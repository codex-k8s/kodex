...
The current `platform-event-log` schema stores the append-only stream and per-consumer checkpoint. Persistent failed/poison diagnostics must be written by the owner service domain model when the service has one. Runtime logs and hooks must stay bounded: event type, schema version, sequence, status, safe code and short summary only.
