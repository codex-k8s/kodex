...
| `staff-gateway` | `openapi/staff-gateway.v1.yaml` предварительный MVP-контракт (`x-kodex-contract-status: mvp-owner-inbox`), Go-модели генерируются в `services/staff/staff-gateway/internal/transport/http/generated` | нет; доменные события публикуют сервисы-владельцы | нет собственного gRPC; вызывает сервисы-владельцы, первый client interface — `interaction-hub` owner inbox и `RecordInteractionResponse` | нет |
