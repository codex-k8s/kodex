...
| `integration-gateway` | `openapi/integration-gateway.v1.yaml` предварительный каркас HTTP-поверхности (`x-kodex-contract-status: mvp-skeleton`) | нет; доменные события публикуют сервисы-владельцы | нет собственного gRPC; вызывает сервисы-владельцы | нет |
