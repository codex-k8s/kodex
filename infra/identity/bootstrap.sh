...
' "$render"
! grep -Eq '__KODEX_[A-Z0-9_]+__|sha256:0{64}' "$render" || fail 'identity render contains placeholders'
...
validate_secret_shape
if [[ "$mode" == apply ]]; then
  kubectl apply --server-side --field-manager=kodex-identity -f "$render" >/dev/null
  ...
  kubectl -n identity patch statefulset keycloak-postgresql --type=merge \
    -p "{\"spec\":{\"template\":{\"metadata\":{\"annotations\":{\"kodex.io/database-tls-revision\":\"$database_tls_revision\"}}}}}" >/dev/null
...
kubectl -n identity exec keycloak-postgresql-0 -- sh -ec '
  ...
  export PGSSLROOTCERT=/var/run/secrets/kodex/postgresql/ca.crt
...
[[ "$(kubectl -n identity get statefulset keycloak-postgresql \
  -o jsonpath='{.spec.template.metadata.annotations.kodex\.io/database-tls-revision}')" == "$database_tls_revision" ]] ||
