...
try:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        wrapper.write_text("#!/usr/bin/env python3\nimport os,sys,subprocess,json\n"
                           ...
                           " current=subprocess.run(['docker','exec',os.environ['STRATEGY_CONTAINER'],'kubectl','-n','kodex-system','get','deployment',name,'-o','jsonpath={.spec.replicas}'],capture_output=True,text=True,check=True).stdout\n"
                           " target='2' if current != '2' else '3'\n"
                           " subprocess.run(['docker','exec',os.environ['STRATEGY_CONTAINER'],'kubectl','-n','kodex-system','scale','deployment/'+name,'--replicas='+target],stdout=subprocess.DEVNULL,check=True)\n"
                           "if '--patch-file' in a:\n"
