...
IMAGE_TAR="${KODEX_BUILD_IMAGE_TAR:-${PROJECT_ROOT}/.local/build/package-hub-smoke-images.tar}"

# shellcheck disable=SC1091
source "${PROJECT_ROOT}/scripts/lib/build_images.sh"

kodex_build_require_env_file "build-package-hub-images" "$ENV_FILE"
...
kodex_build_go_images "build-package-hub-images" "$IMAGE_TAR" "$golang_image" \
