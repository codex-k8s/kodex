...
mattercodex_establish_bootstrap scripts/remote/install-bot-service.sh true true
...
if [ "$DRY_RUN_MODE" = "none" ] && mattercodex_bool "$SHOULD_BUILD_AGENT_RUNNER"; then
  ...
  tar -czf "$AGENT_RUNNER_ARCHIVE" \
    -C "$REPO_ROOT" \
    go.mod \
    go.sum \
    services/jobs/agent-runner \
    --dereference \
    --transform="s|^$BUILD_WRAPPER_FD$|scripts/build-agent-runner-image.sh|" \
    -C "/proc/$$/fd" \
    "$BUILD_WRAPPER_FD"
  if [ "$MATTERCODEX_IMAGE_BUILD_STRATEGY" = "kaniko" ]; then
    run_kaniko_build_remote \
