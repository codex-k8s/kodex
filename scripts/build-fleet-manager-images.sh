...
IMAGE_TAR="${KODEX_BUILD_IMAGE_TAR:-${PROJECT_ROOT}/.local/build/fleet-manager-smoke-images.tar}"

# shellcheck disable=SC1091
source "${PROJECT_ROOT}/scripts/lib/build_images.sh"

kodex_build_require_env_file "build-fleet-manager-images" "$ENV_FILE"
...
kodex_build_go_images "build-fleet-manager-images" "$IMAGE_TAR" "$golang_image" \
