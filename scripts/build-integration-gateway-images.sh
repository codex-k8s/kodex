...
IMAGE_TAR="${KODEX_BUILD_IMAGE_TAR:-${PROJECT_ROOT}/.local/build/integration-gateway-smoke-images.tar}"

# shellcheck disable=SC1091
source "${PROJECT_ROOT}/scripts/lib/build_images.sh"

kodex_build_require_env_file "build-integration-gateway-images" "$ENV_FILE"
...
integration_gateway_image="$(kodex_image_from_repo KODEX_INTEGRATION_GATEWAY_IMAGE KODEX_INTEGRATION_GATEWAY_INTERNAL_IMAGE_REPOSITORY kodex/integration-gateway KODEX_INTEGRATION_GATEWAY_VERSION integration-gateway)"
...
kodex_build_go_images "build-integration-gateway-images" "$IMAGE_TAR" "$golang_image" \
  ...
  "$integration_gateway_image" prod services/external/integration-gateway/Dockerfile \
