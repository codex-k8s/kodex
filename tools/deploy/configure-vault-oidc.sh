...
[[ "$client_id" == kodex-vault-ui && ${#client_secret} -ge 32 ]] || fail 'Vault OIDC Secret is invalid'
root_token=$(<"$root_token_file")

vault_command() {
  printf '%s\n' "$root_token" | kubectl -n kodex-system exec -i vault-0 -- sh -ec '
...
if [[ "$mode" == apply ]]; then
  ...
  vault_json "$temporary_directory/owner-policy.hcl" policy write kodex-owner - >/dev/null
  jq -n --arg audience "$client_id" \
    --arg callback "$vault_public_origin/ui/vault/auth/oidc/oidc/callback" '{
      ...
      bound_claims:{"/realm_access/roles":"kodex-owner"},
      allowed_redirect_uris:[$callback],
      oidc_scopes:["openid","profile","email"],
      token_policies:["kodex-owner"],
      token_ttl:3600,
      token_max_ttl:28800
    }' >"$temporary_directory/role.json"
  vault_json "$temporary_directory/role.json" write auth/oidc/role/kodex-owner - >/dev/null
fi
...
vault_command read -format=json auth/oidc/config | jq -e --arg issuer "$oidc_issuer" --arg client_id "$client_id" '
  ...
  .data.default_role == "kodex-owner"
' >/dev/null || fail 'Vault OIDC config readback failed'
vault_command read -format=json auth/oidc/role/kodex-owner | jq -e \
  --arg callback "$vault_public_origin/ui/vault/auth/oidc/oidc/callback" '
    .data.role_type == "oidc" and .data.allowed_redirect_uris == [$callback] and
    (.data.token_policies | index("kodex-owner") != null)
