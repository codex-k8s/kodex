...
try {
  ...
  try {
    ...
    const result={version:1,protocol:2,revision,go:version,binaries,migrationSHA256:createHash('sha256').update(readFileSync(join(source,'services/internal/internal-rpc-authority/cmd/cli/migrations/20260908000100_authority_bounded_freshness.sql'))).digest('hex'),rendererSHA256:createHash('sha256').update(readFileSync(join(source,'tools/render-image-admission-job.sh'))).digest('hex'),recipe:'CGO_ENABLED=0 GOWORK=off GOOS=linux GOARCH=amd64 GOAMD64=v1 go build -trimpath -buildvcs=false ./cmd/internal-rpc-authority-{issuer,verifier}'};
