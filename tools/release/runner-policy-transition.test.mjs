...
else if(a.includes("patch")){const file=a.find(x=>x.startsWith("--patch-file="))?.slice(13);if(!file||file==="/dev/stdin"||(fs.statSync(file).mode&0o077)!==0)process.exit(5);const p=JSON.parse(fs.readFileSync(file,"utf8"));for(const x of p){if(x.op==="test"){const actual=x.path.endsWith("uid")?s.policy.metadata.uid:s.policy.metadata.resourceVersion;if(actual!==x.value)process.exit(3);}else{s.policy.metadata.annotations=x.value;}}s.patches++;s.policy.metadata.resourceVersion="20";fs.writeFileSync(path,JSON.stringify(s));if(process.env.LOST_ACK==="1")process.exit(4);process.stdout.write("patched");}else process.exit(2);
`, { mode: 0o700 });
