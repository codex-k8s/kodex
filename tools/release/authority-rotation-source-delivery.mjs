...
import {classifyPublisherSourceState,createPublisherRollbackCAS,createPublisherSourceCAS,createSourceMigrationJob,validateRenderedSourceMigration,validateSourceDeliveryPlan,validateSourcePublisher,verifySourceMigrationReadback} from './authority-rotation-source-delivery-model.mjs';
...
async function main(args){
 ...
 while(args.length){const key=args.shift();requireValue(['--context','--source','--revision','--render','--capability','--output','--plan','--evidence','--confirm','--k3s-sudo'].includes(key)&&!Object.hasOwn(options,key),'INVALID_ARGUMENT');options[key]=key==='--k3s-sudo'?true:args.shift();}
 ...
 if(command==='plan'){
  ...
  const source=inspectSource(options['--source']);requireValue(source.revision===options['--revision'],'EXACT_CLEAN_SOURCE_REQUIRED');
  const renderedResources=JSON.parse(run('yq',['eval-all','-o=json','[.]',options['--render']]));
  ...
  const old=inspectSource(publisherState.source),capability=privateJSON(options['--capability']);
  ...
  const currentExecutableSHA256=sourcePublisherExecutable(publisher,get('replicasets,pods',null).items,kube,options['--k3s-sudo']);
 ...
 validateSourceDeliveryPlan(plan);requireValue(plan.context===options['--context']&&plan.namespaceUID===ns.metadata.uid&&inspectSource(plan.source).revision===plan.revision,'SOURCE_DELIVERY_PLAN_DRIFT');
 ...
 if(command==='apply-migration'){
  requireValue(options['--confirm']==='APPLY-STAGING-AUTHORITY-SOURCE-MIGRATION'&&options['--evidence'],'STAGING_CONFIRMATION_REQUIRED');requireValue(!maybeJob(),'EXISTING_INTENT_REQUIRES_OBSERVE');
  const fd=openSync(options['--evidence'],'wx',0o600);try{writeSync(fd,JSON.stringify({at:new Date().toISOString(),status:'INTENT',operation:'CREATE_MIGRATION',planSHA256:fingerprint(plan)})+'\n');fsyncSync(fd);try{kubeInput(JSON.stringify(job),'create','-f','-');}catch{writeSync(fd,JSON.stringify({at:new Date().toISOString(),status:'UNKNOWN',operation:'CREATE_MIGRATION'})+'\n');fsyncSync(fd);}}
  finally{closeSync(fd);}
 }
 if(['apply-migration','observe-migration','apply-publisher','observe-publisher','rollback'].includes(command)){
  const actual=maybeJob();requireValue(actual,'SOURCE_MIGRATION_MISSING');verifySourceMigrationReadback(actual,job);
 ...
 if(command==='rollback'){
  ...
  requireValue(sourcePublisherExecutable(publisher,get('replicasets,pods',null).items,kube,options['--k3s-sudo'])===plan.publisher.beforeExecutableSHA256,'SOURCE_PUBLISHER_ROLLBACK_EXECUTABLE_MISMATCH');process.stdout.write(JSON.stringify({status:'ROLLED_BACK',intentID:plan.intentID})+'\n');return;
 }
 ...
 kube('rollout','status','deployment/internal-rpc-authority-publisher','--timeout=300s');publisher=get('deployment','internal-rpc-authority-publisher');
 requireValue(sourcePublisherExecutable(publisher,get('replicasets,pods',null).items,kube,options['--k3s-sudo'])===plan.capability.publisherSHA256,'SOURCE_PUBLISHER_EXECUTABLE_MISMATCH');
