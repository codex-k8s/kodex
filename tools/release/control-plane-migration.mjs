...
export function planControlPlaneMigration(before, source, inspect = inspectSource, id = randomUUID(), profile = 'legacy') {
  const migrationVersion = profileFor(profile).version;
  ...
  return { version: 1, kind: 'CONTROL_PLANE_MIGRATION', id, source, ...(profile === 'legacy' ? {} : { profile }), migrationVersion, beforeUID: before.metadata.uid, beforeResourceVersion: before.metadata.resourceVersion, beforeSpecSHA256: fingerprint(before.spec), job };
}
...
async function main() {
  ...
  try {
    ...
    if (phase === 'apply' && attempts.length === 0) {
      if (profile === 'mailbox-observation') {
