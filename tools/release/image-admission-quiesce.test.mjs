...
function noWorkLog(phase,{legacy=false}={}){const operation=phase==='claim'?'claim':'claim-promotion',attempts=legacy?[1,12,24,36,48,60,72,84,96,108,120]:[1];return [...attempts.map(attempt=>`image admission bridge claim retry: operation=${operation} attempt=${attempt}/120 class=no-work`),`image admission failed: owner ${phase==='claim'?'admission':'promotion'} work is unavailable`].join('\n')+'\n';}

test('terminal classifier accepts current and transition no-work diagnostics',()=>{assert.equal(classifyNoWorkLog('claim',noWorkLog('claim')),'NO_WORK');assert.equal(classifyNoWorkLog('promote',noWorkLog('promote')),'NO_WORK');assert.equal(classifyNoWorkLog('claim',noWorkLog('claim',{legacy:true})),'NO_WORK');
