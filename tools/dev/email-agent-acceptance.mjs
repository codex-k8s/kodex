...
export function emailOperationProfile(p) {
  check(p && Object.keys(p).every((key) => ['version', 'prefix', 'projectRef', 'agentRef', 'connectionRef', 'configurationRef', 'mailboxRevisionRef', 'accountRef', 'model', 'defaultReasoningEffort', 'capabilityKey', 'input'].includes(key)) && p.version === 1 && /^mvp[a-z0-9-]{4,56}$/.test(p.prefix ?? ''), 'PROFILE_INVALID');
...
export async function emailAgentPlan(profile, agentRef, get) {
...
export async function emailAgentAcceptance({ phase, profile, journal, get, request, preflight }) {
  ...
  if (phase === 'plan') {
    ...
    const plan = await emailAgentPlan(profile, agentRef, get);
  ...
  if (phase === 'launch') {
    ...
    check(sha(await emailAgentPlan(profile, agentRef, get)) === plan.planSHA256, 'PLAN_STALE');
  ...
  return checkpoint(journal, 'capture', { ...launched, ...current, status: current.invocationState === 'SUCCEEDED' ? 'CAPTURED' : current.invocationState, runtimeRevisionRef: ref(revision.ref), runtimeRevisionVersion: version(revision.version), runtimeRevisionDigest: digest(revision.revisionDigest), turnRef: ref(revision.turnRef), lastEventSequence: after, runState: run.state, runTerminal: ['SUCCEEDED', 'FAILED', 'CANCELLED'].includes(run.state), receipt: 'NOT_RUN' });
}
...
async function main() {
  ...
  check(['agent', 'plan', 'launch', 'capture', 'receipt'].includes(phase), 'PHASE_INVALID');
  check(Object.keys(options).every((key) => ['--origin', '--storage-state', '--state', '--profile', '--serving-manifest', '--timeout-ms', '--confirm'].includes(key)), 'ARGUMENT_UNKNOWN');
  ...
  const manifestBytes = privateInput(options['--serving-manifest']); emailServingManifest(JSON.parse(manifestBytes));
  ...
  try {
    ...
    const result = await emailAgentAcceptance({ phase, profile, journal, get, request, preflight: async () => { const response = await client.observe('/api/v1/session', { signal: AbortSignal.timeout(Math.min(30000, timeoutMs)) }); check(response.status === 200, 'SESSION_PREFLIGHT_FAILED'); await response.body?.cancel(); } });
