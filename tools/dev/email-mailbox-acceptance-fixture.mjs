...
export function emailFixtureTransport(state, path, method, body, mode = '') {
  ...
  const connection = () => ({ ref: 'iconn_email', version: state.version ?? 1, definitionKey: 'email', state: state.healthy ? 'CONNECTED' : 'NOT_CONNECTED', grants: state.granted ? [{ ref: 'igrant_email', version: 1, capabilityKey: 'email.message.send', agentRef: 'agt_email', enabled: true }] : [] });
  const view = () => ({ connectionRef: 'iconn_email', connectionVersion: state.version ?? 1, mailboxRef: 'mailbox_email', configuration: { ref: 'mcfg_email', kind: 'EMAIL_MAILBOX', managedBy: 'UI', version: state.configVersion ?? 1 }, revision: { ref: 'mrev_email', state: state.revisionState ?? 'DRAFT', digest: mode === 'drift' ? 'b'.repeat(64) : hash }, ...(state.bound ? { publication: { ref: 'empub_email', revision: 2, digest: hash, configurationRevisionRef: 'mrev_email', state: mode === 'pending' ? 'PENDING' : mode === 'publication-failed' ? 'FAILED' : 'READY' } } : {}) });
  ...
  if (path.includes('/email-mailbox/credential-receipt?') && method === 'GET') return { body: { connectionRef: 'iconn_email', connectionVersion: state.version, name: 'email-recovered', generation: 1 } };
  if (path === '/api/v1/integration-connections' && method === 'POST') { state.version = 1; return { status: 201, body: connection() }; }
  if (path === '/api/v1/integration-connections/iconn_email' && method === 'GET') return { body: connection() };
  ...
  if (path.endsWith('/validation') && method === 'POST') { state.revisionState = mode === 'invalid' ? 'INVALID' : 'VALID'; state.configVersion = 2; return { body: view() }; }
  if (path.endsWith('/publication') && method === 'POST') { state.revisionState = 'PUBLISHED'; state.configVersion = 3; return { body: view() }; }
  if (path.endsWith('/binding') && method === 'POST') { state.bound = true; state.version++; state.configVersion++; return { body: view() }; }
  if (path.endsWith('/commands') && method === 'POST') { state.version++; state.healthy = mode !== 'health-failed'; return { body: connection() }; }
