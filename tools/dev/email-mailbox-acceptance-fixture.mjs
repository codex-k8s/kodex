...
export function emailFixtureTransport(state, path, method, body, mode = '') {
  ...
  const view = () => ({ connectionRef: 'iconn_email', connectionVersion: state.version ?? 1, mailboxRef: 'mailbox_email', configuration: { ref: 'mcfg_email', kind: 'EMAIL_MAILBOX', managedBy: 'UI', version: state.configVersion ?? 1 }, revision: { ref: state.revisionRef ?? 'mrev_email', ...(state.parentRef ? { parentRevisionRef: state.parentRef } : {}), state: state.revisionState ?? 'DRAFT', digest: mode === 'drift' ? 'c'.repeat(64) : state.revisionDigest ?? hash }, boundRevisionRef: state.bound && !state.superseded ? state.revisionRef ?? 'mrev_email' : '', nextActions: [{ action: 'BIND', enabled: !state.bindingDenied }], ...(state.bound ? { publication: { ref: state.rebound ? 'empub_email_forward' : 'empub_email', revision: state.rebound ? 5 : 2, digest: hash, configurationRevisionRef: state.revisionRef ?? 'mrev_email', state: state.superseded ? 'SUPERSEDED' : mode === 'pending' ? 'PENDING' : mode === 'publication-failed' ? 'FAILED' : 'READY' } } : {}) });
  ...
  if (path.endsWith('/binding') && method === 'POST') { if (state.superseded) { state.rebound = true; state.superseded = false; } state.bound = true; state.version++; state.configVersion++; return { body: view() }; }
