...
import { artifactDetail, roleImageTransportFixture } from "./role-image-acceptance-fixture.mjs";
...
test("managed prepare, forward publication and exact historical restore use isolated bindings", async (t) => {
  const journal = privateJournal(fixture(t), { version: 1 });
  const transport = roleImageTransportFixture(); const { request, revisions } = transport;
  for (const phase of ["prepare", "advance", "restore"]) {
    ...
    assert.equal(result.status, "PASS"); assert.equal(result.runtimeJob, "NOT_RUN"); assert.equal(result.revisionRef, `revision${transport.generation}`);
    const previous = transport.mutations; await runAcceptance({ phase, journal, request, preflight: async () => {}, prefix: "fixture-test", runnerDigest: `sha256:${digest}` }); assert.equal(transport.mutations, previous);
  }
