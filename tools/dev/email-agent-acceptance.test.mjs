...
globalThis.fetch=async(url,init={})=>{if(url.origin!==${JSON.stringify(origin)})throw new Error('ORIGIN_UNEXPECTED');
 ...
 if(mode==='diff-pending'&&url.pathname.endsWith('/runtime-revision-diff'))return new Response('{}',{status:404,headers:{'Content-Type':'application/json'}});
...
test('combined runtime binding treats the initial RUNNING revision 404 as bounded pending',t=>{const f=combinedLaunched(t);f.setMode('diff-pending');const output=join(f.directory,'pending-binding.json');const r=f.run('runtime-binding',['--runtime-binding-output',output]);assert.equal(r.status,0,r.stderr);assert.deepEqual(JSON.parse(r.stdout),{status:'PENDING',runRef:'run_fixture'});assert.equal(f.calls().filter(c=>c.method==='POST').length,1);assert.throws(()=>readFileSync(output),/ENOENT/);});
