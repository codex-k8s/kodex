...
export function prepareEmailInput(env, protocol, prefix, readCA = (path) => readFileSync(path, 'utf8')) {
  ...
  const profile = { prefix, specification: { enabled: true, receiveProtocol: protocol, sender: value('ADDRESS'), replyTo: value('ADDRESS'), recipients: [value('RECIPIENT')], allowedFolders: ['INBOX'], folder: 'INBOX', helloName: 'kodex.works', smtp: endpoint('SMTP', 'smtp'), [protocol === 'IMAP' ? 'imap' : 'pop']: endpoint(protocol, protocol === 'IMAP' ? 'imap' : 'pop'), limits: { attachmentBytes: 1048576, maxAttachments: 1, maxRecipients: 1, messageBytes: 2097152, pageSize: 5, scanMessages: protocol === 'POP3' ? 1000 : 20, timeoutSeconds: 30 }, policies: all.map((operation) => ({ operation, policy: reads.has(operation) ? 'ALLOW' : operation === 'SEND' ? 'HUMAN_GATE' : 'DENY', folders: ['INBOX'] })) } };
