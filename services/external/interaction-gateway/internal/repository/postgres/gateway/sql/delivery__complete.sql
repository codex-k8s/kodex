...
SET state = 'DELIVERED', delivery_recorded_at = CASE WHEN owner_gate_id IS NULL THEN delivery_recorded_at ELSE clock_timestamp() END,
    ...
    owner_delivery_recorded_at = CASE WHEN owner_delivery_fence IS NULL THEN owner_delivery_recorded_at ELSE clock_timestamp() END,
    owner_delivery_token_ciphertext = CASE WHEN owner_delivery_fence IS NULL THEN owner_delivery_token_ciphertext ELSE NULL END,
