...
WHERE binding.organization_id = @arg1::uuid AND binding.project_id = @arg2::uuid
  AND binding.agent_stable_key = @arg3::text AND identity.provider_user_id = @arg4::text
  AND binding.provider_generation = @arg5::bigint AND identity.provider_generation = @arg5::bigint
  AND binding.status = 'AVAILABLE' AND identity.status = 'AVAILABLE'
  AND watermark.provider_generation = @arg5::bigint AND watermark.admitted;
