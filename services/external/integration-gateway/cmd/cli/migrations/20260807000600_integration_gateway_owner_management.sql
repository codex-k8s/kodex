...
CREATE TABLE integration_gateway.provider_credential_generations (
    ...
    masked_account text NOT NULL DEFAULT '',
    masked_label text NOT NULL DEFAULT '',
    observed_usage bigint NOT NULL CHECK (observed_usage >= 0),
    ...
    window_duration_seconds bigint NOT NULL CHECK (window_duration_seconds > 0),
...
CREATE TABLE integration_gateway.management_effects (
    ...
    dispatch_state text NOT NULL DEFAULT 'PENDING' CHECK (dispatch_state IN ('PENDING', 'DISPATCHED', 'COMPLETED')),
    provider_phase text NOT NULL DEFAULT 'PENDING' CHECK (provider_phase IN ('PENDING', 'DISPATCHED', 'SUCCEEDED', 'UNKNOWN', 'SKIPPED')),
    secret_phase text NOT NULL DEFAULT 'PENDING' CHECK (secret_phase IN ('PENDING', 'SUCCEEDED')),
    control_plane_phase text NOT NULL DEFAULT 'PENDING' CHECK (control_plane_phase IN ('PENDING', 'SUCCEEDED')),
