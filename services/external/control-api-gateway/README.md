...
Каждая `Resource`-проекция содержит закрытый server-authored `nextActions`;
PWA не выводит lifecycle из локального state. Конфигурационная ownership-
проекция содержит `managedBy`, `source`, immutable `revision`, опциональный
content digest и авторитетный `drift=NOT_APPLICABLE|IN_SYNC|DRIFTED|UNKNOWN`.
`UNKNOWN` является закрытым readback для старого Git-owned состояния, а
`IN_SYNC` выставляется только после успешной exact Git reconciliation.
...
| #191, owner читает ресурс/список | `GET /api/v1/resources[/{id}]` | `control.resource.get|list` → `GetResource|ListResources` | resource сначала разрешён control-plane внутри текущей области; verified actor predicate находится в named SQL до `ORDER BY/LIMIT` | `200`+`ETag` для single; cursor вычисляется по последней eligible строке, скрытый и отсутствующий одинаково `404`; authoritative read path содержит closed `nextActions` и, для конфигурации, source/revision/drift без secret values |
...
| WS subscribe | exact Origin, session, CSRF subprotocol+cookie, ≤8 channel, ≤32 project-scoped kind, bounded frame/connection | policy close без snapshot |
