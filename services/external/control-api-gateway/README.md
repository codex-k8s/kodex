...
| Restore claim/materialization | `QUEUED` → `ASSIGNED` → admission `ADMITTED` → `MATERIALIZING` → `READY` → `RUNNING` | verifier независимо пересчитывает `sourceAuthoritySHA256` из полного private source tuple; operation/generation/digest входят в EffectiveInput, WorkloadTicket и signed restore ticket; admission до Kubernetes/PVC/S3 атомарно сверяет current generation/revoke watermark и потребляет generation | stale/revoked/reused ticket закрыто отклоняется до первого внешнего эффекта; Bind принимает только уже consumed current generation; private tuple не выходит в browser/логи |
...
11. Между выдачей restore claim и первым Kubernetes/S3 effect выполнить
   cancel/expiry: admission должен закрыто отклонить stale generation по
   durable revoke watermark. Для current claim проверить, что admission сначала
   потребляет generation, verifier пересчитывает private tuple, а повтор старого
   ticket отклоняется. Только target `SUCCEEDED` даёт `RESTORED` и `READ_BACKUP`.
