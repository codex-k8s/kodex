...
func (stub *providerCommandStub) replyAccount() *controlplanev1.ProviderAccount {
	if stub.account != nil {
		return stub.account()
	}
...
func (stub *providerCommandStub) VerifyProviderAccountDeviceAuthorization(_ context.Context, request *controlplanev1.VerifyProviderAccountDeviceAuthorizationRequest, _ ...grpc.CallOption) (*controlplanev1.VerifyProviderAccountDeviceAuthorizationResponse, error) {
	stub.verify = request
	return &controlplanev1.VerifyProviderAccountDeviceAuthorizationResponse{Account: stub.replyAccount()}, nil
}
...
func (stub *providerCommandStub) ReauthorizeProviderAccountDeviceCode(_ context.Context, request *controlplanev1.ReauthorizeProviderAccountDeviceCodeRequest, _ ...grpc.CallOption) (*controlplanev1.ReauthorizeProviderAccountDeviceCodeResponse, error) {
	...
	return &controlplanev1.ReauthorizeProviderAccountDeviceCodeResponse{Account: stub.replyAccount()}, nil
}
...
func (stub *providerCommandStub) DeleteProviderAccount(_ context.Context, request *controlplanev1.DeleteProviderAccountRequest, _ ...grpc.CallOption) (*controlplanev1.DeleteProviderAccountResponse, error) {
	stub.delete = request
	return &controlplanev1.DeleteProviderAccountResponse{Account: stub.replyAccount()}, nil
}
...
func TestProviderAccountSafeStatusReasonMatchesOwnerRegistry(t *testing.T) {
	for _, reason := range []string{"AUTHORIZED", "ACCOUNT_DISABLED", "ACCOUNT_REVOKED", "REAUTHORIZATION_REQUIRED", "DEVICE_AUTHORIZATION_PENDING", "CREDENTIAL_CONFIGURATION_REQUIRED", "ACCOUNT_STATE_UNKNOWN", "DEVICE_AUTHORIZATION_EXPIRED", "DEVICE_AUTHORIZATION_FAILED", "CREDENTIAL_MATERIALIZATION_FAILED", "private upstream text", "FUTURE_REASON"} {
		...
		account.Usage = providerUsageAccountFixture().Usage
		account.Usage.AccountVersion = account.Version
		account.Usage.Context = nil
		account.Usage.AgentVersion = 0
		account.Usage.EligibleForSelection = false
		account.Usage.OperationalState = controlplanev1.ProviderUsageState_PROVIDER_USAGE_STATE_NOT_EVALUATED
		account.Usage.ModelCompatibility = &controlplanev1.ProviderUsageDimension{State: controlplanev1.ProviderUsageState_PROVIDER_USAGE_STATE_NOT_EVALUATED, Reason: controlplanev1.ProviderUsageReason_PROVIDER_USAGE_REASON_CONTEXT_REQUIRED, Remediation: controlplanev1.ProviderUsageRemediation_PROVIDER_USAGE_REMEDIATION_SELECT_CONTEXT}
		account.Usage.ActorEligibility = proto.Clone(account.Usage.ModelCompatibility).(*controlplanev1.ProviderUsageDimension)
...
func TestProviderCommandsRejectUnboundAccountReadback(t *testing.T) {
	for _, missing := range []bool{false, true} {
		stub := &providerCommandStub{account: func() *controlplanev1.ProviderAccount {
			if missing {
				return nil
			}
			account := providerTestAccount()
			account.Ref = "pacc_foreign01"
			account.Name = "private foreign account"
			return account
		}}
		...
		for name, invoke := range map[string]func(*httptest.ResponseRecorder){
			"verify": func(w *httptest.ResponseRecorder) {
				server.VerifyProviderAccountDeviceAuthorization(w, httptest.NewRequest("POST", "/", nil), "pacc_primary01", generated.VerifyProviderAccountDeviceAuthorizationParams{IfMatch: `"4"`, IdempotencyKey: "verify-binding"})
			},
			"reauthorize": func(w *httptest.ResponseRecorder) {
				server.ReauthorizeProviderAccountDeviceCode(w, httptest.NewRequest("POST", "/", nil), "pacc_primary01", generated.ReauthorizeProviderAccountDeviceCodeParams{IfMatch: `"4"`, IdempotencyKey: "reauthorize-binding"})
			},
			"delete": func(w *httptest.ResponseRecorder) {
				server.DeleteProviderAccount(w, httptest.NewRequest("DELETE", "/", nil), "pacc_primary01", generated.DeleteProviderAccountParams{IfMatch: `"4"`, IdempotencyKey: "delete-binding"})
			},
		} {
			w := httptest.NewRecorder()
			invoke(w)
			if w.Code != 502 || !strings.Contains(w.Body.String(), "INVALID_UPSTREAM_RESPONSE") || strings.Contains(w.Body.String(), "private foreign account") || strings.Contains(w.Body.String(), "pacc_foreign01") {
				t.Fatalf("%s accepted unbound account: status=%d", name, w.Code)
			}
		}
	}
}
