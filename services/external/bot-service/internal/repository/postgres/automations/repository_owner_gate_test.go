...
func TestAutomationOwnerGateDurabilityAndBoundaries(t *testing.T) {
	...
	prematureResolution := automationsrepo.ResolveOwnerGateInput{
		...
		MattermostChannelID: "automation-channel", MattermostRootPostID: "owner-gate-root",
		MattermostResponsePostID: "owner-response-before-delivery", MattermostResponseCreateAt: 900, Now: now.Add(2 * time.Minute),
	}
	...
	setPostInput := automationsrepo.SetOwnerAttentionPostInput{
		...
		MattermostPostCreateAt: 1_000,
		ClaimToken:             confirmationClaim.ClaimToken, Fence: confirmationClaim.Fence, Now: now.Add(4*time.Minute + time.Second),
	}
	delayedPreCardResolution := automationsrepo.ResolveOwnerGateInput{
		ProjectID: projectID, ActorUserID: "root-owner-id", ActorUserName: "root-owner",
		MattermostChannelID: "automation-channel", MattermostRootPostID: "owner-gate-root",
		MattermostResponsePostID: "owner-response-before-card", MattermostResponseCreateAt: 900, Now: now.Add(6 * time.Minute),
	}
	...
	go func() {
		<-raceStart
		resolvedRun, duplicate, resolveErr := restarted.ResolveOwnerGate(ctx, delayedPreCardResolution)
	...
	if !errors.Is(decision.err, automationsrepo.ErrNotFound) || decision.duplicate || decision.run.ID != 0 {
		t.Fatalf("созданный до карточки ответ прошёл гонку binding: %#v", decision)
	}
	posted, err := restarted.GetOwnerAttentionDelivery(ctx, run.ID)
	if err != nil || posted.MattermostPostID != "attention-post-1" || posted.MattermostPostCreateAt != 1_000 {
		t.Fatalf("delivery proof=%#v error=%v", posted, err)
	}
	resolver := statusservice.NewAutomationService(statusservice.AutomationServiceConfig{
		Repository:   restarted,
		Catalog:      &ownerGateCatalogStub{},
		StorageReady: true,
		Now:          func() time.Time { return now.Add(6 * time.Minute) },
	})
	delayed, err := resolver.ResolveOwnerDecision(ctx, statusservice.AutomationOwnerDecisionCommand{
		ProjectID: projectID, ActorUserID: "root-owner-id", ActorUserName: "root-owner",
		MattermostChannelID: "automation-channel", MattermostRootPostID: "owner-gate-root",
		MattermostResponsePostID: "owner-response-before-card", MattermostResponseCreateAt: 900,
	})
	if err != nil || delayed.Handled {
		t.Fatalf("задержанный pre-card ответ принят после restart/binding: result=%#v error=%v", delayed, err)
	}
	if err := pool.QueryRow(ctx, `select status from matter_codex_owner_attention_requests where id = $1`, delivery.AttentionID).Scan(&automationAttentionStatus); err != nil || automationAttentionStatus != "open" {
		t.Fatalf("pre-card ответ закрыл attention: status=%q error=%v", automationAttentionStatus, err)
	}
	validResolution := statusservice.AutomationOwnerDecisionCommand{
		ProjectID: projectID, ActorUserID: "root-owner-id", ActorUserName: "root-owner",
		MattermostChannelID: "automation-channel", MattermostRootPostID: "owner-gate-root",
		MattermostResponsePostID: "owner-response-1", MattermostResponseCreateAt: 2_000,
	}
	valid, err := resolver.ResolveOwnerDecision(ctx, validResolution)
	if err != nil || !valid.Handled || valid.Duplicate || valid.Run.Status != string(value.AutomationRunStatusSucceeded) || valid.Run.FinishedAt.IsZero() {
		t.Fatalf("валидный post-card ответ: result=%#v error=%v", valid, err)
	}
	decision = decisionResult{run: valid.Run, duplicate: valid.Duplicate}
	...
	wrongActor := automationsrepo.ResolveOwnerGateInput{
		...
		MattermostChannelID: "automation-channel", MattermostRootPostID: "owner-gate-root", MattermostResponsePostID: "owner-response-1", MattermostResponseCreateAt: 2_000, Now: now.Add(4 * time.Minute),
	}
	...
	replayed, replayErr := statusservice.NewAutomationService(statusservice.AutomationServiceConfig{
		Repository: postgresrepo.NewRepository(pool), Catalog: &ownerGateCatalogStub{}, StorageReady: true,
		Now: func() time.Time { return now.Add(7 * time.Minute) },
	}).ResolveOwnerDecision(ctx, validResolution)
	if replayErr != nil || !replayed.Duplicate || !replayed.Handled || replayed.Run.ID != resolved.ID || replayed.Run.FinishedAt.IsZero() {
		t.Fatalf("resolution replay result=%#v error=%v", replayed, replayErr)
	}
	var attentionStatus, resolvedUserID, resolvedPostID string
	var resolvedPostCreateAt int64
	var resolvedAt time.Time
	if err := pool.QueryRow(ctx, `select status, resolved_at, resolved_by_user_id, resolved_by_post_id, automation_resolved_by_post_create_at from matter_codex_owner_attention_requests where id = $1`, delivery.AttentionID).Scan(&attentionStatus, &resolvedAt, &resolvedUserID, &resolvedPostID, &resolvedPostCreateAt); err != nil {
		t.Fatal(err)
	}
	if attentionStatus != "resolved" || resolvedAt.IsZero() || resolvedUserID != "root-owner-id" || resolvedPostID != "owner-response-1" || resolvedPostCreateAt != 2_000 {
		t.Fatalf("attention status=%q resolved_at=%s actor=%q post=%q create_at=%d", attentionStatus, resolvedAt, resolvedUserID, resolvedPostID, resolvedPostCreateAt)
	}
...
type ownerGateCatalogStub struct {
