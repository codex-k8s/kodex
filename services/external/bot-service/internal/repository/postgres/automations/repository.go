...
func (repo *Repository) ResolveOwnerGate(ctx context.Context, input automationsrepo.ResolveOwnerGateInput) (entity.ScheduledRun, bool, error) {
	...
	if duplicate {
		...
		tag, err := txRepo.db.Exec(ctx, query("automation_owner_attention__resolve.sql"), candidate.attentionID, strings.TrimSpace(input.ActorUserID), strings.TrimSpace(input.MattermostResponsePostID), input.MattermostResponseCreateAt, input.Now)
		if err != nil {
...
func scanOwnerAttentionDelivery(row pgx.Row) (entity.AutomationOwnerAttentionDelivery, error) {
	...
	var mattermostPostCreateAt *int64
	err := row.Scan(
		&item.AttentionID,
		&item.ScheduledRunID,
		&item.ScheduledRunPublicID,
	...
	if mattermostPostCreateAt != nil {
		item.MattermostPostCreateAt = *mattermostPostCreateAt
	}
	return item, err
}
