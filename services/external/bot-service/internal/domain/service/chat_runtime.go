...
func (svc *ChatRunService) EnqueueAgentTurn(ctx context.Context, request AgentTurnRequest) (AgentTurnQueued, error) {
	...
	if agentSessionRuntimeShouldBeEnsured(session) {
		...
		started, err = svc.startAgentSessionRuntime(ctx, session, tokenSession, request.Role, openAIAccount.SecretRef, gitHubSecretName, repo, runtimeEnv, runtimeRevisionForPod, runtimeRevisionForPod.ID != runtimeRevision.ID)
		if err != nil {
