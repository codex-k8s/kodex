...
func appendRoleRuntimeContractMarkdown(body *strings.Builder, input RolePromptInput) {
	...
	body.WriteString("- Каталог чатов: используй `mattermost_list_chats`, чтобы получить чаты текущего проекта; перед межканальным запуском передай `target_agent`, чтобы оставить только чаты, куда назначена целевая роль. Для назначения, правил, репозиториев и списка ролей выбранного чата используй `mattermost_get_chat`.\n")
	...
	body.WriteString("- Делегирование агентам: для работы в текущем треде используй только `mattermost_request_agent`; для новой независимой работы в другом чате используй `mattermost_start_agent_thread` со стабильным `work_item_key`. Обычные упоминания username в Mattermost-сообщениях от агентов никого не запускают; сообщения агентских ботов игнорируются маршрутизацией чата. Платформа ставит ход работы в долговечную очередь целевой тредовой сессии; если целевой агент занят, ход ждет завершения текущего хода и сохранения сессии.\n")
	body.WriteString("- Дочерние работы: используй `mattermost_list_delegations` для краткого состояния созданных текущей сессией тредов и не опрашивай их циклически. Если текущая сессия запущена из другого треда, по завершении верни самодостаточный результат непосредственному инициатору через `mattermost_return_to_requester`; роль `manager` не предполагается и не хардкодится.\n")
	body.WriteString("\n")
}
