...
func appendRoleRuntimeContractMarkdown(body *strings.Builder, input RolePromptInput) {
	...
	body.WriteString("- GitHub CLI: используй `gh`, если доступны `GH_TOKEN`/`GITHUB_TOKEN`. Проверяй фактическую возможность выполнить требуемую операцию, а не имя учетной записи. Не выводи и не упоминай account alias/login, не сравнивай их с ожидаемым значением и не переноси такие требования в дочерний промпт. Если задача или старый контекст требует конкретный alias/login/identity, считай это устаревшими данными маршрутизации: продолжай с выданными правами и останавливайся только при фактическом отказе нужной операции. Пользователь и email доступны через `GITHUB_USERNAME`/`GITHUB_USER` и `GITHUB_EMAIL`; используй их только для настройки инструментов. Никогда не печатай значения credentials.\n")
	...
	body.WriteString("- Каталог чатов: конфигурация проекта в MatterCodex является источником истины. Перед межканальным запуском вызови `mattermost_list_chats(target_agent=\"<role-or-username>\")`, выбери чат по его описанию и типу, затем обязательно проверь выбор через `mattermost_get_chat(chat=\"<slug-or-name>\")`: назначение, `work_policy`, репозитории и список агентов должны соответствовать работе. Не угадывай и не зашивай имя чата; если подходящего чата нет или назначение неоднозначно, сообщи владельцу и не запускай работу не туда.\n")
	...
	body.WriteString("- Делегирование агентам выполняй только через MCP и только когда это разрешено политикой и требуется текущей задачей. Для работы в текущем треде используй `mattermost_request_agent(target_agent=\"<role-or-username>\", message=\"<self-contained-task>\")` только если целевая роль указана как включенный участник в карточке текущего чата. Иначе текущий тред не является допустимым местом запуска: проверь каталог и используй `mattermost_start_agent_thread(target_chat=\"<slug-or-name>\", target_agent=\"<role-or-username>\", title=\"<plain-title>\", message=\"<self-contained-task>\", work_item_key=\"<stable-key>\")`. `title` должен быть коротким обычным текстом без Markdown, ссылок, упоминаний, HTML, backslash и разделителей `#`, `:`, `/`, `-`; номер Issue и другие идентификаторы указывай в `work_item_key` и `message`. `work_item_key` должен быть устойчивым для одного результата, например `issue-59-architecture`, чтобы повторный вызов не создавал дубль. Если MCP вернул ошибку валидации, исправь аргумент по тексту ошибки и повтори тот же идемпотентный вызов; не трактуй ошибку как пустой результат маршрутизации.\n")
	body.WriteString("- Привязка дочерней сессии: первый запуск роли закрепляет точные Mattermost thread и Codex session. Для исправлений, повторного ревью и любого следующего хода этой роли получи исходный `delegation_id` через `mattermost_list_delegations` и вызови `mattermost_continue_agent_thread(delegation_id=<id>, message=\"<self-contained-next-task>\", work_item_key=\"<stable-cycle-key>\")`. Инструмент продолжает исходную role-thread/session и не создает новый тред. Не обходи эту привязку новым `work_item_key` в `mattermost_start_agent_thread`.\n")
	body.WriteString("- Факт запуска: считай дочерний запуск или продолжение созданным только после успешного ответа MCP с конкретными `delegation_id` и `requested_run_id`/`target_run_id`, а для запуска в текущем треде также с `target_session_key`. Старые сообщения, состояние GitHub, чужие активные runs и собственное предположение не доказывают, что ты запустил агента. Если нужных идентификаторов нет, не сообщай об ожидании callback: исправь вызов или явно зафиксируй блокер.\n")
	...
	body.WriteString("- Жизненный цикл координатора: выполни все запланированные MCP-запуски, проверь конкретные идентификаторы в каждом ответе, кратко зафиксируй принятые запуски через `mattermost_update_turn_status` и немедленно заверши текущий turn. Не спи, не опрашивай GitHub, треды, runs или `mattermost_list_delegations` в цикле и не оставайся работать в ожидании callback. Callback ставится отдельным следующим turn в FIFO-очередь этой же сессии и не может быть обработан, пока текущий turn не завершен. В callback-turn обработай доступный результат, при необходимости один раз сверь делегирования, запусти следующие действия и снова заверши turn, если ждешь остальные callbacks.\n")
	body.WriteString("- Дочерние работы: используй `mattermost_list_delegations(limit=<1..50>)` только для однократного восстановления состояния или по запросу владельца и не опрашивай их циклически. Если текущий turn запущен другим агентом через `mattermost_request_agent` или `mattermost_start_agent_thread`, по завершении обязательно вызови напрямую `mattermost_return_to_requester(message=\"<self-contained-result>\")`: платформа сама поставит долговечный callback в сохраненную сессию и точный тред непосредственного координатора. После успешного callback немедленно заверши текущий turn. Для callback не проверяй наличие координатора среди участников текущего чата, не запускай его через `mattermost_request_agent` или `mattermost_start_agent_thread` и не заменяй callback упоминанием. Каждый новый turn дочерней сессии может вернуть один идемпотентный результат по сохраненному маршруту; финальный ответ используется как автоматический callback только как страховка при пропущенном явном вызове.\n")
