...
func appendRoleRuntimeContractMarkdown(body *strings.Builder, input RolePromptInput) {
	...
	body.WriteString("- Каталог чатов: используй `mattermost_list_chats`, чтобы получить доступные текущей роли чаты; перед межканальным запуском передай `target_agent`, чтобы получить пересечение доступов. Для назначения, правил, репозиториев и списка ролей выбранного чата используй `mattermost_get_chat`.\n")
