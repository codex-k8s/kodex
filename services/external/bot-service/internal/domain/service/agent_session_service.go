...
func (svc *AgentSessionService) ClaimNextTurn(ctx context.Context, sessionKey string, token string) (AgentSessionTurnClaim, error) {
	...
	err = svc.withCurrentSessionPersistenceFence(ctx, session, "agent_session.claim_turn.side_effect", func(current entity.AgentSession, guardedStore adminrepo.Repository) error {
		...
		if _, updateErr = guardedStore.UpdateAgentRunArtifacts(ctx, adminrepo.UpdateAgentRunArtifactsInput{RunID: turn.RunID, Status: agentSessionTurnRunning}); updateErr != nil &&
			!(turn.RuntimeRevisionID == 0 && errors.Is(updateErr, adminrepo.ErrNotFound)) {
			return updateErr
		}
		if svc.cfg.ThreadPublisher != nil {
...
func (svc *AgentSessionService) CompleteTurn(ctx context.Context, sessionKey string, token string, command CompleteAgentSessionTurnCommand) error {
	...
	err = svc.withCurrentSessionPersistenceFence(ctx, session, "agent_session.complete_turn_persist.side_effect", func(current entity.AgentSession, guardedStore adminrepo.Repository) error {
		...
		if agentSessionTurnTerminal(currentTurn.Status) {
			...
			if _, atomic := guardedStore.(adminrepo.AgentSessionArchiveRepository); !atomic {
				terminalRetry = true
				return nil
			}
		}
		...
		if currentTurn.Status != agentSessionTurnRunning {
			return adminrepo.ErrClusterAdminAdmissionDenied
		}
		...
		turn, completeErr = guardedStore.CompleteAgentSessionTurn(ctx, adminrepo.CompleteAgentSessionTurnInput{
			...
			ErrorMessage:     command.ErrorMessage,
			Artifacts:        artifacts,
			CompletionPodUID: strings.TrimSpace(command.PodUID),
		})
		return completeErr
	})
	if err != nil {
	...
	if alreadyCompleted || terminalRetry {
