...
func TestKubernetesPostgresJobHasKillSafeLifetimeWithoutPrivilegeExpansion(t *testing.T) {
	...
	if readiness == nil || readiness.Exec == nil || !slices.Equal(readiness.Exec.Command, expectedReadinessCommand) ||
		readiness.PeriodSeconds != 1 || readiness.TimeoutSeconds != 1 || readiness.SuccessThreshold != 1 || readiness.FailureThreshold != 120 {
		t.Fatalf("PostgreSQL readiness probe = %#v", readiness)
	}
}

func TestKubernetesPortForwardArgvUsesCanonicalLoopbackForm(t *testing.T) {
	argv := kubernetesPortForwardArgv("disposable-tests", "mc-postgres-test-abc", 25432)
	expected := []string{
		"kubectl",
	...
	if !slices.Equal(argv, expected) {
		t.Fatalf("kubectl port-forward argv = %#v, ожидалось %#v", argv, expected)
	}
}
...
func TestKubernetesPostgresReadinessCancellationStopsLiveTunnel(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	process := newFakePostgresPortForward()
	ready, err := waitKubernetesPostgresReady(ctx, func() (postgresPortForward, error) {
		return process, nil
	}, func(context.Context) error {
		return errors.New("synthetic backend connection refused")
	}, waitControllerPostgresRetry)
	if !errors.Is(err, context.Canceled) || ready != nil || process.stopCalls != 1 {
		t.Fatalf("canceled readiness: ready=%T stops=%d error=%v", ready, process.stopCalls, err)
	}
}
