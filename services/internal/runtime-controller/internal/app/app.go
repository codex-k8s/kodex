...
func Run(
	...
	cluster, err := kubeadapter.InCluster(kubeadapter.Config{
		...
		RuntimeServiceAccount: config.RuntimeServiceAccount, ReadClusterRole: config.ReadClusterRole,
		...
		MaximumPods: config.MaximumPods, MaximumCPU: config.MaximumCPUMilli,
