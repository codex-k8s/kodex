...
control-plane. Issue #263 добавляет authoritative typed owner readbacks,
не меняя владение provider effect или внешний gateway. Provider Team
effect/readback реализован unit #235, Agent bot
catalog/effect/readback/receipt — отдельным interaction-gateway unit #264.
Mattermost path считается полностью материализованным только при совместном
наличии #235 и #264; provider/Git path готов только вместе с deploy/readiness
unit #236.
...
| `SCHEDULE` binding | create/update owner basic intent; bind/rebind exact configuration | typed owner get/list и существующие occurrence paths |
...
## Authoritative owner readbacks Issue #263
...
```text
verified authority
  -> exact full method / workload / SPIFFE / permission
  -> owner eligibility (organization + project + actor)
...
`Get`/`List`/history/mutation refresh применяют один owner predicate. Hidden,
unknown, deleted, partial либо stale dependency возвращается как not found,
conflict или typed `STALE|INELIGIBLE`; fake display lookup и browser UUID join
не используются.

### Lifecycle/authority matrix readback
...
| `DOM-MC-003/011`, interaction-gateway provider readback profile | interaction-gateway #264 | `ManageAgentMattermostBotIdentity` bind/rebind/revoke | exact signed receipt связывает issuer/purpose/workload/SPIFFE/full method/actor/org/project/workspace/team/Agent target/action/effect/version/generation/digest/expiry/JTI; team и bot ref выводятся из receipt | owner transaction one-use consume issuer+purpose+JTI+target+intent вместе с masked state/history/command receipt/audit и bounded immutable result snapshot; exact replay возвращает прежнюю версию даже после последующей mutation | #264 владеет authoritative catalog, one-winner checkpoint, provider/Vault recovery, signer, generated call site и protected readback/readiness; raw provider IDs/credential отсутствуют в source DTO |
...
| `DOM-MC-011`, interaction-gateway provider readback profile | interaction-gateway #235 | `ManageWorkspaceMattermostMapping` bind/relink/unlink | Project является Workspace; signed receipt связывает exact mapping target/stable key/intent, team ref и effect; mapping generation monotonic; graph lock проверяет Workspace→Chat→Session→Turn/delivery | one-use JTI consume + mapping/revoke watermark/receipt/audit одной transaction; open graph relink/unlink отклоняется | #235 владеет Team effect/readback signer, generated call site и joined current mapping readiness |
