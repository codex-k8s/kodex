...
func TestBootstrapComponent(t *testing.T) {
	...
	t.Run("provider queued cancellation preserves snapshot and exact receipt", func(t *testing.T) {
		testProviderQueuedWorkSnapshot(t, ctx, repository)
	})
	t.Run("retry requires fresh target launch before receipt and version", func(t *testing.T) {
		testRetryTargetAuthority(t, ctx, repository)
	})
	t.Run("provider authorization reservation survives replay reauth and delete", func(t *testing.T) {
		testProviderAuthorizationReservation(t, ctx, repository)
	})
	...
	t.Run("effective capability readiness survives public redaction", func(t *testing.T) { testEffectiveCapabilityReadinessRedaction(t, ctx, repository) })
	...
	t.Run("ineligible runtime candidate does not poison healthy claims", func(t *testing.T) {
		testRuntimeCandidateIsolation(t, ctx, repository)
	})
...
`, ownerScope.organizationID, owner.ActorID, sttProviderAccountRef); err != nil {
	...
	if _, err := pool.Exec(ctx, queryProviderSTTDeleting, sttProviderAccountRef); err != nil {
		t.Fatal(err)
	}
	deletingSTT, err := service.GetSystemSTTConfiguration(ctx, owner)
	if err != nil || deletingSTT.Ready || deletingSTT.RevisionRef != sttConfiguration.RevisionRef || !contains(deletingSTT.ReadinessBlockers, "STT_PROVIDER_ACCOUNT_INELIGIBLE") {
		t.Fatalf("deleting account left speech ready or rewrote config: %v", err)
	}
...
func testSessionProviderAffinityAfterPolicyMutation(
	...
	failedContinuation, err := service.GetRun(ctx, owner, continued.Run.Ref)
	if err != nil || failedContinuation.State != "FAILED" {
		t.Fatalf("unobserved credential did not close the continuation: %v", err)
	}
	seedObservedCatalogFixture(t, ctx, repository)
	continued, err = service.Execute(ctx, command.Command{Kind: command.RetryRun, Principal: owner,
		Mutation: value.Mutation{IdempotencyKey: "provider-affinity-retry-observed", ExpectedVersion: &failedContinuation.Version},
		Payload:  command.RunCommandInput{RunRef: failedContinuation.Ref}})
	if err != nil || continued.Run == nil {
		t.Fatalf("retry continuation after fresh observation: %v", err)
	}
...
LIMIT 1`).Scan(&artifactRef, &projectRef); err != nil {
	...
	assertRevokedDelete := prepareDeletedReceiptAuthorityFixture(t, ctx, repository, service, owner, &deleteCommand, "RUNTIME_ENVIRONMENT", first.Ref, first.ProjectRef, "runtime.environment.delete")
...
func testSystemAssistantCorePromptUpgrade(t *testing.T, ctx context.Context, repository *Repository, pool *pgxpool.Pool) {
	...
	const upgradedRevision = "system-assistant-core-v17"
	const upgradedPrompt = "Platform-owned system assistant core prompt revision seventeen."
...
GROUP BY connection.id`, created.Connection.Ref).Scan(&activeTests, &cancelledTests, &enabledGrants); err != nil ||
	...
	assertRevokedDelete := prepareDeletedReceiptAuthorityFixture(t, ctx, repository, service, owner, &deleteCommand, "INTEGRATION", created.Connection.Ref, "", "integration.manage")
	visibleBeforeDelete, err := service.GetIntegrationConnection(ctx, deleteCommand.Principal, created.Connection.Ref)
	if err != nil {
		t.Fatalf("read deleting actor projection: %v", err)
	}
	deleted, err := service.Execute(ctx, deleteCommand)
	if err != nil || deleted.Connection == nil || deleted.Connection.State != "DELETED" ||
		...
		!reflect.DeepEqual(deleted.Connection.Grants, visibleBeforeDelete.Grants) ||
...
func testIntegrationEffectLifecycle(t *testing.T, ctx context.Context, repository *Repository, pool *pgxpool.Pool) {
	...
	gateVersion := int64(1)
	openGate, gateErr := service.GetOwnerGate(ctx, owner, stringMap(rejected, "gateRef"))
	if gateErr != nil || openGate.IntegrationIntent == nil || openGate.IntegrationIntent.ConnectionRef != created.Connection.Ref || len(openGate.DecisionConsequences) != 3 {
		t.Fatalf("integration gate preview missing: err=%v", gateErr)
	}
	...
	if rejection.Gate.IntegrationIntent == nil || rejection.Event == nil || rejection.Event.Delta.Gate == nil || rejection.Event.Delta.Gate.IntegrationIntent == nil || rejection.Gate.DecisionConsequences[1].TerminalForRun {
		t.Fatal("resolved integration gate lost intent or changed consequence")
	}
	replayedGate, replayErr := service.Execute(ctx, command.Command{Kind: command.ResolveOwnerGate, Principal: owner,
		Mutation: value.Mutation{IdempotencyKey: "integration-effect-reject", ExpectedVersion: &gateVersion},
		Payload:  command.GateResolutionInput{GateRef: stringMap(rejected, "gateRef"), Decision: "REJECT", Comment: "Reject exact journal write"}})
	if replayErr != nil || replayedGate.Gate == nil || replayedGate.Gate.IntegrationIntent == nil {
		t.Fatalf("gate receipt lost intent: %v", replayErr)
	}
	gatePage, _, _, pageErr := service.ListOwnerGates(ctx, owner, query.Filter{State: "REJECTED", Page: query.Page{Size: 100}})
	if pageErr != nil {
		t.Fatal(pageErr)
	}
	foundGate := false
	for _, gate := range gatePage {
		if gate.Ref == openGate.Ref {
			foundGate = gate.IntegrationIntent != nil && gate.IntegrationIntent.EffectKey == openGate.IntegrationIntent.EffectKey
		}
	}
	if !foundGate {
		t.Fatal("terminal gate list lost exact intent")
	}
	testGateIntentReadBoundary(t, ctx, repository, owner, *rejection.Gate)
	projectBound := owner
	otherProject, otherErr := service.Execute(ctx, command.Command{Kind: command.CreateProject, Principal: owner, Mutation: value.Mutation{IdempotencyKey: "gate-signed-project"}, Payload: command.ProjectInput{Name: "Other gate project", Purpose: "Signed project boundary", Language: "en"}})
	if otherErr != nil || otherProject.Project == nil {
		t.Fatalf("create signed project fixture: %v", otherErr)
	}
	projectBound.ProjectRef = gateTestProjectID(t, ctx, repository, owner, otherProject.Project.Ref)
	if _, err := service.GetOwnerGate(ctx, projectBound, openGate.Ref); !errors.Is(err, domainerrs.ErrNotFound) {
		t.Fatalf("signed project single gate allowed: %v", err)
	}
	hiddenGates, hiddenTotal, _, err := service.ListOwnerGates(ctx, projectBound, query.Filter{Page: query.Page{Size: 20}})
	if err != nil || len(hiddenGates) != 0 || hiddenTotal != 0 {
		t.Fatalf("signed project gate count leaked: total=%d err=%v", hiddenTotal, err)
	}
	if _, err := service.Execute(ctx, command.Command{Kind: command.ResolveOwnerGate, Principal: projectBound,
		Mutation: value.Mutation{IdempotencyKey: "integration-effect-reject", ExpectedVersion: &gateVersion},
		Payload:  command.GateResolutionInput{GateRef: openGate.Ref, Decision: "REJECT", Comment: "Reject exact journal write"}}); !errors.Is(err, domainerrs.ErrNotFound) {
		t.Fatalf("signed project gate receipt allowed: %v", err)
	}
	gateEvents, _, _, eventErr := service.ListRunEvents(ctx, owner, query.Filter{ResourceRef: openGate.RunRef, Page: query.Page{Size: 100}})
	if eventErr != nil {
		t.Fatalf("integration gate event read: %v", eventErr)
	}
	for _, event := range gateEvents {
		if event.Delta.Gate != nil && event.Delta.Gate.Ref == openGate.Ref && event.Delta.Gate.IntegrationIntent == nil {
			t.Fatal("gate event lost intent")
		}
	}
...
func testScheduleLifecycle(t *testing.T, ctx context.Context, repository *Repository) {
	...
	assertRevokedDelete := prepareDeletedReceiptAuthorityFixture(t, ctx, repository, service, owner, &deleteCommand, "SCHEDULE", archived.Schedule.Ref, archived.Schedule.ProjectRef, "schedule.manage")
	deleted, err := service.Execute(ctx, deleteCommand)
...
func testHumanGateLifecycle(t *testing.T, ctx context.Context, repository *Repository) {
	...
	cardWorkflow, cardErr := service.GetWorkflow(ctx, owner, published.Workflow.Ref)
	if cardErr != nil || cardWorkflow.CardSummary == nil || cardWorkflow.CardSummary.ActiveRunCount != 1 || cardWorkflow.CardSummary.PendingGateCount != 1 || cardWorkflow.CardSummary.LastActivityAt == nil {
		t.Fatalf("workflow gate card projection: %#v %v", cardWorkflow.CardSummary, cardErr)
	}
	cardProject, cardErr := service.GetProject(ctx, owner, project.Project.Ref)
	if cardErr != nil || cardProject.ActiveRunCount != 1 || cardProject.PendingGateCount != 1 || cardProject.LastActivityAt == nil {
		t.Fatalf("project gate card projection: %#v %v", cardProject, cardErr)
	}
	...
	testGateSourceAndHistoricalConsequences(t, ctx, repository, service, owner, *resolved.Gate)
	_, err = service.Execute(ctx, command.Command{Kind: command.ResolveOwnerGate, Principal: owner,
...
func testProviderCredentialRefreshAndCapacity(t *testing.T, ctx context.Context, repository *Repository, pool *pgxpool.Pool) {
	...
	usageAccount, usageErr := service.GetProviderAccountWithUsage(ctx, owner, stringMap(firstLease, "providerAccountRef"), &entity.ProviderAccountUsageContext{Purpose: "LAUNCH", AgentRef: agent.Ref})
	if usageErr != nil || usageAccount.Usage == nil || usageAccount.Usage.ActiveExecutions != 1 || usageAccount.Usage.MaximumConcurrentExecutions != 1 || usageAccount.Usage.Capacity.Reason != "CAPACITY_EXHAUSTED" || !usageAccount.Usage.AllowedToSubmit {
		t.Fatalf("actual claimed lease capacity projection: %v", usageErr)
	}
...
func testProviderCredentialCleanupLifecycle(t *testing.T, ctx context.Context, repository *Repository, pool *pgxpool.Pool) {
	...
	ensureProviderCleanupTerminalLease(t, ctx, repository, service, revokeOwner, defaultAccountID)
	...
	if err != nil || revoked.ProviderAccount == nil || revoked.ProviderAccount.State != "DELETING" || revoked.ProviderAccount.Deletion == nil ||
		revoked.ProviderAccount.Deletion.PendingCleanup != 3 || len(revoked.ProviderAccount.Deletion.Blockers) != 6 {
	...
	if scheduledCount != 3 || acceleratedCount != 3 {
		t.Fatalf("delete cleanup schedule = %d/%d, want 3/3", scheduledCount, acceleratedCount)
	}
	...
	completeProviderAuthorizationCleanupFixture(t, ctx, repository, accountRef)
	if _, err := pool.Exec(ctx, `
	...
	if _, err := repository.CompleteProviderCredentialCleanupTask(ctx, secondTask.Ref,
		"provider-cleanup-component", secondTask.Generation, entity.ProviderAuthorizationCleanupResult{TerminalReceipt: "cleanup-stale-receipt"}); !errors.Is(err, domainerrs.ErrConflict) {
	...
	completed, err := repository.CompleteProviderCredentialCleanupTask(ctx, secondTask.Ref,
		"provider-cleanup-component", reclaimed[0].Generation, entity.ProviderAuthorizationCleanupResult{TerminalReceipt: cleanupReceipt})
	...
	testProviderDeletionTerminalReadback(t, ctx, repository, service, deleteOwner)
}
...
func testProviderAuthRejectionLifecycle(t *testing.T, ctx context.Context, repository *Repository, pool *pgxpool.Pool) {
	...
	if err != nil || pending.ProviderAccount == nil || state != "PENDING_AUTHORIZATION" || currentCredentialID != rotatedCredentialID || pendingVersion != rejectedVersion+1 {
	...
	// До атомарной замены новый attempt не удаляет прежнюю credential.
