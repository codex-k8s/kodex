...
func testRuntimeSecretDraftLifecycle(t *testing.T, ctx context.Context, r *Repository) {
	...
	t.Run("failed_without_effect_leaves_recovery_queue", func(t *testing.T) {
		pendingInput := input
		pendingInput.Name = "draft-no-effect"
		pendingInput.Mutation.IdempotencyKey = "draft-no-effect-save"
		pendingWork := claim(prepare(pendingInput))
		failed, err := s.FinishRuntimeSecretDraft(ctx, principal("operations.fail"), repoport.RuntimeSecretDraftWorkInput{
			Action: "FAIL", OperationRef: pendingWork.OperationRef, ClaimantID: pendingWork.ClaimantID,
			ClaimGeneration: pendingWork.ClaimGeneration, FailureCode: "KUBERNETES_UNAVAILABLE",
		})
		if err != nil || failed.State != "FAILED" {
			t.Fatalf("fail before materialization: %v", err)
		}
		for range 2 {
			recovered, err := finish(pendingWork, "RECOVER", nil, nil)
			if err != nil || recovered.State != "FAILED" || !recovered.Completed || recovered.EncryptedAction != "KEEP" || recovered.MaterializationAction != "KEEP" {
				t.Fatalf("no-effect recovery completion: %+v %v", recovered, err)
			}
			...
			for _, item := range listed {
				if item.OperationRef == pendingWork.OperationRef {
					t.Fatal("completed no-effect cleanup remained in recovery queue")
				}
			}
		}
	})
