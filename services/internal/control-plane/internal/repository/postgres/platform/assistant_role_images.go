...
func (repository *Repository) assistantRoleImageUpdateInput(
	...
	if previous.ProjectRef != payload.ProjectRef || previous.State != "ACTIVE" || shippedRoleImage(previous) {
	...
	baseline, err := repository.roleImageCatalogResolver(entity.RoleEnvironmentSelection{EnvironmentKey: previous.Input.EnvironmentKey})
	if err != nil || roleImageDigest(baseline) != roleImageDigest(previous.Input) {
		return roleimagerepo.ManageInput{}, entity.RoleImageRecipe{}, errs.ErrConflict
	}
	input.Recipe, err = repository.roleImageCatalogResolver(payload.Environment)
...
func (repository *Repository) hydrateAssistantRoleImageUpdate(
	...
	if projectRef == "" || !onlyAssistantFields(operation.Parameters, "projectRef", "recipeRef", "name", "environmentKey") {
	...
	previous := detail.Recipe
	if previous.ProjectRef != projectRef || previous.State != "ACTIVE" || shippedRoleImage(previous) {
	...
	_, _, err = repository.assistantRoleImageUpdateInput(ctx, tx, current, mutation,
		command.AssistantRoleImageUpdateInput{ProjectRef: projectRef, RecipeRef: ref, Name: name,
			Environment: entity.RoleEnvironmentSelection{EnvironmentKey: key}})
	if err != nil {
		return entity.AssistantPlanOperation{}, err
	}
	...
	after["name"], after["environmentKey"] = name, key
...
func rehydrateEditedAssistantRoleImageUpdate(original, edited entity.AssistantPlanOperation) (entity.AssistantPlanOperation, error) {
	if original.Type != "UPDATE_ROLE_IMAGE_RECIPE" || original.Key != edited.Key ||
		...
		!onlyAssistantFields(edited.Parameters, "projectRef", "recipeRef", "name", "environmentKey") ||
	...
	parameters["name"], parameters["environmentKey"] = name, key
