-- name: queries_getagent_select_agents_organization_id_ref_system_key :one
SELECT a.ref,COALESCE(p.ref,''),role.ref,role.name,COALESCE(a.system_key,''),a.name,a.purpose,a.role_description,a.avatar_url,
		COALESCE(avatar.ref,''),COALESCE(a.avatar_artifact_revision,0),a.state,a.enabled,a.version,
		a.runtime_key,r.name,config.provider,config.model,r.runtime_revision,a.capabilities,
		...
		FROM control_plane.agents a LEFT JOIN control_plane.projects p ON p.id=a.project_id JOIN control_plane.role_definitions role ON role.id=a.role_definition_id JOIN control_plane.runtime_profiles r ON r.stable_key=a.runtime_key JOIN control_plane.agent_runtime_config_versions config ON config.id=a.current_runtime_config_id LEFT JOIN control_plane.artifacts avatar ON avatar.id=a.avatar_artifact_id
