-- name: runtime_secret_has_active_operation :one
...
WHERE operation.secret_id = @secret_id::uuid
  AND operation.kind IN ('CREATE', 'ROTATE', 'REVOKE')
  AND operation.state IN ('PREPARED', 'CLAIMED')
