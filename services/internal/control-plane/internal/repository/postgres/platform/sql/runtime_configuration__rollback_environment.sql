...
WITH source AS (
    SELECT source_version.id, source_version.non_secret_values, source_version.secret_descriptors,
           source_version.role_image_artifact_id, source_version.selected_tools, source_version.digest
    ...
    SELECT @version_ref, @organization_id::uuid, @environment_id::uuid, @version_number,
           ...
           source.role_image_artifact_id, source.selected_tools, source.digest, @created_by::uuid
    FROM source
    RETURNING id
