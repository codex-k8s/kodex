...
WHERE account.enabled AND account.state = 'AUTHORIZED' AND definition.enabled
  ...
  AND (EXISTS (SELECT 1 FROM control_plane.provider_account_verifications verification
          WHERE verification.provider_account_id = account.id AND verification.account_version = account.version
            AND verification.provider_credential_revision_id = credential.id
            AND verification.state = 'PENDING' AND verification.model_catalog_task_id IS NULL
            AND verification.deadline > clock_timestamp())
       OR NOT EXISTS (SELECT 1 FROM control_plane.provider_model_catalog_tasks task
          WHERE task.provider_account_id = account.id AND task.state = 'COMPLETED' AND task.account_version = account.version
