...
WITH account AS (
    ...
    ORDER BY provider_account.created_at, provider_account.id
    LIMIT 1
), policy AS (
    ...
    SELECT @policy_ref, @organization_id::uuid, @agent_id::uuid, 1, 'FIXED',
           jsonb_build_array(jsonb_build_object('accountRef', account.ref, 'weight', 1)),
           encode(digest(convert_to('FIXED', 'UTF8') || decode('00', 'hex') ||
                         convert_to('[{"accountRef":"' || account.ref || '","weight":1}]', 'UTF8') || decode('00', 'hex'), 'sha256'), 'hex'),
           @created_by::uuid
    FROM account
