...
func (repository *Repository) authorizeCommand(ctx context.Context, tx pgx.Tx, current scope, input command.Command) error {
	...
	if err := repository.authorizeManagedRoleImageSource(ctx, tx, current, input); err != nil {
		return err
	}
	switch input.Kind {
	...
	if input.Kind == command.AddSessionTurn {
		return repository.authorizeSessionTurnTarget(ctx, tx, current, input)
	}
...
func (repository *Repository) commandAccessTarget(ctx context.Context, tx pgx.Tx, current scope, input command.Command) (string, resolvedAccessTarget, error) {
	...
	case command.ProviderAccountInput:
		...
		switch input.Kind {
		case command.CancelProviderAccountQueuedWork:
			permission = "provider.account.view"
		case command.StartProviderDeviceAuth, command.AuthorizeProviderAPIKey, command.RefreshProviderAuthorization, command.VerifyProviderAuthorization:
