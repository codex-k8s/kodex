...
| Terminal success/error | Текущий владелец lease с exact fence | Agent-runner сохраняет bounded sanitized Markdown в signed handoff с exact Session/Turn/attempt/input/runtime revision/artifact ID+version+SHA. Runtime-controller проверяет digest/lineage и передаёт его generated `CompleteRuntimeExecution`; одна owner transaction блокирует execution, lease, turn, attempt, exact current `ProcessRun`, создаёт Artifact/result binding и durable interaction deliveries, отзывает claims/grants и закрывает применимые occurrence/ScheduledRun | `RUNNING/ADMITTED -> SUCCEEDED/FAILED`; Turn и ProcessRun получают совместимый terminal state, lease закрыта, fence возрастает; delivery rows получают тот же terminal owner commit | Audit + authoritative read RuntimeExecution/Artifact и отдельные `FINAL_MARKDOWN`, `PUBLISH_ARTIFACT`, `STATUS`, `RUN_CARD`, при failure `INCIDENT`; provider receipt возвращается exact fenced командой | Transaction не коммитит terminal RuntimeExecution/Turn либо частичный result envelope при живом ProcessRun, неверном digest или недоступном delivery row. Только первый terminal CAS побеждает; replay возвращает receipt только для exact current outcome без successor |
...
| Scheduled producer и current digest | #193/#221, `ARCH-MC-004`, `DOM-MC-009` | `automation-scheduler` получает exact workload/SPIFFE и schedule permissions; schedule/occurrence/process/Turn IDs и digest назначает сервер | `ManageSchedule` → `ClaimDueSchedules` → `ClaimScheduleOccurrence`; затем `agent-runner` вызывает `ClaimTurn`, а `runtime-controller` — runtime claim/admit | До materialization `Schedule.EffectiveInputSHA` и queued occurrence фиксируют immutable snapshot target/prompt/artifact/revision/session policy. После materialization occurrence переключается на exact execution digest Turn, snapshot сохраняется отдельно в immutable `ScheduledRun.EffectiveInputSHA256`, current — в `ScheduledRun.CurrentInputSHA256`. Repository `UPDATE` явно сохраняет `ScheduleOccurrence.EffectiveInputSHA256`; adapter fake повторяет список изменяемых SQL-полей | Due eligibility и next-run используют PostgreSQL clock. Claim key + stable semantic receipt; recovery и overlap `SKIP` сначала отдельными owner transactions фиксируют terminal/retry/dead-letter, authority cleanup и audit, после чего отдельная selection transaction повторяет idempotency и под occurrence→Schedule locks проверяет отсутствие другой active occurrence или любого historical open run. Некорректная нематериализованная queued-строка после полного rollback отдельной owner transaction повторно проверяется на отсутствие receipt, blocker, claim и execution binding, получает `DEAD_LETTER`+audit и не блокирует следующий schedule. Повреждённый expired claimed graph не терминализируется частично: occurrence→Schedule locks, PostgreSQL deadline и receipt occurrence+attempt фиксируют один blocked-recovery audit, после чего maintenance продолжает backlog. Только затем snapshot пересчитывается и Session/RuntimeRevision/Turn/Process/occurrence/run фиксируются атомарно. `ARCHIVE/UPDATE/DELETE` до receipt требуют пустой authoritative occurrence/run open-set; `PAUSED` допускает queued retry, но новый claim ждёт `ACTIVATE` | Schedule outbox возникает только при реальном `Schedule.Version` bump (`CREATE`, due watermark, manage transition) и имеет sequence новой версии; occurrence/run transitions дают audit + authoritative read и не создают новый AsyncAPI fact. No-next poll не отменяет уже committed maintenance fact | Stale snapshot/current digest, tuple, generation, fence, deadline, historical open run, закрытый Schedule либо duplicate aggregate/version sequence закрывают materialization; replay не повторяет Turn/lease/attempt/audit/outbox; старая scheduler lease/receipt после requeue не возвращается; concurrent winner запрещает quarantine |
