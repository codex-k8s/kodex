...
| Scheduled producer и current digest | #221, `ARCH-MC-004`, `DOM-MC-005` | `automation-scheduler` получает exact workload/SPIFFE и schedule permissions; schedule/occurrence/process/Turn IDs и digest назначает сервер | `ManageSchedule` → `ClaimDueSchedules` → `ClaimScheduleOccurrence`; затем `agent-runner` вызывает `ClaimTurn`, а `runtime-controller` — runtime claim/admit | До materialization `Schedule.EffectiveInputSHA` и queued occurrence фиксируют immutable snapshot target/prompt/artifact/revision/session policy. После materialization occurrence переключается на exact execution digest Turn, snapshot сохраняется отдельно в immutable `ScheduledRun.EffectiveInputSHA256`, current — в `ScheduledRun.CurrentInputSHA256`. Repository `UPDATE` явно сохраняет `ScheduleOccurrence.EffectiveInputSHA256`; adapter fake повторяет список изменяемых SQL-полей | Claim key + stable semantic receipt; schedule snapshot повторно вычисляется под locks, затем Session/RuntimeRevision/Turn/Process/occurrence/run фиксируются атомарно. `FAILED` completion и scheduler lease expiry завершают прежний run, восстанавливают queued occurrence digest из pinned snapshot этого run и очищают старый execution/claim tuple единым helper. Более новая Schedule snapshot не подменяет pinned значение: requeue закрыто отклоняется | Schedule outbox возникает только при реальном `Schedule.Version` bump (`CREATE`, due watermark, manage transition) и имеет sequence новой версии; occurrence/run transitions дают audit + authoritative read и не создают новый AsyncAPI fact | Stale snapshot/current digest, tuple, generation, fence, deadline либо duplicate aggregate/version sequence закрывают всю transaction; replay не повторяет Turn/lease/attempt/audit/outbox; старая scheduler lease/receipt после requeue не возвращается |
...
| `FAILED -> QUEUED` либо scheduler lease expiry `-> QUEUED` | Новый queued digest восстанавливается только из immutable `ScheduledRun.EffectiveInputSHA256`; locked current Schedule обязана всё ещё иметь тот же snapshot | Прежний ScheduledRun сначала получает terminal outcome; `requeueScheduledOccurrence` увеличивает attempt ровно на один, очищает claim key/token/lease/generation и весь execution tuple, сохраняет snapshot через тот же SQL UPDATE. Следующая materialization создаёт новый execution digest и новый run | Более новая/иная Schedule snapshot, stale attempt/token, неterminalized run или старый execution digest дают полный rollback; прежний claim receipt больше не раскрывает token. Exact completion receipt допускается только пока current row остаётся именно этой queued result и до successor claim |
...
| `CompleteScheduleOccurrence`, `CancelScheduleOccurrence`, scheduler lease recovery | read-only occurrence по ID либо expired candidate | по candidate current Turn вызывается общий resolver, поэтому возможный RuntimeExecution предшествует occurrence; затем exact occurrence ID/attempt/token/deadline recheck. Requeue использует уже locked Schedule и immutable snapshot уже locked ScheduledRun, не ищет более ранние rows | OwnerGate берётся последним; queued cancel и exact completion-receipt replay после requeue используют подмножество occurrence→schedule без execution rows. Requeue очищает старый execution/claim tuple и сохраняет snapshot digest одной OCC записью |
