...
`CLAIMED`, `WAITING_OWNER`, `CONTINUATION`, `FAILED`.
...
| `CompleteTurn`, `RetryTurn`, `CancelTurn`, stale `ClaimTurn` | read-only Turn; stale lease query не содержит `FOR UPDATE` | общий resolver до любого Turn/lease effect; `WAITING_OWNER` и `FAILED` occurrence не теряются; stale lease/deadline повторно проверяется после locks | attempt/lease/claim и возможный OwnerGate идут после ProcessRun; retry создаёт свежий tuple |
...
| scheduler claim, overlap `SKIP/FORBID`, schedule delete | только `QUEUED` occurrence либо read-only open predicate | execution graph ещё не существует; применяется совместимое подмножество schedule/queued occurrence, а открытые `WAITING_OWNER/CONTINUATION` запрещают claim/skip/delete | новые Session/Turn/Process создаются после claim; существующий graph не блокируется в обратном порядке |
