...
| Heartbeat/renew | Владелец той же admitted lease и exact workload/SPIFFE | Row locks exact RuntimeExecution и зависимого TurnLease, общий attempt/input/generation, token digest и PostgreSQL clock; обе lease части продлеваются одной transaction до одного deadline | `ADMITTED/RUNNING -> RUNNING`, version/fence возрастают, общий deadline движется только вперёд | Authoritative read | Несовпавшие либо истёкшие RuntimeExecution/TurnLease, stale fence/attempt/generation получают `FailedPrecondition`; generic `RenewTurn` не является runtime authority, новый grant не оживляет прежнюю lease |
...
| `ManageWorkClaim(CREATE/RENEW/RELEASE)` | CREATE получает caller Turn только как данные проверки; RENEW/RELEASE читают unlocked WorkClaim candidate и выводят Turn owner-side | общий resolver блокирует Session→Turn→ProcessRun и более ранние runtime/scheduled rows, если они есть; после него повторно сверяются current lineage, owner, workload, version и expiry | WorkClaim блокируется/создаётся после graph; terminal RuntimeExecution запрещает replay и mutation |
...
| integration decision/begin/terminal/materialization | read-only IntegrationContinuation snapshot | source RuntimeExecution и общий graph, затем pinned Integration/Credential | IntegrationContinuation последняя; terminal materialization и scheduled rebind остаются в той же transaction |
...
| legacy Session/Turn/Process/WorkClaim/OwnerGate/Schedule | Общий owner graph resolver и явная RuntimeExecution disposition выполняются до receipt; `WorkClaim(RENEW)` дополнительно проверяет PostgreSQL `ExpiresAt`, bounded expiry без exact target не раскрывает receipt прошлой строки | Только exact current resource/result; live WorkClaim может replay-ить RENEW, потерянный ответ expiry без доказуемого current candidate закрывается | Live RuntimeExecution для generic terminal, истёкший WorkClaim, non-`REJOINED` session continuation, иной current Turn/process/gate/occurrence или successor |
...
| `ManageSession` | CLOSE/CANCEL допускают runtime-absent unscheduled open graph; ARCHIVE/CLEANUP только без open graph | Запрещён: команда откатывается и требует specialized runtime transition | Open WAITING/SUSPENDED graph и любая non-`REJOINED` continuation блокируют retention; terminal closed history не оживляется |
...
27. Потерять ответ `ManageWorkClaim(RENEW)` и повторить до expiry — получить
    exact current receipt. Повторить на границе/после PostgreSQL expiry и новым
    key: оба пути закрыто отказываются, claim не воскресает.
