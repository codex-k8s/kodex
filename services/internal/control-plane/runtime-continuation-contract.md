...
просроченной lease или gate выполняется без row lock. Общий
`lockOwnerGraphByTurn` затем получает строки в этом порядке и повторно сверяет
...
| legacy Turn/Process/WorkClaim/OwnerGate/Schedule | Общий owner graph resolver и явная RuntimeExecution disposition выполняются до receipt; bounded expiry без exact target не раскрывает receipt прошлой строки | Только exact current resource/result; потерянный ответ expiry без доказуемого current candidate закрывается, а не возвращает чужой terminal result | Live RuntimeExecution для generic terminal, иной current Turn/process/gate/occurrence или successor |
