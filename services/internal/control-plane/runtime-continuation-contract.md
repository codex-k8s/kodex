...
| Scheduled producer и current digest | #221, `ARCH-MC-004`, `DOM-MC-005` | `automation-scheduler` получает exact workload/SPIFFE и schedule permissions; schedule/occurrence/process/Turn IDs и digest назначает сервер | `ManageSchedule` → `ClaimDueSchedules` → `ClaimScheduleOccurrence`; затем `agent-runner` вызывает `ClaimTurn`, а `runtime-controller` — runtime claim/admit | До materialization `Schedule.EffectiveInputSHA` и queued occurrence фиксируют immutable snapshot target/prompt/artifact/revision/session policy. После materialization occurrence переключается на exact execution digest Turn, snapshot сохраняется отдельно в initial fields `ScheduledRun.EffectiveInputSHA256`, current — в `ScheduledRun.CurrentInputSHA256` | Claim key + stable semantic receipt; schedule snapshot повторно вычисляется под locks, затем Session/RuntimeRevision/Turn/Process/occurrence/run фиксируются атомарно. ClaimTurn propagation проверяет current digest и версии до lease/receipt | Schedule outbox возникает только при реальном `Schedule.Version` bump (`CREATE`, due watermark, manage transition) и имеет sequence новой версии; occurrence/run transitions дают audit + authoritative read и не создают новый AsyncAPI fact | Stale snapshot/current digest, tuple, generation, fence, deadline либо duplicate aggregate/version sequence закрывают всю transaction; replay не повторяет Turn/lease/attempt/audit/outbox |
