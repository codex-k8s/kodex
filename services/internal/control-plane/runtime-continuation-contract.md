...
Документ фиксирует границу GitHub Issues
[#221](https://github.com/codex-k8s/matter-codex/issues/221) и
[#231](https://github.com/codex-k8s/matter-codex/issues/231). Он дополняет
...
а `runtime-controller`, `integration-gateway` и `agent-runner` не
...
| Terminal success/error | Текущий владелец lease с exact fence | Agent-runner сохраняет bounded sanitized Markdown и для scheduled execution закрытый outcome `no_action/action_taken/requires_human/failed` в signed handoff с exact Session/Turn/attempt/input/runtime revision/occurrence/artifact ID+version+SHA. Runtime-controller проверяет digest/lineage и передаёт generated `CompleteRuntimeExecution`; одна owner transaction блокирует execution, lease, turn, attempt, exact current `ProcessRun`, создаёт Artifact/result binding, отзывает claims/grants и закрывает применимые occurrence/ScheduledRun. Durable delivery создаётся только после owner-side проверки snapshotted notification policy и exact RoomID | Обычный path: `RUNNING/ADMITTED -> SUCCEEDED/FAILED`; `requires_human`: RuntimeExecution `SUSPENDED`, Turn/ProcessRun/occurrence/run `WAITING_OWNER` и один OwnerGate. Lease закрыта, fence возрастает | Audit + authoritative read RuntimeExecution/Artifact. `no_action`/`AUDIT_ONLY` не создают Mattermost work; eligible `action_taken/failed` создают exact-room delivery. OwnerGate доставляет `interaction-gateway` | Произвольный outcome/route/policy, живой ProcessRun, неверный digest или недоступный обязательный delivery row откатывают transaction. Только первый terminal/gate CAS побеждает; replay возвращает receipt только для exact current outcome без successor |
...
| Independent restore proof | Только `runtime-restore-verifier`, exact SPIFFE, отдельные audience, `RUNTIME_RESTORE_VERIFIER_GRANT`, readiness profile и закрытый набор `restore.verify|restore.bind|rehydrate.complete`; archive writer, OIDC и cleanup authorizer не подходят | Row lock; exact execution/revision/input/attempt/generation и archive checksum; workload, SPIFFE, grant generation, proof reference/checksum сохраняются append-only | Restore state `VERIFIED`, version/fence возрастают; rehydrate использует те же exact operation bindings | Authoritative read contract и verifier deployable/readiness материализованы в #231 | Неизвестный operation либо несовпавший workload/SPIFFE/grant закрыто отклоняется; control-api-gateway не может подписать proof |
...
| Scheduled producer и current digest | #193/#221, `ARCH-MC-004`, `DOM-MC-009` | `automation-scheduler` получает exact workload/SPIFFE и organization-scoped discovery grant; project/occurrence/process/Turn IDs и digest назначает сервер. Unbound grant не имеет execution authority | Owner `ManageSchedule`/`RunScheduleNow`; polling `ClaimDueSchedules` → reservation `ClaimScheduleOccurrence` → exact-capability `MaterializeScheduleOccurrence`; затем `agent-runner` вызывает `ClaimTurn`, а `runtime-controller` — runtime claim/admit/complete | Первый/следующий `next_run_at` вычисляет owner по PostgreSQL clock; manual occurrence watermark не меняет. Durable cursor выбирает project. `RESERVED` хранит one-time materialization capability exact project/occurrence/attempt/input/generation/full method/workload/SPIFFE; materialization потребляет её, создаёт один ProcessRun для AGENT/PLAYBOOK и выдаёт отдельную completion capability. Retry сохраняет root ProcessRun и создаёт fresh attempt/Turn/RuntimeRevision/grant. `scheduled-result.v1` закреплён одним snapshot в RuntimeRevision/Turn и проходит generated read path до runner input | Short-lived unbound bearer JTI/revision/digest исключены только из semantic receipt, но transport replay и exact actor/org/workload/SPIFFE/full-method/permission сохраняются. Bound capability JTI/digest/generation остаются authority. Eligibility применяется до bounded `LIMIT`; overlap batch ограничен. Invalid potentially-live recovery получает `RECOVERY_BLOCKED`, не потребляет watchdog LIMIT и разрешается owner command только с exact version/attempt/evidence | Schedule/occurrence/capability/run transitions имеют audit и authoritative read; AsyncAPI Schedule fact отсутствует | Caller project, чужая/повторно потреблённая capability, stale snapshot/current tuple/generation/fence/deadline, historical open run либо закрытый Schedule закрыто отклоняются; rotation exact replay не повторяет effect |
| Restore/cleanup boundary | #221/#231, `GUIDE-DOC-003/006` | Отдельные signed grants `RUNTIME_RESTORE_VERIFIER_GRANT`, `RUNTIME_RESTORE_EFFECT_GRANT` и `RUNTIME_CLEANUP_AUTHORIZER_GRANT`; runtime-controller получает только exact materialization/consume operations | `/VerifyRuntimeRestore`, `/BindRuntimeRestoreTarget`, `/CompleteRuntimeRehydrate`, `/AuthorizeRuntimeRestoreEffect`, cleanup RPC и protected readiness каждого профиля | RuntimeExecution owner row; attestation pin-ит archive checksum/execution/revision/input/attempt/generation; effect-side transaction повторно сверяет current generation/digest и durable revoke watermark | OCC version/fence + restore/cleanup generation; admission и каждый effect profile расходуют отдельный exact generation slot, а replay повторно читает owner state; PostgreSQL clock управляет expiry/reissue | Authoritative read; verifier/effect deployable и issuer profiles материализованы в #231, cleanup сохраняет отдельный fail-closed profile | OIDC/control-api-gateway, archive writer, чужой SPIFFE, stale checksum/proof/generation, `PENDING` restore ticket, foreign/mixed owner rows и consumed reissue получают закрытый отказ |
...
| --- | --- | --- | --- |
| `QUEUED/RESERVED`, ещё не materialized | `QUEUED`: execution/capability binding пуст; `RESERVED`: execution tuple пуст, claim binding указывает exact materialization capability | `ClaimDueSchedules` вставляет плановый snapshot; `RunScheduleNow` — отдельный immediate snapshot без изменения watermark. `ClaimScheduleOccurrence` только резервирует; `MaterializeScheduleOccurrence` после exact capability lock одним `materializeScheduledOccurrence` переключает digest | Unbound execution, чужая capability, любой execution residue, иной snapshot, caller-owned project/anchor или unknown/mixed tuple отклоняется до Session/Turn/Process effect |
| `CLAIMED`/`WAITING_OWNER`/`CONTINUATION` | `ScheduleOccurrence.EffectiveInputSHA256 == Turn.EffectiveInputSHA256 == ScheduledRun.CurrentInputSHA256`; `ScheduledRun.EffectiveInputSHA256` неизменно хранит исходный schedule snapshot. Waiting/run outcome равен `requires_human` | `rebindScheduledOccurrence` атомарно меняет полный current Session/Turn/Process/RuntimeRevision tuple и execution digest для retry, OwnerGate и integration successor; scheduled continuation сохраняет occurrence ID в RuntimeExecution/handoff | Ручное частичное присваивание, incomplete binding, stale run/occurrence tuple, несовпавший current digest или свободный outcome запрещены source guard |
...
| Terminal без requeue | Current execution digest и полный predecessor tuple остаются readback provenance; ScheduledRun хранит snapshot/current и только `no_action/action_taken/failed` | Completion/owner decision/expiry/cancel/dead-letter очищают scheduler authority и сохраняют terminal occurrence/run атомарно | Terminal row нельзя превратить в queued ручным SQL; `requires_human` не terminal, произвольный outcome запрещён; retry создаётся только утверждённым path |
...
| --- | --- | --- |
| первый Mattermost turn | `interaction-gateway` принимает событие только после проверки transport signature, регистрирует входной Artifact и вызывает `ManageSession`/`EnqueueTurn`. Control-plane одной owner transaction/receipt назначает Session, свежую `RuntimeRevision`, Turn/attempt и immutable input digest; `ClaimRuntimeExecution` затем создаёт первую server-owned execution tuple до появления role Pod | Payload не назначает control IDs, actor, provider account или runtime identity; неизвестный conversation mapping, несовпавший artifact digest/version, stale idempotency либо недоступная materialization boundary закрыто отклоняются до claim |
...
32. Выполнить полный scheduled producer path `ManageSchedule(CREATE)` →
    ...
    lease/attempt/audit/outbox. Schedule не должен создавать AsyncAPI/outbox
    fact; fake/repository обязан отклонить второй
