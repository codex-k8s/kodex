...
| Scheduled producer и current digest | #193/#221, `ARCH-MC-004`, `DOM-MC-009` | `automation-scheduler` получает exact workload/SPIFFE и organization-scoped grant; project/occurrence/process/Turn IDs и digest назначает сервер | Owner `ManageSchedule`/`RunScheduleNow`; polling `ClaimDueSchedules` → `ClaimScheduleOccurrence`; затем `agent-runner` вызывает `ClaimTurn`, а `runtime-controller` — runtime claim/admit/complete | Первый/следующий `next_run_at` вычисляет owner по PostgreSQL clock; manual occurrence watermark не меняет. Durable cursor выбирает project до project-scoped transaction. Queued occurrence хранит immutable schedule snapshot; после materialization current Turn digest переходит в occurrence/run. Retry сохраняет один root ProcessRun и создаёт fresh attempt/Turn/RuntimeRevision/grant | Short-lived unbound bearer JTI/revision/digest исключены только из semantic receipt, но transport replay и exact actor/org/workload/SPIFFE/full-method/permission сохраняются. Eligibility paused/FORBID/open run применяется до bounded `LIMIT`. Recovery отдельной owner transaction восстанавливает broken binding из coherent ScheduledRun либо dead-letter-ит доказанно отсутствующий graph; potentially live graph не терминализируется частично. Maintenance effect учитывается после commit независимо от последующего `NotFound` | Schedule/occurrence/run transitions имеют audit и authoritative read; AsyncAPI Schedule fact отсутствует, `automation-scheduler` не объявлен NATS consumer | Caller project, stale snapshot/current tuple/generation/fence/deadline, historical open run, закрытый Schedule либо semantic mutation закрыто отклоняются; rotation exact replay не повторяет effect, старая scheduler lease после requeue не возвращается |
...
| --- | --- | --- | --- |
| `QUEUED`, ещё не materialized | `ScheduleOccurrence.EffectiveInputSHA256 == Schedule.EffectiveInputSHA`; execution tuple, claim key/token/generation/lease пусты | `ClaimDueSchedules` вставляет плановый snapshot; `RunScheduleNow` — отдельный immediate snapshot без изменения watermark. `ClaimScheduleOccurrence` под occurrence→Schedule locks повторно вычисляет snapshot и одним `materializeScheduledOccurrence` переключает digest | Любой execution/claim residue, иной snapshot, caller-owned project/anchor или unknown/mixed tuple отклоняется до Session/Turn/Process effect |
