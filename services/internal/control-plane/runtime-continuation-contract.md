...
| Turn claim/current tuple | `agent-runner`, exact SPIFFE и `controlplane.turn.claim`; grant pin-ит server-owned Turn/attempt/input/generation | После canonical owner graph locks и проверки старой exact binding переход `QUEUED -> CLAIMED` одним helper переносит новую `Turn.Version` в `ProcessRun.Current*` и, если запуск scheduled, в `ScheduleOccurrence.Execution*`/`ScheduledRun.Current*`; обновлённая `ProcessRun.Version` также попадает в обе scheduled строки. Unscheduled path не создаёт schedule rows | Turn и ProcessRun получают ровно по одному OCC bump; schedule attempt/state не меняются, lease fence равен новой Turn version | Claim receipt, audit и применимые outbox records фиксируются в той же transaction; следующий `ClaimRuntimeExecution` читает уже согласованный tuple | Любое исходное расхождение ProcessRun/occurrence/run, stale grant/input/fence либо конкурентный version bump откатывает Turn, ProcessRun, lease, attempt и receipt целиком; exact replay не создаёт новый bump |
