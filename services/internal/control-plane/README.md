...
| Управляемая конфигурация | target `RoleDefinition`/`Agent`/`InstructionSet`/`ProviderPool` хранит server-assigned `managed_by=UI|GIT`; обычный UI create/update не назначает Git ownership, Git-owned объект изменяет только exact reconcile RPC с immutable source/revision/digest; `InstructionSet` detach атомарно очищает source binding, copy создаёт новую UI entity; legacy `ROLE`/`PROMPT_PROFILE` общие detach/copy не принимают                                                                                                                                                                                                                                                             |
...
| Ревизия среды исполнения | перед каждой новой target-сессией/ходом сервер повторно разрешает active `AgentAssignment`, Agent, published InstructionSet, ProviderPool, server-owned RuntimeProfile и exact credential binding; создаётся immutable target `RuntimeRevision`, а в отдельной таблице — immutable derived ROLE/PROMPT projection для существующего exact `runtime-controller` read; projection не даёт legacy mutation authority                                                                                                                                                                                                                                                         |
...
| Расписание               | owner-friendly binding принимает stable selections, а сервер lock/resolve exact Agent/Instruction/Runtime и active Workspace/Room `AgentAssignment`; каждая materialization для `NEW`, `PERSISTENT` и `ROLLING` повторяет assignment resolution и stale/revoked fail closed; остальной Schedule lifecycle, recovery, retry/dead-letter, overlap и polling остаются одним источником истины #231                                                                                                                                                                                                                                                                                                             |
| Owner-конфигурация       | Agent pause/resume и enable/disable — отдельные commands; bot bind/rebind/revoke и mapping bind/relink/unlink принимают только typed interaction-gateway receipt; provider ref — typed integration-gateway receipt; mapping relink/unlink отклоняется при open Workspace→Chat→Session→Turn/delivery graph; Incident release фактически закрывает runtime graph; Run timeline листается stable cursor `(occurred_at,id)`; backup/restore terminal выполняет bounded in-process reconciler, а не browser                                                                                                                                                       |
