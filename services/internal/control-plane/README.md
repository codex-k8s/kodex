...
| Ревизия среды исполнения | перед каждой новой target-сессией/ходом сервер повторно разрешает active `AgentAssignment`, Agent, published InstructionSet, ProviderPool, RuntimeProfile и credential через единый selector с expiry headroom/freshness/capacity/durable weighted slot; `RuntimeRevision` хранит target tuple в top-level digest/readback, а Components содержит только принятые runtime-controller kinds и immutable derived ROLE/PROMPT; Prompt pin-ит CLEAN content Artifact, который materializes как `AGENTS.md`, без legacy mutation authority                                                                                                                                              |
...
| Расписание               | `CreateScheduleFromOwnerSelections` принимает stable Agent/Instruction/Pool/Room и display name Artifact и под locks назначает ID и exact Workspace/Runtime/Assignment tuple; `BindScheduleConfiguration` остаётся update-only; каждая materialization для `NEW`, `PERSISTENT` и `ROLLING` повторяет assignment resolution и сравнивает тот же Session tuple; stale/revoked fail closed, остальной lifecycle #231 остаётся единственным источником истины                                                                                                                                                                                         |
| Owner-конфигурация       | Agent state actions специализированы; provider receipts включают exact protected target/intent и one-use consume в owner transaction; bot/mapping producer ожидает #235, Git/provider producer — #236; mapping relink/unlink отклоняется при open graph; Incident release закрывает runtime graph, retry требует соответствующий terminal execution; Run lineage охватывает root/children/all attempts/artifacts со stable cursor; backup включает все исторические Session с exact archive либо целиком откатывается                                                                                                                                           |
