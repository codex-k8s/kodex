...
ALTER TABLE control_plane.projects
    DROP CONSTRAINT projects_lifecycle_check,
    ADD COLUMN deleted_at timestamptz,
    ADD COLUMN purge_after timestamptz,
    ADD COLUMN deleted_by uuid REFERENCES control_plane.subjects(id),
    ADD CONSTRAINT projects_lifecycle_check CHECK (
        lifecycle IN ('ACTIVE', 'ARCHIVED', 'TRASHED', 'PURGE_PENDING')
    ) NOT VALID,
    ADD CONSTRAINT projects_trash_consistency CHECK (
        (lifecycle IN ('ACTIVE', 'ARCHIVED')
            AND deleted_at IS NULL AND purge_after IS NULL AND deleted_by IS NULL)
        OR (lifecycle IN ('TRASHED', 'PURGE_PENDING')
            AND deleted_at IS NOT NULL AND purge_after > deleted_at AND deleted_by IS NOT NULL)
    ) NOT VALID;

ALTER TABLE control_plane.projects VALIDATE CONSTRAINT projects_lifecycle_check;
ALTER TABLE control_plane.projects VALIDATE CONSTRAINT projects_trash_consistency;

CREATE INDEX projects_trash_retention
    ON control_plane.projects (purge_after, id)
    WHERE lifecycle IN ('TRASHED', 'PURGE_PENDING');

-- Общий каталог обязан скрывать все project-scoped дочерние ресурсы,
-- включая результаты поиска по организации и прямые ссылки на объект.
-- Операторская корзина читает проект отдельным ограниченным путём.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION control_plane.catalog_resource_visible(
...
AS $$
    SELECT EXISTS (
        ...
        WHERE binding.organization_id = tenant AND binding.state = 'ACTIVE'
          AND (target_project IS NULL OR EXISTS (
              SELECT 1 FROM control_plane.projects project
              WHERE project.id = target_project AND project.organization_id = tenant
...
-- Некоторые read paths используют view напрямую без повторного вызова
-- catalog_resource_visible. В ней тоже нужен тот же project fence.
CREATE OR REPLACE VIEW control_plane.catalog_access_targets WITH (security_invoker = true) AS
...
JOIN control_plane.projects project ON project.id = agent.project_id AND project.lifecycle NOT IN ('TRASHED', 'PURGE_PENDING')
...
JOIN control_plane.projects project ON project.id = workflow.project_id AND project.lifecycle NOT IN ('TRASHED', 'PURGE_PENDING')
...
WHERE (run.project_id IS NULL AND run.target_type = 'SYSTEM_ASSISTANT') OR project.lifecycle NOT IN ('TRASHED', 'PURGE_PENDING')
UNION ALL
...
LEFT JOIN control_plane.agents agent ON run.target_type = 'AGENT' AND agent.ref = run.target_ref AND agent.organization_id = run.organization_id
LEFT JOIN control_plane.workflows workflow ON run.target_type = 'WORKFLOW' AND workflow.ref = run.target_ref AND workflow.organization_id = run.organization_id
WHERE artifact.project_id IS NULL OR project.lifecycle NOT IN ('TRASHED', 'PURGE_PENDING')
UNION ALL
...
LEFT JOIN control_plane.projects project ON project.id = schedule.project_id
...
WHERE schedule.lifecycle_state <> 'DELETED'
  AND (schedule.project_id IS NULL OR project.lifecycle NOT IN ('TRASHED', 'PURGE_PENDING'));
