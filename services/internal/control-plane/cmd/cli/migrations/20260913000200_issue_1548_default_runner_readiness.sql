...
CREATE FUNCTION control_plane.agent_runtime_contract_ready(
 ...
 p_default_role_image_digest text
...
WITH requested(ref) AS (
    ...
    JOIN control_plane.image_artifacts artifact
     ...
     AND (artifact.signature_identity <> 'platform-owned-bootstrap'
          OR artifact.manifest_digest = p_default_role_image_digest)
...
$$;
-- +goose StatementEnd
REVOKE ALL ON FUNCTION control_plane.agent_runtime_contract_ready(uuid,uuid,text[],bigint,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION control_plane.agent_runtime_contract_ready(uuid,uuid,text[],bigint,text,text) TO control_plane_runtime;
RESET ROLE;
