...
-- Однократно удаляем накопленные просроченные контексты. В рабочем пути
-- каждая PostgreSQL-сессия далее обслуживает только собственные записи и не
-- конфликтует с SERIALIZABLE-транзакциями других реплик control-plane.
...
BEGIN
    ...
    IF stored_version <> 20260814000100 THEN
        RAISE EXCEPTION 'control-plane runtime context contention source version is invalid'
            USING ERRCODE = '55000';
    END IF;
...
-- +goose Down
-- Forward-only: возврат глобальной очистки в hot path небезопасен.
SELECT 1;
