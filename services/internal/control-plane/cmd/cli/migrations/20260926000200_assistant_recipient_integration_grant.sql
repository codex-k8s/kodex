...
CREATE OR REPLACE FUNCTION control_plane.assistant_context_projection(
...
WITH resources AS (
    ...
    UNION ALL
    SELECT 'SCHEDULE', target.kind, target.organization_id, target.ref, target.id, target.project_id,
           target.owner_id, target.related_ids, schedule.name, schedule.version, 'schedule.view'
    FROM control_plane.catalog_access_targets target
    JOIN control_plane.schedules schedule ON target.kind='SCHEDULE' AND schedule.id=target.id
      AND schedule.organization_id=target.organization_id AND schedule.ref=target.ref
    WHERE schedule.lifecycle_state='ACTIVE'
    UNION ALL
    ...
    WHERE resource.organization_id=tenant AND resource.context_kind=assistant_context_projection.context_kind
      ...
      AND (resource.context_kind<>'SCHEDULE' OR resource.project_id=conversation_project)
    ...
    ('','PUBLISH_INTEGRATION_DEFINITION','organization.manage',3),
    ...
    ('PROJECT','CREATE_RUNTIME_ENVIRONMENT_DRAFT','project.manage',5),
    ('PROJECT','CREATE_ROLE_IMAGE_RECIPE','image.build',6),
    ('PROJECT','UPDATE_ROLE_IMAGE_RECIPE','image.build',7),
    ('AGENT','UPDATE_AGENT','agent.manage',1), ('AGENT','CHANGE_CAPABILITY','agent.manage',2),
    ('AGENT','LAUNCH_RUN','agent.launch',3), ('AGENT','ARCHIVE_AGENT','agent.manage',4),
    ('AGENT','BIND_AGENT_RUNTIME_ENVIRONMENT','agent.manage',5),
    ('AGENT','CREATE_INSTRUCTION_DRAFT','agent.manage',6),
    ('ENVIRONMENT','PREPARE_RUNTIME_ENVIRONMENT_REVISION','project.manage',1),
    ('WORKFLOW','UPDATE_WORKFLOW','workflow.manage',1), ('WORKFLOW','LAUNCH_RUN','workflow.launch',2),
    ('WORKFLOW','ARCHIVE_WORKFLOW','workflow.manage',3),
    ('SCHEDULE','UPDATE_SCHEDULE','schedule.manage',1),
    ('INTEGRATION_CONNECTION','UPDATE_INTEGRATION_CONNECTION','integration.manage',1),
    ('INTEGRATION_CONNECTION','TEST_INTEGRATION_CONNECTION','integration.manage',2)
)
SELECT eligible.project_id, eligible.project_ref, eligible.name, eligible.version,
       ARRAY(SELECT operation.operation FROM operations operation
             WHERE operation.context_kind=eligible.context_kind
               AND (operation.operation <> 'PREPARE_RUNTIME_ENVIRONMENT_REVISION' OR eligible.project_id IS NOT NULL)
               ...
               AND (operation.operation NOT IN ('CREATE_ROLE_IMAGE_RECIPE','UPDATE_ROLE_IMAGE_RECIPE') OR (
                   control_plane.catalog_resource_visible(tenant,actor,'image.source.view',eligible.kind,eligible.id,
                       eligible.project_id,eligible.owner_id,eligible.related_ids,evaluated_at)
                   AND control_plane.catalog_resource_visible(tenant,actor,'image.source.manage',eligible.kind,eligible.id,
                       eligible.project_id,eligible.owner_id,eligible.related_ids,evaluated_at)))
       ...
       || CASE WHEN eligible.context_kind IN ('AGENT','WORKFLOW') AND EXISTS (
            SELECT 1 FROM control_plane.integration_grant_admission(tenant,actor,signed_project,
                '',eligible.project_ref,eligible.context_kind,eligible.ref,'','GRANT',NULL) admission WHERE admission.reason='READY'
          ) THEN ARRAY['CHANGE_INTEGRATION_GRANT'] ELSE '{}'::text[] END
