...
func TestNodePullRepositoriesAreClosedToBootstrapAndRuntime(t *testing.T) {
	t.Parallel()
	repositories := []string{"mattercodex/agent-runner", "mattercodex/roles"}
	for _, path := range []string{
		"/v2/mattercodex/agent-runner/manifests/sha256:abc",
		"/v2/mattercodex/roles/blobs/sha256:abc",
	} {
	...
	for _, path := range []string{
		"/v2/mattercodex/control-plane/manifests/sha256:abc",
		"/v2/evidence/role-image-admission/manifests/sha256:abc",
		"/v2/mattercodex/roles/tags/list",
	} {
		if pathInRepositories(path, repositories) {
