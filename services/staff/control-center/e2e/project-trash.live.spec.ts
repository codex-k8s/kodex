...
test("диагностика доступности корзины", async ({ page }) => {
  ...
  const response = page.waitForResponse(
    (item) => new URL(item.url()).pathname === "/api/v1/projects/trash",
  );
  ...
  expect(
    ...
    JSON.stringify({
      code: body.code,
      query: new URL(result.url()).search,
      plainStatus: plain.status(),
      plainCode: plainBody.code,
    }),
  ).toBe(200);
});

test("проект с сотрудником проходит корзину, восстановление и физическое удаление", async ({
  page,
}) => {
  const credentials = {
    username: process.env.KODEX_E2E_OWNER_USERNAME ?? "",
  ...
  const projectName = `local-trash-${String(Date.now())}`;
  ...
  await create
    ...
    .fill("Изолированная проверка корзины и дочерних объектов");
  ...
  await page.goto("/projects");
  const active = page
    .locator(".project-list__item")
    .filter({ hasText: projectName });
  await expect(active).toBeVisible();
  await active.getByRole("button", { name: "Переместить в корзину" }).click();
  await page
    .getByRole("dialog", { name: "Переместить в корзину" })
    .getByRole("button", { name: "Переместить в корзину" })
    .click();
  ...
  await page
    .getByRole("dialog", { name: "Восстановить Проект" })
    .getByRole("button", { name: "Восстановить Проект" })
    .click();
  ...
  await expect(
    page.getByText(`${projectName}-agent`, { exact: true }).first(),
  ).toBeVisible();
  ...
  await purge
    .getByLabel("Название проекта для подтверждения")
    .fill(projectName);
