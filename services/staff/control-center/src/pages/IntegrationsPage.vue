...
import type { IntegrationConfiguration } from "@/shared/api/generated/openapi/types.gen";
...
const selected = ref<IntegrationConfiguration | null>(null);
...
function edit(value?: IntegrationConfiguration): void {
...
async function save(): Promise<void> {
  ...
  if (!def || !conn || form.capabilities.length === 0) return;
  const value = selected.value;
  const ok = await store.saveIntegration(
    {
      ...(value ? { configurationRef: value.configurationRef } : {}),
      stableKey: form.stableKey.trim(),
      definitionRef: def.definitionRef,
      definitionVersion: def.version,
      definitionDigestSha256: def.digestSha256,
...
async function test(value: IntegrationConfiguration): Promise<void> {
  const ok = await store.runIntegrationTest({
    connectionRef: value.connectionRef,
    ...
    <ModalDialog
      ...
      ><form class="form-grid" @submit.prevent="save">
        <label class="form-field form-field--full"
          ...
          ><input v-model="form.stableKey" required maxlength="80" /></label
        ><label class="form-field"
          ...
          ><select v-model="form.connectionRef" required>
            ...
            <option
              v-for="item in store.connections.data.filter(
                (connection) => connection.state === 'VALID',
              )"
