...
async function save(): Promise<void> {
  ...
  const ok = await store.saveInstruction(
    {
...
async function command(
  item: Resource,
  action: "VALIDATE" | "PUBLISH" | "ROLLBACK" | "DETACH" | "COPY" | "ARCHIVE",
  targetVersion?: number,
): Promise<void> {
  if (
    !window.confirm(
      t("instructions.confirmAction", { action, name: item.name }),
    )
  )
    return;
  await store.saveInstruction(
    {
      action,
      resourceRef: item.id,
      ...(targetVersion ? { targetVersion } : {}),
      ...(action === "COPY" ? { name: `${item.name}-copy` } : {}),
    },
    item.version,
  );
}
                ...
                <td>
                  <div class="data-table__actions">
                    ...
                    ><button
                      class="button button--text"
                      type="button"
                      @click="command(item, 'VALIDATE')"
                    >
                      <FileCheck2 :size="14" aria-hidden="true" />{{
                        $t("instructions.validate")
                      }}</button
                    ><button
                      v-if="item.spec.instructionSet?.validationSucceeded"
                      class="button button--text"
                      type="button"
                      @click="command(item, 'PUBLISH')"
                    >
      ...
      ><div class="section-stack">
        ...
        <div class="timeline">
          ...
          >
            ...
            <button
              v-if="entry.resource.version !== selected?.version"
              class="button button--text"
              type="button"
              @click="
                selected &&
