...
const availableConnections = computed(() =>
  store.connections.data.filter((item) => item.state === "VALID"),
);

async function beginPool(value?: ProviderPoolView): Promise<void> {
  if (value) {
    await store.loadConfigurationSource(value.poolRef, "PROVIDER_POOL");
    if (
...
async function archivePool(value: ProviderPoolView): Promise<void> {
  ...
  await store.savePool(
    { action: "ARCHIVE", poolRef: value.poolRef },
    value.version,
  );
}
  ...
  <div class="page">
    ...
    <ModalDialog
      ...
      ><form class="form-grid" @submit.prevent="savePool">
        <label class="form-field"
          ><span>{{ $t("providers.stableKey") }}</span
          ><input v-model="poolForm.stableKey" required maxlength="80" /></label
        ><label class="form-field"
          ><span>{{ $t("common.name") }}</span
          ><input
