...
export function consumePendingBrowserIntent(
  storage: Pick<Storage, "getItem" | "removeItem">,
  now = Date.now(),
): OidcIntent {
  const raw = storage.getItem(oidcReauthIntentStorageKey);
  storage.removeItem(oidcReauthIntentStorageKey);
  // Только возврат к форме; purpose и полномочия проверяет BFF по своей login transaction.
  return raw === null
    ? { kind: "login" }
    : parseReauthIntent(JSON.parse(raw), now);
}
