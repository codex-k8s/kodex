...
watch(
  ...
  ([projectRef, ref], _previous, onCleanup) => {
    ...
    void unwrap(
      ...
      .catch(() => {
        if (!controller.signal.aborted) agentProblem.value = true;
      });
  },
  { immediate: true },
);
...
<template>
  <div class="assistant-agent-environment-binding-form">
    <p class="assistant-plan-friendly__hint">
      {{ $t("assistant.planEditor.bindingBoundary") }}
    </p>
    <p
      v-if="agentProblem || environmentProblem"
      class="field-error"
      role="alert"
    >
    ...
    <p
      v-if="agent && agent.version !== operation.value.expectedVersion"
      class="field-error"
      role="alert"
    >
      {{ $t("assistant.planEditor.bindingStale") }}
    </p>
    <p v-if="beforeEnvironmentName">
      {{
        $t("assistant.planEditor.bindingCurrent", {
          environment: beforeEnvironmentName,
        })
      }}
    </p>
    <label class="field">
      ...
      <AsyncEntityPicker
        ...
        :selected="
          environment && { ref: environment.ref, title: environment.name }
        "
    ...
    <p
      v-if="environment && !environment.ready"
      class="field-error"
      role="alert"
    >
...
<style scoped>
.assistant-agent-environment-binding-form {
  display: grid;
  gap: 12px;
}
</style>
