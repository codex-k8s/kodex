...
const providerSubmissionAllowed = ref(false);
...
const providerUsageContext = computed(() => ({
  purpose: "CONFIGURE" as const,
  agentRef: props.agentRef,
  runtimeProfileRef: form.runtimeProfileRef,
  providerDefinitionKey: selectedProvider.value,
  model: form.model,
  reasoningEffort:
    view.value && form.model
      ? overlayEffort(
          view.value.publishedOverlay.content,
...
watch(
  ...
  () => {
    providerSubmissionAllowed.value = false;
  },
  { flush: "sync" },
);
watch(
  () => form.providerAccounts.map((item) => item.accountRef).join("|"),
  () => {
    providerSubmissionAllowed.value = false;
  },
  { flush: "sync" },
      ...
      <article class="runtime-panel panel">
            ...
            :label="
              $t(
                selectedRuntime?.ready
                  ? 'providerUsage.profileAvailable'
                  : 'providerUsage.profileUnavailable',
              )
            "
          />
        </div>
        ...
        <section class="runtime-panel__account-capability">
          ...
          <ProviderAccountSelector
            ...
            :usage-context="providerUsageContext"
            :disabled="!canEdit || busy"
            @eligibility-state-change="providerAccountEligibility = $event"
            @submission-allowed-change="providerSubmissionAllowed = $event"
          />
        </section>
...
.runtime-panel__account-heading > [role="status"] {
  white-space: nowrap;
  flex-shrink: 0;
}
