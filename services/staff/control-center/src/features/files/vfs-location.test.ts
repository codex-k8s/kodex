...
describe("VFS navigation location", () => {
  ...
  it("разрешает возврат только в глобальный VFS или файлы текущего проекта", () => {
    const filtered =
      "/projects/one/files?view=vfs&vfsState=DELETED&vfsKinds=SKILL,MEMORY";
    expect(safeVfsReturn(filtered, "one")).toBe(filtered);
    ...
    for (const value of [
      ...
      "/files?vfsState=UNKNOWN",
      "/files?vfsKinds=UNKNOWN",
      "/files?vfsState=ACTIVE&vfsState=DELETED",
      "/\\external.invalid/files",
    ])
