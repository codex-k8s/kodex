...
| `deploy/base/integration-gateway-public/**` | Public `Ingress` для GitHub webhook route `integration-gateway`; защита только HMAC, не OAuth. |
...
| `cmd/bootstrap-operational-acceptance` | Read-only операционная приёмка после deploy: Kubernetes workloads, jobs, secrets, registry, public web/webhook contour, HTTPS/OAuth redirect и HMAC-only webhook reject без раскрытия значений env. |
...
`Certificate` для `platform.kodex.works`, создаёт публичный `Ingress` только на
`oauth2-proxy` для UI и отдельный HMAC-only `Ingress` на
`/v1/provider-webhooks/github` для `integration-gateway`. Прямой публичный
`Ingress` на `web-console` запрещён: если OAuth secret не может быть создан из
существующего Kubernetes `Secret` или env seed-полей, команда завершается до
создания публичных `Ingress`.
...
- `web`: `web-console`.
- `web-public`: Traefik/cert-manager foundation, `oauth2-proxy` перед
  `web-console` и HMAC-only GitHub webhook route в `integration-gateway`.
...
- для `web-public` ring не запускает registry, Kaniko, PostgreSQL и backend
  migrations; он применяет только public web/webhook-contour;
...
`oauth2-proxy`, HTTPS/OAuth redirect и HMAC-only GitHub webhook ingress.
...
## Операционная приёмка после deploy

После применения нужных rings и перед ручной проверкой UI владелец или агент
запускает read-only приёмку текущего кластера:

```bash
go run ./cmd/bootstrap-operational-acceptance \
  --env-file bootstrap/host/config.env
```

Команда не меняет кластер: не выполняет `kubectl apply`, не запускает jobs, не
собирает и не пушит образы. Она проверяет:

- `services.yaml` и builder/registry image refs через `libs/go/stackinventory`;
- доступность Kubernetes API `/readyz`;
- готовность deployments, PostgreSQL statefulset и обязательных migration jobs;
- отсутствие активных или failed backend build jobs;
- наличие Service объектов и сохранение внутренних сервисов как `ClusterIP`;
- наличие обязательных ключей в `kodex-postgres`, `kodex-platform-runtime` и
  `kodex-web-oauth2-proxy` без вывода значений;
- доступность локального registry endpoint на loopback;
- операторскую HTTP-поверхность через локальный `kubectl port-forward`:
  `staff-gateway` OpenAPI, `web-console` root и `oauth2-proxy` ping;
- что public web `Ingress` ведёт только на `oauth2-proxy`, а не напрямую на
  `web-console`;
- что public GitHub webhook `Ingress` ведёт только на `integration-gateway` по
  точному path `/v1/provider-webhooks/github` и не попадает в `oauth2-proxy`;
- готовность `Certificate` и `ClusterIssuer`;
- что публичный `web-console` root редиректит на GitHub OAuth без вывода
  query/state;
- что неподписанный запрос к публичному GitHub webhook endpoint получает
  `401/signature_invalid`, а не OAuth redirect.

Если команда завершилась строкой `OK: operational acceptance completed`,
платформа готова к ручной UI-проверке владельца через
`https://platform.kodex.works`. Интерактивный вход owner-аккаунтом не входит в
автоматическую приёмку: агент не должен печатать cookie, OAuth `state`, токены
или содержимое callback.

## Регистрация GitHub webhook

Публичный endpoint для репозитория `codex-k8s/kodex`:

```text
https://platform.kodex.works/v1/provider-webhooks/github
```

Этот endpoint не находится за OAuth. Единственная защита входа — GitHub
`X-Hub-Signature-256`, body limit, route rate limit/backpressure и разрешённый
`provider_slug=github`. Значение `KODEX_GITHUB_WEBHOOK_SECRET` хранится в
Kubernetes `Secret` `kodex-platform-runtime` и не попадает в `ConfigMap`,
документы, Issue, PR или логи.

Регистрацию webhook выполняет владелец или бот с достаточными правами GitHub
после применения `--ring web-public`. Секрет не выводить и не копировать в
документы; команда ниже передаёт его только через переменную процесса:
...
WEBHOOK_SECRET="$(
  kubectl -n "$KODEX_PRODUCTION_NAMESPACE" \
    get secret kodex-platform-runtime \
    -o jsonpath='{.data.KODEX_GITHUB_WEBHOOK_SECRET}' | base64 -d
)"

GH_TOKEN="${KODEX_GIT_BOT_TOKEN:-${KODEX_GITHUB_PAT:-}}" \
  gh api repos/codex-k8s/kodex/hooks \
    --method POST \
    -f name=web \
    -f active=true \
    -f events[]=push \
    -f events[]=pull_request \
    -f config[url]=https://platform.kodex.works/v1/provider-webhooks/github \
...
GitHub repository webhook не фильтрует `push` по ветке на уровне регистрации.
`provider-hub` получает safe envelope и оставляет бизнес-фильтрацию `main` в
owner-контуре. Лишние GitHub events не включать.
