...
| `bootstrap/host/deploy_backend_ring.sh` | Реальное применение backend-колец, `staff-gateway`, `platform-mcp-server` и `web-console`: registry, Kubernetes `Secret`, Kaniko-сборки, PostgreSQL, миграции и сервисы выбранного кольца. |
...
`staff-gateway`, `platform-mcp-server` и `web-console` не входят в `all`, чтобы
повторный backend deploy не менял edge-, MCP- и frontend-контуры без явного
выбора оператора.
...
через локальный `kubectl port-forward`.
