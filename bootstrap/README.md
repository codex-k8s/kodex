...
| `cmd/bootstrap-operational-acceptance` | Read-only операционная приёмка после deploy: Kubernetes workloads, jobs, secrets, registry, public web contour и HTTPS/OAuth redirect без раскрытия значений env. |
...
- что public `Ingress` ведёт только на `oauth2-proxy`, а не напрямую на
  `web-console`;
