...
функциональные и архитектурные границы MatterCodex. `docs/idea/**` и
`docs/strategy/**` сохраняют исторический контекст и не переопределяют
...
- До отдельного решения владельца о завершении прототипа активен профиль
  `Prototype`: быстрые форматирование, статический разбор, компиляция/сборка и
  проверка diff, а также небольшие быстрые unit-тесты только для действительно
  сложной чистой или security-critical логики.
- В профиле `Prototype` отсутствие полного test coverage, integration/E2E,
  contract/deploy/render/lifecycle/oracle suites и общего baseline не является
  блокером PR и не может становиться review finding. Reviewer проверяет
  production-код, достижимые сценарии и границы доверия, но не требует
  отсутствующую тяжёлую оснастку как условие принятия прототипа.
- Простому glue-коду тесты ради покрытия не добавляются. Тяжёлые harnesses,
  fixtures и test scripts не включаются в функциональный PR прототипа.
- Полная поддерживаемая стратегия тестирования проектируется и вводится
  отдельной owner-approved волной после готовности прототипа:
...
- Перед staging/production владелец отдельно активирует поддерживаемый профиль,
  включающий применимые lint, contract, build, unit, integration, security и
  use case проверки; само описание будущего контура не делает его обязательным.
- Архивная или экспериментальная проверка не становится обязательной без
  внесения в `GOV-DOC-003` и публичную точку входа CI.
