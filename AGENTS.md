...
- До отдельного решения владельца не считать выбранной модель изоляции agent run: один namespace `mattermost` или отдельный namespace на run.

## Документация

- Активные продуктовые и архитектурные решения хранить в `docs/strategy/**`.
- Исходные документы идеи находятся в `docs/idea/**`.
- Документы должны описывать целевое состояние и явно помечать открытые решения.
