...
Платформа находится в активной стадии разработки и проходит программу рефакторинга новой версии.
Главная текущая каноника: `refactoring/task.md` и `refactoring/README.md`.

Старые sprint/epic и product baseline документы перенесены в `docs/deprecated/pre-refactor/**` и оставлены только для исторического анализа.
Новые волны реализации пишутся с нуля в новых сервисах и каталогах целевой архитектуры, без доработки устаревшего кода как стартовой базы.

Пока репозиторий опубликован без права форка.
...
Для понимания исторически реализованных возможностей можно посмотреть следующие этапы и связанные артефакты:
...
По факту этапы выше были выполнены менее чем за сутки. Это исторический пример старого delivery-подхода; новая модель проектируется в `refactoring/**`.
Далее в старой модели шли этапы `qa -> release -> postdeploy -> ops`: каждый запускался своим label на задаче и выполнялся отдельным агентом со своей ролью и инструкциями.
...
Исторически `kodex` запускал агентные роли по GitHub-лейблам (`run:*`) и вёл задачу по stage-модели:
