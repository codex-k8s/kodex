...
`docs/external/**` хранит необязательные submodule-ссылки на внешние репозитории документации. Содержимое этих репозиториев не копируется в активную канонику `docs/**`, но остаётся доступным рядом с ней для синхронизации и справки.

Внешние пакеты документации считаются reusable baseline. Локальная копия в `docs/design-guidelines/**` может дополнять их проектными требованиями `kodex`, если эти требования не подходят для общего пакета.
