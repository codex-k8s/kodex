...
Если фактический хеш совпадает, простаивающий pod можно переиспользовать. Если изменились переменные окружения, авторизация, образ, подключенные тома, `PriorityClass`, ServiceAccount либо memory requests/limits, перед захватом следующего хода пересоздается только pod. PVC, неизменяемый session token Secret и архивы не удаляются и не пересоздаются. Несовместимый уже существующий utility Job закрыто останавливает запуск до отдельной сверки: его нельзя молча переиспользовать вне действующего guard. Изменения не прерывают выполняющийся ход.
...
Все поддерживаемые session и utility pod получают один отдельный `PriorityClass` и одинаковые memory request/limit. Scoped `ResourceQuota` с областью этого класса ограничивает сумму обоих значений. Допуск выполняет Kubernetes ResourceQuota admission, поэтому конкурентные запросы не обходят суммарную границу. Допустимые BinarySI Quantity сначала приводятся к KiB и ограничиваются значением `9007199254740991`; переполнение или насыщение запрещено одинаково в shell, render и Go.

Недоверенный agent pod получает только namespace ServiceAccount для чтения. Он не может создавать Pod/Job, изменять или удалять scoped `ResourceQuota` и менять или удалять `PriorityClass`; штатного `cluster-admin` credential в shared-node runtime нет. Сохранённое назначение роли с `kubernetes_access=cluster-admin` остаётся фактом доменной политики и аудита, но его материализация в этом runtime закрыто отклоняется до создания Kubernetes workload. Административный сценарий требует отдельного управляемого шлюза вне недоверенного pod.

Допуск также учитывает доступные ресурсы узла, requests, квоту namespace, ожидающие pod и класс ресурсов. При нехватке ресурсов запуск остается в очереди с понятной причиной. Перед отказом контроллер может удалить самый старый прогретый простаивающий pod, не затрагивая активные сессии.
