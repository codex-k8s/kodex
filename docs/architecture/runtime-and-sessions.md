...
В переходном контуре MVP канонический типизированный манифест и его SHA-256 сохраняются в неизменяемой `RuntimeRevision`. Манифест содержит только безопасные идентификаторы, ссылки на Kubernetes Secret и необратимые отпечатки конфигурации; значения Secret, `auth.json`, kubeconfig и session token в него не входят. Каждый ход ссылается на свою ревизию, а сессия отдельно хранит желаемую и подтверждённо применённую ревизии.

При несовпадении digest выполняющийся ход не прерывается. Простаивающий pod удаляется только по точному имени и UID и создаётся заново с теми же PVC и основным либо неизменяемым session-token Secret. Очередь ходов и workspace сохраняются. Если точную конфигурацию отложенной ревизии нельзя безопасно воспроизвести, запуск закрывается с ошибкой, а не использует другую ревизию.
...
Метаданные сессии хранятся в PostgreSQL. Целевой контракт предусматривает архив сессии в S3-совместимом хранилище с контрольной суммой и версией. До реализации S3-переноса переходный контур MVP хранит ограниченный по размеру неизменяемый архив в PostgreSQL; PVC остаётся обязательной сохраняемой рабочей копией и не удаляется автоматически.

После каждого хода runner:

1. завершает процесс поставщика;
2. собирает архив сессии;
3. вычисляет контрольную сумму и размер ограниченного архива;
4. одной транзакцией сохраняет новую неизменяемую версию архива, завершает `Turn` и обновляет метаданные сессии;
5. публикует события результата и outbox.

Перед восстановлением runner проверяет размер и контрольную сумму подтверждённой версии, повторно валидирует структуру tar/gzip и только затем атомарно заменяет каталог `sessions`. Ошибка проверки не изменяет уже существующий каталог.
...
Полный эксплуатационный контракт и шлюз миграции определены в `docs/operations/runtime-retention.md`. Воспроизводимый безопасный сценарий переходного контура описан в `docs/runbooks/runtime-session-revision.md`.
