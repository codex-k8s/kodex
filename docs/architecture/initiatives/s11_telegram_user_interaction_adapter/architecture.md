...
| Raw webhook intake и secret-token verification | Telegram adapter contour | Telegram Bot API | Telegram-specific webhook authenticity завершается до входа в `codex-k8s`; `api-gateway` не обязан разбирать raw Telegram updates | Webhook/auth contract, normalized callback family |
