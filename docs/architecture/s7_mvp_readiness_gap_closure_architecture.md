...
| S7-E13 `run:qa:revise` policy coverage | Product policy + `control-plane` stage resolver | SA + QA + Dev | Contract impact в label/stage policy path | Runtime impact: добавить revise route в stage transitions | Риск конфликтов label set; mitigation: resolver precedence + `need:input` fallback |
