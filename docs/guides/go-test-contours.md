...
## Статус в фазе прототипа

Описанные ниже полные контуры сохраняются как проектный материал для будущей
поддерживаемой стратегии, но до отдельного решения владельца о завершении
прототипа не являются обязательным baseline функциональных PR. Отсутствие
`make test-go`, PostgreSQL matrix, integration/E2E/contract/deploy/render,
lifecycle/oracle suites или полного покрытия не блокирует прототип и не должно
становиться review finding.

В активном профиле выполняются быстрые компиляция/сборка/форматирование/diff и
небольшие unit-тесты только сложной чистой либо security-critical логики.
Простой glue-код не получает тест ради покрытия. Полное переутверждение,
упрощение и реализация контуров выполняются отдельной owner-approved волной:
...
Оснастка работает только внутри кластера, требует уже созданный явно тестовый namespace с меткой `mattercodex.dev/disposable-postgres-tests=true` и не создаёт namespace, RBAC или ServiceAccount. Через `client-go` она создаёт только Job и внутренний ClusterIP Service с уникальной run identity, отключённым токеном ServiceAccount, ограниченными ресурсами и без добавленных Linux capabilities. Job имеет server-owned `activeDeadlineSeconds`, нулевой `backoffLimit` и `ttlSecondsAfterFinished`; Service принадлежит точному UID Job, поэтому deadline, TTL-контроллер и garbage collector прекращают полномочие и очищают ресурсы после SIGKILL/OOM runner. Готовность Pod подтверждается `pg_isready`, после чего контроллер проверяет реальное подключение к PostgreSQL через loopback `port-forward`; ранний отказ backend или завершение tunnel приводят к ограниченному контекстом повтору без произвольной задержки. Перед обычным удалением Job и Service повторно сверяются label и точный UID; Kubernetes UID precondition запрещает удалить replacement. Если политика или права тестового namespace отсутствуют, режим имеет исход `NOT RUN`. Промышленный namespace и production apply/deploy этим контуром запрещены.
...
Дайджесты проверены 2026-07-21 непосредственно через Docker Registry HTTP API V2 для `registry-1.docker.io`; один ref нельзя переиспользовать для другой major-версии. Контейнер получает уникальные имя и lease-label, случайный порт только на `127.0.0.1` и не получает общий том. Закреплённый образ запускается через прямой `timeout` без shell, с `--rm`, `--init`, ограниченным временем остановки и независимым self-deadline; persistent custom network не создаётся. Поэтому завершение по сроку или OOM очищается Docker daemon без callback runner. Перед обычным `docker rm --force` сверяются точный container ID, lease и auto-remove contract; replacement не удаляется. Недоступный daemon даёт `NOT RUN`, а не успех. Kubernetes mode использует те же закреплённые ref и отвергает mutable tag или неверное соответствие major-версии до создания Job.
...
Оснастка отклоняет промышленную конечную точку и базу данных, известные DSN выполнения и миграций, канонические базы данных, внешнюю конечную точку без разрешения и одноразового доказательства, а также несовпадающую основную версию. После admission дочерняя команда получает минимальный allowlist переменных среды: `MATTERCODEX_DATABASE_DSN`, `MATTERCODEX_MIGRATIONS_DATABASE_DSN`, `MATTERCODEX_POSTGRES_*`, `PG*`, `DATABASE_URL`, `POSTGRES_*` и управляющие matrix/proof-переменные родителя не наследуются. До замены `HOME` trusted controller запускает `go env` с `GOENV=off`, пустым `GOFLAGS` и `GOWORK=off`, материализует абсолютные default `GOMODCACHE`, `GOCACHE` и `GOPATH` исходного server-owned home и передаёт их child. Внешние cache paths игнорируются; точное безопасное значение `GOPROXY=off` можно сохранить для доказательства повторного запуска без сети. В дочерний процесс передаются только эти toolchain/cache paths, scoped disposable DSN/marker, обязательная major-версия и sentinel.
...
| Архив сессии | Перед упаковкой исходные session-файлы атомарно санитизируются. Лимиты до чтения и `WriteHeader`: 8 MiB на файл, 32 MiB суммарно, 512 файлов и 1024 записи tar, включая корень `sessions`, каталоги, regular files и каждый допустимый header. Restore сначала полностью проверяет USTAR directory-root, canonical порядок, уникальность, parents, типы и пределы, затем повторно извлекает только в private sibling staging. Target не изменяется при ошибке; успешное дерево публикуется одним `renameat2` exchange/noreplace с проверяемым rollback. |
...
Единый inventory запуска строится из фактически выданных `env`/`extraEnv` и credential-файлов GitHub, OpenAI/Codex, Kubernetes ServiceAccount, MCP-сессии, `KUBECONFIG` и других scoped `_FILE` источников. Все пути канонизируются и дедуплицируются. До чтения действуют server-owned пределы: 512 canonical sources, 8 MiB совокупного содержимого, 8192 производных представления и 96 MiB их совокупного размера; 513 записей `KUBECONFIG` отклоняются. Любое непустое sensitive-значение сохраняется в inventory. Значение короче 16 байт считается неподдерживаемым для безопасного поиска фрагментов и закрыто отклоняет запуск до child, публикации и сети; короткая общая строка не передаётся глобальному replacer. Точное kubelet-managed значение `/var/run/secrets/kubernetes.io/serviceaccount/token` является ожидаемо ротируемым источником: runner ограниченно опрашивает его во время child process, сохраняет каждое замеченное значение в inventory и делает обязательный финальный readback до санитизации. Невозможность безопасно собрать этот набор закрыто отклоняет результат.

Перед первым чтением source создаётся fail-closed event guard для файла и его parent boundary. События create/write/delete/rename/replace, ошибка watcher и переполнение очереди отменяют subprocess; барьерное событие перед запуском и после завершения доказывает обработку уже поставленных в очередь событий. Финальная metadata/digest-проверка остаётся дополнительной, но не является источником истории ротации.

Каждая запись `KUBECONFIG` разбирается типизированным `client-go`: inline token/password/client certificate/key data входят в inventory, а `tokenFile`, `client-key`, `client-certificate` и `certificate-authority` снимаются в private copies и переписываются внутри нового kubeconfig. `exec`, `auth-provider` и неизвестные extensions отклоняются до child. Значения файлов копируются в уникальный каталог `0700` как файлы `0600`; `KUBECONFIG`, `MATTERCODEX_GITHUB_TOKEN_FILE`, scoped `_FILE` и весь per-run `CODEX_HOME` в child env переписываются на снимок. Runtime `auth.json` тоже находится под event guard. Исходные пути, которые child всё ещё технически способен открыть, контролируются весь период `cmd.Run`. Для всех project/provider credentials любое небезопасное событие навсегда помечает текущий запуск, удаляет raw staging и session source и запрещает events, final, archive, status, completion и любой последующий HTTP-вызов этого запуска. Единственное исключение - штатная ротация точного kubelet-managed ServiceAccount token: она не отменяет turn, но все замеченные версии токена проходят тот же bounded redaction boundary до любой публикации.
