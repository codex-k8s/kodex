...
В профиле `Prototype` production-дефект виден из статического достижимого пути
и остаётся замечанием без проверки в живом окружении. Отсутствие полного
покрытия и integration/E2E/contract/deploy/render/lifecycle suite само по себе
замечания не образует. Быстрые format/build/static/diff и небольшие тесты
сложной чистой либо security-critical логики выполняются по `GOV-DOC-003`;
полный контур отложен в Issue #216.
