...
6. активный профиль из `GOV-DOC-003` и только фактически выполненные быстрые
   проверки;
7. ссылку на follow-up тестовой стратегии, если полное покрытие и тяжёлые
   suites отложены.

В профиле `Prototype` структура unit не обязана включать test harness,
fixtures, integration/E2E/contract/deploy/render/lifecycle/oracle suites или
полный baseline. Их отсутствие не является нарушением структуры и не может
становиться review finding. Допустимы небольшие быстрые unit-тесты сложной
чистой или security-critical логики; простому glue-коду тест ради покрытия не
нужен. Полный поддерживаемый контур отслеживается в
