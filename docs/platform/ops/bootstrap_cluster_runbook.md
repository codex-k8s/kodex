...
- `cmd/bootstrap-backend-deploy` выполняет реальный deploy выбранного backend-кольца:
  ...
  не запускаются PostgreSQL foundation и backend migrations. Значения env, DSN,
...
12. Проверить, что завершились jobs `kodex-build-*`,
...
13. При необходимости повторить проверку первого кольца без пересборки:
...
Фронтенд разворачивается только как внутренний `web-console` Service через
явный `--ring web`. `staff-gateway` не входит во второе backend-кольцо и не
...
повторном backend deploy. Публичный ingress для `web-console` не выбирается в
этом контуре. Ingress controller и cert-manager остаются отдельной
