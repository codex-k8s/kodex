...
- `cmd/bootstrap-deploy-plan` также проверяет готовность `self-deploy` до
  rollout `agent-manager`: `KODEX_AGENT_MANAGER_SELF_DEPLOY_SIGNAL_PROJECT_ID`
  должен быть заполнен UUID, signal consumer, governance gate и build dispatch
  должны быть включены, а rendered `agent-manager` manifest должен содержать
  обязательные env/Secret refs. При `--require-kubernetes` команда проверяет
  наличие нужных ключей в `kodex-platform-runtime` без вывода значений.
...
3. Выполнить план backend deploy:
   ...
   Для финального `self-deploy` rollout этот шаг обязан пройти до применения
   `agent-manager`; если не заполнен project id, сначала выполнить штатный
...
5. Если k3s и foundation уже установлены, выполнить план backend deploy с
   ...
   refs, service accounts, self-deploy service tokens, `agent-manager`
   self-deploy flags, runtime Kubernetes executor refs, source auth SecretRef,
   Kaniko builder image из `services.yaml`, rendered surface `agent-manager` и
   `runtime-manager`, а также наличие read-only команды
   `cmd/self-deploy-chain-acceptance`. Команда печатает строки `READINESS`
   только со статусами, safe codes и следующим действием; значения env,
   `Secret`, DSN, kubeconfig и manifest payload не выводятся.
6. Для готовности live `self-deploy` после изменения access bootstrap выполнить
...
7. Перед финальным live self-deploy прогоном выполнить read-only проверку
   цепочки:
...
8. Финальная live-проверка `self-deploy` выполняется одной последовательностью:
   ...
   не используются.
9. После явного разрешения владельца выполнить install, если registry
   foundation ещё не применён.
10. Для первого backend-кольца выполнить:
   `bash bootstrap/host/deploy_backend_ring.sh --env-file <env>`.
11. Для второго backend-кольца выполнить:
...
17. После применения всех нужных rings выполнить операционную приёмку:
...
18. При необходимости повторить проверку первого кольца без пересборки:
...
- Если план backend deploy падает с `self_deploy_*` или `runtime_executor_*`
  safe code, сначала выполнить указанное `next_action`: обычно это
  idempotent `onboarding-runner --apply`, заполнение защищённого env-файла,
  подготовка `kodex-platform-runtime` Secret key или применение runtime
  manifest/RBAC штатным deploy path. Значения Secret и полный manifest не
  выводить в отчётах.
