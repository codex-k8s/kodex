...
7. После явного разрешения владельца выполнить install, если registry
   foundation ещё не применён.
8. Для первого backend-кольца выполнить:
   `bash bootstrap/host/deploy_backend_ring.sh --env-file <env>`.
9. Для второго backend-кольца выполнить:
...
15. После применения всех нужных rings выполнить операционную приёмку:
...
16. При необходимости повторить проверку первого кольца без пересборки:
