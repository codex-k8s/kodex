...
5. Если k3s и foundation уже установлены, выполнить план backend deploy с
   ...
   refs и self-deploy service tokens без изменения кластера.
...
8. После явного разрешения владельца выполнить install, если registry
   foundation ещё не применён.
9. Для первого backend-кольца выполнить:
   `bash bootstrap/host/deploy_backend_ring.sh --env-file <env>`.
10. Для второго backend-кольца выполнить:
...
16. После применения всех нужных rings выполнить операционную приёмку:
...
17. При необходимости повторить проверку первого кольца без пересборки:
