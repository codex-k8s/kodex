...
6. После явного разрешения владельца выполнить install.
7. После install проверить rollout `deployment/kodex-registry` в production namespace.
8. Запустить registry/Kaniko smoke и проверить завершение jobs `kodex-registry-mirror-smoke`, `kodex-registry-pull-smoke`, `kodex-kaniko-build-smoke`.
