...
- Если migration job не завершился, проверить состояние PostgreSQL,
  ...
  значений DSN.
- Если live-схема расходится с ожидаемой, подготовить новую миграцию владельца схемы или явный
  runbook/deploy шаг. Не исправлять live drift ручным SQL и не редактировать уже применённый
  migration-файл.
