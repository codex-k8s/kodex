...
- `cmd/bootstrap-backend-deploy` выполняет реальный deploy выбранного backend-кольца:
  ...
  публичный `Ingress`, который ведёт только на `oauth2-proxy`. Значения env,
  DSN, токены, домены, адреса и kubeconfig не печатаются.
- `cmd/bootstrap-operational-acceptance` выполняет read-only приёмку уже
  ...
  Ingress через `oauth2-proxy`, `Certificate`/`ClusterIssuer` и HTTPS redirect
  на GitHub OAuth. Команда не выполняет `kubectl apply`, не запускает jobs, не
  собирает образы и не печатает значения env.
...
- Публичный web-contour развёрнут после явного `--ring web-public`:
  ...
  готов, а публичный `Ingress` ведёт только на `oauth2-proxy`.
- Post-deploy operational acceptance завершается `OK: operational acceptance
  ...
  root редиректит на GitHub OAuth.
