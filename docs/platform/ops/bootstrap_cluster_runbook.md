...
- `cmd/bootstrap-backend-deploy` выполняет реальный deploy выбранного backend-кольца:
  ...
  `staff-gateway` после готовности `agent-manager` и `interaction-hub`. Значения
...
режимом `--ring staff`. Ingress controller и cert-manager остаются отдельной
