...
- Backend smoke после deploy: `KODEX_SMOKE_ENV_FILE=<env> bash bootstrap/host/smoke_backend_contour.sh`.
...
- `cmd/bootstrap-deploy-plan` выполняет безопасный план первого backend deploy:
  ...
  platform event-log migrations, registry/Kaniko smoke manifests и manifests
  ...
  service smoke.
- `cmd/bootstrap-backend-deploy` выполняет реальный deploy первого кольца:
  применяет registry foundation, подготавливает Kubernetes `Secret`, запускает
  Kaniko build jobs, PostgreSQL, базы, migrations и deployments для
  `access-manager`, `project-catalog`, `package-hub`, `provider-hub`. Значения
...
8. Проверить, что завершились jobs `kodex-build-*`,
   `kodex-postgres-bootstrap-databases`, `platform-event-log-migrations` и
   migrations первого кольца.
9. Запустить backend smoke:
   `KODEX_SMOKE_ENV_FILE=<env> bash bootstrap/host/smoke_backend_contour.sh`.
   По умолчанию smoke повторяет `deploy_backend_ring.sh --skip-build`, чтобы
   не перезаписать Kubernetes `Secret` устаревшими значениями из env-файла.
...
- Backend smoke запускается после `deploy_backend_ring.sh`; штатный режим
  проверяет первый backend-набор через идемпотентный deploy без пересборки
  образов. Legacy service-specific scripts включаются только явным
  `KODEX_BACKEND_SMOKE_LEGACY_SERVICE_SCRIPTS=true`.

## Границы

Этот runbook не разворачивает frontend и полный backend-набор за пределами
первого кольца. Ingress controller и cert-manager остаются отдельной
