...
- После merge/push в `main` для собственного репозитория платформы `provider.repository.changed` остаётся provider-owned фактом, а `agent-manager` использует его только как trigger. Для создания self-deploy plan `agent-manager` вызывает `project-catalog.GetSelfDeploySignal`; только `ready` project-side input передаётся в `CreateSelfDeployPlanFromSignal` и порождает pending state с project/repository refs, source/merge commit refs, affected service keys, path categories, `services.yaml` ref/digest, expected runtime job types и governance refs. `provider_signal_ref` является ключом повтора: тот же signal/fingerprint возвращает существующий plan, а другой fingerprint по тому же signal ref конфликтует. Такой план не запускает build/deploy автоматически. Перед постановкой runtime-заданий orchestration передаёт в `governance-manager.PrepareSelfDeployPlanGate` только безопасное deploy-plan evidence: provider merge refs, project/repository refs, changed-file summary ref, path categories, service keys, digest/version и bounded summary. `governance-manager` классифицирует риск, создаёт или находит required gate request, учитывает существующее решение владельца, показывает required gate counts в summary и возвращает `pending`/`approved`/`rejected`/`blocked` summary. Runtime-задания разрешаются только после `approved`; для build orchestration дополнительно вызывает `project-catalog.GetSelfDeployBuildPlan`, сверяет expected `services.yaml` digest/fingerprint/version и получает per-service `BuildExecutionSpec`-совместимые refs из проверенной `ServicesPolicy`. Для корневого `services.yaml` source of truth build policy живёт в `spec.images`/`spec.versions`; `project-catalog` выводит из него image ref/tag, Dockerfile ref/target и builder image ref, но не создаёт build context PVC. Если checked build context ref/digest ещё не подготовлены, build plan возвращает `build_context_unavailable`, и `JOB_TYPE_BUILD` не создаётся. Runtime-owned путь подготовки build context фиксирует project/repository/provider/source refs, merge commit SHA, affected service keys и build plan fingerprint через `runtime-manager.PrepareBuildContext`; без checked source snapshot запись остаётся `pending` с `source_snapshot_unavailable`, а ready context допускается только после `ReportBuildContextProgress` с `build_context_ref/digest`. `agent-manager` не парсит `services.yaml`, а `runtime-manager` не читает project policy и не делает скрытый checkout из GitHub/GitLab. Если build plan возвращает другой non-ready статус, `JOB_TYPE_BUILD` не создаётся до reconcile проектной политики или исправления service build spec. Задание `deploy` после build требует successful build и отдельного `project-catalog.GetSelfDeployDeployPlan`, который связывает project/repository/policy/services.yaml/build output с checked manifest bundle, rollout targets и expected image refs. `runtime-manager` может хранить только уже проверенный `DeployExecutionSpec` как ожидающее задание, но не выводит manifest bundle из репозитория или build output, не вызывает `kubectl apply` и не выкатывает production автоматически. Весь контур работает без чтения raw webhook/diff/YAML, provider response, логов или значений секретов.
