...
Для merge/push-понимания runner принимает safe блок `repository_change`: имя события, `ref`/`base_branch`, commit sha, ограниченный список путей, действия и опциональные `sha256` digests объектов. На выходе он строит локальный safe plan summary: категории путей, флаги `services_policy_changed`/`deploy_relevant_changed`, количество путей и синтетический `change_digest`. Категория `services_policy` соответствует root `services.yaml`; deploy-relevant категории покрывают `deploy/**`, `services/**`, `packages/**`, `proto/**`, `specs/**`, `Dockerfile`, `go.mod`, `go.sum`, `Makefile` и `.github/workflows/**`. После локального плана runner читает `provider-hub GetRepositoryChangeSignal`/`ListRepositoryChangeSignals` и ждёт live provider-owned signal по provider refs, branch, commit и kind. Для `push` signal path summary должен быть `ready` или `truncated`; для `pull_request_merged` допускается `unavailable`, потому что GitHub webhook не содержит безопасного списка путей. Сырые webhook body, provider response, diff, полный YAML, токены, DSN, kubeconfig и приватные ссылки в runner output и product API input не попадают. Этот summary и live signal являются готовым safe input для ручного или управляемого deploy/update plan, но build/deploy не запускают.
