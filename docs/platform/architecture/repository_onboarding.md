...
Для merge/push-понимания runner принимает safe блок `repository_change`: имя события, `ref`/`base_branch`, commit sha, ограниченный список путей, действия и опциональные `sha256` digests объектов. На выходе он строит только safe summary: категории путей, флаги `services_policy_changed`/`deploy_relevant_changed`, количество путей и синтетический `change_digest`. Категория `services_policy` соответствует root `services.yaml`; deploy-relevant категории покрывают `deploy/**`, `services/**`, `packages/**`, `proto/**`, `specs/**`, `Dockerfile`, `go.mod`, `go.sum`, `Makefile` и `.github/workflows/**`. Сырые webhook body, provider response, diff, полный YAML, токены, DSN, kubeconfig и приватные ссылки в runner output и product API input не попадают. Этот summary является готовым safe input для `agent-manager CreateSelfDeployPlanFromSignal` или ручного deploy/update plan, но build/deploy не запускает.

Полный live-сценарий получения push/merge signal от публичного webhook ingress зависит от пограничного контура, который принимает внешний webhook и доставляет проверенный сигнал в `provider-hub`. Текущий runner готовит product-side state и проверочный путь через публичные gRPC API, но не реализует HTTP ingress, не создаёт auto deploy и не переносит orchestration/runtime/governance decisions в onboarding.
