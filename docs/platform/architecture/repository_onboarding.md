...
related_issues: [281, 282, 761, 794, 810, 818]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
10. Webhook или сверка провайдера фиксирует merge; provider projection связывает merged `PR/MR` с `project_id` и `repository_id`; внутренний контур передаёт в `project-catalog ImportBootstrapServicesPolicy` только проверенный сигнал: provider target, `base_branch`, `source_ref`, commit, `content_hash`, watermark и нормализованный `services.yaml`.
