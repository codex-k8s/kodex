...
6. `provider-hub` создаёт репозиторий у провайдера или использует заранее созданный пустой репозиторий, создаёт ветку и PR с bootstrap-набором файлов.
...
- Комментарий: документ фиксирует согласованную гибридную модель bootstrap/adoption; реализация #281/#282 остаётся отдельными срезами.
