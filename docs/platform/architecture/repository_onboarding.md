...
related_issues: [281, 282, 761, 794, 810, 818, 840, 864, 865, 881, 883, 917, 1011, 1015]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
- После merge/push в `main` для собственного репозитория платформы `provider.repository.changed` остаётся provider-owned фактом, а `agent-manager` использует его только как trigger. Для создания self-deploy plan `agent-manager` вызывает `project-catalog.GetSelfDeploySignal`; только `ready` project-side input передаётся в `CreateSelfDeployPlanFromSignal` и порождает pending state с project/repository refs, source/merge commit refs, affected service keys, path categories, `services.yaml` ref/digest, expected runtime job types и governance refs. `provider_signal_ref` является ключом повтора: тот же signal/fingerprint возвращает существующий plan, а другой fingerprint по тому же signal ref конфликтует. Такой план не запускает build/deploy автоматически. Перед постановкой runtime-заданий orchestration передаёт в `governance-manager.PrepareSelfDeployPlanGate` только безопасное обоснование deploy plan: provider merge refs, project/repository refs, changed-file summary ref, path categories, service keys, digest/version и ограниченный summary. `governance-manager` классифицирует риск, создаёт или находит required gate request, учитывает существующее решение владельца, показывает required gate counts в summary и возвращает `pending`/`approved`/`rejected`/`blocked` summary. Runtime-задания разрешаются только после `approved`. Для build orchestration `agent-manager` вызывает `project-catalog.GetSelfDeployBuildPlan`, сверяет expected `services.yaml` digest/fingerprint/version и получает рецепт сборки для каждого сервиса из проверенной `ServicesPolicy`; bare branch ref вроде `main` и полный ref `refs/heads/main` нормализуются как один источник, но raw SHA/digest/tag refs не подменяются branch-эквивалентностью. Merge commit кода валидируется и используется для build/deploy specs, но для code-only deploy-relevant изменений он может быть новее `ServicesPolicy.SourceCommitSHA`; freshness checked policy держится на source ref, digest, fingerprint и version. Для корневого `services.yaml` источник истины build policy живёт в `spec.images`/`spec.versions`; `project-catalog` выводит из него image ref/tag, Dockerfile ref/target и builder image ref, но не создаёт PVC контекста сборки. Если checked build context ref/digest ещё не подготовлены, build plan возвращает `build_context_required` с fingerprint рецепта и item; `agent-manager` фиксирует `runtime_build_status=preparing_context` и не создаёт `JOB_TYPE_BUILD`. Runtime-owned путь подготовки build context фиксирует project/repository/provider/source refs, merge commit SHA, affected service keys и build plan fingerprint через `runtime-manager.PrepareBuildContext`; Kubernetes materializer для self-repo получает source auth только через `SecretRef`, создаёт PVC/ref, проверяет commit/source snapshot, считает digest prepared context и доводит запись до `ready` через `ReportBuildContextProgress` с `build_context_ref/digest`. Повторный вызов `project-catalog.GetSelfDeployBuildPlan` с materialized context возвращает `BuildExecutionSpec`-совместимые items без хранения динамического PVC/digest в `services.yaml`. `agent-manager` не парсит `services.yaml`, а `runtime-manager` не читает project policy и не делает скрытый checkout из GitHub/GitLab. Если build/deploy plan возвращает `policy_stale`, старый approved plan становится terminal `failed` с safe action `replan_required`; latest policy не подставляется в старый plan, а следующий запуск начинается с нового checked provider/project signal. Если provider signal сообщает изменение root `services.yaml`, `GetSelfDeploySignal` возвращает `needs_services_policy_reconcile`, пока новая проверенная policy не импортирована. Если build plan возвращает другой статус неготовности, `JOB_TYPE_BUILD` не создаётся до reconcile проектной политики, подготовки контекста или исправления service build spec. Задание `deploy` после build требует successful build и отдельного `project-catalog.GetSelfDeployDeployPlan`, который связывает project/repository/policy/services.yaml/build output с ожидаемым build plan fingerprint, тем же materialized context, checked manifest bundle, rollout targets и expected image refs. `runtime-manager` принимает только уже проверенный `DeployExecutionSpec`, не выводит manifest bundle из репозитория или build output, применяет checked bundle controlled executor-ом через Kubernetes SDK/server-side apply и наблюдает rollout targets без raw manifest payload или kubeconfig в БД. Весь контур работает без чтения raw webhook/diff/YAML, provider response, логов или значений секретов.
- Если transient blocker был записан по уже созданному terminal runtime job, повтор self-deploy использует bounded `runtime_job_attempt_ref`, выведенный из старых job refs/status/error. Это даёт новый deterministic command id для `runtime-manager.CreateJob`, но повтор recovery с тем же attempt не создаёт дубли, а terminal результат самого retry attempt остаётся явным blocker для оператора.
- Для self-repo `codex-k8s/kodex` branch `main` live provider webhook `push` создаёт provider-owned `RepositoryChangeSignal` в `provider-hub`: commit sha, provider refs, `services_policy_changed`, `deploy_relevant_changed`, path category counters, `path_digest` и `change_fingerprint`. GitHub `pull_request closed + merged` создаёт merge fact с `path_summary_status=unavailable`, если webhook не несёт список изменённых файлов; детальная deploy-relevant сводка берётся из `push` signal или будущей безопасной сверки. Provider signal может оставаться не привязанным к project/repository refs на стороне `provider-hub`; `project-catalog` связывает этот provider-owned факт с уже привязанным repository binding по явному project/repository context и канонической provider identity, а не ходит напрямую в GitHub/GitLab.
...
Минимальный проверочный контур onboarding живёт в `cmd/onboarding-runner`. Это отдельный Go runner, а не `shell` smoke и не скрытый consumer. Он использует только публичные gRPC product API `project-catalog` и `provider-hub`, не читает внутренние таблицы и не вызывает GitHub/GitLab напрямую. Локальный `services.yaml` допускается читать только в операционном import-контуре runner, где файл преобразуется в checked JSON input для `ImportServicesPolicy`; доменный `project-catalog` не читает рабочее дерево репозитория.
...
Если `project_id` не передан, runner использует `organization_id`, project slug и project display name как безопасный project bootstrap input. Сначала он вызывает публичный `project-catalog ListProjects`, ищет active project по slug или имени, а в apply-режиме при отсутствии project вызывает `CreateProject` с детерминированным idempotency key. На успешном завершении runner всегда печатает safe summary с `project_id`; это значение используется как вход для `KODEX_AGENT_MANAGER_SELF_DEPLOY_SIGNAL_PROJECT_ID`. Если `organization_id` неизвестен, runner останавливается с безопасной диагностикой и не создаёт project обходным способом.

Режим apply включается только явным `--apply` или `KODEX_ONBOARDING_RUNNER_APPLY=true` и требует safe target policy: разрешённый provider owner, префикс тестового repository name для `bootstrap_setup.create_repository` или точное имя разрешённого репозитория через `--allowed-provider-repository`/`KODEX_ONBOARDING_RUNNER_ALLOWED_REPOSITORY` для `repository_binding`. Apply вызывает `ReconcileBootstrapMergeSignal` и `ReconcileAdoptionMergeSignal` только при наличии проверенного входа сценария: safe provider merge refs, artifact ref/digest/version, `content_hash`, watermark payload и нормализованный checked `validated_payload_json`. Эти значения передаются в product API как typed input, но runner не печатает сырой YAML, webhook body, provider response, diff, token, DSN, private URL или полный checked payload.
...
Для live-safe self-repo reconcile runner поддерживает блок `services_policy_import`. В dry-run он читает указанный локальный файл `--services-policy-file`/`KODEX_ONBOARDING_RUNNER_SERVICES_POLICY_FILE` с ограничением размера, нормализует YAML или JSON в JSON-объект, вычисляет `content_hash` как SHA-256 от нормализованного JSON и показывает только safe refs/digest/commit summary. В apply он использует уже найденные `project_id` и `repository_id`, берёт `source_ref`/`source_commit_sha` из блока `services_policy_import`, из live `RepositoryChangeSignal` или из явных флагов, затем вызывает публичный `project-catalog ImportServicesPolicy` с `validation_status=valid`. Повтор того же файла, source path и commit использует детерминированный `CommandMeta` idempotency key и не создаёт дубли projection/descriptors. Runner не печатает сырой YAML, полный `validated_payload_json`, provider response, diff, token, DSN, private URL или значения секретов.

После успешного `ImportServicesPolicy`, `ReconcileBootstrapMergeSignal` или `ReconcileAdoptionMergeSignal` runner вызывает публичный `project-catalog GetProjectOnboardingStatus` с теми же project/repository refs, source refs, commit, `content_hash`, `ServicesPolicy` id/version и service keys. Это контрольное чтение подтверждает, что ручной bootstrap/adoption дошёл до активного repository binding, checked `services.yaml` projection и активных `ServiceDescriptor`, либо возвращает типизированную причину неготовности вроде `services_policy_not_found`, `services_policy_stale` или `service_descriptors_not_found`. Проверка остаётся product API контуром onboarding: она не читает GitHub/GitLab, не обращается к БД и не запускает self-deploy build/deploy.
...
Для self-adoption репозитория `codex-k8s/kodex` runner поддерживает сценарий `repository_binding`: dry-run проверяет, что существующий provider repo может быть привязан к project через публичный `project-catalog AttachRepository`, а apply при точной safe target policy сначала ищет существующий active repository binding через `ListRepositories`. Если binding уже есть, повторный запуск возвращает тот же `repository_id` и не создаёт дубль; если binding отсутствует, runner вызывает `AttachRepository` и получает `repository_id` из ответа. Для self-repo используется provider `github`, provider owner `codex-k8s`, provider name `kodex`, базовая ветка `main` и root `services.yaml` как policy artifact path. Runner не создаёт и не меняет GitHub-репозиторий в этом сценарии и не требует `repository_name_prefix`, если задано точное разрешённое имя `kodex`.

После создания или нахождения self-repo binding runner в apply-режиме выдаёт через публичный `access-manager PutAccessRule` узкие сервисные правила. Для собственного service actor runner создаёт или переиспользует `repository.read` на конкретный binding: `subject.type=service`, `subject.id=onboarding-runner`, `resource.type=repository`, `resource.id=<repository_id>`, `scope=project/<project_id>`. Это правило готовится до `project-catalog GetRepository`, если `repository_id` уже известен, и повторно проверяется через `CheckAccess`, когда binding найден или создан. Для чтения project policy runner создаёт правила `subject.id=agent-manager` и `subject.id=staff-gateway`, `action=project.policy.read`, `resource.type=services_policy`, `scope=project/<project_id>`. Эти правила нужны, чтобы `agent-manager` мог выполнить `project-catalog GetSelfDeploySignal`/`GetSelfDeployBuildPlan`, а `staff-gateway` мог показать pre-plan остановку через `project-catalog GetSelfDeploySignal`, когда `SelfDeployPlan` ещё не создан.

Для self-deploy governance path runner дополнительно создаёт минимальные project-scoped правила, необходимые доменным сервисам после появления `SelfDeployPlan`. `service/agent-manager` получает `governance.risk.evaluate`, `governance.risk.read`, `governance.gate.request` и `governance.gate.read` на соответствующие governance resources в `scope=project/<project_id>`; этого хватает для `PrepareSelfDeployPlanGate`, чтения уже созданного gate и safe summary. `service/staff-gateway` получает только read-набор `governance.risk.read` и `governance.gate.read` в том же project scope для `GET /v1/self-deploy/summary`. Сервисам не выдаются `governance.gate.decide`, release decision write/admin и policy manage права: решение `approve`/`reject`/`request_changes` остаётся за владельцем или другим явно разрешённым субъектом. Если в runner передан безопасный `owner_user_ref` через сценарий, `--owner-user-ref`, `KODEX_ONBOARDING_RUNNER_OWNER_USER_REF` или `KODEX_BOOTSTRAP_OWNER_USER_REF`, runner создаёт прямые owner-facing правила `subject.type=user`: `governance.risk.read`, `governance.gate.read` и `governance.gate.decide` в `scope=project/<project_id>` для self-deploy governance resources. Email владельца остаётся seed для identity/bootstrap и не записывается в `AccessRule.subject_id`. `governance-manager` для self-deploy target checks использует `ProjectContext.ProjectRef` как access scope; `SubmitGateDecision` сначала читает gate, затем для gate с `RiskAssessmentID` авторизует решение по target/project связанного assessment, без широкого owner rule в `scope=global`.

Для self-deploy runtime path runner создаёт только те runtime rules, которые нужны после owner approval. `service/agent-manager` получает `runtime.build_context.prepare`, `runtime.build_context.read`, `runtime.job.create` и `runtime.job.read` на `runtime_build_context`/`runtime_job` в `scope=project/<project_id>`; этого хватает для `runtime-manager.PrepareBuildContext`, повторного чтения context, постановки `JOB_TYPE_BUILD`/`JOB_TYPE_DEPLOY` и наблюдения job state. `service/runtime-manager-build-context-materializer` получает project-scoped `runtime.build_context.report` на `runtime_build_context`, потому что materializer переводит build context в `running`/`ready`/`failed` до создания build job. Фактический worker actor `service/runtime-manager-kubernetes-executor` получает `runtime.job.claim` на `runtime_job` в `scope=global`, потому что текущий `runtime-manager ClaimRunnableJob` авторизуется до выбора конкретного project/job через `jobResource(nil,nil)`. Остальные worker-переходы остаются project-scoped: `runtime.job.step.report`, `runtime.job.complete` и `runtime.job.fail` на `runtime_job` в `scope=project/<project_id>`. `runtime.job.cancel` не выдаётся, пока Kubernetes executor не вызывает cancel path. Runner не создаёт wildcard/admin-доступ, не выдаёт runtime rights владельцу UI и проверяет каждый grant через `CheckAccess`.

Перед live rollout self-deploy контур проверяется read-only preflight-ом bootstrap/deploy: project id, включённые self-deploy consumer/gate/build/decision флаги, rendered `agent-manager`/`runtime-manager` surface, runtime executor namespace/image/service account refs, source auth SecretRef, service tokens, Kaniko builder image из корневого `services.yaml` и готовность Kubernetes foundation. Этот preflight не создаёт project state и не читает raw payload; создание и проверка access rules остаются за idempotent `onboarding-runner --apply`. Dynamic registry Secret refs, checked build context refs, build/deploy job refs и итоговый статус проверяются после controlled trigger через `cmd/self-deploy-chain-acceptance`, потому что они появляются только из `GetSelfDeployBuildPlan`, `PrepareBuildContext` и runtime job lifecycle.

Если включён `services_policy_import`, runner дополнительно выдаёт своему service actor узкое право `project.policy.import` на `services_policy` в том же project scope и проверяет его через `CheckAccess` перед вызовом `ImportServicesPolicy`. Runner не создаёт широкие admin-права, не выдаёт доступ к секретам и после каждого `PutAccessRule` проверяет итоговое решение `CheckAccess`; если существующая политика оставляет deny, runner останавливается с безопасной причиной.

Для merge/push-понимания runner принимает safe блок `repository_change`: имя события, `ref`/`base_branch`, commit sha, ограниченный список путей, действия и опциональные `sha256` digests объектов. На выходе он строит локальный safe summary: категории путей, флаги `services_policy_changed`/`deploy_relevant_changed`, количество путей и синтетический `change_digest`. Категория `services_policy` соответствует root `services.yaml`; deploy-relevant категории покрывают `deploy/**`, `services/**`, `packages/**`, `proto/**`, `specs/**`, `Dockerfile`, `go.mod`, `go.sum`, `Makefile` и `.github/workflows/**`. После локального плана runner читает `provider-hub GetRepositoryChangeSignal`/`ListRepositoryChangeSignals` и ждёт live provider-owned signal по provider refs, branch, commit и kind. Для `push` signal path summary должен быть `ready` или `truncated`; для `pull_request_merged` допускается `unavailable`, потому что GitHub webhook не содержит безопасного списка путей. Сырые webhook body, provider response, diff, полный YAML, токены, DSN, kubeconfig и приватные ссылки в runner output не попадают; в `ImportServicesPolicy` передаётся только нормализованный checked JSON input с refs/digests/commit metadata.

Provider-owned `RepositoryChangeSignal` не передаётся напрямую в `agent-manager` как полный plan input. `agent-manager` получает `provider.repository.changed` из event log только как trigger с provider signal ref/source refs, затем вызывает `project-catalog GetSelfDeploySignal`. `project-catalog` сначала читает signal по точному `provider_signal_id`/`provider_signal_key`; если provider-owned запись не содержит project/repository refs или прямое чтение не нашло связанный project context, метод использует явный `project_id`/`repository_id`, активный repository binding и `provider-hub ListRepositoryChangeSignals` по canonical provider slug, owner/name, provider repository id, branch и commit. Сопоставление GitHub owner/repository выполняется в каноническом нижнем регистре, отображаемая строка не является источником истины. Если binding отсутствует, signal относится к другому provider/repository или из provider signal identity нельзя извлечь repository/branch/commit, read surface возвращает точный safe reason (`repository_binding_not_found`, `provider_signal_repository_ref_mismatch`, `provider_signal_identity_unavailable`), а не общий транспортный `NotFound`. После успешного binding lookup `project-catalog` связывает provider signal с проверенной `ServicesPolicy`, `services_yaml_digest`, версией/fingerprint проекции и `ServiceDescriptor`. Если root `services.yaml` изменён, но checked projection ещё не импортирована с commit сигнала, project-side read surface возвращает `needs_services_policy_reconcile`, а не ложный готовый сигнал. Если provider path summary недоступен, возвращается `needs_repository_change_summary`. Только статус `ready` даёт safe project-side input для `agent-manager CreateSelfDeployPlanFromSignal`: `provider_signal_ref`, project/repository refs, source ref, commit sha, `services_yaml_ref`, `services_yaml_digest`, affected service keys, path categories, expected runtime job type hints и признак обязательного owner/governance gate. После owner/governance approval `agent-manager` вызывает `project-catalog GetSelfDeployBuildPlan` с теми же refs, affected service keys и expected policy digest/fingerprint/version. `project-catalog` нормализует `main` и `refs/heads/main` только как безопасные branch refs; raw SHA, digest, tag/provider-specific refs и небезопасные refs требуют буквального совпадения. Первый read может вернуть `build_context_required` и items только с рецептом сборки; `agent-manager` сохраняет `preparing_context`, вызывает или переиспользует `runtime-manager.PrepareBuildContext` и повторяет read с `materialized_build_contexts` после готовности runtime context refs/digests. `project-catalog` возвращает `BuildExecutionSpec`-совместимые build items только из checked `ServicesPolicy` и переданных runtime context refs/digests; при `policy_stale`, `service_not_found`, `build_plan_unavailable`, `build_context_required` или `build_context_invalid` постановка runtime build jobs блокируется безопасным статусом. Build не запускается до approval и готового build plan; deploy запускается только после successful build, checked `GetSelfDeployDeployPlan`, совпадения build output с ожидаемым build plan/context и typed `DeployExecutionSpec`, без обхода через provider payload или raw manifests.

Полный live-сценарий получения push/merge signal от публичного webhook ingress зависит от пограничного контура, который принимает внешний webhook и доставляет проверенный сигнал в `provider-hub`. Runner готовит product-side state, проверяет наличие live `RepositoryChangeSignal` через публичные gRPC API и показывает безопасную диагностику ожидания delivery, но не реализует HTTP ingress, не создаёт auto deploy и не переносит orchestration/runtime/governance decisions в onboarding.
