...
| `provider-hub` | Выполняет provider-native операции: читать репозиторий, создать или обновить ветку, Issue, PR/MR, комментарий, связь и зеркало. Создание самого репозитория у провайдера является отдельной операцией/срезом и не смешивается с bootstrap branch/PR. |
...
3. Пользователь выбирает способ подготовки:
...
4. Для агентного режима `agent-manager` запускает bootstrap-роль, которая формирует начальный `services.yaml`, базовую структуру документации и описание ожидаемого состава проекта.
5. Для детерминированного режима `package-hub` отдаёт manifest шаблона, а кодовый исполнитель создаёт файлы, `services.yaml` и локальные инструкции по шаблону без анализа агентом.
6. `provider-hub` использует заранее созданный пустой репозиторий, создаёт или обновляет bootstrap branch и PR с подготовленным bootstrap-набором файлов; создание самого репозитория у провайдера остаётся отдельным provider-срезом.
7. Владелец проверяет PR у провайдера и подтверждает переход через merge.
8. Webhook или сверка провайдера фиксирует merge; `project-catalog` импортирует проверенную политику из commit и переводит repository binding в активное состояние.
...
4. Если репозиторий подходит под выбранный шаблон и конфликтов нет, детерминированный исполнитель готовит PR: добавляет или обновляет `services.yaml`, локальные инструкции, скелет документации и ссылки на руководящие пакеты.
...
8. Результатом является отчёт подключения и, если нужно, PR с добавлением или исправлением `services.yaml`, документационных ссылок и минимальных политик.
9. Владелец принимает решение по отчёту и подтверждает переход через merge bootstrap/adoption PR.
10. После merge `project-catalog` импортирует проверенную проекцию и включает репозиторий в проектный рабочий контур.
