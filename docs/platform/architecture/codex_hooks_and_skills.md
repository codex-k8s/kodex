...
3. Hook emitter отправляет событие в `platform-mcp-server` или локальный агентный sidecar, если прямой MCP-вызов недоступен. В обоих случаях целевая граница остаётся MCP-поверхностью: sidecar только буферизует и нормализует событие, но не принимает доменные решения.
...
Подробная граница сервиса и план поставки зафиксированы в `docs/domains/platform-mcp-server/**`. Этот документ продолжает фиксировать решение #698, но реализация hook emitter и hook ingress выполняется отдельными срезами.
