...
- `web-console` не принимает доменных решений и не собирает состояние напрямую из БД нескольких сервисов-владельцев. Первый активный интерфейсный срез в `services/staff/web-console` вызывает только `staff-gateway`, использует сгенерированный TypeScript-клиент из `specs/openapi/staff-gateway.v1.yaml` и оставляет отсутствующие серверные сценарии как пустые или отключённые состояния.
