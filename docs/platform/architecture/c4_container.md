...
|---|---|---|
| Пограничный слой и интерфейс | `web-console`, `user-gateway`, `staff-gateway`, `integration-gateway`, `platform-mcp-server`, `codex-hook-ingress` | Пользовательский интерфейс, входящие HTTP/webhook/MCP запросы, Codex hook events, авторизация и маршрутизация по направлениям доступа. |
...
System_Boundary(kodex, "kodex") {
  ...
  Container(hookIngress, "codex-hook-ingress", "Go", "Входной контур нормализованных Codex hook events")
...
Rel(hookIngress, agent, "Маршрутизирует hook events жизненного цикла", "gRPC")
Rel(hookIngress, runtime, "Маршрутизирует hook events среды исполнения", "gRPC")
Rel(hookIngress, providerHub, "Маршрутизирует provider artifact signals", "gRPC")
Rel(hookIngress, interaction, "Маршрутизирует запросы разрешений и обратной связи", "gRPC")
...
Rel(access, vault, "Хранит и проверяет ссылки на секреты платформы", "Vault/Secret metadata")
Rel(providerHub, vault, "Получает секрет через resolver-клиент после разрешения доступа", "Vault/Kubernetes Secret/env")
...
- `web-console` не принимает доменных решений, не собирает состояние напрямую из БД нескольких сервисов-владельцев и не утверждает actor identity в production-сборке. Первый активный интерфейсный срез в `services/staff/web-console` вызывает только `staff-gateway`, использует сгенерированный TypeScript-клиент из `specs/openapi/staff-gateway.v1.yaml`, получает `X-Kodex-Actor-*` от public edge после OAuth2-proxy проверки владельца и маппинга `X-Forwarded-Email` в platform actor id, а отсутствующие серверные сценарии оставляет как пустые или отключённые состояния.
...
- Первый активный контур `staff-gateway` обслуживает `web-console` для входящих решений владельца, просмотра runtime summary одного `AgentRun`, safe activity timeline и self-deploy наблюдения: owner inbox идёт через gRPC-вызовы `interaction-hub`, runtime summary/activity timeline/self-deploy plan summary — через `agent-manager.GetAgentRunRuntimeStatus`, `agent-manager.ListAgentActivities` и `agent-manager.ListSelfDeployPlans`, а pre-plan self-deploy readiness при отсутствии plan уточняется только через существующие read RPC `project-catalog.GetSelfDeploySignal` и `project-catalog.ListRepositories`. Self-deploy summary показывает безопасные стадии `signal`, `plan`, `gate`, `approval`, `build_context`, `build`, `deploy`, `health` по данным `agent-manager`, но не читает Kubernetes и не управляет runtime jobs. Gateway не хранит decision state, не владеет `Run`/runtime job/activity timeline/deploy plan/project/provider/governance lifecycle и не агрегирует provider/agent/runtime/governance состояние за пределами явно типизированных safe DTO.
...
## Порядок реализации gateway-сервисов

Общий `api-gateway` не вводится. HTTP-поверхности разделяются по направлению доступа, чтобы операторские сценарии, внешние пользовательские сценарии и входящие интеграции не смешивали разные политики доступа и жизненные циклы.

| Компонент | Когда реализуется | Причина |
|---|---|---|
| `staff-gateway` | Первый узкий контур доступен для owner inbox/Human gate decision path; следующие срезы расширяют его по мере появления экранов `web-console`. | Это основной HTTP-вход для консоли сотрудников MVP; пользовательский интерфейс не должен ходить напрямую во внутренние gRPC-сервисы. |
| `integration-gateway` | После базового `provider-hub`, когда определён набор webhook, обратных вызовов и входящих событий от провайдеров и внешних интеграций. | `integration-gateway` должен принимать внешние события, но маршрутизировать их в `provider-hub` и другие сервисы-владельцы без собственной доменной логики. |
| `user-gateway` | На границе MVP или после MVP, когда появятся подтверждённые сценарии внешних пользователей. | Это не блокирует внутренний MVP консоли сотрудников, потому что начальные сценарии выполняются сотрудниками и владельцем через `web-console` и `staff-gateway`. |
...
- Vault или другой настроенный secret backend хранит секреты платформы и её зависимостей; проекты могут использовать свои хранилища секретов.
- Доменные сервисы не получают секреты через публичные API: после проверки доступа они используют общий resolver-клиент, который держит значение только в памяти процесса на время операции.
