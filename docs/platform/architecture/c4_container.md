...
|---|---|---|
| `staff-gateway` | Непосредственно перед `web-console` или сразу после первого среза пользовательского интерфейса, когда понятен набор OpenAPI-эндпоинтов для операторских и административных экранов. | Это основной HTTP-вход для консоли сотрудников MVP; пользовательский интерфейс не должен ходить напрямую во внутренние gRPC-сервисы. |
