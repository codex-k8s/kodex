...
related_issues: [599, 600, 601, 602, 655, 711, 747]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
- `platform-mcp-server` даёт инструментальную поверхность для agent-manager, агентов в слотах, hook emitter и внешних интеграций. Agent-manager и agent-runner обращаются к нему как клиенты MCP, а hook emitter или локальный sidecar отправляет Codex hook events через отдельный входной контур hook-событий. Сам `platform-mcp-server` проверяет источник, очищает payload и маршрутизирует разрешённые инструменты и события во все сервисы-владельцы по gRPC. Он не становится владельцем агентных запусков, заданий, состояния провайдера, пакетов или проектов.
