...
- Первый активный контур `staff-gateway` обслуживает `web-console` для входящих решений владельца и просмотра runtime summary одного `AgentRun`: owner inbox идёт через gRPC-вызовы `interaction-hub`, а runtime summary — через `agent-manager.GetAgentRunRuntimeStatus`. Gateway не хранит decision state, не владеет `Run`/runtime job и не агрегирует provider/agent/runtime состояние за пределами явно типизированных safe DTO.
