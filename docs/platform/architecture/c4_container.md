...
- `platform-mcp-server` даёт инструментальную поверхность для agent-manager, агентов в слотах, hook emitter и внешних интеграций. Agent-manager и agent-runner обращаются к нему как клиенты MCP, а сам `platform-mcp-server` проверяет источник, очищает payload и маршрутизирует разрешённые инструменты во все сервисы-владельцы по gRPC. Он не становится владельцем агентных запусков, заданий, состояния провайдера, пакетов или проектов.
