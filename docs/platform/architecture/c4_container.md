...
- Первый активный контур `staff-gateway` обслуживает `web-console` для входящих решений владельца: список, карточка и безопасный ответ идут через OpenAPI наружу и gRPC-вызовы `interaction-hub` внутри. Gateway не хранит decision state и не агрегирует provider/agent/runtime состояние.
