...
| Webhook inbox privacy-hardening | частично готово | PRV-4 inbox хранит `payload_json` как safe envelope only с `payload_sha256` и provider refs; canonical provider body не является штатной опорой storage. Replay/conflict идёт по delivery id и fingerprint, наружу read RPC/events/diagnostics отдают только safe envelope/digest. Осталось решить encryption-at-rest/KMS и re-fetch/reprocess для случаев, где без raw body нужна повторная нормализация. |
...
Следующий provider-срез — интеграция provider tools с `agent-manager`/platform MCP, GitLab write/webhook adapter, encryption/refetch продолжение webhook inbox privacy-hardening или расширение callback routes после новых owner-service contracts. Project-side импорт bootstrap/adoption должен читать lightweight snapshot и safe provider merge signal через provider-hub read surface, а producer-side route/signal можно проверять через staged fixture checks; проверка `services.yaml`, deep workspace scan/report и активация binding остаются в `project-catalog`/`agent-manager`.
