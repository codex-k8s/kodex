...
| PRV-7b | #729 | готово | Общий конвейер команд операций записи реализован в `provider-hub`: типизированные gRPC handlers, casters, доменный pipeline, идемпотентная запись `ProviderOperation`, проверка `expected_version`, policy context и `approval_gate_ref`, но без реальных GitHub/GitLab write-вызовов. |

## Текущий бэклог

| Срез | Статус | Почему не завершён |
|---|---|---|
| PRV-7c | готово к началу после PRV-7b | GitHub-адаптер записи для операций из каталога должен использовать уже готовый общий конвейер команд, журнал операций, лимиты, проекции и события. |
...
- операции записи используют тот же resolver-контур; PRV-7b уже закрепил общий command pipeline, но реальные внешние GitHub/GitLab write-вызовы остаются в PRV-7c.
...
- `agent-manager` и `platform-mcp-server` могут использовать ускоряющие сигналы `provider-hub`; provider-инструменты записи получили типизированный контракт и общий pipeline, но реальные операции записи остаются зависимостью до PRV-7c.

## Рекомендуемый следующий шаг

Следующий provider-срез — PRV-7c: GitHub-адаптер записи поверх уже готового общего command pipeline. Не смешивать его с MCP, `agent-manager`, UI или bootstrap/adoption.
