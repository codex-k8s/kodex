...
|---|---|---|
| PRV-7 | заблокировано частично | Операции записи в провайдера уже могут использовать общий resolver-контракт, но требуют согласованный каталог MCP/agent-manager инструментов, идемпотентность команд и политику approval. Действия доступа уже есть. |
...
Следующий provider-срез — PRV-7: платформенные provider-операции записи для agent-manager/MCP. Не начинать запись до согласования каталога инструментов, идемпотентности команд и approval-политики.
