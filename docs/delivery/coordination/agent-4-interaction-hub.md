...
|---|---|---|
| IH-8+ | ожидает отдельные срезы | MCP, concrete channel packages, runtime worker, gateway/UI-связки и cross-domain inbox aggregation должны поставляться малыми PR. |
