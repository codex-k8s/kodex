...
Агент #3 временно выполняет AGO-0 в домене `agent-orchestration`, чтобы зафиксировать стартовые границы `agent-manager` и его зависимость от `package-hub` для чтения установленных руководящих пакетов. Код `package-hub` в этом срезе не меняется.
