...
После PKG-7 независимого package-hub среза без соседних доменов почти не осталось. PKG-5.3b остаётся заблокированным до появления access-контракта заполненности секретов установки, а интеграционные сценарии магазина ждут `provider-hub`, внешний адаптер магазина и runtime/fleet-контур.
