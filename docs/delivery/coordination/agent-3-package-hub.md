...
|---|---|---|
| PKG-5.3b | следующий срез после контракта доступа | `secretresolver.Checker` для проверки без возврата значения готов; `access-manager` получает контракт выдачи безопасных ссылок секретов установки, а `package-hub` должен подключить его в следующем срезе. |
...
|---|---|---|
| `access-manager` | Пересчёт `secret_binding_status` установки. | Срез доступа добавляет контракт ссылок секретов установки; после его слияния `package-hub` проверяет доступность только через `secretresolver.Checker` без возврата значений. |
...
После PKG-7 независимого package-hub среза без соседних доменов почти не осталось. Общий secret resolver уже есть, срез доступа добавляет контракт безопасных ссылок установки, поэтому после его слияния следующий локальный шаг агента #3 — PKG-5.3b в `package-hub`. Интеграционные сценарии магазина продолжают ждать `provider-hub`, внешний адаптер магазина и runtime/fleet-контур.
