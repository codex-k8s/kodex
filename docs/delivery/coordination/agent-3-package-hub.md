...
|---|---|---|
| PKG-5.3b | заблокировано частично | `secretresolver.Checker` для проверки без возврата значения готов, но `RefreshPackageInstallationSecretStatus` всё ещё требует контракт `access-manager` для выдачи ссылок секретов конкретной установки. |
...
|---|---|---|
| `access-manager` | Пересчёт `secret_binding_status` установки. | Нужен контракт ссылок секретов установки; после получения ссылок `package-hub` проверяет доступность только через `secretresolver.Checker` без возврата значений. |
