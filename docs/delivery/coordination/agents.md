...
Параллельная разработка разделена на три доменных потока. Агент #1 подтвердил, что `project-catalog` Wave 8, `runtime-manager` RTM-0..RTM-7 и `fleet-manager` FLEET-0 готовы. Для `project-catalog` остаются интеграционные хвосты по bootstrap/adoption репозиториев, которые требуют `provider-hub`; для runtime/fleet текущий MVP не блокируется соседними доменами, но организационные runtime-политики, workspace с руководящими пакетами, сценарии исполнения runtime-нагрузок и целевой `ResolvePlacement` требуют новых междоменных контрактов. Для `provider-hub` сняты базовые блокировки по provider-действиям, `ResolveExternalAccountUsage` и ускоряющим сигналам, но полная GitHub-сверка и provider-операции записи ждут общий secret resolver/Vault/Kubernetes Secret клиент. Агент #3 довёл `package-hub` до специализации видов пакетов и read-only сценария руководящих пакетов; пересчёт заполненности секретов установки всё ещё ждёт `access-manager`.
...
| `access-manager` | `fleet-manager` | FLEET-0 не блокируется; кодовые срезы требуют ключей действий и проверки прав на scope, серверы, кластеры и placement rules. | Завести ключи действий в FLEET-1 и подключить проверки доступа в FLEET-3. |
...
| `package-hub` | `fleet-manager` | Runtime-требования пакетов и плагинов могут влиять на placement. | Fleet не запускает runtime-нагрузки; в FLEET-5 нужен вход `runtime_requirements`, полученный через `runtime-manager` или согласованный контракт чтения `package-hub`. |
...
| `fleet-manager` | `runtime-manager` | Целевой `ResolvePlacement`, реестр нескольких scope/серверов/кластеров и cluster health. | До FLEET-5 runtime использует bootstrap default refs; после FLEET-5 перейти на fleet decision отдельным интеграционным PR. |
...
| `agent-manager` и `platform-mcp-server` | `provider-hub` | Ускоряющие сигналы уже имеют локальный gRPC-контракт; provider-операции записи требуют согласованного набора инструментов, идемпотентности и прав. | Сигналы передают выбранный `external_account_id` и не требуют секрета; команды записи делать после secret resolver и MCP-контракта. |
