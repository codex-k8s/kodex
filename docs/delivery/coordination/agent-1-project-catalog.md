...
| `platform-mcp-server` | готово: #747, #753, #760, #771, #780, #830, #841 | MCP-0 фиксирует границы, группы инструментов, безопасность и план поставки; MCP-1 фиксирует стратегию контрактов через MCP SDK, JSON Schema и snapshot-проверки `tools/list`, а также отделяет Codex hooks в `codex-hook-ingress`; MCP-2 добавляет сервисный каркас; MCP-3 подключает первые маршруты к `agent-manager`; MCP-3g подключает gate lifecycle tools к `governance-manager`; MCP-3r подключает risk evaluator tools к `governance-manager`; MCP-4 подключает маршруты чтения и записи provider-данных к `provider-hub` без хранения бизнес-состояния в MCP. |
...
- `agent-manager` зависит от `platform-mcp-server` для MCP-инструментов run/session и будущих flow/gate-связок; MCP не владеет flow/role/prompt и не конфликтует с AGO-3. Risk assessment и gate decision state остаются у `governance-manager`, а MCP-3g/MCP-3r дают только тонкие governance routes.
...
Для агента #1 нет незавершённого локального Wave 8, RTM или FLEET среза, который нужно закрыть до соседних доменов. После MCP-3r рационально идти в один из трёх вариантов:

- продолжить MCP governance routes после GOV-5/GOV-7: review signals, release decision package и release decision без хранения decision state в MCP;
