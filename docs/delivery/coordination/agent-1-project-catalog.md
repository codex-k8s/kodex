...
| Реальный исполнитель platform jobs | первый Kubernetes-срез готов: #940, надёжность worker-а готова: #949, контракт `agent_run` готов: #961, первый executor `agent_run` готов: #966, рабочая нагрузка runner-а готова: #975, contract-first Codex execution spec готов: #990 | `runtime-manager` исполняет безопасное задание `health_check` и `agent_run` с валидным `AgentRunExecutionSpec` через включаемый явно Kubernetes-исполнитель. Worker устойчив к остановке процесса, повторному claim и повторной сверке уже созданного Kubernetes Job. `agent_run` без spec не claim-ится, а `agent_run` со spec запускается через фиксированный runner command, workspace PVC mount и safe refs без значений секретов. `CodexSessionExecutionSpec` фиксирует безопасные refs/digest для будущего Codex CLI запуска и не передаёт raw prompt text. `agent-runner` проверяет смонтированный context и execution spec, сообщает безопасную диагностику в `agent-manager`; фактический вызов Codex CLI остаётся следующим runtime/runner-срезом. Задания `build`/`deploy`, нагрузки slot-агента, исполнитель workspace materialization и расширенные типы заданий остаются отдельными срезами после согласования с `agent-manager`/ops-контуром. |
