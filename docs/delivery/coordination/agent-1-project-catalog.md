...
- adoption checked artifact/import signal после provider adoption merge, чтобы existing repository path не импортировал `services.yaml` на основе lightweight scan snapshot;
...
- продолжить bootstrap пустого репозитория следующим ONB-срезом: закрыть детерминированный template executor без переноса шаблонов в `project-catalog` либо перейти к adoption import signal для существующего репозитория;
