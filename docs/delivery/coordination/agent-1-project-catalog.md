...
|---|---|---|
| Bootstrap пустого репозитория | требует выбора владельца: #281 | Проектная часть готова в `project-catalog`; вариантная архитектура зафиксирована в `docs/platform/architecture/repository_onboarding.md`. Предварительно рекомендована гибридная модель: `services.yaml` как Git-декларация, установки пакетов хранятся в `package-hub`, provider-native запись идёт через `provider-hub`, материализация workspace выполняется в `runtime-manager`. |
| Adoption существующего репозитория | требует выбора владельца: #282 | Проектная часть готова в `project-catalog`; перед provider/adoption срезом нужно утвердить, как `services.yaml`, внешняя документация и пакеты попадают в репозиторий и workspace. |
...
- #281 и #282 требуют provider-native операций: создать или просканировать репозиторий, открыть bootstrap PR, связать provider Issue/PR/MR с локальными проектом и репозиторием; перед реализацией нужен выбор владельца по вариантам bootstrap/adoption;
