...
| MCP-3r | #841 | готово | Подключены governance risk evaluator tools поверх GOV-5: `governance.risk.evaluate/reevaluate/get/list` маршрутизируются в `governance-manager`, принимают только typed refs/bounded summaries и возвращают safe assessment refs/status/risk class, matched rule refs/counts, required gate refs, version/timestamps. |
| MCP-3d | #852 | готово | Подключены release decision lifecycle tools поверх GOV-6: package prepare/get/list, decision request/submit/get/list, blocking signal record/resolve/list и safety-loop record/get маршрутизируются в `governance-manager`, возвращают только safe refs/status/summary/counts/version/timestamps и не хранят release state в MCP. |
...
| `platform-mcp-server` | готово: #747, #753, #760, #771, #780, #830, #841, #852 | MCP-0 фиксирует границы, группы инструментов, безопасность и план поставки; MCP-1 фиксирует стратегию контрактов через MCP SDK, JSON Schema и snapshot-проверки `tools/list`, а также отделяет Codex hooks в `codex-hook-ingress`; MCP-2 добавляет сервисный каркас; MCP-3 подключает первые маршруты к `agent-manager`; MCP-3g подключает gate lifecycle tools к `governance-manager`; MCP-3r подключает risk evaluator tools к `governance-manager`; MCP-3d подключает release decision lifecycle tools к `governance-manager`; MCP-4 подключает маршруты чтения и записи provider-данных к `provider-hub` без хранения бизнес-состояния в MCP. |
...
- фактический вызов `ImportBootstrapServicesPolicy` из webhook/reconciliation контура после provider projection merge-сигнала;
...
- `agent-manager` зависит от `platform-mcp-server` для MCP-инструментов run/session и будущих flow/gate-связок; MCP не владеет flow/role/prompt и не конфликтует с AGO-3. Risk assessment, gate decision, release decision package, release decision, blocking signal и safety-loop state остаются у `governance-manager`, а MCP-3g/MCP-3r/MCP-3d дают только тонкие governance routes.
