...
| RTM-RUNNER-2 | #990 | готово | Зафиксирован contract-first `CodexSessionExecutionSpec`: instruction/result schema refs и digest, session/workspace snapshot, hook/callback refs, timeout, fixed runner profile, output/result refs и allowed secret refs без значений. `runtime-manager` сохраняет spec и передаёт runner-у bounded JSON env, а `agent-runner` валидирует spec и без фактического Codex executor завершает job `agent_execution_contract_unavailable`. |
...
| Реальный исполнитель platform jobs | первый Kubernetes-срез готов: #940, надёжность worker-а готова: #949, контракт `agent_run` готов: #961, первый executor `agent_run` готов: #966, рабочая нагрузка runner-а готова: #975, contract-first Codex execution spec готов: #990, фактический запуск `codex exec` со стороны runner-а готов в RTM-RUNNER-3, отчёт runner-а в `agent-manager` через `SecretKeyRef` готов в RTM-RUNNER-E2E, контрактная/storage/service модель `build`/`deploy` готова в RTM-BD-1 | `runtime-manager` исполняет безопасное задание `health_check` и `agent_run` с валидным `AgentRunExecutionSpec` через включаемый явно Kubernetes-исполнитель. Worker устойчив к остановке процесса, повторному claim и повторной сверке уже созданного Kubernetes Job. `agent_run` без spec не claim-ится, а `agent_run` со spec запускается через фиксированный runner command, workspace PVC mount и safe refs без значений секретов. `CodexSessionExecutionSpec` фиксирует безопасные refs/digest для Codex CLI запуска и не передаёт raw prompt text. `agent-runner` production image содержит `/usr/local/bin/codex`; runner проверяет смонтированный context, execution spec и workspace refs `workspace://.kodex/execution/...`, вызывает `codex exec` фиксированным executable/args, сообщает `running`/`completed`/`failed` через `ReportAgentRunState` и сохраняет только bounded summary/result digest/schema ref. `runtime-manager` передаёт runner-у адрес `agent-manager` и ссылку на token через Kubernetes `SecretKeyRef`, не читая и не сохраняя значение token. `build`/`deploy` jobs теперь принимают только типизированные `BuildExecutionSpec`/`DeployExecutionSpec` с проверенными refs/digest/fingerprint и не claim-ятся без spec; Kaniko build executor, Kubernetes rollout executor, нагрузки slot-агента, исполнитель workspace materialization и механизм чтения object-store input остаются отдельными срезами после согласования с `agent-manager`/ops-контуром. |
