...
| Реальный исполнитель platform jobs | первый Kubernetes-срез готов: #940, надёжность worker-а готова: #949, контракт `agent_run` готов: #961 | `runtime-manager` умеет исполнять только безопасное задание `health_check` через включаемый явно Kubernetes-исполнитель. Worker устойчив к остановке процесса, повторному claim и повторной сверке уже созданного Kubernetes Job. `agent_run` имеет typed `AgentRunExecutionSpec` для будущего runner-среза и не claim-ится для исполнения без spec. Реальный запуск agent runner, задания `build`/`deploy`, нагрузки slot-агента, исполнитель workspace materialization и расширенные типы заданий остаются отдельными срезами после согласования с `agent-manager`/ops-контуром. |
