...
| RTM-K8S-1 | #940 | готово | Основа Kubernetes executor: включаемый явно исполнитель `health_check` jobs получает cluster secret ref через `fleet-manager.GetKubernetesCluster`, создаёт ограниченный Kubernetes Job через `client-go` и завершает runtime job через `ReportJobStepProgress`, `CompleteJob` или `FailJob` без хранения kubeconfig, значений секретов и больших логов. |
...
| Реальный исполнитель platform jobs | первый Kubernetes-срез готов: #940 | `runtime-manager` умеет исполнять только безопасный `health_check` job через включаемый явно Kubernetes executor. Задания `build`/`deploy`, нагрузки slot-агента, исполнитель workspace materialization и расширенные типы заданий остаются отдельными срезами после согласования с `agent-manager`/ops-контуром. |
