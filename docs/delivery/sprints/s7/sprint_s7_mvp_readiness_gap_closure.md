...
| Day 7+ | Dev/QA/Release/Postdeploy/Ops/Doc-Audit | P0/P1 | implementation issues `#243..#260`, `#274` (`run:dev`) | in-progress (`#243` и `#244` completed + owner-approved; `#245`, `#246`, `#247/#248/#249` и `#274` in-review; `#250/#257` закрываются doc-actualization pass как уже поглощённые cleanup-потоками; remaining standalone backlog: `#251..#256`, `#258..#260`) |
...
- После combined cleanup `#247/#248/#249/#274` standalone issues `#250` и `#257` закрываются без отдельного `run:dev`, поэтому фактический remaining backlog начинается с `#251`.
...
- Remaining Sprint S7 standalone execution backlog после этой актуализации: `#251..#256`, `#258..#260`.
