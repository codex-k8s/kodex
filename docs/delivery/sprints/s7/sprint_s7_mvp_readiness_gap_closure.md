...
| Day 7+ | Dev/QA/Release/Postdeploy/Ops/Doc-Audit | P0/P1 | implementation issues `#243..#260`, `#274` (`run:dev`) | in-progress (`#243` и `#244` completed + owner-approved; `#245`, `#246`, `#247/#248/#249`, `#251`, `#252`, `#253`, `#258` и `#274` реализованы в execution streams; `#250/#257` закрываются doc-actualization pass как уже поглощённые cleanup-потоками; remaining standalone backlog: `#254..#256`, `#259..#260`) |
...
- После combined cleanup `#247/#248/#249/#274` standalone issues `#250` и `#257` закрываются без отдельного `run:dev`; после реализации `#252` и `#253` фактический remaining backlog начинается с `#254`.
...
- Remaining Sprint S7 standalone execution backlog после этой актуализации: `#254..#256`, `#259..#260`.
