...
- `provider-hub`: базовое gRPC-чтение safe scan snapshots и provider merge signals для project/adoption контура есть; GitLab parity делать только при отдельном решении владельца.
...
2. Проверить, что внешний образ можно зеркалировать в реестр, а внутренний образ можно собрать через Kaniko и сохранить в тот же реестр.
3. Развернуть сервисы с готовыми манифестами и миграциями.
4. Проверить readiness, gRPC-связность, event-log/outbox, migration jobs и smoke-команды.
5. Зафиксировать ошибки в логах pod и закрывать их малыми PR.
6. Не подключать web-console как обязательный критерий этого этапа.
