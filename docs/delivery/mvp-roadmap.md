...
related_issues: [78, 281, 282, 294, 380, 582, 586, 698]
related_prs: []
related_docsets:
  - docs/platform/**
  - docs/domains/**
...
- Первый удалённый deploy начинается до реализации полноценного фронтенда, как только готов минимальный backend-набор сервисов, миграции, манифесты и smoke-команды.
...
- понимать, когда начинать deploy на удалённый сервер;
...
- `provider-hub`: дать удобное чтение safe scan snapshots и provider merge signals для project/adoption контура; GitLab parity делать только при отдельном решении владельца.
...
- `runtime-manager` и `fleet-manager`: использовать уже готовые jobs, slots, placement и health как основу для удалённой проверки.
...
### Этап 2. Первый удалённый deploy backend-контура
...
1. Поднять базовые зависимости: PostgreSQL, platform event log, Keycloak или выбранный IdP-контур, registry/ingress по текущей инфраструктурной модели.
2. Развернуть сервисы с готовыми манифестами и миграциями.
3. Проверить readiness, gRPC-связность, event-log/outbox, migration jobs и smoke-команды.
4. Зафиксировать ошибки в логах pod и закрывать их малыми PR.
5. Не подключать web-console как обязательный критерий этого этапа.

Критерий завершения:

- backend MVP-контур развёрнут на удалённом сервере;
...
- новые срезы можно проверять на удалённом контуре без ручного чтения всех внутренних БД.
...
- Удалённый deploy позволяет агентам смотреть логи pod, readiness и smoke-вывод.
