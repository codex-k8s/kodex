...
- Новое окружение координируется ролями `manager`, `architect`, `reviewer`, `tester`, `sre` и исполнителем задачи; прежняя раскладка шести доменных агентов остаётся архивом.
...
Активная координация ведётся через `docs/delivery/coordination/roles.md`. Прежняя раскладка шести доменных агентов больше не является моделью назначения задач.
...
- Исполнитель задачи делает кодовый или документационный срез по Issue и открывает PR.
- Reviewer проверяет PR по задаче, архитектуре, гайдам, тестам, безопасности данных и live-safe правилам.
- Tester отвечает за приёмку, интеграционные проверки, UI-сценарии, fixture-наборы и регрессии.
