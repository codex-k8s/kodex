...
- Ближайшая цель MVP: получить сквозной рабочий контур, который можно проверить в Kubernetes через проверки готовности, логи pod и затем через операторский интерфейс.
- Первый серверный deploy начинается до реализации полноценного фронтенда, как только готов минимальный backend-набор сервисов, миграции, манифесты и Go integration runners.
...
7. Увидеть runtime jobs, slots, pod readiness, статус проверок и короткие диагностические хвосты без чтения сырого секрета или raw payload.
8. Проверить всё это сначала через Go checks/логи Kubernetes, затем через `staff-gateway` и web-console.
...
- `provider-hub`: базовое gRPC-чтение safe scan snapshots и provider merge signals для project/adoption контура есть; producer-side checks проверяет GitHub bootstrap/adoption webhook fixtures -> safe merge signal -> outbox path; GitLab parity делать только при отдельном решении владельца.
...
- runbook или CLI-команды могут пройти основной backend-сценарий без web-console;
- сервисы пишут безопасные события и диагностические summaries;
- ошибки видны через логи, readiness и проверки готовности, а не только через unit-тесты.
...
- у сервисов MVP есть Dockerfile, Kubernetes manifests, migration jobs, env inventory, health/readiness, runbook;
...
3. В строгом ручном deploy plan проверить Kubernetes foundation, runtime `Secret` и согласованность derived keys; готовность `self-deploy` проверять только отдельным явным режимом, чтобы она не блокировала обычную MVP-выкладку.
4. Проверить, что внешний образ можно зеркалировать в реестр, а внутренний образ можно собрать через Kaniko и сохранить в тот же реестр.
5. Развернуть сервисы с готовыми манифестами и миграциями после отдельного разрешения владельца.
6. Первым кольцом поднять PostgreSQL, platform event log migrations,
   `access-manager`, `project-catalog`, `package-hub` и `provider-hub` через
   локальный deploy-путь с Kaniko-сборкой и Kubernetes `Secret`, без Docker
   daemon и без раскрытия значений env.
7. Проверить readiness, gRPC-связность, event-log/outbox, migration jobs и Go integration runners.
8. Зафиксировать ошибки в логах pod и закрывать их малыми PR.
9. Не подключать web-console как обязательный критерий этого этапа.
...
- строгий ручной deploy plan завершается без требования `self-deploy` и заранее ловит дрейф runtime Secret keys типизированной диагностикой;
- агенты могут использовать логи pod и статус проверок при проверке PR;
- владелец может видеть фактическое состояние deploy через runbook и проверки готовности, даже без фронтенда.
...
- ошибки можно диагностировать через UI, логи pod, Go integration runners и
  тонкие bootstrap/deploy обвязки проверок;
...
- Сквозной backend-срез проходит проверку готовности без UI.
...
- Серверный deploy позволяет агентам смотреть логи pod, readiness и вывод проверок.
...
- Риск: первый deploy откладывается до “идеального MVP”.
  Решение: backend deploy начинать раньше, как только есть минимальные манифесты и проверка готовности.
