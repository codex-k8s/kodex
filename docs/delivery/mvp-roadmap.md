...
- Ближайшая цель MVP: пройти сквозной обычный пользовательский контур, который можно проверить в Kubernetes через проверки готовности, логи pod и операторский интерфейс.
- Первый серверный deploy уже выполнен ручным MVP-путём; дальше он остаётся основным способом выкладки до отдельного возврата к самовыкладке.
...
- Новое окружение координируется ролями `manager`, `architect`, `developer`, `docs`, `reviewer`, `qa` и `sre`; прежняя раскладка шести доменных агентов остаётся архивом.

## Назначение

Этот документ задаёт маршрут до MVP и порядок параллельной работы ролей. Он не заменяет доменные документы в `docs/domains/**` и не переписывает архитектурную канонику в `docs/platform/**`.
...
Статус: базовый ручной MVP deploy выполнен.

Фактическая точка перехода:

- по задаче `#1111` свежий `main` `9cca0f4c` развернут штатным ручным MVP-путём;
- применены backend-кольца, `staff`, `web` и `web-public`;
- эксплуатационная приёмка прошла;
- deployments готовы, активных и failed jobs нет;
- публичный UI закрыт OAuth, неподписанный GitHub webhook отклоняется без OAuth redirect;
- самовыкладка не запускалась и не является критерием готовности MVP.
...
## Координация после перехода в ролевое окружение

Активная координация ведётся через драфт `docs/delivery/coordination/roles.md`. Прежняя раскладка шести доменных агентов больше не является моделью назначения задач. Драфт правил должен дорабатываться новыми агентами по мере обкатки приложения.

Роли:

- Manager ведёт план MVP, создаёт GitHub Issues, фиксирует зависимости и не даёт работе распадаться на несвязанные срезы.
- Architect проверяет доменные границы, контракты, миграции, события и live-safe порядок обновления.
- Developer делает кодовый срез по Issue и открывает PR.
- Docs делает документационный срез по Issue, синхронизирует инструкции, runbook, карты связей и доменные документы.
- Reviewer проверяет PR по задаче, архитектуре, гайдам, тестам, безопасности данных и live-safe правилам.
- QA отвечает за приёмку, интеграционные проверки, UI-сценарии, fixture-наборы и регрессии.
- SRE ведёт путь выкладки, Kubernetes, registry/Kaniko, Secrets, готовность, задания, сертификаты, ingress и эксплуатационную приёмку.

Правило для дальнейшей работы: каждый новый срез должен иметь GitHub Issue с целевым результатом, проверками и ролью, а не назначение вида `агент #N`.

Ближайший порядок после ручной выкладки:

1. Проверить живой обычный MVP-сценарий без самовыкладки: проект, репозиторий, проверенный `services.yaml`, обычный `AgentRun`, активность, приёмка, артефакт провайдера, решение владельца и governance-сводка.
2. Закрыть блокеры, найденные в этом сценарии, малыми PR с обновлением доменной документации и runbook.
3. Довести `staff-gateway` и `web-console` до удобного сценария владельца и оператора без ручного ввода технических идентификаторов.
4. Вернуться к самовыкладке только отдельным решением, когда обычный сценарий и ручная выкладка стабильно работают.
