...
| #829 | `services/external/integration-gateway/Dockerfile`, `deploy/base/integration-gateway/**`, `deploy/base/postgres/secrets.yaml.tpl`, `services.yaml`, `scripts/build-integration-gateway-images.sh`, `scripts/smoke-integration-gateway.sh`, `bootstrap/host/**`, `bootstrap/local/steps/**`, `docs/domains/integration-gateway/**`, `docs/delivery/coordination/agent-2-provider-hub.md` | IGW-5 | готово | Добавлен deploy-контур `integration-gateway`: Dockerfile, Kubernetes manifests, config/secret refs, smoke, runbook, monitoring и rollback без БД, inbox, provider business state и раскрытия секретов. |
| #853 | `services/external/integration-gateway/**`, `specs/openapi/integration-gateway.v1.yaml`, `tools/codegen/openapi/integration-gateway.oapi-codegen.yaml`, `services.yaml`, `deploy/base/integration-gateway/**`, `scripts/smoke-integration-gateway.sh`, `docs/domains/integration-gateway/**`, `docs/domains/interaction-hub/architecture/api_contract.md`, `docs/delivery/coordination/agent-2-provider-hub.md` | IGW-6 | готово | Активирован generic callback route channel/package -> `interaction-hub.RecordChannelCallback`: gateway проверяет source/signature/size/backpressure, передаёт safe envelope и не хранит callback lifecycle или бизнес-состояние. |
| #895 | `fixtures/provider-webhooks/**`, `scripts/smoke-provider-merge-signal.sh`, `services/external/integration-gateway/**`, `services/internal/provider-hub/**`, `docs/domains/integration-gateway/**`, `docs/domains/provider-native-work-items/**`, `docs/delivery/coordination/agent-2-provider-hub.md` | Provider merge signal smoke | готово | Staged smoke использует safe GitHub `pull_request closed + merged` fixture, проверяет provider webhook route wiring до `provider-hub.IngestWebhookEvent` и оставляет merge signal/outbox producer-side проверку во владельце `provider-hub`. |
| #909 | `fixtures/provider-webhooks/**`, `scripts/smoke-provider-merge-signal.sh`, `services/external/integration-gateway/**`, `services/internal/provider-hub/**`, `docs/domains/integration-gateway/**`, `docs/domains/provider-native-work-items/**`, `docs/delivery/coordination/agent-2-provider-hub.md` | Provider adoption merge signal smoke | готово | Staged smoke расширен на bootstrap/adoption fixtures: gateway route проверяет HTTP/HMAC boundary для обоих payload, не хранит state и не публикует provider business events. |
