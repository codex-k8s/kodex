...
| #281, #282 | `docs/platform/architecture/repository_onboarding.md`, `docs/domains/agent-orchestration/**` | междоменное решение | требуют выбора владельца | `agent-manager` должен запускать bootstrap/adoption роли, готовить отчёт и PR через provider-контур, но не владеть проектной политикой, пакетами или файловой системой workspace. |
