...
| #753 | `docs/domains/platform-mcp-server/architecture/contract_strategy.md`, `docs/domains/codex-hook-ingress/**`, `docs/platform/architecture/codex_hooks_and_skills.md` | MCP-1 | готово | Codex hooks отделены от MCP-инструментов: `agent-manager` будет потреблять hook events через `codex-hook-ingress`, а MCP tools через `platform-mcp-server`. |
