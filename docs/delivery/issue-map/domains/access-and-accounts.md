...
| без отдельного Issue | `docs/platform/architecture/data_model.md`, `docs/domains/access-and-accounts/architecture/design.md`, `docs/domains/access-and-accounts/architecture/data_model.md`, `docs/domains/access-and-accounts/delivery/wave7_access_and_accounts.md`, `libs/go/eventlog/**`, `services/internal/access-manager/**` | волна 7 | MVP-доставка событий готова | Общий PostgreSQL-журнал событий и checkpoint API реализованы в `libs/go/eventlog`; `access-manager` по умолчанию публикует outbox-события через штатный канал `postgres-event-log`. Реальные бизнес-потребители добавляются в доменах-потребителях. |
