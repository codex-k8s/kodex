...
| без отдельного Issue | `docs/platform/architecture/data_model.md`, `docs/domains/access-and-accounts/architecture/design.md`, `docs/domains/access-and-accounts/architecture/data_model.md`, `docs/domains/access-and-accounts/delivery/wave7_access_and_accounts.md`, `libs/go/eventlog/**`, `services/internal/access-manager/**`, `services/internal/platform-event-log/**`, `deploy/base/postgres/postgres.yaml.tpl` | волна 7 | MVP-доставка событий готова | Общий PostgreSQL-журнал событий и checkpoint API реализованы в `libs/go/eventlog`; миграции общего журнала вынесены в контур `platform-event-log`, а `access-manager` публикует outbox-события через штатный канал `postgres-event-log` по отдельному DSN. Реальные бизнес-потребители добавляются в доменах-потребителях. |
