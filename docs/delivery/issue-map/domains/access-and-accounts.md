...
| без отдельного Issue | `docs/platform/architecture/data_model.md`, `docs/domains/access-and-accounts/architecture/design.md`, `docs/domains/access-and-accounts/architecture/data_model.md`, `docs/domains/access-and-accounts/delivery/wave7_access_and_accounts.md`, `services/internal/access-manager/**`, `specs/asyncapi/access-manager.v1.yaml` | волна 7 | outbox-доставщик готов | Добавлены аренда outbox-записей, повтор временных ошибок, постоянные сбои без бесконечного повтора, защита отметок номером попытки и контрактная сверка реализованных `access.*` событий с AsyncAPI. Зафиксировано, что MVP-доставка потребителям пойдёт через общий PostgreSQL event log и inbox/checkpoint отдельным обязательным срезом, а диагностический канал в лог не является MVP-доставкой. |
