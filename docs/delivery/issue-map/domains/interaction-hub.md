...
| не назначено | `docs/domains/interaction-hub/delivery/interaction_hub_delivery.md` | IH-5+ | planned | Следующие срезы должны выделяться в отдельные Issue: notification/subscription lifecycle, delivery attempts, channel contract integration, callback, MCP и ops. |
