...
| #830 | `services/internal/platform-mcp-server/**`, `docs/domains/platform-mcp-server/architecture/api_contract.md`, `docs/domains/platform-mcp-server/architecture/design.md`, `docs/domains/platform-mcp-server/delivery/platform_mcp_server_delivery.md`, `docs/platform/architecture/mcp_and_interaction_model.md`, `docs/delivery/coordination/agent-1-project-catalog.md` | MCP-3g | готово | Подключены governance gate lifecycle tools поверх GOV-4: request/get/list/submit decision/cancel/expire идут только через `governance-manager`, MCP не хранит decision state и возвращает только safe refs/status/summary. Risk evaluator, release decision engine, delivery/callback и provider write pipeline не входят. |
