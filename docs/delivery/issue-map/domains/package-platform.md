...
| не назначено | `services/internal/package-hub/**`, `docs/domains/package-platform/**` | PKG-5.3b | запланировано | Сверка статуса заполненности секретов установки после согласования контракта ссылок установки в `access-manager`; checker без выдачи значения готов в общем secret resolver. |
| #711 | `libs/go/secretresolver/**`, `docs/platform/architecture/secret_resolution.md`, `docs/domains/package-platform/**` | междоменный срез секретов | готово | `package-hub` может проверять доступность секрета без значения через `secretresolver.Checker`, но не получает сырое значение и всё ещё ждёт access-контракт ссылок установки. |
