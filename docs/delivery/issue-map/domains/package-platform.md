...
| #281, #282 | `docs/platform/architecture/repository_onboarding.md`, `docs/domains/package-platform/**` | междоменное решение | требуют выбора владельца | Bootstrap/adoption должны учитывать установки пакетов и руководящих пакетов через `package-hub`; тексты пакетов и руководства не копируются в БД, а workspace собирается платформой из установленных пакетов и `source_ref`. |
