...
| #723 | `proto/kodex/packages/**`, `services/internal/package-hub/**`, `libs/go/secretresolver/**`, `docs/domains/package-platform/**`, `docs/platform/architecture/secret_resolution.md` | PKG-5.3b | готово | `RefreshPackageInstallationSecretStatus` получает безопасные ссылки секретов установки из `access-manager`, проверяет доступность через `secretresolver.Checker` без чтения значения и обновляет `secret_binding_status`. |
...
| #718 | `proto/kodex/access_accounts/**`, `services/internal/access-manager/**`, `docs/domains/access-and-accounts/**`, `docs/domains/package-platform/**` | междоменный срез доступа | готово | `access-manager` добавил `ListPackageInstallationSecretRefs` для безопасной выдачи ссылок и статусов секретов установки без значений; `package-hub` использует этот контракт в PKG-5.3b. |
...
| не назначено | `services/internal/package-hub/**`, `docs/catalogs/plugins/**`, `docs/catalogs/package-store/**` | PKG-6.3b+ | запланировано | Следующие интеграционные сценарии магазина, плагинов и пользовательского контента после согласования внешнего адаптера магазина, provider-native доступа и runtime-размещения. |
| #710 | `services/internal/package-hub/Dockerfile`, `deploy/base/package-hub/**`, `deploy/base/postgres/**`, `services.yaml`, `bootstrap/host/**`, `scripts/build-package-hub-images.sh`, `scripts/smoke-package-hub.sh`, `docs/domains/package-platform/delivery/package_hub.md`, `docs/domains/package-platform/operations/package_hub_runbook.md` | PKG-7 | готово | Эксплуатационный контур `package-hub`: манифесты, migration job, config, health, metrics, smoke-проверка и runbook. |
| #281, #282 | `docs/platform/architecture/repository_onboarding.md`, `docs/domains/package-platform/**` | междоменное решение | модель выбрана, ждёт реализации | Bootstrap/adoption должны учитывать установки пакетов, руководящих пакетов и шаблонов репозиториев через `package-hub`; тексты пакетов и руководства не копируются в БД, а workspace собирается платформой из установленных пакетов, шаблонов и `source_ref`. |
