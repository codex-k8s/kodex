...
| #639 | `docs/design-guidelines/go/**`, `services/internal/project-catalog/**`, `services/internal/access-manager/**`, `libs/go/postgres/**` | после wave 8.2 | запланирована | Системный перевод PostgreSQL-сканеров на штатные помощники `pgx`, где не нужна ручная доменная конвертация. |
