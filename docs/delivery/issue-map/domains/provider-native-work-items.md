...
| #281, #282 | `docs/platform/architecture/repository_onboarding.md`, `docs/domains/provider-native-work-items/delivery/provider_hub_delivery.md` | PRV-8 | требуют выбора владельца | Provider-часть empty repository bootstrap и existing repository adoption должна следовать выбранной модели onboarding. `provider-hub` выполняет provider-native запись и зеркало, но не владеет проектной политикой; сканирование и отчёт выполняет агентная роль через workspace. |
