...
- Day4 architecture (`#340`) закрепляет ownership для projections, commands, provider sync/reconciliation и degraded realtime path и передаёт continuity в design issue `#344`.
...
- Day 5 (Design): follow-up issue `#344`; должна подготовить implementation-ready package по API/data/UI/reconciliation.
