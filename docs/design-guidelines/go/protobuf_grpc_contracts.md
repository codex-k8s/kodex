...
- Транспортные DTO живут в `contracts/proto/` (gRPC) и в `contracts/openapi/**` (HTTP/OpenAPI только для gateway-поверхностей).
...
- В live-кластере breaking change в `.proto` допустим только если все потребители обновляются в
  согласованном срезе и нет live rolling gap. Если такой гарантии нет, сначала добавляется новое
  поле/операция с новым номером, затем потребители переводятся на неё, и только потом старый
  контракт удаляется отдельным срезом.
- Объём `v1`:
