...
- Изменения policy/tool scope сопровождаются синхронным обновлением проектной документации.
  До заполнения `docs/platform/architecture/**` и релевантных доменных пакетов источником правды является `refactoring/**`; старые MCP-документы в `deprecated/docs/architecture/**` используются только как справка.
