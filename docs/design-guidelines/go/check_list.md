...
- Значения секретов, полученные по ссылке, представлены безопасным типом вроде `secretresolver.SecretValue`, не сериализуются в JSON/text/fmt и живут только в памяти процесса на время операции.
