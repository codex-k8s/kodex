...
- Если новая или изменённая библиотека из `libs/go/*` зависит от других локальных контрактов (`contracts/proto/**`, `contracts/openapi/**`, `contracts/asyncapi/**`, `libs/go/accesscatalog`, `libs/go/platformevents/**` и т.п.), каждый deployable-сервис должен иметь в `bumpOn` как саму библиотеку, так и прямые локальные контракты, которые сервис импортирует или использует через неё как часть runtime/build-контракта.
...
- `libs/go/secretresolver/*` — общий контракт безопасного получения значения секрета по разрешённой ссылке и проверки доступности без раскрытия значения; детали реализации хранилища (`kubernetes_mounted_secret`, `env`) не должны протекать в домены-потребители.
...
- транспортные DTO продукта (для этого есть `contracts/proto/` и OpenAPI/AsyncAPI контракты);
