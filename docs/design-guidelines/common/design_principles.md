...
- Доменная логика и доменные модели не зависят от SQL/HTTP/gRPC; транспортные
  DTO Proto/OpenAPI маппятся на границе; границы контекстов проходят по
  API/событиям, а не shared DB/shared tables.
...
- Общий код — в `libs/`; общие транспортные контракты (gRPC) — в `contracts/proto/` (не копипаст по сервисам).
...
- Env остаётся только для bootstrap, secrets и infrastructure/runtime параметров, которые нужны до доступа к БД или не должны редактироваться через control surface.
