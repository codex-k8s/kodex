...
- Dockerfile не фиксирует `EXPOSE` для портов приложения. Порты объявляются в Kubernetes-манифестах
  и runtime-конфигурации сервиса, чтобы Dockerfile не расходился с фактическим контуром запуска;
