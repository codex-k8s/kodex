...
- Контракты транспорта не “вшиты в код строками” и имеют источник правды:
  ...
  - HTTP: OpenAPI YAML только для gateway-поверхностей
...
- Сырые значения секретов не сериализуются, не попадают в БД, события, тело аудита, трассировку, метки метрик или ошибки; для разрешённых ссылок используется общий secret resolver или профильный безопасный контракт.
