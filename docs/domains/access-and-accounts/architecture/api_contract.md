...
`CheckAccess` сначала сверяет действие с системным code-owned каталогом и затем с `AccessAction`: отсутствующее действие, отключённое действие или несовпадение типа ресурса дают объяснимое решение `deny` с audit-записью, а не локальную догадку вызывающего сервиса. Пользователь в состоянии `pending` даёт решение `pending`; пользователь в состояниях `blocked` и `disabled`, неактивная организация, группа или внешний аккаунт дают `deny`. Если `subject_type=user` передан как UUID, `access-manager` проверяет статус пользователя и раскрывает членство через membership graph. Если доверенный edge или `staff-gateway` передаёт безопасный внешний owner ref вроде `user/owner-1`, `CheckAccess` не падает на приведении к UUID, но оценивает только явные прямые `AccessRule` для этого `subject_id`: статус пользователя, членство и allowlist по такому ref не выводятся, а неявные owner-права не появляются.
...
`PutAccessRule` принимает `service` как тип субъекта для внутренних сервисов платформы. Такое правило используется для межсервисных команд, `cmd/onboarding-runner` и сервисных путей потребления событий, где аудит должен показывать реальный сервисный субъект. Для self-deploy onboarding runner создаёт узкие прямые правила `service/agent-manager` и `service/staff-gateway` с `action=project.policy.read`, `resource_type=services_policy`, `scope=project/<project_id>`, чтобы сервисы могли читать `project-catalog.GetSelfDeploySignal` и связанный checked build plan input без широких admin-прав. Для governance части self-deploy runner создаёт сервисам только нужные права: `service/agent-manager` получает `governance.risk.evaluate`, `governance.risk.read`, `governance.gate.request` и `governance.gate.read`, а `service/staff-gateway` только `governance.risk.read` и `governance.gate.read` в project scope. Для собственного service actor runner создаёт `action=repository.read`, `resource_type=repository`, `resource_id=<repository_id>`, `scope=project/<project_id>`, чтобы штатно читать только текущий self-repo binding перед import/reconcile. Это не заменяет `external_account`: внешний аккаунт поставщика типа `service` остаётся внешним аккаунтом с владельцем, привязкой использования и ссылкой на секрет. `service` не добавляется в `Membership`, а проверяется через `CheckAccess` как прямое правило по `subject_type=service` и строковому `subject_id`. Для owner-facing UI допустимо такое же прямое правило по `subject_type=user` и безопасному внешнему `subject_id`, если edge ещё не передаёт UUID пользователя; self-repo apply создаёт такие правила только при явно заданном safe `owner_user_ref`. Owner получает `governance.risk.read`, `governance.gate.read` и `governance.gate.decide` в project scope для self-deploy governance path. Решение gate дополнительно ограничивается в `governance-manager`: команда сначала читает gate, затем для linked risk assessment авторизует действие по target/project assessment. Это временно-совместимый путь авторизации конкретного owner ref, а не membership-модель и не замена штатного пользователя домена доступа; email и другие персональные данные не используются как `AccessRule.subject_id`.
