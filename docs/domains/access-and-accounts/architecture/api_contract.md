...
`PutAccessRule` принимает `service` как тип субъекта для внутренних сервисов платформы. Такое правило используется для межсервисных команд, `cmd/onboarding-runner` и сервисных путей потребления событий, где аудит должен показывать реальный сервисный субъект. Для self-deploy onboarding runner создаёт узкие прямые правила `service/agent-manager` и `service/staff-gateway` с `action=project.policy.read`, `resource_type=services_policy`, `scope=project/<project_id>`, чтобы сервисы могли читать `project-catalog.GetSelfDeploySignal` и связанный checked build plan input без широких admin-прав. Для собственного service actor runner создаёт `action=repository.read`, `resource_type=repository`, `resource_id=<repository_id>`, `scope=project/<project_id>`, чтобы штатно читать только текущий self-repo binding перед import/reconcile. Это не заменяет `external_account`: внешний аккаунт поставщика типа `service` остаётся внешним аккаунтом с владельцем, привязкой использования и ссылкой на секрет. `service` не добавляется в `Membership`, а проверяется через `CheckAccess` как прямое правило по `subject_type=service` и строковому `subject_id`. Для owner-facing UI допустимо такое же прямое правило по `subject_type=user` и безопасному внешнему `subject_id`, если edge ещё не передаёт UUID пользователя; это временно-совместимый путь авторизации конкретного owner ref, а не membership-модель и не замена штатного пользователя домена доступа.
