...
`ListPackageInstallationSecretRefs` проверяет право `package.installation.secret_ref.read` в области установки и возвращает только безопасные поля: логический ключ из схемы пакета, статус настройки, `secret_store_type`, `secret_store_ref`, идентификатор ссылки и безопасные метаданные. Значение секрета, отпечаток значения и любые производные от значения не входят в ответ. Если вызывающий передал список ожидаемых логических ключей, отсутствующие привязки возвращаются со статусом `missing`, чтобы `package-hub` мог пересчитать заполненность установки без локального хранения ссылок. Проверка фактической доступности значения остаётся за `secretresolver.Checker`; `package-hub` не вызывает `Resolver.Resolve`.
