...
- штатный канал публикации `postgres-event-log` пишет событие через отдельное подключение к БД `platform-event-log`, где `id` outbox-события становится `event_id` общего журнала;
