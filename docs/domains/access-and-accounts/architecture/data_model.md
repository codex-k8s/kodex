...
| `published_at` | timestamp | yes | Время успешной передачи события в подключённый канал публикации. |
...
- доставщик забирает событие только если оно не опубликовано, наступило `next_attempt_at` и истекла предыдущая аренда;
