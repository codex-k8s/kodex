...
- Основные компоненты: доменная БД `access-manager`, gRPC API, outbox событий, путь чтения для операторской консоли, интеграция с SSO/OIDC и безопасными ссылками на секреты.
...
|---|---|
| Организации, пользователи, группы, членство, allowlist, внешние аккаунты как субъекты политики, правила доступа, аудит решений, безопасные ссылки на секреты внешних аккаунтов и установок пакетов. | Проекты, репозитории, зеркало провайдера, webhook, лимиты провайдера, агентные запуски, слоты, задания, уведомления, биллинг, значения секретов. |
...
Для пакетных установок `access-manager` хранит привязки логических ключей схемы пакета к `SecretBindingRef`, но не владеет самой установкой пакета и не хранит её состояние. `package-hub` передаёт идентификатор установки, область и ожидаемые ключи, а `access-manager` после проверки права возвращает безопасные ссылки и статусы заполненности без значений секретов.
...
| Каталог поставщиков и действий | Хранит поставщиков внешних аккаунтов, пользовательские и пакетные действия для правил доступа; системные действия задаются в code-owned каталоге с i18n message id. |
...
| Контур `platform-event-log` | Владеет отдельной БД и миграциями общего журнала событий; `access-manager` только публикует туда события через отдельный DSN. |
...
В текущем сервисном контуре реализован PostgreSQL outbox внутри `access-manager`, штатный канал публикации `postgres-event-log` и отдельный контур `platform-event-log` с общим журналом `platform_event_log`. Миграции общего журнала не входят в БД `access-manager`; сервис доступа подключается к журналу отдельным DSN, публикует событие идемпотентно по `event_id` и неизменяемому содержимому, а разные потребители получают независимую веерную доставку через собственный checkpoint. Реальные бизнес-подписчики на `access.*` события добавляются в сервисах-потребителях отдельными сценариями, когда этим сервисам понадобится реакция на изменение доступа.
