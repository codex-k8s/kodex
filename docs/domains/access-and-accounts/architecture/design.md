...
| Outbox-доставщик | Забирает `access.*` события после фиксации транзакции, ставит короткую аренду записи, повторяет временные ошибки с задержкой и передаёт событие в подключённый канал публикации. |
...
В текущем сервисном срезе реализован PostgreSQL outbox и интерфейс канала публикации. Диагностический канал публикации в лог включается отдельной настройкой процесса и не заменяет будущий платформенный транспорт событий для межсервисных подписчиков.
