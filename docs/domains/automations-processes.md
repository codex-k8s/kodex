...
- Перед межкомнатным запуском менеджер проверяет каталог и карточку целевого чата и передаёт `owner_mention` и контракт обратного вызова в `message` вызова `mattermost_start_agent_thread`.
...
- Только запущенная после этого дочерняя сессия публикует через `mattermost_post_thread_update` отдельное сообщение с точным упоминанием владельца, сохраняет собственный `post_id` и возвращает его через `mattermost_return_to_requester`; менеджер проверяет наличие и соответствие `post_id` в обратном вызове.
