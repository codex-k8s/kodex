...
Карточка внимания не содержит agent summary или prompt: её текст, props, payload hash и delivery id формирует сервер. Повтор callback и ограниченное восстановление при старте допубликовывают сохранённую карточку либо связывают уже найденный post. Это восстановление принадлежит автоматизациям и не заменяет общий watchdog или callback outbox среды выполнения.
...
Для запроса, созданного итогом автоматизации, это же решение атомарно переводит точные `ScheduledRun` и occurrence в `succeeded`, задаёт `finished_at` и добавляет audit. Точный replay решения возвращает существующий результат; другой пользователь, канал, корневой тред или новый post после разрешения не создаёт повторного перехода.
