...
type: runbook
title: "codex-hook-ingress — runbook: deploy, smoke и rollback"
status: active
owner_role: SRE
...
# Runbook: codex-hook-ingress — deploy, smoke и rollback
...
- Быстрое восстановление: исправить image/env, перезапустить `Deployment/codex-hook-ingress`, выполнить smoke; при неудачном rollout откатить image tag или вернуть предыдущий ConfigMap.
...
- Локально для smoke нужны `go`, `kubectl` и `curl`.
...
KODEX_PRODUCTION_NAMESPACE=kodex-prod \
  scripts/smoke-codex-hook-ingress.sh
```

Путь проверки:

- рендерит только `deploy/base/codex-hook-ingress`;
- применяет `ServiceAccount`, `ConfigMap`, `Service` и `Deployment`;
- ждёт rollout `Deployment/codex-hook-ingress`;
- проверяет `/health/readyz`, `/health/livez` и `/metrics` через port-forward.
...
- `/metrics` доступен.
- `scripts/smoke-codex-hook-ingress.sh` завершается сообщением `health, readiness and metrics OK`.
...
- Если обнаружен пробел в limits, route diagnostics, manifests, smoke или runbook, обновить документацию вместе с исправлением.
