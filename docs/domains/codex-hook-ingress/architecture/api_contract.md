...
## Состояние реализации CHI-3/CHI-4/CHI-4b/CHI-5/CHI-6a/CHI-7/CHI-8
...
CHI-8 добавляет deploy-контур: Dockerfile, Kubernetes `Deployment/Service/ConfigMap`, health/readiness probes, Prometheus scrape annotations, image/config inventory, smoke, runbook и monitoring. Deploy-контур не добавляет БД, migration job, secret refs, persistent ops feed или physical handler для `SubmitHookEvent`.

В CHI-3/CHI-4/CHI-5/CHI-6a/CHI-7/CHI-8 не создаются proto, OpenAPI, AsyncAPI, HTTP/gRPC handler для `SubmitHookEvent` и network client emitter/sidecar. Служебный HTTP-процесс отдаёт только `/health/livez`, `/health/readyz` и `/metrics`.
