...
CHI-7 расширяет normalized envelope и sanitizer boundary для safe capability refs Codex skills. `capability_context` может нести `capability_context_ref`, `capability_digest`, refs выбора `agent-manager`, refs materialization `runtime-manager` и массив `skill_refs` с `source_ref`, `package_ref`, `package_installation_ref`, `package_version_ref`, `manifest_digest`, `capability_ref`, `capability_kind`, `package_slug`, `package_version_label`, `invocation_policy_ref`, `policy_summary_digest` и `digest`. Эти поля проходят validation/sanitizer/routing как refs/digests и попадают в `agent-manager.RecordAgentActivity` через `safe_refs_json`, если событие маршрутизируется в persistent activity timeline. `codex-hook-ingress` не читает и не хранит `SKILL.md`, manifest payload, materialized workspace path или package installation state.
