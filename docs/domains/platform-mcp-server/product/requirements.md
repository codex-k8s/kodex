---
doc_id: PRD-CK8S-PLATFORM-MCP-0001
type: prd
title: kodex — требования к platform-mcp-server
status: active
owner_role: PM
created_at: 2026-05-14
updated_at: 2026-05-14
related_issues: [747]
related_prs: []
related_docsets:
  - docs/domains/platform-mcp-server/architecture/design.md
  - docs/domains/platform-mcp-server/architecture/api_contract.md
  - docs/domains/platform-mcp-server/delivery/platform_mcp_server_delivery.md
approvals:
  required: ["Owner"]
  status: approved
  request_id: "owner-2026-05-14-platform-mcp-kickoff"
  approved_by: "ai-da-stas"
  approved_at: 2026-05-14
---

# Требования к platform-mcp-server

## TL;DR

- Что строим: платформенный MCP-сервер как безопасную инструментальную поверхность для `agent-manager`, slot-агентов и hook emitter.
- Для кого: быстрый `agent-manager`, ролевые агенты в слотах, локальный hook emitter, будущие управляемые интеграции и операторы диагностики.
- Почему: агентам нужен быстрый и контролируемый способ читать актуальные данные платформы и выполнять разрешённые операции без прямого обхода сервисов-владельцев.
- MVP: приём hook-событий, группы MCP-инструментов для agent/provider/project/runtime/package чтений, ограниченная диагностика, авторизация источника, очистка данных вызова, идемпотентность и correlation id.
- Критерии успеха: MCP не хранит чужую истину, не раскрывает секреты, маршрутизирует операции только к владельцам и даёт `agent-manager` основу для следующих AGO-срезов.

## Проблема и цель

`agent-manager` и slot-агенты должны быстро получать актуальные данные платформы и выполнять разрешённые операции. Прямой доступ к каждому сервису из агента создаёт дублирование политик, смешивает ответственность и усложняет аудит. Нужна единая MCP-поверхность, которая нормализует вызов, проверяет источник, применяет минимальную границу политики и передаёт команду сервису-владельцу.

Цель `platform-mcp-server` — быть безопасным маршрутизатором инструментов, а не новым доменным сервисом.

## Пользователи и вызывающие стороны

| Вызывающая сторона | Потребность |
|---|---|
| Быстрый `agent-manager` | Получать инструменты для запуска, продолжения, приёмки, gate и чтений соседних доменов. |
| Агент в слоте | Запрашивать платформенные действия, обратную связь владельца, статус runtime и безопасные provider-сигналы. |
| Hook emitter | Передавать события жизненного цикла Codex hooks в платформу. |
| Операторский контур | Получать ограниченную диагностику MCP-поверхности и зависимостей. |
| Будущие управляемые интеграции | Использовать те же правила actor/source binding и policy boundary. |

## Функциональные требования

| ID | Требование | Приоритет |
|---|---|---|
| MCP-FR-1 | Сервис должен принимать нормализованные MCP tool calls и hook-события только с явным actor/source/run/session/slot контекстом. | Обязательно |
| MCP-FR-2 | Сервис должен предоставлять MVP-группу hooks: `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PreCompact`, `PostCompact`, `Stop`. | Обязательно |
| MCP-FR-3 | Операции `Run`, session, gates, flow, role и prompt должны маршрутизироваться только в `agent-manager`. | Обязательно |
| MCP-FR-4 | Provider-инструменты должны использовать `provider-hub` write/read операции и не должны ходить напрямую в GitHub/GitLab. | Обязательно |
| MCP-FR-5 | Чтения project/runtime/fleet/package должны идти только к сервисам-владельцам: `project-catalog`, `runtime-manager`, `fleet-manager`, `package-hub`. | Обязательно |
| MCP-FR-6 | Запросы обратной связи, решений владельца и доставки уведомлений должны маршрутизироваться в `interaction-hub`, когда его контракт готов. | Обязательно |
| MCP-FR-7 | Диагностика должна быть ограниченной: readiness, статус зависимостей, классы ошибок, короткие безопасные сообщения без больших логов. | Обязательно |
| MCP-FR-8 | Сервис должен поддерживать идемпотентные команды через `command_id` или idempotency key и единый `correlation_id`. | Обязательно |
| MCP-FR-9 | Сервис должен отклонять вызовы без проверенного source binding или с несовместимым run/slot контекстом. | Обязательно |
| MCP-FR-10 | Сервис должен публиковать или передавать аудит только для решений, отказов, risky operations и permission/gate сценариев. | Обязательно |

## Нефункциональные требования

| Область | Требование |
|---|---|
| Безопасность | Не хранить секреты, сырые данные инструментов, полные prompt, stdout/stderr и исходные данные провайдера. |
| Надёжность | Ошибка одного сервиса-владельца не должна ломать все группы инструментов; ответ должен иметь явный класс зависимости. |
| Производительность | MCP-поверхность должна быть коротким маршрутом без тяжёлой агрегации и без чтения больших данных. |
| Ограничение нагрузки | Нужны rate limits, backpressure, размерные лимиты данных вызова и timeout на вызов владельца. |
| Наблюдаемость | Метрики по группам инструментов, отказам политики, задержкам владельцев, срабатываниям очистки и rate limits. |
| Совместимость | Внешняя MCP-поверхность версионируется отдельно от внутренних gRPC-контрактов владельцев. |

## Критерии приёмки

- Сервисная граница явно запрещает владение business state.
- MVP-группы инструментов описаны без преждевременной детализации proto и AsyncAPI.
- Для каждого класса инструментов указан сервис-владелец.
- Hook-события #698 связаны с MCP-поверхностью, но сама задача #698 остаётся открытой до реализации.
- Безопасность покрывает actor/source/run/slot binding, очистку данных вызова, rate limits, аудит, идемпотентность и correlation id.
- Delivery-план разделяет документацию, контракты, каркас сервиса, hooks, инструментальные маршруты, безопасность и deploy.

## Не-цели

- Не реализовывать MCP tools в этом срезе.
- Не создавать proto, AsyncAPI, миграции или код.
- Не делать `platform-mcp-server` владельцем `Run`, slot, provider artifact, package installation, project policy или диалогов.
- Не начинать `staff-gateway`, `interaction-hub` или полноценный UI.
- Не закрывать #698 этим срезом.

## Риски

| Риск | Митигирующее решение |
|---|---|
| MCP станет скрытым монолитом. | Документы и будущий код требуют маршрутизации к владельцам, без чужой бизнес-логики и чужих таблиц. |
| В БД попадут сырые hook/tool данные. | MVP фиксирует только безопасные сводки, решения, ссылки, отпечатки и ограниченную диагностику. |
| Provider-инструменты начнут обходить `provider-hub`. | Внешняя поверхность остаётся типизированной, а запись идёт через provider write pipeline. |
| `agent-manager` и MCP начнут дублировать gates. | MCP только принимает и маршрутизирует; доменное состояние gate принадлежит `agent-manager` и `interaction-hub`. |

## Апрув

- request_id: `owner-2026-05-14-platform-mcp-kickoff`
- Решение: approved
- Комментарий: требования к `platform-mcp-server` согласованы как целевое состояние MCP-0.
