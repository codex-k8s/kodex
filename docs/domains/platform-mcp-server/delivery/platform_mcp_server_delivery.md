...
| MCP-3r | #841 | Risk evaluator инструменты governance готовы: `governance.risk.evaluate/reevaluate/get/list` маршрутизируются только через `governance-manager`, принимают typed refs и bounded summaries, возвращают assessment refs/status/risk class, matched rule refs/counts, required gate refs, version/timestamps и не хранят risk state в MCP. |
