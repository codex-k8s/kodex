...
| `governance.risk.evaluate` / `governance.risk.reevaluate` | готово | Маршрутизируются в `EvaluateRisk`/`ReevaluateRisk`; MCP принимает только typed refs, bounded evidence, safe classifier summary, expected version/idempotency/correlation и не хранит risk state. |
| `governance.risk.get` / `governance.risk.list` | готово | Маршрутизируются в `GetRiskAssessment`/`ListRiskAssessments`; MCP возвращает bounded assessment refs/status/risk class, matched rule refs/counts, required gate refs, version/timestamps и safe summary. |
