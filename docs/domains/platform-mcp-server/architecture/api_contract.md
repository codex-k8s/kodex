...
| `governance.summary.get` | `governance-manager` | Прочитать безопасную сводку governance по одному selector: target, project/repository context, release candidate, release decision package id или integration ref, включая доменно подготовленный `status` rollup. |
...
| `governance.summary.get` | готово | Маршрутизируется в `GetGovernanceSummary`; MCP принимает ровно один selector, передаёт actor/source/request context, возвращает owner-prepared `status` rollup, pending/completed decisions, risk/gate/release outcomes, linked provider/agent/runtime evidence refs, diagnostics и не хранит governance state. |
