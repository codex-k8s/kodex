...
В сервисном каркасе активны диагностический инструмент `diagnostics.mcp_status.read` и первый набор инструментов `agent-manager`, для которых у владельца уже есть кодовая реализация: сессия, `Run`, запись session snapshot и безопасное чтение run context. Остальные инструменты из этого документа остаются целевой картой и добавляются отдельными срезами после готовности контрактов и бизнес-реализации сервисов-владельцев.
