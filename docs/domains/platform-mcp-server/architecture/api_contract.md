...
related_issues: [747, 753, 760, 771, 780, 830, 841, 852, 933, 322]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
| `governance.summary.get` | `governance-manager` | Прочитать безопасную сводку governance по одному selector: target, project/repository context, release candidate, release decision package id или integration ref. |
...
| `governance.summary.get` | готово | Маршрутизируется в `GetGovernanceSummary`; MCP принимает ровно один selector, передаёт actor/source/request context, возвращает pending/completed decisions, risk/gate/release outcomes, linked provider/agent/runtime evidence refs, diagnostics и не хранит governance state. |
