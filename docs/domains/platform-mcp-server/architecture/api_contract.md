...
- Версионирование: каталог MCP-инструментов версионируется отдельно от внутренних gRPC-контрактов сервисов-владельцев.
...
Машинно-читаемый каталог инструментов живёт в `docs/domains/platform-mcp-server/catalog/tool_catalog.v1.yaml`. Он является источником правды для имён инструментов, групп событий, общего envelope, правил версионирования, классификации ошибок и тестовых примеров.

Этот документ фиксирует верхнеуровневую карту и смысл контрактов. Кодовые срезы должны уже отталкиваться от каталога и создать:

- транспортные DTO для MCP-envelope;
- отдельный DTO входного envelope для событий Codex hooks;
- внутренние client interfaces к сервисам-владельцам;
- тесты совместимости маршрутизации, очистки данных и обратной совместимости каталога.
...
| `hook_event_version` | Версия нормализованной формы hook-события. |
...
| `redaction_status` | Результат очистки: не требовалась, выполнена или отклонена. |
| `retention_class` | Класс хранения нормализованного события у владельца. |
| `sanitized_payload` | Только разрешённые поля события после очистки и ограничения размера. |

## Общий envelope внутреннего события

Внутренние события платформы используют отдельную форму:

| Поле | Назначение |
|---|---|
| `event_name` | Имя платформенного синтетического события. |
| `event_version` | Версия нормализованной формы события. |
| `actor` | Сервис или агент, который инициировал событие. |
| `source` | Внутренний источник события. |
| `scope` | Организация, проект и репозиторий, если применимо. |
| `run_context` | `agent_run_id`, `session_id`, `slot_id`, `turn_id`, если применимо. |
| `event_meta` | Безопасная служебная сводка события. |
| `redaction_status` | Результат очистки. |
| `retention_class` | Класс хранения у сервиса-владельца. |

Внутренние события не являются Codex hooks и не являются MCP tool calls.
...
|---|---|
| `diagnostics.mcp_status.read` | Readiness MCP-сервера, версия каталога инструментов и состояние маршрутов. |
...
| `mcp.tool_not_found` | Инструмент не зарегистрирован в текущем каталоге. |
...
- Новые инструменты добавляются как новые имена или новые версии в `tool_catalog.v1.yaml`.
...
- Удаление инструмента проходит через период устаревания и явное обновление каталога инструментов.
- Каждый новый инструмент должен получить тестовые примеры для валидного вызова, отказа политики или обратной совместимости, если меняется существующая группа.
