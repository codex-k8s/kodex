...
| `provider.artifact_signal.register` | готово | Маршрутизируется в `RegisterProviderArtifactSignal`; payload ограничен типизированным signal и не является сырым provider payload. |
