...
related_issues: [747, 753, 760, 771, 780, 830, 841, 852, 322]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
В сервисном каркасе активны диагностический инструмент `diagnostics.mcp_status.read`, первый набор инструментов `agent-manager`, provider-инструменты для уже реализованной поверхности `provider-hub`, governance gate lifecycle, risk evaluator и release decision lifecycle поверх `governance-manager`. Остальные инструменты из этого документа остаются целевой картой и добавляются отдельными срезами после готовности контрактов и бизнес-реализации сервисов-владельцев.
...
| `governance.risk.reevaluate` | `governance-manager` | Пересчитать существующий risk assessment после новых safe refs, signals или версии policy. |
...
| `governance.release.resolve_blocking_signal` | `governance-manager` | Закрыть blocking signal с bounded resolution summary. |
...
| `governance.gate.get` | готово | Маршрутизируется в `GetGateRequest`; MCP возвращает только safe refs, status, version, timestamps и короткие summary. |
...
| `governance.risk.evaluate` / `governance.risk.reevaluate` | готово | Маршрутизируются в `EvaluateRisk`/`ReevaluateRisk`; затем делают typed owner-read `GetRiskAssessment(include_factors=true)` для bounded matched rule refs/counts. MCP принимает только typed refs, bounded evidence, safe classifier summary, expected version/idempotency/correlation и не хранит risk state. |
| `governance.risk.get` / `governance.risk.list` | готово | Маршрутизируются в `GetRiskAssessment`/`ListRiskAssessments`; list summaries добирают factors через bounded owner-read по assessment id. MCP возвращает bounded assessment refs/status/risk class, matched rule refs/counts, required gate refs, version/timestamps и safe summary. |
| `governance.release.prepare_decision_package` / `governance.release.get_decision_package` / `governance.release.list_decision_packages` | готово | Маршрутизируются в `BuildReleaseDecisionPackage`/`GetReleaseDecisionPackage`/`ListReleaseDecisionPackages`; MCP передаёт только project/provider/runtime/agent refs, review signal ids, bounded evidence refs, risk assessment ref и short limitations summary, не копирует release package payload и не хранит release state. |
| `governance.release.request_decision` / `governance.release.submit_decision` / `governance.release.get_decision` / `governance.release.list_decisions` | готово | Маршрутизируются в `RequestReleaseDecision`/`SubmitReleaseDecision`/`GetReleaseDecision`/`ListReleaseDecisions`; expected version, idempotency replay, actor policy и final decision state остаются у `governance-manager`. |
| `governance.release.record_blocking_signal` / `governance.release.resolve_blocking_signal` / `governance.release.list_blocking_signals` | готово | Маршрутизируются в `RecordBlockingSignal`/`ResolveBlockingSignal`/`ListBlockingSignals`; MCP принимает typed target/source/severity и bounded summaries, возвращает только safe signal refs/status/version/timestamps. |
...
| `governance.signal.record_review` | запланировано | Не регистрируется как MCP-инструмент до отдельного среза поверх готового review signal contract. |
