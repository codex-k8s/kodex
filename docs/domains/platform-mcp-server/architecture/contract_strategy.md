...
## Решение для поставки

- MCP-1 фиксирует стратегию разделения MCP и Codex hooks, а не YAML-каталог.
- MCP-2 должен создавать сервисный каркас `platform-mcp-server` с MCP SDK, но без hook ingress.
- Hook ingress получает отдельный срез и отдельный сервисный пакет документации.
- #698 закрывается только после реализации hook emitter/sidecar и `codex-hook-ingress`.
