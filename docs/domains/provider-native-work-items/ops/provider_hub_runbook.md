...
Миграция privacy-hardening backfill-ит уже существующие `processed`/`ignored` строки: сначала фиксирует digest текущего canonical payload, затем заменяет storage payload safe envelope. `pending`/`failed` строки остаются с canonical payload для retry/reprocess.
