...
Оставшийся privacy backlog: encryption-at-rest/KMS policy для inbox/outbox БД и re-fetch/reprocess strategy через provider adapter для случаев, где повторная нормализация без raw body невозможна.
...
| retry webhook возвращает `payload_unavailable` или `payload_expired` | Inbox хранит только safe envelope либо legacy payload уже очищен | `payload_sha256`, delivery id, `retain_until`, safe envelope; raw payload не восстанавливать вручную |
