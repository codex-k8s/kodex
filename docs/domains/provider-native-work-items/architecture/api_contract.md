...
related_issues: [281, 282, 711, 719, 725, 729, 737, 761, 770, 781, 794, 818, 840, 864, 865, 908, 909, 1100]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
- Основные операции: приём webhook, чтение проекций, операции провайдера, сверка, provider-owned сигналы изменения репозитория, операционное состояние аккаунтов и снимки лимитов.
...
Обычный MVP provider-native сценарий для агентной разработки покрыт тем же контрактом без зависимости от self-deploy: GitHub `issues`, `pull_request`, `issue_comment`, `pull_request_review` и `push` webhook нормализуются в safe projections/signals, а команды `CreateIssue`, `UpdateIssue`, `CreateComment`, `UpdateComment`, `CreatePullRequest`, `UpdatePullRequest` и `CreateReviewSignal` проходят через общий `ProviderOperation` pipeline. Go fixtures проверяют, что webhook route и `provider-hub` не отдают наружу raw webhook body, provider response, token, diff, private URL или полный provider payload.
...
Provider-owned read surface для bootstrap/adoption состоит из `GetRepositoryMergeSignal`, `ListRepositoryMergeSignals`, `GetRepositoryChangeSignal`, `ListRepositoryChangeSignals`, `GetRepositoryAdoptionScanSnapshot` и `ListRepositoryAdoptionScanSnapshots`. Эти методы возвращают уже проверенные provider-side факты и safe refs/digests/status/version/etag, но не возвращают checked artifact metadata, checked payload или нормализованный `services.yaml`. Если точный signal/snapshot ещё не создан, `Get*` отвечает явным `read_status=NOT_FOUND`; если данные требуют будущей freshness-политики, контракт уже имеет статусы `NOT_VERIFIED` и `STALE`, но владелец проверки остаётся за пределами `provider-hub`. `RepositoryChangeSignal` может оставаться provider-owned и не содержать project/repository refs; для self-deploy enrichment `project-catalog` разрешает такой signal через собственный active repository binding по provider slug, owner/name, provider repository id, branch и commit. `project-catalog` связывает provider signal со своим checked artifact input и импортом политики, не читая GitHub/GitLab напрямую.

`RepositoryChangeSignal` фиксирует provider-owned факт изменения содержимого репозитория для self-adoption/update planning. Для GitHub `push` webhook на `refs/heads/<branch>` сохраняет provider owner/repo, provider repository id, branch/ref, head commit sha, previous sha, safe path category counters, `services_policy_changed`, `deploy_relevant_changed`, `path_digest` и `change_fingerprint`. Категория root `services.yaml` помечается как `services_policy`, deploy-relevant категории покрывают сервисный код/конфигурацию, deploy-манифесты, runtime-конфигурацию, proto/specs и платформенную политику. Список путей, diff, содержимое файлов и provider response не сохраняются и не отдаются наружу. Для GitHub `pull_request closed + merged` создаётся сигнал kind `pull_request_merged` с merge refs и `path_summary_status=unavailable`, потому что webhook не несёт проверенного списка изменённых файлов; детальный deploy-relevant summary приходит через соответствующий `push` signal или будущий provider refetch. Повтор по тому же `signal_key` идемпотентен, конфликтующий fingerprint отклоняется до outbox.
...
| `provider.repository.changed` | GitHub `push` или `pull_request merged` зафиксировал safe repository change signal; payload содержит только refs, commit, path summary status, category counts/digests и flags без raw paths/diff/YAML. |
