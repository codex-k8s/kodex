...
Для `UpdatePullRequest` GitHub-адаптер не смешивает в одной команде issue-side поля (`labels`, `assignee_provider_logins`, `milestone`) и PR-specific поля (`base_branch`, `maintainer_can_modify`), потому что GitHub требует разные API-вызовы. Такая смешанная команда отклоняется до внешнего write, чтобы не оставить частичный side effect; вызывающий контур должен разбить изменение на две идемпотентные команды.
