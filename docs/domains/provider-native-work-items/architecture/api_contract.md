...
| `RetryWebhookEventProcessing` | Повторить обработку, если у записи есть безопасные факты или доступен будущий re-fetch путь; для safe-envelope-only записей вернуть безопасный отказ. | Операторский контур | По версии события. |
