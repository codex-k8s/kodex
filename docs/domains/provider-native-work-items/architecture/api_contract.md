...
`CreateBootstrapPullRequest` является provider-side командой модели C: проектный или агентный контур готовит payload и refs заранее, а `provider-hub` только пишет их в существующий пустой репозиторий через provider API, создаёт или обновляет bootstrap branch/PR, обновляет проекцию и фиксирует безопасный журнал операции. Содержимое файлов не хранится в `ProviderOperation`, outbox, событиях, audit payload и логах.
