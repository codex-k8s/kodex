...
- safe envelope содержит provider slug, delivery id, event name, repository provider id, payload sha256 и ограниченные нормализованные указатели вроде repository full name, provider work item/comment id, fact kind, artifact kind и number, если они уже извлечены;
