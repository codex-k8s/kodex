...
Назначение: идемпотентный след ускоряющего сигнала до постановки курсоров сверки.

Сигнал сохраняется отдельно от `ReconciliationRequest`, потому что явный `signal_id`, ключ идемпотентности команды и `command_id` должны быть signal-level ключами, а не частью естественного ключа курсора. Повтор с тем же ключом и тем же содержимым возвращает уже принятую запись и повторно пытается поставить курсоры. Повтор с тем же ключом, но другим `external_account_id`, `target`, `source`, payload, временем наблюдения, scope или набором артефактов считается конфликтом.
