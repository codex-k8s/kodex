...
- если конкурентный повтор уже перевёл событие из `pending` или `failed`, команда повторной обработки перечитывает и возвращает актуальное состояние вместо ложного `not found`.
