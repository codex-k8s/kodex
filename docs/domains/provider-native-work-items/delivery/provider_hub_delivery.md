...
Повтор webhook с тем же delivery id и тем же `payload_sha256` считается replay; тот же delivery id с другим fingerprint считается конфликтом. `RetryWebhookEventProcessing` для safe-envelope-only/expired записей безопасно отказывает с `payload_unavailable` или `payload_expired`; повторная обработка без raw body должна идти из уже сохранённых safe facts или через будущий re-fetch provider adapter. Оставшиеся шаги privacy-hardening для webhook inbox: encryption-at-rest/KMS policy для БД и re-fetch/reprocess strategy.
