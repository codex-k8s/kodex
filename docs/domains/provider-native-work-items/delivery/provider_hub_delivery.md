...
| PRV-6.4 | Общий контракт безопасного разрешения секретов готов: `provider-hub` может подключить resolver-клиент в следующем срезе batch-сверки, не сохраняя токен и не раскрывая его через публичные ответы; доступны реализации хранилища `kubernetes_mounted_secret`, `env` и `vault`. |
...
| Сверка | Готово: сигналы, очередь сверки, batch-обработка и курсоры. | Частично реализовано: PRV-6.1 добавил доменную модель `sync_cursor`, постановку области в очередь, чтение, список и короткую аренду курсора; PRV-6.3 добавил ускоряющий сигнал, который ставит `hot` cursor по provider target и выбранному внешнему аккаунту; общий resolver-контракт для получения секрета по разрешённой ссылке готов в `libs/go/secretresolver` с реализациями хранилища `kubernetes_mounted_secret`, `env` и `vault`. Реальная batch-сверка провайдера, подключение resolver-клиента к `provider-hub` и drift status остаются в PRV-6.2b. |
...
- общий `libs/go/secretresolver`, который в следующем срезе позволит batch-сверке и provider-операциям получить значение секрета после `ResolveExternalAccountUsage` без хранения токена в `provider-hub`.
...
Ограничение текущей сверки: сервис ставит курсоры в очередь, отдаёт их на чтение, выдаёт короткую аренду worker и принимает ускоряющие сигналы. Общий resolver-контракт готов, но сам `provider-hub` ещё не подключает его к worker. Поэтому сервис ещё не читает GitHub API, не продвигает `cursor_value`, не публикует `provider.sync_cursor.advanced` и не выставляет итоговый drift status. Реальная batch-сверка провайдера будет добавлена в PRV-6.2b. Команды записи в провайдера, bootstrap/adoption и эксплуатационный контур пока остаются `Unimplemented`. Kubernetes-манифесты, создание БД в deploy-контуре, migration job, alerts и runbook остаются в PRV-9.
