...
- запуск процесса с HTTP health/readiness, `/metrics` и gRPC server runtime через `libs/go/grpcserver`;
...
- минимальный слой репозитория и доменный сервис для проверки готовности хранилища;
- регистрация полного gRPC-контракта `ProviderHubService`.

Ограничение среза: все gRPC-операции пока возвращают `Unimplemented`. Реализация конкретных команд и чтений добавляется в PRV-3..PRV-8 вместе с соответствующей доменной логикой. Kubernetes-манифесты, создание БД в deploy-контуре, migration job, alerts и runbook остаются в PRV-9.
