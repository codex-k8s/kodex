...
| PRV-8i | GitHub live provider path заменён на backlog Go integration runner с dry-run/apply, safe target policy, idempotency и cleanup policy. |
...
- Live GitHub provider path должен быть отдельным Go integration runner: dry-run/apply, тестовый репозиторий, ветка, PR, merge, payload, gateway/provider checks и cleanup policy не реализуются в shell.
