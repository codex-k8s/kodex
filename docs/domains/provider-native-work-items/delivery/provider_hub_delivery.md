...
| PRV-7 | Платформенные операции записи в провайдера для agent-manager/MCP с аудитом и идемпотентностью. |
