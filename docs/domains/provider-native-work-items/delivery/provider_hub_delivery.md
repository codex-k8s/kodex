...
| PRV-6.4 | Общий контракт безопасного разрешения секретов готов: `provider-hub` может подключить resolver-клиент в следующем срезе batch-сверки, не сохраняя токен и не раскрывая его через публичные ответы. |
...
| Сверка | Готово: сигналы, очередь сверки, batch-обработка и курсоры. | Частично реализовано: PRV-6.1 добавил доменную модель `sync_cursor`, постановку области в очередь, чтение, список и короткую аренду курсора; PRV-6.3 добавил ускоряющий сигнал, который ставит `hot` cursor по provider target и выбранному внешнему аккаунту; общий resolver-контракт для получения секрета по разрешённой ссылке готов в `libs/go/secretresolver`. Реальная batch-сверка провайдера, подключение resolver-клиента к `provider-hub` и drift status остаются в PRV-6.2b. |
