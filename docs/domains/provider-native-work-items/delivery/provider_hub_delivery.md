...
related_issues: [281, 282, 711, 719, 725, 729, 737, 754, 761, 770, 781, 840, 864, 865, 895]
related_prs: []
related_docsets:
...
- `scripts/smoke-provider-merge-signal.sh` проверяет producer-side путь safe fixture -> `integration-gateway` route wiring -> `provider-hub IngestWebhookEvent` -> `RepositoryMergeSignal` read surface -> локальный outbox event; live HTTP режим дополнительно может проверить попадание события в `platform-event-log`, если включён dispatch и доступен безопасный diagnostic DSN.
...
Задачи #281 и #282 остаются открытыми до полного end-to-end bootstrap/adoption. PRV-8a, PRV-8b, PRV-8c, PRV-8d, PRV-8e, PRV-8f и PRV-8g закрывают только provider-side часть создания репозитория, bootstrap/adoption PR, безопасного merge signal, lightweight adoption scan snapshot, чтения этих provider-owned фактов и producer-side smoke/diagnostic проверки.
