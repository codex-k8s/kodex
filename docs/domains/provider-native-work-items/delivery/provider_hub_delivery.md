...
related_issues: [281, 282, 711, 719, 725, 729, 737, 754, 761, 770, 781, 840, 864, 865, 895, 908, 909, 939, 1100]
related_prs: []
related_docsets:
...
| Provider-native MVP checks | Обычный GitHub-контур агентной разработки проверен без self-deploy: `integration-gateway` route принимает safe fixtures `issues`, `pull_request`, `issue_comment`, `pull_request_review` и `push`, `provider-hub` нормализует их в projections/signals и общий write pipeline принимает comment, PR и review signal операции через `ProviderOperation`. |
...
| PRV-8i | Shell smoke для live provider path заменён на Go product API runner: dry-run проверяет `project-catalog`/`provider-hub` read surface, apply явно запускает bootstrap/adoption reconcile с checked input и safe target policy; checked input может готовиться из уже нормализованного JSON payload с обычным SHA-256 `content_hash` и `artifact_digest = content_hash` без raw YAML и provider payload. Runner для пустого репозитория создаёт provider repository и bootstrap PR только через публичные product API `project-catalog`, а self-adoption для существующего `codex-k8s/kodex` привязывает repository binding через `AttachRepository` и строит safe summary изменения `services.yaml`/deploy-relevant путей без прямого GitHub/GitLab, без auto deploy и без вывода подготовленных файлов. |
...
|---|---|---|
| Приём webhook | Готово: `IngestWebhookEvent`, чтение, список, повторная обработка и явная cleanup-операция legacy retained payload. | Реализовано в PRV-4: входящий журнал, дедупликация по `provider_slug + delivery_id`, базовая нормализация GitHub-событий, статусы обработки и outbox-события `provider.webhook.received` / `provider.webhook.normalized`. Privacy-hardening переводит `payload_json` на safe envelope only, хранит `payload_sha256` для replay/conflict, редактирует старые retained payload миграцией и оставляет `CleanupExpiredWebhookPayloads` как совместимость для legacy `retained_for_retry`; retry не читает raw body, поддержанный GitHub onboarding `pull_request` перечитывает через provider API, остальные неподдержанные/expired случаи получают safe diagnostic. Обычный GitHub MVP проверяется fixtures для `issues`, `pull_request`, `issue_comment`, `pull_request_review` и `push`; публичный HTTP webhook endpoint остаётся ответственностью `integration-gateway`. |
...
| Merge bootstrap/adoption PR | Готово: безопасный provider-side merge signal, gRPC-чтение этих сигналов и AsyncAPI события `provider.repository.bootstrap_merged` / `provider.repository.adoption_merged`. | Реализовано в PRV-8d: GitHub `pull_request closed + merged` связывается с уже известной bootstrap/adoption PR-проекцией, сохраняет только safe refs/digest/timestamps, дедуплицируется по signal key и конфликтует при другом commit/source ref. PRV-8f добавляет `GetRepositoryMergeSignal`/`ListRepositoryMergeSignals` со status/version/etag. PRV-8g/PRV-8h добавляют Go checks на bootstrap/adoption fixtures, replay/conflict и leak assertions для safe outputs. `cmd/onboarding-runner` проверяет product API путь: читает provider-owned merge signals/snapshots через `provider-hub` и при явном apply вызывает project-side `ReconcileBootstrapMergeSignal`/`ReconcileAdoptionMergeSignal` с checked input. `provider-hub` не вызывает `project-catalog` напрямую; project-side reconciliation получает safe signal и checked artifact metadata, которые остаются в `project-catalog`. |
| Эксплуатационный контур | Готово: Dockerfile, Kubernetes-манифесты, bootstrap БД, migration job, runbook и monitoring docs. | PRV-9 добавил `deploy/base/provider-hub/**`, подключение `kodex_provider_hub` к PostgreSQL bootstrap/runtime secrets и эксплуатационные документы. Проверка первого кольца выполняется через `bootstrap/host/smoke_backend_contour.sh`. |
...
- webhook inbox хранит `payload_json` как safe envelope only: новые `pending`/`failed`/`processed`/`ignored` записи не сохраняют canonical provider body, а `payload_sha256` остаётся fingerprint для replay/conflict;
- явная очистка истёкших legacy retryable webhook payload через `CleanupExpiredWebhookPayloads`: старые `retained_for_retry` записи после `retain_until` сохраняют safe envelope, `payload_sha256`, статус `failed` и причину `payload_expired`; повторная обработка не нормализует raw body из БД, поддержанный GitHub onboarding `pull_request` перечитывает через provider API, а неподдержанные/expired случаи терминализируются безопасной причиной;
...
- live `push`/`pull_request merged` для репозитория провайдера фиксируется provider-side как `RepositoryChangeSignal`: GitHub `push` по `refs/heads/main` хранит safe owner/repo, branch/ref, commit sha, previous sha, `services_policy_changed`, `deploy_relevant_changed`, category counters, `path_digest` и `change_fingerprint`; GitHub `pull_request closed + merged` хранит merge refs с `path_summary_status=unavailable`, если provider webhook не несёт проверенного списка путей. Raw paths, diff, body, YAML, provider response и секреты не сохраняются.
- обычный provider-native контур для агентной разработки проверяется безопасными GitHub fixtures: `issues`, `pull_request`, `issue_comment`, `pull_request_review` и `push` проходят через `integration-gateway` HMAC route и `provider-hub IngestWebhookEvent`, дают safe work item/comment/review/repository change projections или signals, а сырые webhook-only поля, private URL, полный payload и provider response не попадают в read surface, outbox или diagnostics.
- `GetRepositoryMergeSignal`, `ListRepositoryMergeSignals`, `GetRepositoryChangeSignal`, `ListRepositoryChangeSignals`, `GetRepositoryAdoptionScanSnapshot` и `ListRepositoryAdoptionScanSnapshots` отдают только provider-owned safe refs/status/timestamps/version/etag; checked artifact, checked payload и нормализованный `services.yaml` остаются в `project-catalog`.
...
- Проверочный onboarding path живёт в `cmd/onboarding-runner`: dry-run читает provider-owned merge signals, repository change signals, adoption snapshots и project repository binding через gRPC, apply явно вызывает project-side reconcile с checked input и safe target policy. Runner может подготовить checked input из уже нормализованного JSON payload, вычислить `content_hash` как обычный SHA-256 от байтов payload, передать совместимый `artifact_digest = content_hash`, сформировать ref/version и связать artifact version с provider merge commit. Для пустого тестового репозитория runner умеет по сценарию `bootstrap_setup` вызвать `project-catalog CreateProviderRepository` и `CreateRepositoryBootstrapPullRequest`, получить `repository_id` из product API и подготовить bootstrap PR без прямого GitHub/GitLab. Для существующего self-repo `codex-k8s/kodex` сценарий `repository_binding` вызывает только `project-catalog AttachRepository` при точном allowlist `allowed_provider_owner + allowed_provider_repository`, а `repository_change` строит локальный safe plan и затем ждёт live `RepositoryChangeSignal` от `provider-hub` по provider refs/base branch/commit. Он не читает сырой `services.yaml` для checked import, не печатает подготовленные файлы, не хранит provider payload и не запускает build/deploy.
...
Задачи #281 и #282 остаются открытыми до полного end-to-end bootstrap/adoption. PRV-8a, PRV-8b, PRV-8c, PRV-8d, PRV-8e, PRV-8f, PRV-8g, PRV-8h и PRV-8i закрывают provider-side часть создания репозитория, bootstrap/adoption PR, безопасного merge signal, lightweight adoption scan snapshot, чтения этих provider-owned фактов, producer-side Go-проверки диагностики и подготовку checked input из нормализованного payload для project-side reconcile с совместимым `artifact_digest = content_hash`. Runner также умеет поэтапно подготовить empty-repo bootstrap setup через product API `project-catalog`, self-repo adoption binding для `codex-k8s/kodex` через `AttachRepository` и проверку live `RepositoryChangeSignal` для `main` с safe summary изменения `services.yaml`/deploy-relevant путей. Для создания `SelfDeployPlan` provider signal сначала проходит project-side enrichment в `project-catalog GetSelfDeploySignal`: только там добавляются `services_yaml_digest`, checked policy fingerprint/version и affected service keys. Срез не закрывает auto deploy, операторский интерфейс, слияние владельцем, adoption deep scan/report и пользовательский запуск; публичная регистрация и проверка GitHub webhook остаётся зависимостью пограничного контура.
...
- live `push`/`pull_request merged` `provider-hub` фиксирует как `RepositoryChangeSignal` с safe refs/category counters/digests/status; это вход для self-adoption/update planning, но не запуск build/deploy и не импорт project policy;
...
- `project-catalog` принимает safe bootstrap/adoption merge signal только через checked artifact metadata и вызывает свой import use-case; provider-native факт merge остаётся в `provider-hub`, а webhook inbox хранит только safe envelope/digest, не checked artifact input;
...
`provider-hub` webhook inbox не хранит canonical provider webhook payload в `provider_hub_webhook_events.payload_json` как штатное состояние. При приёме webhook сервис вычисляет `payload_sha256`, нормализует событие и сохраняет safe envelope с `payload_storage=safe_envelope_only`, provider/event/delivery refs и ограниченными нормализованными указателями. Миграция редактирует старые retained payload в safe envelope с причиной `raw_payload_removed`; `CleanupExpiredWebhookPayloads` остаётся для legacy `retained_for_retry` строк и переводит их в `payload_storage=expired_after_retention`. Это внутреннее хранилище `provider-hub`, а не safe read surface для соседних сервисов.
...
Повтор webhook с тем же delivery id и тем же `payload_sha256` считается replay; тот же delivery id с другим fingerprint считается конфликтом. `RetryWebhookEventProcessing` не использует raw/canonical payload из БД. Для поддержанного GitHub `pull_request` safe envelope он перечитывает PR через provider API по stable refs и уже разрешённому onboarding `ProviderOperation`; при отсутствии безопасного refetch-пути или expired payload запись получает терминальную safe diagnostic причину `payload_unavailable`, `payload_expired` или `refetch_unavailable`. Оставшийся шаг privacy-hardening для webhook inbox: encryption-at-rest/KMS policy для БД и расширение refetch-покрытия за пределы GitHub onboarding PR.
