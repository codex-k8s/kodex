...
Межкомнатное делегирование роли с сохранённым назначением `cluster-admin` допускается только для пары role/chat, уже зафиксированной серверным снимком. Перед созданием делегирования, публикацией целевого треда, постановкой хода и связыванием целевой сессии платформа повторно проверяет точные роль, чат, участника, зависимости, отсутствие отзыва и целостность Kubernetes Secret. Этот путь не создаёт и не расширяет назначение. Shared-node Kubernetes adapter не материализует такое делегирование: до создания Job, Pod, PVC или session Secret запуск закрыто отклоняется, потому что недоверенный runtime не получает штатный `cluster-admin` ServiceAccount. Исполнение административной возможности требует будущего управляемого шлюза вне agent pod.

Пользовательские title, message, target chat/agent, work item key, ссылки и audit metadata имеют server-owned UTF-8 byte/rune bounds до storage и dispatcher. `delegation.Title` нормализуется в NFC и становится непрозрачными недоверенными данными: новые значения с Markdown, ссылками, упоминаниями, HTML/code-разделителями, backslash, переводами строк, управляющими, bidi/format и zero-width символами отклоняются до чтения session/token. Каждый из четырёх Mattermost/prompt sink повторно строит единое безопасное представление; legacy row либо проходит ту же нормализацию, либо закрыто отклоняется без вывода исходного значения. В межагентском prompt title передаётся одной JSON-строкой с явной меткой недоверенных данных, а не как инструкция или заголовок секции.
...
Адаптер создает ресурсы из типизированных спецификаций. Бизнес-код не формирует YAML или shell. Контроллер среды выполнения идемпотентно сверяет состояние `Pod`, `PVC`, `Secret` и `ServiceAccount` и записывает условие состояния. Legacy session pod с другим `PriorityClass`, ServiceAccount, memory request/limit или token Secret удаляется по точным UID/resourceVersion preconditions и создаётся заново; PVC, token Secret и архив не затрагиваются. Несовместимый существующий utility Job не переиспользуется и закрыто отклоняется до безопасной отдельной сверки.
...
- Недоверенный shared-node runtime не получает `cluster-admin` credential и не может создавать Pod/Job либо изменять защитные `ResourceQuota` и `PriorityClass`.
- Kubernetes ResourceQuota admission отклоняет превышающий memory budget pod, в том числе при конкурентной границе; render-проверка остаётся отдельным предварительным слоем.
