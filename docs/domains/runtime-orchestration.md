...
Для возврата домен до durable transition строит точный callback prompt и обе точные audit-публикации, включая окончательные byte chunks, общий предел и deadline/concurrency admission. В одной guarded DB-транзакции с `CallbackRunID` создаётся неизменяемый план `matter_codex_agent_delegation_callback_deliveries`: отдельная строка на destination/publication фиксирует channel, root, message, props, SHA-256 payload и детерминированный внешний идентификатор. Delivery worker захватывает только готовую строку через ограниченную lease и `FOR UPDATE SKIP LOCKED`; один destination не отправляется параллельно и порядок его chunks сохраняется. Каждая фактическая попытка имеет server-owned deadline и точный source+child binding/revocation fence.

Mattermost `pending_post_id` используется только как кратковременная подсказка серверу, а не как недоказанная долговечная гарантия. После timeout, перезапуска процесса или ошибки записи подтверждения адаптер ищет в точном треде пост с детерминированным delivery identity и сверяет channel/root/message и полный набор props; конфликт или два совпадения закрыто отклоняются. Повтор при существующем `CallbackRunID` доставляет только незавершённые строки и возвращает ошибку, пока все обязательные публикации не имеют состояния `delivered`. Подтверждённая destination не дублируется; revoke/remap оставляет строку `blocked` с безопасным кодом ошибки, а неоднозначный сетевой исход — `pending/confirmation_ambiguous` либо незавершённую lease, если сама БД недоступна.
