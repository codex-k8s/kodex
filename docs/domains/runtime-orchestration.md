...
Пользовательские title, message, target chat/agent, work item key, ссылки и audit metadata имеют server-owned UTF-8 byte/rune bounds до storage и dispatcher. Для возврата домен до durable transition строит точный callback prompt и обе точные audit-публикации, включая окончательные byte chunks, общий предел и deadline/concurrency admission. Guarded transaction повторно сверяет точные metadata; только после этого разрешены enqueue и запись `CallbackRunID`. Частичный сетевой исход после уже зафиксированного перехода остаётся явным идемпотентным результатом: повтор не создаёт второй callback или audit posts.
