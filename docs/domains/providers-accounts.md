...
- Каждая успешная device-code авторизация создает новую неизменяемую ревизию
  ...
  для отката.
- Повторная авторизация учетной записи, используемой ролью `cluster-admin`,
  доступна только владельцу инстанса. Она атомарно обновляет credential и все
  замороженные снимки зависимостей, запрещена во время активного хода и не
  разрешает удаление либо произвольную замену учетной записи.
