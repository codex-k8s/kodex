...
related_issues: [628, 629, 630, 631, 632, 633, 794, 810, 818, 840, 864, 881, 893, 917, 1099]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
`GetProjectOnboardingStatus` — project-side RPC чтения для ручного bootstrap/adoption через product API. Метод проверяет активный проект, опциональный repository binding, ожидаемые provider/source refs, commit, `content_hash`, версию проверенной `ServicesPolicy` и наличие активных `ServiceDescriptor` для нужных service keys. Вместо общих транспортных `NotFound`/`PermissionDenied` при доменной неготовности возвращается типизированный `ProjectOnboardingStatus` с коротким `safe_reason`: `project_not_found`, `repository_binding_not_found`, `repository_binding_not_active`, `services_policy_not_found`, `services_policy_not_ready`, `services_policy_stale`, `service_descriptors_not_found` и другие статусы. `ready` означает, что project/repository binding, checked `services.yaml` projection и descriptors согласованы для указанного input; метод не читает GitHub/GitLab, не возвращает raw YAML/provider payload и не запускает self-deploy.

`GetSelfDeploySignal` — project-side read RPC для подготовки безопасного входа `agent-manager CreateSelfDeployPlanFromSignal`. Метод принимает `project_id`, опциональный `repository_id` и provider signal identity (`provider_signal_id` или `provider_signal_key`), читает safe `RepositoryChangeSignal` через `provider-hub`, сверяет его с активным repository binding и обогащает checked project-owned данными: `services_yaml_ref`, `services_yaml_digest`, fingerprint/version активной `ServicesPolicy`, affected service keys из `ServiceDescriptor`, safe path categories, expected runtime job type hints и признак обязательного owner/governance gate. Governance requirement может содержать safe policy key без namespace, например `self_deploy.owner_gate`; `agent-manager` канонизирует такой key в typed governance ref перед сохранением локального `SelfDeployPlan`. Если provider-owned signal не содержит project/repository refs или прямое чтение по ключу не находит связанный project context, метод при явном `repository_id` ищет signal через `provider-hub ListRepositoryChangeSignals` по active binding: provider slug, канонические owner/name, provider repository id, branch и commit. Signal для другого repository не связывается; неполная provider identity возвращает safe reason `provider_signal_identity_unavailable`, а отсутствие checked projection возвращается статусом `services_policy_not_found` или `needs_services_policy_reconcile`, не транспортным `NotFound`. `project-catalog` не читает GitHub/GitLab, не подставляет `path_digest` вместо `services_yaml_digest`, не возвращает `validated_payload_json` и не запускает build/deploy. Если provider сообщает изменение root `services.yaml`, а активная checked projection ещё не импортирована с commit сигнала, метод возвращает `needs_services_policy_reconcile`; если path summary недоступен, возвращается `needs_repository_change_summary`. Готовый статус `ready` означает только, что `agent-manager` может создать pending `SelfDeployPlan` из safe refs/fingerprints; runtime jobs всё равно ждут owner/governance approval.

`GetSelfDeployBuildPlan` — project-side read RPC для подготовки безопасного входа runtime `JOB_TYPE_BUILD` после approved `SelfDeployPlan`. Метод принимает project/repository/source refs, provider/self-deploy signal refs, affected service keys и ожидаемые digest/fingerprint/version проверенной `ServicesPolicy`; дополнительно вызывающая сторона может передать `materialized_build_contexts`, подготовленные `runtime-manager` для конкретных service keys. `source_ref` сравнивается как Git branch ref: `main` и `refs/heads/main` считаются одним источником только для безопасных имён веток; raw 40/64-hex SHA, digest, tag/provider-specific refs и небезопасные refs не нормализуются и должны совпадать буквально. `merge_commit_sha` валидируется как commit кода и попадает в runtime specs, но не обязан совпадать с `ServicesPolicy.SourceCommitSHA`: для code-only deploy-relevant изменений проверенная policy может быть импортирована на более раннем commit. Freshness checked policy держится на source ref, ожидаемых digest/fingerprint/version и owner-owned status; если root `services.yaml` менялся, `GetSelfDeploySignal` не отдаёт `ready` до импорта новой policy. Источник статической policy — только checked project-owned projection `services.yaml`: сервис ищет affected service descriptors, берёт typed `service.build`, а для корневого инвентаря платформы также детерминированно выводит image ref/tag, Dockerfile ref/target и builder image ref из `spec.images`/`spec.versions`. `services.yaml` хранит переносимый рецепт сборки: что собирать, из какого repository source, каким Dockerfile/target, в какой image ref/tag, с какими allowed secret refs и output refs. Runtime build context не хранится в `services.yaml`, не выводится из путей репозитория и не создаётся в `project-catalog`: его готовит `runtime-manager` и возвращает вызывающей стороне как `build_context_ref`/`build_context_digest` и optional `dockerfile_digest`. Если context ещё не передан, метод возвращает `build_context_required` и per-service items с рецептом, стабильным recipe/item fingerprint и без `BuildExecutionSpec`; `agent-manager` трактует это как `runtime_build_status=preparing_context`, вызывает `runtime-manager.PrepareBuildContext` и повторяет read с `materialized_build_contexts` после готовности context refs. Когда вызывающая сторона повторяет запрос с валидными materialized contexts и fingerprint совпадает с ранее выданным рецептом, готовый ответ возвращает per-service items, совместимые с `runtime-manager BuildExecutionSpec`: image ref/tag, runtime build context ref/digest, Dockerfile ref/target, builder image ref, allowed secret refs без значений, output refs и стабильный plan fingerprint. Если политика устарела, сервис отсутствует, build spec не задан, context не передан или context не совпадает с рецептом, метод возвращает safe статус `policy_stale`, `service_not_found`, `build_plan_unavailable`, `build_context_required` или `build_context_invalid`; `agent-manager` не должен создавать `JOB_TYPE_BUILD` при статусе неготовности. Метод не читает raw webhook body, provider response, diff, GitHub/GitLab или полный YAML и не подбирает Dockerfile/image refs по файловой системе.

`GetSelfDeployDeployPlan` — project-side read RPC для подготовки безопасного входа runtime `JOB_TYPE_DEPLOY` после successful build. Метод принимает те же project/repository/source refs, affected service keys, ожидаемую версию checked `ServicesPolicy`, refs успешных build jobs, commit-based image tag и optional image digest. Как и build plan, deploy plan различает commit кода и commit импортированной policy: `merge_commit_sha` может быть новее `ServicesPolicy.SourceCommitSHA`, если `services.yaml` не менялся, но expected digest/fingerprint/version проверяются строго. Successful build output должен быть связан с ожидаемым build plan fingerprint и тем же materialized build context ref/digest; output от другого build plan или context возвращает `build_output_invalid`, а отсутствие output — `build_not_ready`. Ответ возвращает per-service `DeployExecutionSpec`, совместимый с `runtime-manager`: image ref/tag/digest, manifest/kustomization refs/digests, checked `manifest_bundle_ref`/`manifest_bundle_digest`, rollout targets, expected image refs, target namespace/cluster ref, allowed secret refs без значений, output refs и стабильный deploy plan fingerprint. Источник связи project/repository/policy/services.yaml/build output -> deploy targets остаётся в `project-catalog` и `agent-manager` contour; `runtime-manager` не выводит manifest bundle из репозитория, `services.yaml` или build output и не получает raw manifests. Если build output, manifest bundle или policy не совпадают с ожидаемым fingerprint/version, метод возвращает typed safe status, и `agent-manager` не создаёт `JOB_TYPE_DEPLOY`.

`cmd/onboarding-runner` использует эти публичные команды как проверочный product API контур для bootstrap/adoption. Dry-run проверяет `ListProjects`, `GetRepository`/`ListRepositories`, provider-owned merge signals, repository change signals, adoption scan snapshots, готовность `bootstrap_setup`, self-repo `repository_binding`, `services_policy_import` и safe `repository_change` plan summary без мутаций. Если `project_id` не передан, runner ищет active project по `organization_id` и slug/display name через `ListProjects`, а apply при отсутствии project вызывает `CreateProject` и печатает safe `project_id` для `KODEX_AGENT_MANAGER_SELF_DEPLOY_SIGNAL_PROJECT_ID`. Apply включается только явно и вызывает `CreateProviderRepository`/`CreateRepositoryBootstrapPullRequest` для пустого тестового репозитория, `AttachRepository` для существующего self-repo binding `codex-k8s/kodex`, `ImportServicesPolicy` для checked self-repo `services.yaml` projection, а после слияния владельцем и появления safe signal — `ReconcileBootstrapMergeSignal`/`ReconcileAdoptionMergeSignal` при наличии проверенного входа сценария. Для self-repo apply runner также вызывает публичный `access-manager PutAccessRule` и создаёт или переиспользует узкие rules: собственный service actor получает `repository.read` на `resource.type=repository`, `resource.id=<repository_id>`, `scope=project/<project_id>` для чтения текущего binding через `GetRepository`; `service/agent-manager` и `service/staff-gateway` получают `project.policy.read` и минимальные governance read/request/evaluate права, нужные для self-deploy summary и gate preparation, после чего runner проверяет итоговый `CheckAccess`. Если задан безопасный `owner_user_ref`, runner дополнительно создаёт прямые owner-facing `subject.type=user` правила: `governance.risk.read`, `governance.gate.read` и `governance.gate.decide` в project scope для self-deploy governance path. Email владельца остаётся seed для identity/bootstrap и не записывается в `AccessRule.subject_id`. Если включён `services_policy_import`, runner дополнительно создаёт или переиспользует правило для своего service actor: `project.policy.import` на `services_policy` в том же project scope, без admin-доступа и без доступа к секретам. Для apply обязательна safe target policy: разрешённый provider owner, префикс тестового repository name для `bootstrap_setup.create_repository` и точное разрешённое имя репозитория через `KODEX_ONBOARDING_RUNNER_ALLOWED_REPOSITORY` для `repository_binding`. Перед `AttachRepository` runner читает `ListRepositories` и повторно использует existing active binding, поэтому повторный запуск не создаёт дубли. Runner может подготовить checked input из уже нормализованного JSON payload: проверяет JSON-объект, вычисляет `content_hash` как обычный SHA-256 от байтов payload, передаёт совместимый `artifact_digest = content_hash`, формирует artifact ref/version и добавляет watermark payload из сценария или безопасного JSON-файла. Отдельный `services_policy_import` читает локальный YAML/JSON файл только в операционном runner-контуре, нормализует его в JSON-объект, вычисляет `content_hash` от нормализованного JSON, берёт source ref/commit из live `RepositoryChangeSignal` или явных параметров и вызывает `ImportServicesPolicy` через gRPC. `repository_change` строит локальные safe categories/counts/fingerprints для root `services.yaml` и deploy-relevant путей, затем читает `provider-hub GetRepositoryChangeSignal`/`ListRepositoryChangeSignals` по provider refs, branch, commit и kind. Для `push` ожидается path summary с категориями; для `pull_request_merged` допускается отсутствие path summary, потому что детальный список путей приходит через `push` или будущий safe refetch. Этот provider-owned signal является входом self-adoption/update planning, но `project-catalog` не запускает build/deploy и не читает GitHub/GitLab напрямую. Runner не читает БД, не вызывает GitHub/GitLab напрямую, не печатает подготовленные файлы, полный `validated_payload_json`, webhook body, provider response, raw YAML, token, DSN или private URL. Содержимое подготовленных файлов используется только как typed input `CreateRepositoryBootstrapPullRequest`; checked artifact и import input остаются безопасными refs/digests/payload projection.
...
| `GetProjectOnboardingStatus` | gRPC query | `project.policy.read` | нет | Возвращает типизированную готовность ручного bootstrap/adoption по project/repository binding, checked `ServicesPolicy`, expected refs/digests/version и active service descriptors без raw YAML/provider payload и побочных эффектов self-deploy. |
...
| `GetSelfDeployBuildPlan` | gRPC query | `project.policy.read` | нет | Читает checked `ServicesPolicy`, сверяет expected digest/fingerprint/version, возвращает рецепт сборки по сервисам без runtime context или готовые `BuildExecutionSpec`-совместимые refs, если вызывающая сторона передала materialized context refs/digests от `runtime-manager`; raw YAML, provider payload и значения секретов не возвращаются. |
| `GetSelfDeployDeployPlan` | gRPC query | `project.policy.read` | нет | После successful build возвращает checked `DeployExecutionSpec` с manifest bundle refs/digest, rollout targets и expected image refs, только если build output связан с ожидаемым build plan fingerprint и materialized context; raw YAML, provider payload, kubeconfig и значения секретов не возвращаются. |
...
| Бизнес-обработчики gRPC и event-driven use-case | Подключены к доменному сервису для проектов, репозиториев, создания provider repo/base ref через `provider-hub`, bootstrap PR по существующему binding, reconciliation safe bootstrap/adoption merge signal, импорта bootstrap/adoption checked policy после merge, проверки типизированной готовности ручного bootstrap/adoption, проверенной проекции `services.yaml`, операторских переопределений, источников документации, правил веток, релизных политик, релизных линий и политики размещения. |
