...
`cmd/onboarding-runner` использует эти публичные команды как проверочный product API контур для bootstrap/adoption. Dry-run проверяет `GetRepository`, provider-owned merge signals и adoption scan snapshots без мутаций. Apply включается только явно и вызывает `ReconcileBootstrapMergeSignal`/`ReconcileAdoptionMergeSignal` при наличии checked scenario input. Runner может подготовить checked input из уже нормализованного JSON payload: проверяет JSON-объект, вычисляет `content_hash`, передаёт совместимый `artifact_digest = content_hash`, формирует artifact ref/version и добавляет watermark payload из сценария или безопасного JSON-файла. Runner не читает БД, не вызывает GitHub/GitLab напрямую, не валидирует сырой YAML, не создаёт provider repo/PR и не печатает checked payload, webhook body, provider response, token, DSN или private URL.
