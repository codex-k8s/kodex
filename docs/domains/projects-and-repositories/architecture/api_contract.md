...
`cmd/onboarding-runner` использует эти публичные команды как проверочный product API контур для bootstrap/adoption. Dry-run проверяет `GetRepository`, provider-owned merge signals, adoption scan snapshots, готовность `bootstrap_setup`, self-repo `repository_binding` и safe `repository_change` summary без мутаций. Apply включается только явно и вызывает `CreateProviderRepository`/`CreateRepositoryBootstrapPullRequest` для пустого тестового репозитория, `AttachRepository` для существующего self-repo binding `codex-k8s/kodex`, а после слияния владельцем и появления safe signal — `ReconcileBootstrapMergeSignal`/`ReconcileAdoptionMergeSignal` при наличии проверенного входа сценария. Для apply обязательна safe target policy: разрешённый provider owner, префикс тестового repository name для `bootstrap_setup.create_repository` и точное разрешённое имя репозитория через `KODEX_ONBOARDING_RUNNER_ALLOWED_REPOSITORY` для `repository_binding`. Runner может подготовить checked input из уже нормализованного JSON payload: проверяет JSON-объект, вычисляет `content_hash` как обычный SHA-256 от байтов payload, передаёт совместимый `artifact_digest = content_hash`, формирует artifact ref/version и добавляет watermark payload из сценария или безопасного JSON-файла. `repository_change` фиксирует только safe categories/counts/fingerprints для root `services.yaml` и deploy-relevant путей, чтобы соседние контуры могли подготовить ручной или управляемый update plan без auto deploy. Runner не читает БД, не вызывает GitHub/GitLab напрямую, не валидирует сырой YAML и не печатает подготовленные файлы, checked payload, webhook body, provider response, token, DSN или private URL. Содержимое подготовленных файлов используется только как typed input `CreateRepositoryBootstrapPullRequest`; checked artifact и import input остаются безопасными refs/digests/payload projection.
