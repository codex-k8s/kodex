...
| `GetSelfDeployDeployPlan` | gRPC query | `project.policy.read` | нет | Следующий dependency для self-deploy deploy: после successful build должен вернуть checked `DeployExecutionSpec` с manifest bundle refs/digest, rollout targets и expected image refs без raw YAML, provider payload, kubeconfig и значений секретов. |
