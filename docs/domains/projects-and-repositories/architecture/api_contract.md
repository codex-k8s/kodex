...
| `ReconcileAdoptionMergeSignal` | domain command / event consumer use-case | `project.policy.import` | `CommandMeta` или provider `signal_key` + source commit replay | Принимает safe adoption merge signal и checked artifact metadata, валидирует signal/artifact/binding, ведёт safe status journal и импортирует checked policy для существующего репозитория. |
