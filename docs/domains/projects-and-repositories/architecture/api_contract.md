...
`cmd/onboarding-runner` использует эти публичные команды как проверочный product API контур для bootstrap/adoption. Dry-run проверяет `GetRepository`, provider-owned merge signals, adoption scan snapshots и готовность `bootstrap_setup` без мутаций. Apply включается только явно и вызывает `CreateProviderRepository`/`CreateRepositoryBootstrapPullRequest` для пустого тестового репозитория, а после слияния владельцем и появления safe signal — `ReconcileBootstrapMergeSignal`/`ReconcileAdoptionMergeSignal` при наличии проверенного входа сценария. Runner может подготовить checked input из уже нормализованного JSON payload: проверяет JSON-объект, вычисляет `content_hash` как обычный SHA-256 от байтов payload, передаёт совместимый `artifact_digest = content_hash`, формирует artifact ref/version и добавляет watermark payload из сценария или безопасного JSON-файла. Runner не читает БД, не вызывает GitHub/GitLab напрямую, не валидирует сырой YAML и не печатает подготовленные файлы, checked payload, webhook body, provider response, token, DSN или private URL. Содержимое подготовленных файлов используется только как typed input `CreateRepositoryBootstrapPullRequest`; checked artifact и import input остаются безопасными refs/digests/payload projection.
