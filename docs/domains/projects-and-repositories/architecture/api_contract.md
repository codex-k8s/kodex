...
| `GetSelfDeployDeployPlan` | gRPC query | `project.policy.read` | нет | После successful build возвращает checked `DeployExecutionSpec` с manifest bundle refs/digest, rollout targets и expected image refs без raw YAML, provider payload, kubeconfig и значений секретов. |
