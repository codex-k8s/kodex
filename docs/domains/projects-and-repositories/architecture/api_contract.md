...
`GetSelfDeployBuildPlan` — project-side read RPC для подготовки безопасного входа runtime `JOB_TYPE_BUILD` после approved `SelfDeployPlan`. Метод принимает project/repository/source refs, provider/self-deploy signal refs, affected service keys и ожидаемые digest/fingerprint/version проверенной `ServicesPolicy`. Источник данных — только checked project-owned projection `services.yaml`: сервис ищет affected service descriptors, берёт typed `service.build`, а для корневого инвентаря платформы также детерминированно выводит image ref/tag, Dockerfile ref/target и builder image ref из `spec.images`/`spec.versions`. Build context не выводится из путей репозитория: для `ready` требуются проверенные `buildContextRef` и `buildContextDigest`, указывающие на уже подготовленный runtime build context. Готовый ответ возвращает per-service build items, совместимые с `runtime-manager BuildExecutionSpec`: image ref/tag, build context ref/digest, Dockerfile ref/target, builder image ref, allowed secret refs без значений, output refs и стабильный plan fingerprint. Если политика устарела, сервис отсутствует, build spec не задан или checked build context ещё не материализован, метод возвращает safe статус `policy_stale`, `service_not_found`, `build_plan_unavailable` или `build_context_unavailable`; `agent-manager` не должен создавать `JOB_TYPE_BUILD` при non-ready статусе. Метод не читает raw webhook body, provider response, diff, GitHub/GitLab или полный YAML и не подбирает Dockerfile/image refs по файловой системе.
