...
| Проверка готовности | Готова. | Первый серверный контур проверяет `project-catalog` через `bootstrap/host/smoke_backend_contour.sh`: PostgreSQL, миграции `platform-event-log`, `access-manager`, миграции `project-catalog`, rollout и `/health/readyz`; доменные проверки должны жить в Go tests или будущем Go integration runner. |
