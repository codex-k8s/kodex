...
| Сборка проверочных образов | Готова. | `scripts/build-project-catalog-images.sh` собирает образы `project-catalog`, его миграций и минимальных зависимостей проверочного контура. |
| Проверка готовности | Готова. | `scripts/smoke-project-catalog.sh` применяет PostgreSQL, миграции `platform-event-log`, `access-manager`, миграции `project-catalog`, выкладку `project-catalog` и проверяет `/health/readyz`. |
