...
| PKG-5.3b | не назначено | Сверка статуса заполненности секретов установки должна быть готова: `RefreshPackageInstallationSecretStatus`, чтение канонических ссылок установки из `access-manager` и проверка доступности через `secretresolver.Checker` без получения значения. |
