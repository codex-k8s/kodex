...
related_issues: [642, 646, 650, 673, 678, 680, 684, 689, 692, 700, 704, 706, 711, 718]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
| Операции следующих срезов | `RefreshPackageInstallationSecretStatus` и runtime-связанные команды пока возвращают `unimplemented`; общий checker без возврата значения доступен в `libs/go/secretresolver`, а `access-manager` предоставляет контракт безопасного чтения ссылок установки. Подключение этого контракта в `package-hub` остаётся следующим срезом. |
