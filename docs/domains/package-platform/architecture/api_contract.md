...
`SyncAvailablePackages` не ходит во внешний Git/store/provider напрямую. Эту работу выполняет адаптер источника: он получает данные, приводит их к нормализованному контракту снимка и вызывает `package-hub`. Так сервис остаётся владельцем локального каталога и проверки manifest, но не становится Git-клиентом магазина. `manifest_digest` сверяется как `sha256:<hex>` от компактного нормализованного JSON manifest; несовпадение считается невалидным снимком.
