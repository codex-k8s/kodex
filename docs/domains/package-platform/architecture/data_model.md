...
Схема создаётся из нормализованного блока `secrets` проверенного manifest при синхронизации доступного каталога. Новая запись появляется только при новом `schema_digest` для версии пакета; повторная синхронизация с тем же набором полей не создаёт дубль. `package-hub` хранит только описание ожидаемых полей и итоговый статус заполненности установки, но не сырые секреты и не канонические ссылки на заполненные значения. Проверка доступности выполняется по безопасным ссылкам из `access-manager` через `secretresolver.Checker`, без возврата значения вызывающему коду; конкретная реализация (`kubernetes_mounted_secret`, `env`, `vault`) остаётся деталью resolver-клиента. Для Vault это проверка наличия пути и текущей версии через запрос только метаданных, а не чтение конкретного поля `#key`.
...
- Канонические ссылки на заполненные секреты хранит `access-manager` как `PackageInstallationSecretRef -> SecretBindingRef`; `package-hub` получает безопасные ссылки только на время пересчёта статуса и не становится владельцем этих ссылок. Если нужен технический health-check ссылки, используется только `secretresolver.Checker`.
