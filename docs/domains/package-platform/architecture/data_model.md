...
Схема создаётся из нормализованного блока `secrets` проверенного manifest при синхронизации доступного каталога. Новая запись появляется только при новом `schema_digest` для версии пакета; повторная синхронизация с тем же набором полей не создаёт дубль. `package-hub` хранит только описание ожидаемых полей и статус заполненности установки, но не сырые секреты и не канонические ссылки на заполненные значения. Проверка доступности значения выполняется по ссылкам из `access-manager` через `secretresolver.Checker`, без возврата значения вызывающему коду; конкретная реализация (`kubernetes_mounted_secret`, `env`, `vault`) остаётся деталью resolver-клиента.
