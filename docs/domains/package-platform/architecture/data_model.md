...
related_issues: [642, 673, 680, 689, 692, 700, 704, 706]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
Схема создаётся из нормализованного блока `secrets` проверенного manifest при синхронизации доступного каталога. Новая запись появляется только при новом `schema_digest` для версии пакета; повторная синхронизация с тем же набором полей не создаёт дубль. `package-hub` хранит только описание ожидаемых полей и статус заполненности установки, но не сырые секреты и не канонические ссылки на заполненные значения.
