...
related_issues: [642, 673, 680, 689, 692, 700, 704, 706, 711]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
При синхронизации доступного каталога новый снимок создаётся для новой версии пакета или при изменении source/manifest данных существующей версии. Повторная синхронизация без изменений не создаёт дубль снимка manifest. Перед записью `package-hub` сверяет `manifest_digest` с компактным нормализованным JSON manifest, чтобы изменение содержимого не могло пройти со старым digest.
...
Схема создаётся из нормализованного блока `secrets` проверенного manifest при синхронизации доступного каталога. Новая запись появляется только при новом `schema_digest` для версии пакета; повторная синхронизация с тем же набором полей не создаёт дубль. `package-hub` хранит только описание ожидаемых полей и статус заполненности установки, но не сырые секреты и не канонические ссылки на заполненные значения. Проверка доступности выполняется по ссылкам из `access-manager` через `secretresolver.Checker`, без возврата значения вызывающему коду; конкретная реализация (`kubernetes_mounted_secret`, `env`, `vault`) остаётся деталью resolver-клиента. Для Vault это проверка наличия пути и текущей версии через запрос только метаданных, а не чтение конкретного поля `#key`.
...
- Канонические ссылки на заполненные секреты хранит `access-manager` как `SecretBindingRef`; `package-hub` получает статус заполненности через команду, чтение или событие и не становится владельцем этих ссылок. Если нужен технический health-check ссылки, используется только `secretresolver.Checker`.
...
Для руководящих пакетов в БД не появляется отдельная таблица документов. `PackageManifestSnapshot` хранит только проверенный manifest, а тексты правил, шаблонов и руководств остаются в репозитории-источнике. Установка руководящего пакета отличается от установки плагина только `package_kind=guidance` и тем, что manifest не имеет runtime-требований, секретов, действий доступа и API платформы.

Для `store` и `platform_content` также не появляются отдельные таблицы. Пакет магазина и пакет пользовательского контента платформы используют те же `PackageEntry`, `PackageVersion`, `PackageManifestSnapshot` и `PackageInstallation`. Отличия фиксируются видом пакета и проверкой manifest:

- `store` может иметь runtime-требования, секреты, действия доступа и API платформы, но БД `package-hub` хранит только пакетную запись, manifest, схему секретов и установку, а не состояние магазина как бизнес-системы.
- `platform_content` не может требовать секреты, действия доступа или API платформы; его файлы сайта, справки и пользовательской документации не сохраняются в PostgreSQL, а остаются в репозитории-источнике или внешнем хранилище пакета.
