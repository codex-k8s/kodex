...
| `store` | Поставляет магазин пакетов как устанавливаемый пакет и источник доступного каталога. | Должен объявить capability `store`. Может иметь runtime, секреты и действия доступа, но бизнес-логика магазина остаётся внутри пакета магазина, а `package-hub` хранит только локальную пакетную истину. |
