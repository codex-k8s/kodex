...
|---|---|
| Источники магазинов, локальный доступный каталог, локальный установленный каталог, версии пакетов, снимки manifest, статусы проверки, схемы секретов, ссылки на секреты, требования API платформы, ценовые метаданные, события `package.*`. | Бизнес-логика магазина, исходники пакетов как Git-истина, webhook провайдера, runtime-нагрузки, Kubernetes, счета биллинга, сырые секреты, проектная документация, пользовательский интерфейс. |
...
| Домен | Связь |
|---|---|
| `access-manager` | Проверяет права на источники, установки, верификацию и управление scope пакета. |
