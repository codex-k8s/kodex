...
related_issues: [322, 380, 769, 790, 815, 827, 845, 856, 869, 886, 907, 919, 957, 972, 976, 1104]
related_prs: []
approvals:
  required: ["Owner"]
  status: pending
...
| `ListGateDecisions` | gRPC query | `governance.gate.read` | gate request или target | Читает final gate decisions по gate request или target; для gate request, связанного с risk assessment, access scope выводится из target/project context assessment. Outcome используется только как уточняющий фильтр. |
| `GetGateRequest` | gRPC query | `governance.gate.read` | нет | Читает gate request, evidence и decision status. |
| `ListGateRequests` | gRPC query | `governance.gate.read` | target или risk assessment | Читает ожидающие, resolved или просроченные gates по target или assessment. При lookup по `risk_assessment_id` сервис сначала читает assessment, выводит target/project context и проверяет `governance.gate.read` в этом project scope; status используется только как уточняющий фильтр. |
| `PrepareSelfDeployPlanGate` | gRPC command | `governance.risk.evaluate` + `governance.gate.request` | `command_id` или `idempotency_key`, `plan_fingerprint` | Принимает safe `SelfDeployPlan` refs от `agent-manager`, создаёт или находит risk assessment и required gate request по target `self_deploy_plan`, сверяет fingerprint, учитывает существующий gate decision и возвращает `pending`/`approved`/`rejected`/`request_changes`/`blocked` summary. |
...
| `GetGovernanceSummary` | gRPC query | существующие `governance.*.read` по выбранному scope | нет | Возвращает доменно подготовленную безопасную сводку для gRPC consumers, `platform-mcp-server` и будущего UI: `status` rollup, pending/completed decisions, risk class, required gate counts, gate/release outcomes и linked provider/agent/runtime evidence refs. Для target-based self-deploy summary `ProjectContext.ProjectRef` используется как access scope, а не как второй selector данных. |
...
`GetGovernanceSummary` — авторитетное чтение для интерфейса владельца, персонала и быстрых manager/slot-агентов поверх уже сохранённого governance state. Запрос обязательно ограничен ровно одним selector: `target`, `project_context`, `release_candidate_ref`, `release_decision_package_id` или безопасным `integration_ref` из release package. Исключение — target-based summary: любой `target` может идти вместе с `project_context`, где `ProjectContext.ProjectRef` задаёт project-scoped access check, но не включает отдельную project-wide выборку. Сервис не объединяет независимые selectors в одном ответе, чтобы карточка владельца или агентная сводка не смешивала решения и evidence от разных задач, PR, release package или agent run. Ответ содержит typed `status`, `pending_decisions`, `completed_decisions` и `evidence_summaries`: общий attention, максимальный `risk_class`, счётчики pending/blocked/completed решений, открытых gates, активных blocking signals, evidence и diagnostics, `summary_code`, `next_action_code`, risk assessment, review signal outcome, gate request/decision, release package/decision, blocking signal, safety-loop state, provider refs, agent/runtime refs, короткие safe summaries, timestamps, digest/version и bounded diagnostics для отсутствующих локальных связей. Для обычной агентной разработки active blocking signals по `pull_request`, `merge`, `runtime_job`, `document` или `policy_change` target попадают в summary и в evidence refs как `blocking_signal.<source_type>`, даже если release package ещё не собран. Если соседний owner-domain ref ещё не загружен или не имеет service-client read-contract, summary возвращает partial response с explicit ref и диагностикой вместо ошибки; прямого lookup по project/run/provider payload нет. `platform-mcp-server` отдаёт owner-prepared `status` и summary через `governance.summary.get`: MCP валидирует один selector, передаёт actor/source/request context, вызывает gRPC и не хранит governance state. `staff-gateway` сохраняет тонкую HTTP -> gRPC границу для существующего `GET /v1/governance/summary`; добавление нового `status` в HTTP DTO и frontend остаётся отдельным срезом `console-and-operations-ux`.

Минимальная связь с security baseline строится через уже существующие governance факты: vulnerability или runtime/infra finding передаётся как bounded risk factor, review signal, blocking signal или release evidence ref, после чего summary показывает максимальный риск, блокирующие решения, открытые gates, активные blocking signals, required gate counts и безопасные diagnostics. Для blocking signals используются типизированные source types `security`, `dependency`, `container` и `infrastructure`, чтобы владелец видел категорию блокера без чтения отчёта сканера. Сканеры зависимостей, container images и runtime/infra probes остаются владельцами своих исходных данных и добавляются отдельными срезами; `governance-manager` не хранит raw scan report, SBOM, provider payload, runtime logs или секреты.

Для self-deploy `codex-k8s/kodex` merge в `main` не является разрешением на автоматический deploy. `agent-manager` перед запуском runtime jobs вызывает `PrepareSelfDeployPlanGate` и передаёт только safe `SelfDeployPlan` refs: `self_deploy_plan_ref`, project/repository/release refs, provider/source/merge commit refs, `services.yaml` ref/digest, changed-file summary ref, path categories, service keys, expected runtime job types, bounded summary и `plan_fingerprint`. `governance-manager` создаёт или находит локальный risk assessment по target `self_deploy_plan`, сверяет fingerprint, классифицирует `R2/R3`, создаёт отсутствующий gate request и возвращает `pending`/`approved`/`rejected`/`request_changes`/`blocked` summary. Для этого пути access checks используют `ProjectContext.ProjectRef` как project scope для `governance.risk.evaluate/read` и `governance.gate.request/read`; `governance.gate.decide` остаётся отдельным правом владельца и не нужно для подготовки или чтения self-deploy gate. `GetGovernanceSummary` с `target=self_deploy_plan` и `ProjectContext` читает gate/review/decision через найденный project-scoped risk assessment и связанные IDs, а также показывает active blocking signals по target при наличии `governance.signal.read`. `SubmitGateDecision` для такого gate принимает финальные outcome `approve`, `reject` и `revise`; `revise` отображается как `request_changes`, чтобы вызывающий сервис не запускал build/deploy jobs и мог показать владельцу требуемые изменения. Runtime build/deploy jobs остаются запрещены, пока response не содержит approved gate decision. Low-risk изменения документации, тестов и неисполняемых описаний остаются `R0/R1` и не требуют Human gate, если policy не требует большего и deploy/runtime jobs не планируются. `services.yaml`, deploy manifests, branch/release policy, DB schema/migrations, runtime/runner, gateway, provider write, RBAC и service account изменения классифицируются как минимум `R2` и требуют owner/governance gate перед release decision `go`. Явные признаки secret values, kubeconfig, private key, auth bypass, отключения SSO/OIDC или raw payload/log leakage классифицируются как `R3` и блокируют deploy до явного устранения или owner decision через governance. `GetGovernanceSummary` по target `self_deploy_plan` показывает созданный gate request/decision и не предлагает дублирующий `request_governance_gate`, если gate request уже есть.

Один gate request получает только одно финальное решение: таблица `governance_manager_gate_decisions` уникальна по `gate_request_id`, а команда требует `expected_version` открытого request. Идемпотентный replay `SubmitGateDecision` сравнивает сохранённые safe поля команды: `gate_request_id`, outcome, actor/policy/source refs, interaction request/delivery/callback/decision refs, reason и conditions summary. Повтор с тем же `command_id` или `idempotency_key` возвращает сохранённое решение, а изменённый replay получает `conflict` и не создаёт новый outbox event. Событие `governance.gate.resolved` содержит только ids/refs, outcome, safe summary, status, target ref, actor/policy/source refs, request metadata и version; raw prompt, provider payload, YAML text, logs и секреты в событие не попадают.
...
| Сводка чтения governance | `GetGovernanceSummary` готовит безопасную модель чтения для владельца, персонала и manager/slot-агентов по target/project/release/package/integration ref: live `status` rollup, pending/completed decisions, risk class, required gate counts, gate/release outcomes, linked provider/agent/runtime evidence refs и partial diagnostics без raw payload; новый `status` доступен через gRPC и `platform-mcp-server`, а HTTP DTO в `staff-gateway` обновляется отдельным срезом. |
