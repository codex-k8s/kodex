...
| `GetGovernanceSummary` | gRPC query | существующие `governance.*.read` по выбранному scope | нет | Возвращает доменно подготовленную безопасную сводку для gRPC consumers, `platform-mcp-server` и будущего UI: `status` rollup, pending/completed decisions, risk class, required gate counts, gate/release outcomes и linked provider/agent/runtime evidence refs. |
