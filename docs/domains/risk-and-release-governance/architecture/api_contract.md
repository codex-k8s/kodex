...
Для self-deploy `codex-k8s/kodex` merge в `main` не является разрешением на автоматический deploy. Provider/project/runtime/orchestration контуры передают в `governance-manager` только безопасный deploy-plan evidence: target/ref, project/repository/release refs, digest, changed-file summary ref, path categories, service keys, risk codes и bounded summaries. Low-risk изменения документации, тестов и неисполняемых описаний остаются `R0/R1` и не требуют Human gate, если policy не требует большего. `services.yaml`, deploy manifests, branch/release policy, DB schema/migrations, runtime/runner, gateway, provider write, RBAC и service account изменения классифицируются как минимум `R2` и требуют owner/governance gate перед release decision `go`. Явные признаки secret values, kubeconfig, private key, auth bypass, отключения SSO/OIDC или raw payload/log leakage классифицируются как `R3` и блокируют deploy до явного устранения или owner decision через governance. `GetGovernanceSummary.status.pending_required_gate_count` и `next_action_code=request_governance_gate` показывают оператору, что нужно создать gate request, если risk assessment уже требует gate, но delivery-запрос владельцу ещё не создан.
