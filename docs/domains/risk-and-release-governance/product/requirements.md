...
| GOV-FR-19 | Домен должен принимать типизированные security baseline blocking signals для обычной агентной разработки: `security`, `dependency`, `container`, `infrastructure`, с безопасным source ref и bounded summary без отчётов сканеров. | Обязательно |
...
| GOV-AC-8 | Если по обычному `PR/MR`, `Run`, документу или policy change приходит active security baseline blocker, `GetGovernanceSummary` показывает blocked status, typed source category и безопасный evidence ref без release/self-deploy context. |
