...
| Сводка чтения governance | `GetGovernanceSummary` собирает безопасную модель чтения из локальных risk/gate/review/release/blocking/safety-loop фактов по target/project/release/package/integration ref; `staff-gateway` получает готовые pending/completed decisions и evidence summaries без вычисления governance-правил. | GOV-7j |
