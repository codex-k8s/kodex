...
Первый сценарий без Git хранит один Markdown размером до 65 536 UTF-8 байт в PostgreSQL. Текущий Mattermost-диалог `/agents` безопасно создаёт и повторно открывает инструкции размером не более 3000 UTF-8 байт; это предел байтовой проверки `textarea.Default`, а не обещание всей ёмкости хранилища через интерфейс. Публикация создаёт новую неизменяемую `InstructionVersion` с монотонным номером, SHA-256 и ссылкой на инициатора, затем атомарно обновляет `InstructionSet.current_version_id` и совместимый `AgentRole.prompt_template`. Пустой prompt допустим и не создаёт набор инструкций.

`managed_by=git` делает объект доступным интерфейсу только для чтения. Явное отсоединение переводит конфигурационный агрегат агента под управление интерфейса, добавляет версионированную запись в цепочку происхождения без замены прежнего объекта и создаёт аудит; только после этого разрешены публикация новой версии и изменение bot identity.
