...
При создании дочернего треда менеджер передаёт `owner_mention` и контракт возврата `post_id` прямо в `message` вызова `mattermost_start_agent_thread`. Дочерний агент вызывает `mattermost_post_thread_update` в собственном текущем треде, публикует отдельное короткое сообщение с точным `@username` владельца, сохраняет возвращённый `post_id` и включает его в `mattermost_return_to_requester`. Менеджер только проверяет наличие `post_id`; контракт не предполагает публикацию менеджером в дочерний тред или отсутствующий инструмент с выбором целевого треда.
