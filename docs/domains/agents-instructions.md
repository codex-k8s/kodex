...
Перед межкомнатным запуском координатор проверяет каталог и карточку целевого чата. Платформа проверяет capability и relationship policy, сохраняет точное сообщение запуска и наследует root initiator. Внутри треда волны координатор запускает назначенные роли через `mattermost_request_agent`; этот путь также создает долговечное делегирование. Дочерняя сессия в обоих режимах возвращает результат разрешенному координатору через `mattermost_return_to_requester`; обычное завершение, статус и callback не упоминают человека.
