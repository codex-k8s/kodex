...
`build` без spec остаётся в `pending` с диагностикой `build_execution_spec_required` и не попадает в `ClaimRunnableJob`. Если `build_context_ref` не является поддержанным PVC ref, исполнитель завершает задание безопасной ошибкой `invalid_build_context_ref`; создатель задания должен сначала материализовать checked build context и передать его ref/digest. Digest собранного образа фиксируется только если он уже безопасно известен в spec или появится отдельный проверенный output ref; парсинг логов Kaniko для получения digest не используется. После создания Kaniko Job исполнитель наблюдает Kubernetes Job по детерминированному имени и runtime labels, сверяет managed labels, различает `pending`, `running`, `succeeded`, `failed`, `timed_out` и `cancelled`, а таймаут Kubernetes Job сохраняет как runtime status `timed_out`. Короткий хвост лога сохраняется только в ограниченном виде и при небезопасных маркерах заменяется безопасной диагностикой. `deploy` имеет типизированный `DeployExecutionSpec`, но исполнитель Kubernetes-выкладки не подключён: `deploy` без spec остаётся в `pending` с `deploy_execution_spec_required`, а `deploy` со spec остаётся в `pending` с `deploy_executor_unavailable`, доступен через `GetJob`/`ListJobs` и не попадает в `ClaimRunnableJob`. Runtime-manager не вызывает `kubectl apply`, не читает manifest YAML и не запускает автоматическую выкладку production.
