...
Типы `build` и `deploy` уже имеют типизированную runtime-модель для будущего саморазвёртывания, но текущий Kubernetes executor их не исполняет. Для этих jobs исполнимый вход должен приходить через `BuildExecutionSpec` или `DeployExecutionSpec`: source ref/commit SHA, `service_key`, image refs/digests, build context или manifest/kustomization refs/digests, target namespace/cluster refs, fingerprint проверенного плана и `allowed_secret_refs` без значений. `build`/`deploy` без spec остаются в `pending` с диагностикой `build_execution_spec_required` или `deploy_execution_spec_required` и не попадают в `ClaimRunnableJob`. Если оператор видит такую диагностику, исправлять нужно producer/deploy-plan контур, а не вручную добавлять raw JSON, env, kubeconfig или команды в runtime job.
