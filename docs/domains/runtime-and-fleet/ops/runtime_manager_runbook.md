...
`build` без spec остаётся в `pending` с диагностикой `build_execution_spec_required` и не попадает в `ClaimRunnableJob`. Если `build_context_ref` не является поддержанным PVC ref, executor завершает задание безопасной ошибкой `invalid_build_context_ref`; producer должен сначала материализовать checked build context и передать его ref/digest. Digest собранного образа фиксируется только если он уже безопасно известен в spec или появится отдельный проверенный output ref; парсинг логов Kaniko для получения digest не используется. `deploy` имеет типизированный `DeployExecutionSpec`, но Kubernetes rollout executor не подключён: `deploy` без spec остаётся в `pending` с `deploy_execution_spec_required`, а `deploy` со spec не исполняется этим executor-ом.
