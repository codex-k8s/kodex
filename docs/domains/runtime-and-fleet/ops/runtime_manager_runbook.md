...
Build context для self-deploy готовится отдельным runtime-owned lifecycle до создания `JOB_TYPE_BUILD`. Orchestration вызывает `PrepareBuildContext` по project/repository/provider/source refs, merge commit SHA, affected service keys и build plan fingerprint. Если checked source snapshot ref/digest ещё не готов, состояние остаётся `pending` с `source_snapshot_unavailable` и `next_action=provide_checked_source_snapshot`; это означает, что owner provider/project/source-artifact контура должен передать проверенный snapshot, а runtime не должен выполнять скрытый checkout. После получения checked source snapshot trusted materializer/controller вызывает `ReportBuildContextProgress`: `running` требует source snapshot ref/digest, `ready` требует `build_context_ref` и `build_context_digest`. Только `ready` context пригоден для `BuildExecutionSpec`. В таблице runtime хранятся только refs, digest, status, безопасная ошибка и timestamps; содержимое snapshot/context, raw webhook body, diff, provider response, полный YAML, kubeconfig и значения секретов не сохраняются.

`build` без spec остаётся в `pending` с диагностикой `build_execution_spec_required` и не попадает в `ClaimRunnableJob`. Если `build_context_ref` не является поддержанным PVC ref, исполнитель завершает задание безопасной ошибкой `invalid_build_context_ref`; создатель задания должен сначала материализовать checked build context и передать его ref/digest. Перед созданием Kaniko Job executor делает предварительную проверку готовности: PVC должен существовать и быть `Bound`, registry Secret ref должен быть указан и доступен, а service account должен существовать, если он задан. Неготовность фиксируется как `build_context_ref_required`, `build_context_pvc_unavailable`, `build_context_pvc_not_ready`, `build_context_pvc_access_denied`, `build_context_pvc_status_unavailable`, `build_registry_secret_ref_required`, `build_registry_secret_unavailable`, `build_registry_secret_access_denied`, `build_registry_secret_status_unavailable`, `kubernetes_service_account_unavailable`, `kubernetes_service_account_access_denied`, `kubernetes_service_account_status_unavailable` или `kubernetes_job_create_access_denied`; это означает, что build-контур не готов до запуска Pod, а не что Kaniko выполнил сборку с ошибкой. `next_action` указывает ближайшее действие: `prepare_build_context_pvc`, `provide_build_registry_secret_ref` или `fix_runtime_kubernetes_rbac`. Digest собранного образа фиксируется только если он уже безопасно известен в spec или появится отдельный проверенный output ref; парсинг логов Kaniko для получения digest не используется. После создания Kaniko Job исполнитель наблюдает Kubernetes Job по детерминированному имени и runtime labels, сверяет managed labels, различает `pending`, `running`, `succeeded`, `failed`, `timed_out` и `cancelled`, а таймаут Kubernetes Job сохраняет как runtime status `timed_out`. Короткий хвост лога сохраняется только в ограниченном виде и при небезопасных маркерах заменяется безопасной диагностикой. `deploy` имеет типизированный `DeployExecutionSpec`, но исполнитель Kubernetes-выкладки не подключён: `deploy` без spec остаётся в `pending` с `deploy_execution_spec_required`, а `deploy` со spec остаётся в `pending` с `deploy_executor_unavailable`, доступен через `GetJob`/`ListJobs` и не попадает в `ClaimRunnableJob`. Валидный deploy spec должен приходить из будущего `project-catalog.GetSelfDeployDeployPlan` после successful build и содержать checked `manifest_bundle_ref`/`manifest_bundle_digest`, rollout targets и expected image refs. Runtime-manager не вызывает `kubectl apply`, не читает raw manifest YAML из Git provider, не выводит manifest bundle из `services.yaml` или build output и не запускает автоматическую выкладку production.
...
Если задание падает с `cluster_secret_unavailable`, `cluster_ref_unavailable`, `kubernetes_client_init_failed`, `kubernetes_job_create_failed`, `kubernetes_job_create_access_denied`, `build_context_ref_required`, `build_context_pvc_unavailable`, `build_context_pvc_not_ready`, `build_context_pvc_access_denied`, `build_context_pvc_status_unavailable`, `build_registry_secret_ref_required`, `build_registry_secret_unavailable`, `build_registry_secret_access_denied`, `build_registry_secret_status_unavailable`, `kubernetes_service_account_unavailable`, `kubernetes_service_account_access_denied` или `kubernetes_service_account_status_unavailable`, проверять нужно secret ref в `fleet-manager`, настройки `KODEX_RUNTIME_MANAGER_SECRET_RESOLVER_*`, RBAC service account `runtime-manager`, наличие default namespace/image, готовность PVC build context и наличие registry Secret ref. Значения kubeconfig, токенов, DSN и содержимое Secret не выводить в Issue/PR и не прикладывать к отчётам.
