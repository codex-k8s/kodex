...
| RTM-BD-1 | без отдельного Issue | Зафиксирована типизированная runtime-модель для будущих jobs саморазвёртывания `build` и `deploy`: `BuildExecutionSpec` и `DeployExecutionSpec` принимают только проверенные source/service/image/build-context/manifest/kustomization/target refs, checked manifest bundle refs, rollout targets, expected image refs, digest/fingerprint и secret refs без значений. `CreateJob` сохраняет specs как канонический `job_input_json`, `Job` возвращает типизированную проекцию, а jobs без spec остаются ожидающими с безопасной диагностикой и не claim-ятся. |
| RTM-BD-2 | без отдельного Issue | Kubernetes executor исполняет `JOB_TYPE_BUILD` с валидным `BuildExecutionSpec` через Kaniko: build context монтируется как PVC только для чтения, Dockerfile path остаётся внутри context, destination image ref/tag и target передаются фиксированными args, registry credentials подключаются только через Kubernetes `Secret` ref, а runtime сохраняет Kubernetes Job/namespace refs и destination image ref без raw env, webhook payload, provider response, полного YAML, diff, kubeconfig или значений секретов. |
| RTM-BD-3 | без отдельного Issue | Kubernetes executor наблюдает Kaniko build Job до терминального состояния: сверяет managed labels, умеет резервный поиск по runtime labels, различает `pending`, `running`, `succeeded`, `failed`, `timed_out` и `cancelled`, сохраняет таймаут как runtime status `timed_out` и пишет только безопасную ограниченную диагностику без сырых логов, Kubernetes events, kubeconfig или значений секретов. |
| RTM-BD-4 | без отдельного Issue | Deploy job готов как безопасный типизированный жизненный цикл для следующего перехода саморазвёртывания после успешного build: `DeployExecutionSpec` сохраняется и читается через `GetJob`/`ListJobs`, повтор `CreateJob` идемпотентен, а задание остаётся `pending` с диагностикой `deploy_executor_unavailable` и не выдаётся через `ClaimRunnableJob`, пока исполнитель выкладки не согласован. |
...
| Build context materialization | Готова contract-first поверхность: `PrepareBuildContext`, `ReportBuildContextProgress`, `GetBuildContext`. | Runtime хранит отдельную запись `BuildContext` для self-deploy build context, идемпотентную по `context_fingerprint`. Запись содержит только refs/digest/status/diagnostics/timestamps и не содержит raw source snapshot, webhook body, diff, provider response, полный YAML, kubeconfig или значения секретов. `pending` с `source_snapshot_unavailable` фиксирует, что соседний provider/project/source-artifact контур ещё не передал checked source snapshot ref/digest; `ready` допускается только с `build_context_ref` и `build_context_digest`. |
| Platform jobs | Готов: создание, claim с `lease_token`, progress, complete/fail/cancel, чтения и события `runtime.job.*`. | Команды `CreateJob`, `ClaimRunnableJob`, `ReportJobStepProgress`, `CompleteJob`, `FailJob`, `CancelJob`, чтения `GetJob`/`ListJobs`, PostgreSQL repository, проверка доступа и gRPC-подключение готовы. `CreateJob` без slot получает fleet-ссылки через `ResolvePlacement`; `CreateJob` со slot наследует refs из slot. Исполнитель получает короткий lease и одноразовый `lease_token`. `build` и `deploy` имеют типизированные `BuildExecutionSpec`/`DeployExecutionSpec` для будущего саморазвёртывания: source ref/commit SHA, `service_key`, image refs/digests, build context refs или manifest/kustomization refs, checked manifest bundle refs, rollout targets, expected image refs, target refs, plan fingerprint, `allowed_secret_refs` без значений и `output_refs`. Raw `job_input_json` для этих типов не принимается, а jobs без spec остаются видимыми, но не claim-ятся. `build` с валидным spec исполняется через Kaniko, если build context уже материализован как PVC и registry Secret ref готов: исполнитель до создания Pod проверяет, что PVC находится в фазе `Bound`, метаданные registry Secret и ссылку service account, возвращает типизированную диагностику блокера при неготовности, затем создаёт Kubernetes Job с фиксированной командой `/kaniko/executor`, context mount только для чтения, destination image ref/tag, фиксированным target и registry `Secret` volume только по allowed secret ref. После запуска он наблюдает состояние по имени и runtime labels, проверяет managed labels, различает `pending`, `running`, `succeeded`, `failed`, `timed_out` и `cancelled`, а таймаут сохраняет как runtime status `timed_out`. `deploy` появляется только после successful build и checked `project-catalog.GetSelfDeployDeployPlan`: валидный `DeployExecutionSpec` обязан содержать `manifest_bundle_ref`/`manifest_bundle_digest`, rollout targets и expected image refs, сохраняется и читается как типизированное задание, но остаётся `pending` с диагностикой `deploy_executor_unavailable` и не выдаётся через `ClaimRunnableJob`, пока исполнитель выкладки не реализован. Runtime не вызывает `kubectl apply`, не выводит manifest bundle из репозитория, `services.yaml` или build output, не читает raw manifest YAML, kubeconfig и значения секретов и не запускает автоматическую выкладку production. `agent_run` является отдельным каноническим типом задания для agent Run и не подменяется `build`/`deploy`/`housekeeping`; typed `AgentRunExecutionSpec` хранит безопасные refs/digest/fingerprint, сверяется со slot и завершённой materialization, а задание без spec остаётся ожидающим с безопасной диагностикой и не claim-ится для исполнения. Kubernetes-исполнитель готов для заданий `health_check`, `agent_run` с валидным spec и `build` с валидным spec: он включается явным env-флагом, читает ссылку на секрет кластера через `fleet-manager`, создаёт ограниченный Kubernetes Job через `client-go`, различает остановку worker-а и терминальные ошибки Kubernetes Job, переиспользует детерминированный Kubernetes Job при повторном claim и завершает runtime job через штатные lifecycle-команды. Для `agent_run` используются image/profile/context/workspace refs из spec, фиксированная команда runner-а, PVC mount и env только со safe refs без значений секретов. Адрес `agent-manager` передаётся runner-у строкой, а gRPC token подключается через Kubernetes `SecretKeyRef`, поэтому `runtime-manager` не читает и не сохраняет значение token. Optional `CodexSessionExecutionSpec` добавляет safe refs Codex CLI запуска без prompt body: checked execution input материализуется отдельным объектом или файлом workspace и читается только по ref/digest. `agent-runner` проверяет смонтированный context, digest, fingerprint и execution spec; для workspace refs `workspace://.kodex/execution/...` вызывает `codex exec` фиксированным executable/args, передаёт instruction через stdin и schema через `--output-schema`, затем через `agent-manager.ReportAgentRunState` фиксирует безопасный `running`/`completed`/`failed` state. Object-store refs требуют отдельного безопасного механизма чтения и не исполняются через fallback. Исполнитель Kubernetes-выкладки остаётся отдельным runtime-срезом. |
