...
`BuildExecutionSpec` фиксирует проверенный source ref/commit SHA, `service_key` из проверенного инвентаря стека `services.yaml`, target image ref/tag и optional digest, build context ref/digest, Dockerfile ref/digest, фиксированный target, утверждённый builder image ref, build plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`. `DeployExecutionSpec` фиксирует проверенный source ref/commit SHA, `service_key`, image ref/tag/digest, manifest/kustomization refs/digests, target namespace, fleet-owned target cluster ref, optional slot ref, deploy plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`. Задания `build` и `deploy` без spec остаются видимыми в `pending` с `build_execution_spec_required` или `deploy_execution_spec_required`, но не выдаются через `ClaimRunnableJob`. `build` с валидным spec исполняется через Kaniko, если build context уже материализован как PVC; `deploy` с валидным spec сохраняется как ожидающее задание с `deploy_executor_unavailable` и не выдаётся через `ClaimRunnableJob` до согласования исполнителя Kubernetes-выкладки.
