...
Типы `build` и `deploy` выделены для цепочки саморазвёртывания после GitHub merge, deploy plan и owner/governance approval. `runtime-manager` не переносит в себя `cmd/bootstrap-backend-deploy` и не становится shell-обвязкой Kaniko или `kubectl`: он хранит job lifecycle, steps, short log tail, runtime artifact refs и безопасную диагностику. Исполнимый вход для этих типов принимается только как типизированный `BuildExecutionSpec` или `DeployExecutionSpec`; произвольный `job_input_json`, raw env, kubeconfig, webhook body, provider response, полный YAML и большие логи не принимаются.

`BuildExecutionSpec` фиксирует проверенный source ref/commit SHA, `service_key` из проверенного инвентаря стека `services.yaml`, target image ref/tag и optional digest, build context ref/digest, Dockerfile ref/digest, фиксированный target, утверждённый builder image ref, build plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`. `DeployExecutionSpec` фиксирует проверенный source ref/commit SHA, `service_key`, image ref/tag/digest, manifest/kustomization refs/digests, обязательный checked `manifest_bundle_ref`/`manifest_bundle_digest`, rollout targets, expected image refs, target namespace, fleet-owned target cluster ref, optional slot ref, deploy plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`. Задания `build` и `deploy` без spec остаются видимыми в `pending` с `build_execution_spec_required` или `deploy_execution_spec_required`, но не выдаются через `ClaimRunnableJob`. `build` с валидным spec исполняется через Kaniko, если build context уже материализован как PVC; `deploy` с валидным checked manifest bundle исполняется controlled deploy executor-ом через Kubernetes SDK/server-side apply и rollout watch.

Перед реальным исполнением `agent_run` должен иметь typed `AgentRunExecutionSpec`; для agent Run его собирает `agent-manager` после `PrepareRuntime`, но передаёт в `CreateJob` только когда slot уже `ready`, workspace materialization `completed`, а `RuntimeContext.workspace_pvc_ref` доступен как safe ref. В spec попадают только безопасные refs и контрольные значения: `agent_run_id`, `slot_id`, ожидаемая workspace materialization, workspace mount/PVC/workspace refs, `.kodex/context/agent-run.json` ref/digest, `runner_profile_ref`, `runner_image_ref`, фиксированный `runner_mode`, ссылки на разрешённые секреты без значений и цели отчёта runner. Вложенный `CodexSessionExecutionSpec` описывает запуск Codex CLI: checked instruction object ref/digest, result schema ref/digest, session или workspace snapshot ref, hook endpoint/callback refs, timeout, фиксированный runner profile, output/result refs и allowed secret refs без значений. Для текущего runner instruction и result schema refs должны быть уже материализованы в workspace и иметь вид `workspace://.kodex/execution/...`; `object://` refs не считаются готовым execution input для runtime job. `CreateJob` сверяет spec с slot и завершённой materialization, отклоняет invalid typed spec до записи задания; задание без spec остаётся в ожидающем состоянии с безопасной диагностикой и не забирается исполнителем.

Первый реальный исполнитель Kubernetes находится внутри `runtime-manager` и выключен по умолчанию. После включения он забирает `health_check`, `agent_run` с валидным `AgentRunExecutionSpec`, `build` с валидным `BuildExecutionSpec` и `deploy` с валидным `DeployExecutionSpec`, получает `cluster_id` из сохранённого placement, читает через `fleet-manager.GetKubernetesCluster` только безопасную ссылку на kubeconfig/service account secret и создаёт ограниченный Kubernetes Job или controlled deploy operation через `client-go`. Runtime не вызывает `kubectl`, не читает БД `fleet-manager`, не хранит kubeconfig и не пишет полный лог или Kubernetes events в PostgreSQL. Для self-deploy deploy `project-catalog.GetSelfDeployDeployPlan` готовит checked `DeployExecutionSpec` после successful build, `agent-manager` создаёт `JOB_TYPE_DEPLOY` только по этому готовому spec, а runtime executor применяет checked bundle без raw provider/manifests.

Для `health_check` поля `namespace`, `service_account`, `image` и `labels` проходят строгую проверку; значения `env`, annotations, команды контейнера, значения секретов, prompt, transcript и provider payload не принимаются. Команда контейнера фиксирована для проверки здоровья. Для `agent_run` произвольные поля не принимаются: executor читает только `agent_run_execution_spec`, требует workspace PVC ref, создаёт контейнер `runtime-agent-runner` с image из `runner_image_ref`, фиксированной командой `/kodex/bin/agent-runner run`, выключенным automount service account token, PVC mount в фиксированную точку `/workspace` и env только со safe refs/digest/fingerprint. Если присутствует `CodexSessionExecutionSpec`, executor передаёт его runner-у как bounded JSON env с безопасными refs; checked instruction input и result schema должны быть отдельными materialized workspace refs `workspace://.kodex/execution/...` и не передаются как текст. Для отчёта результата executor передаёт runner-у адрес `agent-manager` и gRPC token только как Kubernetes `SecretKeyRef` на operator-managed Secret; значение токена не проходит через `runtime-manager`. Для `build` executor читает только `build_execution_spec`, требует PVC build context ref, запускает фиксированную Kaniko command `/kaniko/executor`, монтирует context только для чтения в `/workspace/context`, передаёт destination image ref/tag и Dockerfile path только как проверенные args, а registry credentials подключает только как Kubernetes `Secret` volume по allowed secret ref. Перед созданием Kaniko Job executor проверяет, что PVC существует и находится в `Bound`, метаданные registry Secret доступны, а заданный service account существует. Неготовые PVC/Secret/service account refs становятся безопасной типизированной диагностикой до запуска Pod. Secret refs остаются ссылками; значения секретов не записываются в состояние runtime, события, логи или краткую сводку.

Kubernetes worker не считает остановку сервиса ошибкой platform job. Если процесс завершается во время ожидания Kubernetes Job, worker прекращает текущую попытку без `FailJob`; после истечения lease задание может быть забрано повторно, а детерминированное имя Kubernetes Job позволяет продолжить сверку уже созданного объекта без дубля. Для `build` исполнитель сначала выполняет preflight readiness и только потом создаёт Kaniko Job с `activeDeadlineSeconds`, наблюдает состояние по имени и runtime labels, проверяет managed labels и нормализует фазы `pending`, `running`, `succeeded`, `failed`, `timed_out` и `cancelled`. Терминальными ошибками считаются таймаут, условие `JobFailed`, удалённый/отменённый Kubernetes Job и невозможность получить статус Kubernetes Job; таймаут сохраняется как status `timed_out`. Повторные ошибки claim/report/complete выполняются с увеличивающейся задержкой. Короткий хвост лога остаётся ограниченным по размеру, проходит безопасную очистку для `build` и сохраняется через штатную границу `ReportJobStepProgress`/`CompleteJob`/`FailJob`.
