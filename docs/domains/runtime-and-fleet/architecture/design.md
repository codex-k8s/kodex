...
Перед реальным исполнением `agent_run` должен иметь typed `AgentRunExecutionSpec`; для agent Run его собирает `agent-manager` после `PrepareRuntime`, но передаёт в `CreateJob` только когда slot уже `ready`, workspace materialization `completed`, а `RuntimeContext.workspace_pvc_ref` доступен как safe ref. В spec попадают только безопасные refs и контрольные значения: `agent_run_id`, `slot_id`, ожидаемая workspace materialization, workspace mount/PVC/workspace refs, `.kodex/context/agent-run.json` ref/digest, `runner_profile_ref`, `runner_image_ref`, фиксированный `runner_mode`, ссылки на разрешённые секреты без значений и цели отчёта runner. Вложенный `CodexSessionExecutionSpec` описывает запуск Codex CLI: checked instruction object ref/digest, result schema ref/digest, session или workspace snapshot ref, hook endpoint/callback refs, timeout, фиксированный runner profile, output/result refs и allowed secret refs без значений. `CreateJob` сверяет spec с slot и завершённой materialization; задание без spec или без валидного вложенного execution spec остаётся в ожидающем состоянии с безопасной диагностикой и не забирается исполнителем.
