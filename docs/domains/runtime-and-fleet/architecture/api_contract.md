...
| Операция | Назначение | Вызывает | Идемпотентность |
|---|---|---|---|
| `PrepareBuildContext` | Зафиксировать runtime-owned запрос подготовки build context для self-deploy по project/repository/provider/source refs, commit SHA, affected service keys и build plan fingerprint. | `agent-manager`, self-deploy controller | `command_id` или `idempotency_key`; дополнительная идемпотентность по `context_fingerprint`. |
| `ReportBuildContextProgress` | Обновить статус подготовки build context от доверенного materializer/controller. | runtime controller | `command_id + expected_version`. |
| `GetBuildContext` | Прочитать состояние build context по id или `context_fingerprint`. | `agent-manager`, операторский контур | Read-only. |

`BuildContext` — отдельная runtime-owned модель подготовки build context для self-deploy, а не slot workspace materialization и не runtime job. Она идемпотентна по `context_fingerprint`, вычисленному из project/repository/provider/source refs, source commit SHA, affected service keys и `build_plan_fingerprint`. В БД сохраняются только refs, digest, status, `next_action`, короткая безопасная ошибка и timestamps: raw webhook body, diff, provider response, полный YAML, kubeconfig, token values, содержимое source snapshot и содержимое build context не сохраняются. `runtime-manager` не ходит напрямую в GitHub/GitLab и не делает скрытый `git clone`: готовность требует checked source snapshot ref/digest от владельца provider/project/source-artifact контура.
...
Self-deploy orchestration plan в `agent-manager` не вызывает `CreateJob` до owner/governance approval, включая путь `CreateSelfDeployPlanFromSignal` от provider/project safe signal. Он хранит только safe refs, affected service keys, `services.yaml` digest/fingerprint и expected job types `build`, `deploy`, `health_check` как подготовку к owner/governance approval. После `approved` gate `agent-manager` получает build recipe через `project-catalog.GetSelfDeployBuildPlan`; если ответ требует context, он вызывает или переиспользует `runtime-manager.PrepareBuildContext` и хранит `runtime_build_status=preparing_context`. Runtime-owned path подготовки build context фиксируется отдельным lifecycle: `PrepareBuildContext` принимает project/repository/provider/source refs, merge commit SHA, affected service keys и build plan fingerprint; Kubernetes materializer для self-repo получает source auth только через `SecretRef`, скачивает фиксированный provider archive по commit, создаёт PVC/ref, проверяет source snapshot и считает digest подготовленного context. Ready context допускается только с `build_context_ref` и `build_context_digest`. После передачи ready refs как `materialized_build_contexts` `project-catalog.GetSelfDeployBuildPlan` возвращает ready build items без хранения динамического PVC/digest в `services.yaml`, а `agent-manager` создаёт `JOB_TYPE_BUILD` через typed `CreateJob`. Deploy job появляется только после successful build и checked `project-catalog.GetSelfDeployDeployPlan`, который возвращает `DeployExecutionSpec` с manifest bundle refs/digest, rollout targets и expected image refs. Health-check jobs остаются отдельным approval-driven переходом. В build/deploy dispatch не передаются raw webhook body, diff, полный `services.yaml`, raw manifests, секреты или token values.

`JOB_TYPE_BUILD` и `JOB_TYPE_DEPLOY` являются каноническими типами runtime job для будущего саморазвёртывания после provider merge signal, deploy plan и owner/governance approval. Эти типы не выражают произвольную shell-команду или перенос bootstrap deploy tool внутрь `runtime-manager`: сервис хранит lifecycle/status/artifact refs/short log tail, а исполнимый вход принимает только как типизированный spec. Для self-deploy build статический рецепт готовит `project-catalog.GetSelfDeployBuildPlan` из checked `ServicesPolicy`; materialized build context готовит `runtime-manager` для конкретного repository/source ref/commit и отдаёт вызывающей стороне refs/digest, которые затем передаются обратно в `project-catalog` для сборки executable `BuildExecutionSpec`. `runtime-manager` не читает project policy и не парсит `services.yaml`. `BuildExecutionSpec` содержит проверенный source ref и commit SHA, `service_key` из проверенного инвентаря стека `services.yaml`, target image ref/tag и optional digest, build context ref/digest, Dockerfile ref/digest, фиксированный Dockerfile target, утверждённый builder image ref, build plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`. Если `project-catalog` возвращает `build_context_required`, runtime job не создаётся: `agent-manager` фиксирует `preparing_context`, а runtime-owned path должен сначала подготовить materialized build context PVC/ref и digest. `DeployExecutionSpec` создаётся только из checked deploy plan: он содержит проверенный source ref и commit SHA, `service_key`, image ref/tag/digest, manifest/kustomization refs/digests, обязательные `manifest_bundle_ref` и `manifest_bundle_digest`, rollout targets, expected image refs, target namespace, fleet-owned target cluster ref, optional slot ref, deploy plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`.

`runtime-manager` сохраняет build/deploy specs как канонические `job_input_json.build_execution_spec` и `job_input_json.deploy_execution_spec`, возвращает их в типизированных полях `Job.build_execution_spec`/`Job.deploy_execution_spec` и не принимает старый свободный payload вместе со spec. `build` без `BuildExecutionSpec` и `deploy` без `DeployExecutionSpec` можно создать и увидеть через `GetJob`/`ListJobs`, но такие задания остаются ожидающими с диагностикой `build_execution_spec_required` или `deploy_execution_spec_required` и не попадают в `ClaimRunnableJob`. `deploy` со spec без checked manifest bundle ref/digest, rollout targets или expected image refs не считается валидным typed deploy input и отклоняется до записи. `build` с валидным `BuildExecutionSpec` исполняется Kubernetes-исполнителем через Kaniko, если `build_context_ref` указывает на уже подготовленный PVC build context и registry push Secret передан как allowed ref. Перед созданием Kaniko Job executor проверяет, что PVC существует и находится в фазе `Bound`, registry Secret metadata доступна по ref, а указанный service account доступен, если он задан в executor config. Неготовый build context или недоступный registry Secret фиксируются как typed безопасная ошибка до создания Kubernetes Job. `deploy` с валидным `DeployExecutionSpec` исполняется controlled executor через Kubernetes SDK: executor монтирует checked manifest bundle PVC только для чтения, применяет объекты server-side apply, сверяет namespace/rollout targets/expected image refs и наблюдает rollout до terminal состояния. Runtime не читает manifest YAML из Git provider, не выводит manifest bundle из `services.yaml`, не принимает произвольный shell и не логирует kubeconfig или значения секретов.
...
Для исполнения `agent_run` используется typed `AgentRunExecutionSpec`, который формирует `agent-manager` при opt-in dispatch после готовности slot/materialization. Spec содержит только безопасные refs, digest и fingerprint: `agent_run_id`, `slot_id`, ожидаемые `workspace_materialization_id` и fingerprint, workspace mount/PVC/workspace refs, ссылку и digest `.kodex/context/agent-run.json`, `runner_profile_ref`, `runner_image_ref`, фиксированный `runner_mode`, `allowed_secret_refs` без значений и `reporting_target_refs`. `PrepareRuntime` возвращает `RuntimeContext.workspace_pvc_ref` как runtime-owned safe ref на workspace PVC; `agent-manager` переносит его в `AgentRunExecutionSpec` и не выводит PVC ref из project/provider данных. Для запуска Codex-сессии `agent-manager` передаёт вложенный `CodexSessionExecutionSpec`: `instruction_object_ref`/digest, `result_schema_ref`/digest, `session_snapshot_ref` или `workspace_snapshot_ref`, `hook_endpoint_ref`, callback refs, ограниченный timeout, фиксированный runner profile, output/result refs и allowed secret refs без значений. `instruction_object_ref` и `result_schema_ref` для текущего runner принимаются только как `workspace://.kodex/execution/...`; безопасные `object://` refs не являются готовым execution input и должны оставлять `Run` в `execution_input_unavailable` до `CreateJob`. `runtime-manager` сохраняет spec как канонический `job_input_json.agent_run_execution_spec`, сверяет slot, завершённую workspace materialization и fingerprint, не принимает legacy raw payload вместе со spec, отклоняет invalid typed spec и не принимает произвольную команду. Для Kubernetes-исполнения `workspace_pvc_ref` обязателен: первый executor принимает safe PVC ref как `pvc://<namespace>/<claim>` или `k8s://pvc/<claim>` и монтирует workspace в фиксированную точку контейнера. `agent_run` без spec остаётся созданным заданием с безопасной диагностикой `agent_run_execution_spec_required` и не попадает в claim для исполнения; invalid `CodexSessionExecutionSpec` отклоняется до записи задания или завершается в runner-е безопасной диагностикой `agent_execution_contract_unavailable`, если несовместимый spec был передан через старую очередь.

Исполнитель Kubernetes в `runtime-manager` обрабатывает ограниченные типы `health_check`, `agent_run`, `build` и `deploy`. Он забирает задание через `ClaimRunnableJob`, читает выбранный кластер через `fleet-manager.GetKubernetesCluster`, получает только ссылку `secret_store_type`/`secret_store_ref` и разрешает kubeconfig в памяти через `secretresolver`. Значение kubeconfig, raw Kubernetes objects, events и полный лог не пишутся в БД. Запуск фиксируется через `ReportJobStepProgress` с `RuntimeArtifactRef` на Kubernetes Job/namespace или controlled deploy artifact ref, для `agent_run` дополнительно сохраняется image ref runner-а, для `build` — destination image ref и digest, если digest уже безопасно известен. Для `build` исполнитель перед созданием Kaniko Job выполняет предварительную проверку: проверяет PVC build context, метаданные registry Secret и service account ref без вывода значений секретов. Коды `build_context_ref_required`, `build_context_pvc_unavailable`, `build_context_pvc_not_ready`, `build_context_pvc_access_denied`, `build_context_pvc_status_unavailable`, `build_registry_secret_ref_required`, `build_registry_secret_unavailable`, `build_registry_secret_access_denied`, `build_registry_secret_status_unavailable`, `kubernetes_service_account_unavailable`, `kubernetes_service_account_access_denied`, `kubernetes_service_account_status_unavailable` и `kubernetes_job_create_access_denied` являются типизированной диагностикой блокера build-контура, а не падением Kaniko внутри Pod. После успешной предварительной проверки исполнитель создаёт Kaniko Job с `activeDeadlineSeconds`, затем сверяет состояние Kubernetes Job по детерминированному имени и резервному поиску по runtime labels, проверяет managed labels и различает `pending`, `running`, `succeeded`, `failed`, `timed_out` и `cancelled`. Для `deploy` executor запускает фиксированный binary `runtime-deployer`, передаёт только typed bundle/target args, применяет manifest bundle через Kubernetes SDK и ждёт rollout targets. Завершение идёт через `CompleteJob` или `FailJob`; таймаут Kubernetes Job или rollout сохраняется как runtime job status `timed_out` с безопасной диагностикой.
...
Для `build` Kubernetes Job создаётся с детерминированным именем runtime job и фиксированным контейнером `runtime-kaniko-build`. Executor принимает только `BuildExecutionSpec`, монтирует `build_context_ref` как PVC только для чтения в `/workspace/context`, поддерживает ref-формы `pvc://<namespace>/<claim>`, `k8s://pvc/<claim>` и короткое имя PVC в default namespace, а `dockerfile_ref` принимает только путь внутри context: `context://<relative-path>` или безопасный относительный путь. Команда контейнера фиксирована как `/kaniko/executor`, args фиксируют `--context=dir:///workspace/context`, `--dockerfile`, `--destination`, `--target`, `--cache=false`, `--snapshot-mode=redo` и `--verbosity=info`. `builder_image_ref` задаёт утверждённый Kaniko image ref; `image_ref` и `image_tag` формируют destination image ref, но executor не создаёт tag/digest из логов. Registry credentials обязательны для Kaniko executor и подключаются только как Kubernetes `Secret` volume по `allowed_secret_refs` с kind `registry`, `registry_docker_config` или `docker_config`; ref вида `secret://runtime/<secret-name>` или `k8s://secret/<secret-name>` монтируется как `/kaniko/.docker/config.json`. `runtime-manager` проверяет Secret через metadata-запрос и не записывает значение секрета, token или docker config в БД, события, логи или диагностику. Raw env, OAuth tokens, webhook body, provider response, полный `services.yaml`, diff и произвольная команда не принимаются.

Рабочая нагрузка `services/jobs/agent-runner` является исполняемым образом для этой команды. Production image содержит `/kodex/bin/agent-runner` и Codex CLI по фиксированному пути `/usr/local/bin/codex`; Dockerfile проверяет наличие CLI и команды `codex exec` при сборке. Runner читает только `.kodex/context/agent-run.json` из смонтированного workspace, сверяет `context_digest`, `workspace_fingerprint`, `agent_run_id`, `slot_id`, materialization refs и фиксированный `runner_mode=codex_agent`. Связь с `agent-manager` выполняется через существующие команды `GetAgentRunRuntimeStatus`, `ReportAgentRunState` и `RecordAgentActivity`, если сервисный адрес и токен переданы как конфигурация runner-а. `runtime-manager` передаёт `CodexSessionExecutionSpec` в runner как bounded JSON env с одними refs/digest. Проверенный execution input материализуется отдельно в workspace и читается runner-ом только по refs вида `workspace://.kodex/execution/...`; runner сверяет instruction и result schema digest, передаёт instruction в `codex exec` через stdin, result schema через `--output-schema`, workspace через `--cd` и sandbox из поддержанного фиксированного runner profile. Его содержимое не хранится в БД и не становится частью `agent-run.json`. Если spec отсутствует, неполон, не проходит safe-валидацию, указывает неподдержанный ref/profile или digest не совпадает, runner завершает job безопасной диагностикой `agent_execution_contract_unavailable`; произвольная shell-команда, prompt body из `agent-run.json`, transcript, raw tool input/output, provider payload, kubeconfig, secret values и полный stdout/stderr не принимаются и не логируются. Успешное завершение сохраняет только bounded summary, result digest/schema ref и безопасные refs через `ReportAgentRunState` и activity timeline.

`JobInputJSON` для `health_check` не является произвольным manifest. Поддержаны только ограниченные поля `namespace`, `service_account`, `image` и `labels`; значения `env`, annotations, команды контейнера, значения секретов, prompt, transcript, provider payload и большие тексты не принимаются. Команда контейнера остаётся фиксированной проверкой здоровья. Для `agent_run`, `build` и `deploy` произвольный `job_input_json` также запрещён: исполнимый payload хранится только как соответствующий typed spec, а executor получает фиксированный набор refs/digest/fingerprint. Нагрузки slot-агента и workspace materialization executor не исполняются этим Kubernetes-исполнителем.

Worker Kubernetes использует lease job как границу владения исполнением. Повторный claim после истечения lease переиспользует детерминированный Kubernetes Job по имени и проверяет, что объект действительно создан `runtime-manager` для того же runtime job. Если прямое чтение по имени недоступно, observe делает резервный поиск по runtime labels; неоднозначный или несовпадающий объект считается безопасной ошибкой статуса, а не основанием создать дубль. Остановка процесса или отмена контекста не переводит platform job в `failed`: claim остаётся для повторной сверки после истечения lease. Реальный таймаут Kubernetes Job сохраняется как `timed_out`, условие `JobFailed`, удалённый/отменённый Kubernetes Job или недоступный статус фиксируются классифицированной ошибкой через `FailJob`. Ошибки claim/report/complete сдерживаются повтором с увеличивающейся задержкой, чтобы worker не входил в частый цикл запросов.
