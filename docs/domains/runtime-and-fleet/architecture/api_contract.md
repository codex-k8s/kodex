...
`runtime-manager` сохраняет build/deploy specs как канонические `job_input_json.build_execution_spec` и `job_input_json.deploy_execution_spec`, возвращает их в типизированных полях `Job.build_execution_spec`/`Job.deploy_execution_spec` и не принимает старый свободный payload вместе со spec. `build` или `deploy` без соответствующего spec можно создать и увидеть через `GetJob`/`ListJobs`, но такие задания остаются ожидающими с диагностикой `build_execution_spec_required` или `deploy_execution_spec_required` и не попадают в `ClaimRunnableJob`. Это позволяет producer-контуру сначала создать видимое задание, а затем следующий runtime executor-срез подключить Kaniko build или Kubernetes rollout без смены модели.
