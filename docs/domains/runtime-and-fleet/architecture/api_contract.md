...
Self-deploy orchestration plan в `agent-manager` не вызывает `CreateJob` до owner/governance approval, включая путь `CreateSelfDeployPlanFromSignal` от provider/project safe signal. Он хранит только safe refs, affected service keys, `services.yaml` digest/fingerprint и expected job types `build`, `deploy`, `health_check` как подготовку к owner/governance approval. После `approved` gate `agent-manager` получает build items через `project-catalog.GetSelfDeployBuildPlan` и создаёт только `JOB_TYPE_BUILD` через typed `CreateJob`; если `project-catalog` вернул `build_context_required` или старый совместимый статус неготового контекста, `agent-manager` хранит `runtime_build_status=preparing_context` и не вызывает runtime job. Runtime-owned path подготовки build context фиксируется отдельным lifecycle: `PrepareBuildContext` принимает project/repository/provider/source refs, merge commit SHA, affected service keys или build plan fingerprint; без checked source snapshot artifact запись остаётся `pending` с диагностикой `source_snapshot_unavailable`; trusted materializer/controller передаёт checked source snapshot ref/digest и готовый PVC через `ReportBuildContextProgress`, а `ready` допускается только с `build_context_ref` и `build_context_digest`. Пока ready context refs не переданы в build-plan enrichment, `project-catalog.GetSelfDeployBuildPlan` возвращает `build_context_required` с рецептом сборки и стабильными fingerprints; после передачи ready refs как `materialized_build_contexts` enrichment возвращает ready build items без хранения динамического PVC/digest в `services.yaml`. Deploy job появляется только после successful build и отдельного checked `project-catalog.GetSelfDeployDeployPlan`, который возвращает `DeployExecutionSpec` с manifest bundle refs/digest, rollout targets и expected image refs. Health-check jobs остаются отдельным approval-driven переходом. В build/deploy dispatch не передаются raw webhook body, diff, полный `services.yaml`, raw manifests, секреты или token values.
