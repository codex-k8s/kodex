...
Self-deploy orchestration plan в `agent-manager` не вызывает `CreateJob` до owner/governance approval, включая путь `CreateSelfDeployPlanFromSignal` от provider/project safe signal. Он хранит только safe refs, affected service keys, `services.yaml` digest/fingerprint и expected job types `build`, `deploy`, `health_check` как подготовку к owner/governance approval. После `approved` gate orchestration сначала подготавливает runtime-owned build context через `runtime-manager.PrepareBuildContext`: входом служат project/repository/provider/source refs, merge commit SHA, affected service keys или build plan fingerprint. Если проверенный source snapshot artifact ещё не передан, build context остаётся `pending` с диагностикой `source_snapshot_unavailable` и `next_action=provide_checked_source_snapshot`; это блокер подготовки build context, а не падение Kaniko. Когда trusted materializer/controller получает checked source snapshot ref/digest и готовит PVC, он вызывает `ReportBuildContextProgress` и переводит запись в `ready` только с `build_context_ref` и `build_context_digest`. Следующий orchestration/project-catalog срез должен передать эти ready refs в build-plan enrichment, чтобы `project-catalog.GetSelfDeployBuildPlan` мог вернуть ready build items без хранения динамического PVC/digest в `services.yaml`; до этого build plan остаётся `build_context_unavailable` и `JOB_TYPE_BUILD` не создаётся. В build dispatch не передаются raw webhook body, diff, полный `services.yaml`, секреты или token values.

`JOB_TYPE_BUILD` и `JOB_TYPE_DEPLOY` являются каноническими типами runtime job для будущего саморазвёртывания после provider merge signal, deploy plan и owner/governance approval. Эти типы не выражают произвольную shell-команду или перенос bootstrap deploy tool внутрь `runtime-manager`: сервис хранит lifecycle/status/artifact refs/short log tail, а исполнимый вход принимает только как типизированный spec. Для self-deploy build `BuildExecutionSpec` готовит `project-catalog.GetSelfDeployBuildPlan` из checked `ServicesPolicy`; `agent-manager` только оркестрирует approval и создание job, а `runtime-manager` не читает project policy и не парсит `services.yaml`. `BuildExecutionSpec` содержит проверенный source ref и commit SHA, `service_key` из проверенного инвентаря стека `services.yaml`, target image ref/tag и optional digest, build context ref/digest, Dockerfile ref/digest, фиксированный Dockerfile target, утверждённый builder image ref, build plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`. Если `project-catalog` возвращает `build_context_unavailable`, runtime job не создаётся: создатель плана должен сначала подготовить checked build context PVC/ref и digest через runtime build context lifecycle. `DeployExecutionSpec` содержит проверенный source ref и commit SHA, `service_key`, image ref/tag/digest, manifest ref/digest, kustomization ref/digest, target namespace, fleet-owned target cluster ref, optional slot ref, deploy plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`.

`runtime-manager` сохраняет build/deploy specs как канонические `job_input_json.build_execution_spec` и `job_input_json.deploy_execution_spec`, возвращает их в типизированных полях `Job.build_execution_spec`/`Job.deploy_execution_spec` и не принимает старый свободный payload вместе со spec. `build` без `BuildExecutionSpec` и `deploy` без `DeployExecutionSpec` можно создать и увидеть через `GetJob`/`ListJobs`, но такие задания остаются ожидающими с диагностикой `build_execution_spec_required` или `deploy_execution_spec_required` и не попадают в `ClaimRunnableJob`. `build` с валидным `BuildExecutionSpec` исполняется Kubernetes-исполнителем через Kaniko, если `build_context_ref` указывает на уже подготовленный PVC build context и registry push Secret передан как allowed ref. Перед созданием Kaniko Job executor проверяет, что PVC существует и находится в фазе `Bound`, registry Secret metadata доступна по ref, а указанный service account доступен, если он задан в executor config. Неготовый build context или недоступный registry Secret фиксируются как typed безопасная ошибка до создания Kubernetes Job. `deploy` с валидным `DeployExecutionSpec` фиксируется как типизированное задание для будущего перехода саморазвёртывания после успешного build, остаётся `pending` с диагностикой `deploy_executor_unavailable`, доступен через `GetJob`/`ListJobs` и не выдаётся через `ClaimRunnableJob`, пока отдельный исполнитель выкладки и межсервисный контракт выкладки не согласованы. Runtime не вызывает `kubectl apply`, не читает manifest YAML и не запускает автоматическую выкладку production.
