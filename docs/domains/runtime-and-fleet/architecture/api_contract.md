...
Self-deploy orchestration plan в `agent-manager` не вызывает `CreateJob` до owner/governance approval, включая путь `CreateSelfDeployPlanFromSignal` от provider/project safe signal. Он хранит только safe refs, affected service keys, `services.yaml` digest/fingerprint и expected job types `build`, `deploy`, `health_check` как подготовку к owner/governance approval. После `approved` gate `agent-manager` получает build items через `project-catalog.GetSelfDeployBuildPlan` и создаёт только `JOB_TYPE_BUILD` через typed `CreateJob`; если build context refs/digest ещё не готовы, `agent-manager` хранит `runtime_build_status=preparing_context` и не вызывает runtime job. Runtime-owned path подготовки build context фиксируется отдельным lifecycle: `PrepareBuildContext` принимает project/repository/provider/source refs, merge commit SHA, affected service keys или build plan fingerprint; без checked source snapshot artifact запись остаётся `pending` с диагностикой `source_snapshot_unavailable`; trusted materializer/controller передаёт checked source snapshot ref/digest и готовый PVC через `ReportBuildContextProgress`, а `ready` допускается только с `build_context_ref` и `build_context_digest`. Пока ready context refs не переданы в build-plan enrichment, `project-catalog.GetSelfDeployBuildPlan` возвращает `build_context_unavailable`; после передачи ready refs enrichment возвращает ready build items без хранения динамического PVC/digest в `services.yaml`. Deploy job появляется только после successful build и отдельного checked `project-catalog.GetSelfDeployDeployPlan`, который возвращает `DeployExecutionSpec` с manifest bundle refs/digest, rollout targets и expected image refs. Health-check jobs остаются отдельным approval-driven переходом. В build/deploy dispatch не передаются raw webhook body, diff, полный `services.yaml`, raw manifests, секреты или token values.

`JOB_TYPE_BUILD` и `JOB_TYPE_DEPLOY` являются каноническими типами runtime job для будущего саморазвёртывания после provider merge signal, deploy plan и owner/governance approval. Эти типы не выражают произвольную shell-команду или перенос bootstrap deploy tool внутрь `runtime-manager`: сервис хранит lifecycle/status/artifact refs/short log tail, а исполнимый вход принимает только как типизированный spec. Для self-deploy build `BuildExecutionSpec` готовит `project-catalog.GetSelfDeployBuildPlan` из checked `ServicesPolicy`; `agent-manager` только оркестрирует approval и создание job, а `runtime-manager` не читает project policy и не парсит `services.yaml`. `BuildExecutionSpec` содержит проверенный source ref и commit SHA, `service_key` из проверенного инвентаря стека `services.yaml`, target image ref/tag и optional digest, build context ref/digest, Dockerfile ref/digest, фиксированный Dockerfile target, утверждённый builder image ref, build plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`. Если `project-catalog` возвращает `build_context_unavailable`, runtime job не создаётся: `agent-manager` фиксирует `preparing_context`, а runtime-owned path должен сначала подготовить checked build context PVC/ref и digest. `DeployExecutionSpec` создаётся только из checked deploy plan: он содержит проверенный source ref и commit SHA, `service_key`, image ref/tag/digest, manifest/kustomization refs/digests, обязательные `manifest_bundle_ref` и `manifest_bundle_digest`, rollout targets, expected image refs, target namespace, fleet-owned target cluster ref, optional slot ref, deploy plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`.
