...
Self-deploy orchestration plan в `agent-manager` не вызывает `CreateJob` до owner/governance approval, включая путь `CreateSelfDeployPlanFromSignal` от provider/project safe signal. Он хранит только safe refs, affected service keys, `services.yaml` digest/fingerprint и expected job types `build`, `deploy`, `health_check` как подготовку к owner/governance approval. После `approved` gate `agent-manager` получает рецепт сборки через `project-catalog.GetSelfDeployBuildPlan`; если ответ `build_context_required`, текущая реализация фиксирует blocked build dispatch с recipe/fingerprint и не создаёт `JOB_TYPE_BUILD`. Контракт поддерживает следующий шаг: `runtime-manager` готовит materialized build context и создаёт context ref/digest для конкретного repository/source ref/commit, после чего повторный `GetSelfDeployBuildPlan` с materialized context refs возвращает готовые build items. Deploy и health-check jobs остаются отдельными approval-driven переходами. В build dispatch не передаются raw webhook body, diff, полный `services.yaml`, секреты или token values.

`JOB_TYPE_BUILD` и `JOB_TYPE_DEPLOY` являются каноническими типами runtime job для будущего саморазвёртывания после provider merge signal, deploy plan и owner/governance approval. Эти типы не выражают произвольную shell-команду или перенос bootstrap deploy tool внутрь `runtime-manager`: сервис хранит lifecycle/status/artifact refs/short log tail, а исполнимый вход принимает только как типизированный spec. Для self-deploy build статический рецепт готовит `project-catalog.GetSelfDeployBuildPlan` из checked `ServicesPolicy`; materialized build context готовит `runtime-manager` для конкретного repository/source ref/commit и отдаёт вызывающей стороне refs/digest, которые затем передаются обратно в `project-catalog` для сборки executable `BuildExecutionSpec`. Пока этот handshake не подключён в `agent-manager`, статус `build_context_required` блокирует создание runtime job безопасной диагностикой. `runtime-manager` не читает project policy и не парсит `services.yaml`. `BuildExecutionSpec` содержит проверенный source ref и commit SHA, `service_key` из проверенного инвентаря стека `services.yaml`, target image ref/tag и optional digest, build context ref/digest, Dockerfile ref/digest, фиксированный Dockerfile target, утверждённый builder image ref, build plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`. Если context refs/digest невалидны или не совпадают с recipe fingerprint, `project-catalog` возвращает `build_context_invalid`. `DeployExecutionSpec` содержит проверенный source ref и commit SHA, `service_key`, image ref/tag/digest, manifest ref/digest, kustomization ref/digest, target namespace, fleet-owned target cluster ref, optional slot ref, deploy plan fingerprint, `allowed_secret_refs` без значений и ограниченные `output_refs`.
