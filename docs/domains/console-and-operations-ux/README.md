...
- self-deploy наблюдение: блок командного центра читает `GET /v1/self-deploy/summary`, показывает реальные safe refs/status/digests или честное `unavailable`, без кнопки автоматического deploy;
...
Для self-deploy сценария командный центр показывает отдельный owner-facing блок наблюдения поверх `GET /v1/self-deploy/summary`. Блок отображает, где остановилась цепочка, следующий безопасный шаг, последний safe provider signal ref, связанный repository/project, branch/commit refs, изменённые service keys или path categories, `services.yaml` digest/fingerprint, deploy plan status, governance/owner decision refs/status и safe error/reason summary. Если provider/project/governance/runtime цепочка ещё не дала доменный факт, карточка показывает `unavailable`, `pending` или конкретный `chain_status`, не подменяя состояние демо-данными. Raw webhook body, provider response, diff, полный YAML, tokens, OAuth state/cookies, secrets и автоматический deploy через `web-console` не доступны.
