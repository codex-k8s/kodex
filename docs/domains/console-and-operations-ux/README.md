...
Для self-deploy наблюдения `staff-gateway` отдаёт `GET /v1/self-deploy/summary`. Endpoint сначала вызывает `agent-manager.ListSelfDeployPlans` и возвращает latest visible plan по actor/request context и scope: provider signal ref/status, project/repository refs, source branch/commit refs, affected service keys, path categories, `services.yaml` digest, plan fingerprint, self-deploy plan status, governance gate/release refs и safe summary. Если plan ещё не создан, но caller передал safe project/provider signal refs, gateway уточняет readiness через `project-catalog.GetSelfDeploySignal`; если provider signal ещё не передан, но project известен, gateway проверяет active repository binding через `project-catalog.ListRepositories`. Для pending self-deploy gate gateway дополнительно читает `governance-manager.GetGovernanceSummary` по target `self_deploy_plan` и возвращает `gate_request_id`, `gate_request_version`, `gate_policy_ref` и `allowed_actions`, чтобы UI мог отправить optimistic command без разбора внутренних строковых refs. Ответ всегда содержит owner-facing `chain_status` и `next_step`: `not_configured`, `project_missing`, `repository_binding_missing`, `waiting_for_provider_signal`, `provider_signal_found`, `needs_services_policy_reconcile`, `plan_created`, `governance_gate_pending`, `approved_ready_for_build` или `blocked`. Runtime job refs/status не придумываются: пока доменный контракт не публикует их в self-deploy plan read model, runtime summary остаётся `unavailable` или `pending` с expected runtime job types. Raw webhook body, provider response, diff, полный YAML, tokens, OAuth state/cookies, secret values и автоматический deploy через endpoint не доступны.

Self-deploy решение владельца отправляется через `POST /v1/self-deploy/gates/{gate_request_id}/decision`. Endpoint принимает только `approve`, `reject` или `request_changes`, bounded comment, `idempotency_key`, `expected_version`, expected pending status и safe refs, затем вызывает `governance-manager.SubmitGateDecision`. `approve` снимает governance gate, но не запускает build/deploy напрямую: дальнейшее продвижение self-deploy цепочки остаётся у `agent-manager` и runtime-контуров. Gateway не хранит собственную модель решений и не записывает raw webhook body, diff, provider response, полный YAML, tokens, OAuth state/cookies или secret values.
...
Для self-deploy сценария командный центр показывает отдельный owner-facing блок наблюдения поверх `GET /v1/self-deploy/summary`. Блок отображает, где остановилась цепочка, следующий безопасный шаг, последний safe provider signal ref, связанный repository/project, branch/commit refs, изменённые service keys или path categories, `services.yaml` digest/fingerprint, deploy plan status, governance/owner decision refs/status и safe error/reason summary. Если governance gate находится в pending status и summary содержит `allowed_actions` с `gate_request_id`/`gate_request_version`, владелец может выбрать `approve`, `reject` или `request_changes`; UI отправляет bounded comment и idempotency key через `staff-gateway`, а затем перечитывает сводку. Если provider/project/governance/runtime цепочка ещё не дала доменный факт, карточка показывает `unavailable`, `pending` или конкретный `chain_status`, не подменяя состояние демо-данными. Raw webhook body, provider response, diff, полный YAML, tokens, OAuth state/cookies, secrets и автоматический deploy через `web-console` не доступны.
