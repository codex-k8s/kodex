...
Текущая интерфейсная итерация доводит эти экраны до демонстрируемого состояния без новых backend-ручек: shell адаптируется под узкую ширину, командный центр показывает отдельно работающие зоны и зоны, ожидающие `staff-gateway` endpoint, owner inbox использует master-detail паттерн с безопасными ошибками и ответом только через `allowed_actions`, а экран исполнений явно работает как поиск одного `Run` по safe id.
