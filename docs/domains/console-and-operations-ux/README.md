...
Для self-deploy наблюдения `staff-gateway` отдаёт `GET /v1/self-deploy/summary`. Endpoint является тонким фасадом над `agent-manager.ListSelfDeployPlans`: запрашивает latest visible plan по actor/request context и scope, возвращает provider signal ref/status, project/repository refs, source branch/commit refs, affected service keys, path categories, `services.yaml` digest, plan fingerprint, self-deploy plan status, governance gate/release refs и safe summary. Если self-deploy plan ещё не создан или недоступен для выбранного scope, gateway возвращает успешный ответ с `availability=unavailable` и safe reason, а не демо- или фиктивные данные. Runtime job refs/status не придумываются: пока доменный контракт не публикует их в self-deploy plan read model, runtime summary остаётся `unavailable` или `pending` с expected runtime job types. Raw webhook body, provider response, diff, полный YAML, tokens, OAuth state/cookies, secret values и автоматический deploy через endpoint не доступны.
...
Для self-deploy сценария командный центр показывает отдельный owner-facing блок наблюдения поверх `GET /v1/self-deploy/summary`. Блок отображает последний safe provider signal ref, связанный repository/project, branch/commit refs, изменённые service keys или path categories, `services.yaml` digest/fingerprint, deploy plan status, governance/owner decision refs/status и safe error/reason summary. Если provider/project/governance/runtime цепочка ещё не дала доменный факт, карточка показывает `unavailable` или `pending`, не подменяя состояние демо-данными. Raw webhook body, provider response, diff, полный YAML, tokens, OAuth state/cookies, secrets и автоматический deploy через `web-console` не доступны.
