...
Для self-deploy сценария командный центр показывает отдельный owner-facing блок наблюдения, но не подменяет отсутствующую доменную read-модель демо-данными. Блок закрепляет безопасный состав будущей поверхности: последний GitHub signal, связанный repository/project, branch/commit refs, изменённые service keys или path categories, deploy plan status, governance/owner decision status и safe error/reason summary. Пока `staff-gateway` не имеет согласованного endpoint, связывающего provider webhook/merge signal, project-side deploy plan, governance decision и runtime/deploy status, карточка отображается как ожидающая backend-поверхность. Raw webhook body, provider response, diff, полный YAML, tokens, OAuth state/cookies, secrets и автоматический deploy через `web-console` не доступны.
