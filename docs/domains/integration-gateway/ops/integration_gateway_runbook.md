...
- Симптом: `integration-gateway` не стартует, не проходит readiness, не отдаёт OpenAPI, не отклоняет неподписанный GitHub webhook или возвращает `503` при маршрутизации в `provider-hub`.
- Быстрая диагностика: проверить образ, `Deployment`, `/health/readyz`, `/metrics`, OpenAPI endpoint, runtime secret refs и доступность `provider-hub`.
...
- Доступ к логам `integration-gateway` и `provider-hub`.
...
- выполняет safe negative checks для GitHub webhook route без реального секрета: неподписанный GitHub webhook должен получить `401/signature_invalid`, неподдержанный provider slug — `400/source_not_allowed`.
...
Readiness подтверждает, что HTTP router, OpenAPI validator и route registry собраны. Доступность `provider-hub` проверяется через route smoke и логи `downstream_unavailable`, потому что gateway не хранит собственное состояние и не открывает БД.
...
Нельзя печатать webhook secret, подпись `X-Hub-Signature-256`, provider payload или provider-hub gRPC token.
...
Gateway не создаёт очередь и не хранит retry state. Повтор delivery id проходит в `provider-hub`, где находится webhook inbox и дедупликация.
