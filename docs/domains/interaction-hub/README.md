...
- `interaction-hub` хранит request lifecycle, delivery attempts, callback records и решения, но не владеет package installation, UI или внешним HTTP gateway.
...
|---|---|
| Диалоги, feedback request, approval request, Human gate, уведомления, подписки, delivery attempts, reminders, callback, решения человека, события `interaction.*`. | Flow, `Run`, session, acceptance, provider write pipeline, provider projections, runtime jobs, package catalog/installations, UI state, внешний HTTP gateway, биллинг. |
