...
Первый кодовый PR после этого документационного среза должен делать только контракты: proto, AsyncAPI, события и действия доступа. Сервисный каркас идёт следующим отдельным PR.
...
| IH-1 | не назначено | gRPC и AsyncAPI контракты `interaction-hub`, события `interaction.*`, действия доступа и channel contract DTO готовы; сервисная реализация не входит. |
...
| IH-3 | не назначено | PostgreSQL-модель thread, message, request, decision, notification, subscription, route, delivery attempt, callback, command result и service-local outbox готова. |
| IH-4 | не назначено | Lifecycle feedback, approval и Human gate requests готов: создать, прочитать, решить, отменить, истечь, идемпотентность и события. |
...
| IH-8 | не назначено | Связка с `agent-manager`, `codex-hook-ingress` и `provider-hub` готова для PermissionRequest, owner feedback, approval gate refs и событий решения. |
...
- ключи действий доступа для request, decision, delivery, callback и subscription;
...
|---|---|---|
| `CreateConversationThread` / `RecordConversationMessage` | запланировано | IH-1, IH-3, IH-4 |
| `GetConversationThread` / `ListConversationMessages` | запланировано | IH-1, IH-3, IH-4 |
| `RequestFeedback` | запланировано | IH-1, IH-4 |
| `RequestApproval` | запланировано | IH-1, IH-4 |
| `RequestHumanGate` | запланировано | IH-1, IH-4 |
| `SubmitInteractionDecision` | запланировано | IH-1, IH-4 |
| `CancelInteractionRequest` | запланировано | IH-1, IH-4 |
| `ExpireInteractionRequests` | запланировано | IH-1, IH-4 |
| `GetInteractionRequest` / `ListInteractionRequests` | запланировано | IH-1, IH-3, IH-4 |
| `RequestNotification` | запланировано | IH-1, IH-5 |
| `UpsertSubscription` / `DisableSubscription` / `ListSubscriptions` | запланировано | IH-1, IH-5 |
| `PlanDelivery` | запланировано | IH-1, IH-5 |
| `RecordDeliveryResult` | запланировано | IH-1, IH-5, IH-6 |
| `RecordChannelCallback` | запланировано | IH-1, IH-6 |
| `GetDeliveryStatus` | запланировано | IH-1, IH-5 |

## Синхронизация с параллельными доменами

| Домен или сервис | Когда синхронизироваться | Причина |
|---|---|---|
| `agent-manager` | Перед IH-4 и IH-8 | Нужен общий lifecycle Human gate, feedback request и события решения без владения `Run` в `interaction-hub`. |
...
| `provider-hub` | Перед IH-4 и IH-8 | Approval gate refs нужны provider write pipeline, но provider write остаётся у `provider-hub`. |
