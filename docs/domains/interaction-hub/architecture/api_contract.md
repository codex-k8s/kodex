...
related_issues: [582, 768, 781, 783, 800, 806, 821, 835, 843, 853, 867, 882, 911, 921]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
Owner action для UI не вводит отдельный lifecycle: `staff-gateway` или другой edge вызывает существующий `RecordInteractionResponse` с `request_id`, `response_action`, safe actor/source refs, bounded summary/object ref, `CommandMeta.command_id` или `idempotency_key` и `expected_version` из owner inbox detail. `interaction-hub` фиксирует response lifecycle и публикует safe event, а `agent-manager`, `governance-manager` или другой decision owner применяет business decision у себя.
