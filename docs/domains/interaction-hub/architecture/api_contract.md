...
| `RecordDeliveryResult` | runtime boundary или channel package runtime -> `interaction-hub` | Зафиксировать safe status `accepted`, `delivered`, `failed`, `expired` или совместимый retry/reject result без raw external payload. |
