...
`ListOwnerInboxItems` отдаёт только доменную read-поверхность `interaction-hub` по собственным сущностям. `GetOwnerInboxItem` открывает один item для будущего UI/gateway flow `list -> detail -> safe action`: detail возвращает те же safe refs и summaries, добавляет допустимые действия ответа и текущую `version` request для optimistic concurrency. Это подготовка для `staff-gateway`, UI и `operations-hub`, но не их cross-domain read model.
