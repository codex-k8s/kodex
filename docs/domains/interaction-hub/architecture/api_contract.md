...
| `ListInteractionRequests` | gRPC query | `interaction.request.read` | нет | Список по scope, status, kind, source и deadline. |
| `RequestNotification` | gRPC command | `interaction.notification.request` | `command_id` | Создаёт уведомление или reminder intent. |
...
| `RecordDeliveryResult` | gRPC command | `interaction.delivery.update` | `delivery_id` | Фиксирует ответ channel package на delivery command. |
...
| `GetDeliveryStatus` | gRPC query | `interaction.delivery.read` | нет | Читает состояние request и delivery attempts. |
...
|---|---|---|
| `DeliverInteraction` | `interaction-hub` -> channel package runtime | Передать delivery command для request или notification. |
| `RecordDeliveryResult` | channel package runtime -> `interaction-hub` | Зафиксировать, что пакет принял, отложил или отклонил попытку доставки. |
...
| `request_kind` | Закрытый enum `feedback`, `approval`, `human_gate`, `notification`. |
...
| `runtime-manager` | Будущая техническая доставка command в runtime-нагрузку channel package через согласованный контур. | Jobs и workloads остаются у runtime. |
...
| `interaction.feedback.requested` | request | `request_id`, `scope`, `source_service`, `deadline_at` |
