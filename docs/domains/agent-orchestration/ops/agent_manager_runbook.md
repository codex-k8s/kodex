...
- Если `project-catalog.GetSelfDeploySignal` возвращает `ready` с governance policy key без namespace, consumer сохраняет его как typed ref `governance:gate_policy/<key>`; bare policy key не должен попадать в `SelfDeployPlan` как `governance_gate_policy_ref`.
