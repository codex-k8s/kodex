...
- Если `SelfDeployPlan` уже существует в `pending_approval`, но в нём нет `governance_gate_decision_ref`, стартовая сверка `agent-manager` повторно применяет штатный `EnsureSelfDeployPlanGovernanceGate` для настроенного self-project. Этот путь не меняет checkpoint event-log, не требует ручного SQL и не использует ручной replay/redelivery. После успешного вызова `governance-manager` в plan появляются safe governance refs, а уже resolved owner decision переводит plan в `approved`/`rejected`/`failed` и для `approved` продолжает build dispatch. Если risk assessment и gate request уже созданы для target `self_deploy_plan`, `agent-manager` находит active risk assessment через safe `governance-manager` read API по target/project/fingerprint, при пустом результате повторяет lookup по target без project-фильтра и локально требует совпадения target + evidence fingerprint. Затем он читает gate requests по найденному `risk_assessment_id`, локально сверяет target/status и для `resolved` gate дополнительно читает `ListGateDecisions` по `gate_request_id`; найденный decision ref записывается в plan только если существующие governance refs пустые или совпадают. Такой порядок не требует отдельного target-wide gate read для recovery, не принимает risk с другим fingerprint и не создаёт второй gate. При ошибке в лог попадает только stage code: `plan_list_failed`, `plan_lookup_failed`, `gate_replay_failed`, `gate_prepare_failed`, `existing_risk_lookup_failed`, `existing_risk_access_denied`, `existing_risk_fingerprint_mismatch`, `existing_risk_conflict`, `existing_gate_request_lookup_failed`, `existing_gate_request_access_denied`, `existing_gate_not_found`, `existing_gate_mismatch`, `existing_gate_conflict`, `existing_gate_decision_lookup_failed`, `existing_gate_decision_access_denied`, `existing_gate_decision_not_found`, `existing_gate_decision_mismatch`, `gate_response_invalid` или `plan_governance_refs_update_failed`.
- После owner/governance approval build jobs создаются только если `project-catalog.GetSelfDeployBuildPlan` вернул `ready` для affected service keys и ожидаемой `ServicesPolicy` digest/fingerprint/version. `agent-manager` не парсит `services.yaml`, не подбирает Dockerfile/image refs сам и не передаёт `runtime-manager` значения секретов; non-ready build plan блокирует `JOB_TYPE_BUILD` безопасной причиной. Статус `build_context_required` означает, что image/Dockerfile рецепт уже найден в checked `ServicesPolicy`, но materialized build context PVC/ref и digest ещё не подготовлены; `agent-manager` сохраняет `runtime_build_status=preparing_context`, не создаёт job и ждёт повторной проверки после появления runtime-owned context refs. Если `ready` build plan пришёл без safe context/image/Dockerfile refs или fingerprint, план переходит в `blocked` с `invalid_build_execution_spec`, а runtime job не создаётся. `build_context_invalid` означает, что переданные context refs/digest или fingerprint не совпали с рецептом.
- Если уже `approved` plan с сохранённым `governance_gate_decision_ref` остановился на retryable runtime blocker после исправления доступа или конфигурации, оператор не повторяет owner approval и не трогает checkpoint. Стартовая runtime-сверка `agent-manager` вызывает `EnsureSelfDeployPlanRuntime` для такого plan, сбрасывает только безопасно повторяемый runtime step и снова проходит `PrepareBuildContext`, `JOB_TYPE_BUILD` или `JOB_TYPE_DEPLOY`. Если blocker пришёл от уже созданного terminal runtime job, новый job получает отдельный bounded `runtime_job_attempt_ref`, выведенный из старых job refs/status/error; повтор recovery переиспользует этот attempt и не создаёт дубли. Если сам retry attempt завершился terminal, автоматический recovery не создаёт бесконечную цепочку jobs и оставляет точный runtime blocker для оператора. Retryable считаются `permission_denied`, `dependency_unavailable`, `conflict`, deadline/cancelled и явные config/materializer readiness codes. `failed_precondition`, `invalid_argument`, fingerprint/digest mismatch, unsafe contract mismatch, `policy_stale` и stale checked policy остаются terminal/blocker диагностикой и не ретраятся. В логах для approved-runtime recovery используется отдельный stage code `runtime_reconcile_failed`, а не `gate_replay_failed`.
- Если build или deploy dispatch фиксирует `policy_stale`, это terminal состояние старого approved plan: `status=failed`, `runtime_build_status=blocked` или `runtime_deploy_status=blocked`, safe reason показывает конкретный guard (`services_policy_source_ref_mismatch`, `services_policy_source_commit_mismatch`, `services_policy_digest_mismatch`, `services_policy_fingerprint_mismatch`, `services_policy_version_mismatch`). Оператор не должен повторно approve-ить старый gate, делать checkpoint reset, redelivery или ручной SQL. Нужно обновить checked `services.yaml` projection штатным import/reconcile path и получить новый deploy-relevant provider/project signal; новый signal создаёт новый `SelfDeployPlan` с актуальными digest/fingerprint/version. `project-catalog` считает `main` и `refs/heads/main` одной branch ref только для безопасных имён веток; raw SHA, digest, tag/provider-specific refs и небезопасные refs не склеиваются. Поэтому `services_policy_source_ref_mismatch` после этого означает реальную смену source snapshot, несовместимую ref-форму или некорректный импорт policy, а не обычное отличие `main` от `refs/heads/main`.
