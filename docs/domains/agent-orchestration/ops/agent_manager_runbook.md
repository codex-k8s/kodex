...
- После owner/governance approval `agent-manager` сначала вызывает `runtime-manager.PrepareBuildContext` для affected service keys, source commit SHA и build plan fingerprint. Если runtime возвращает `source_snapshot_unavailable`, нужно передать checked source snapshot ref/digest через согласованный materializer/controller; runtime не делает скрытый checkout. Build jobs создаются только после готового runtime build context и `project-catalog.GetSelfDeployBuildPlan(status=ready)` для ожидаемой `ServicesPolicy` digest/fingerprint/version. `agent-manager` не парсит `services.yaml`, не подбирает Dockerfile/image refs сам и не передаёт `runtime-manager` значения секретов; non-ready build plan блокирует `JOB_TYPE_BUILD` безопасной причиной. Статус `build_context_unavailable` означает, что image/Dockerfile policy уже найдена в checked `ServicesPolicy`, но checked build context PVC/ref и digest ещё не подключены к build-plan enrichment.
