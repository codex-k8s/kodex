...
- Если `SelfDeployPlan` уже существует в `pending_approval`, но `governance_risk_assessment_ref` и `governance_gate_request_ref` пустые, стартовая сверка `agent-manager` повторно применяет штатный `EnsureSelfDeployPlanGovernanceGate` для настроенного self-project. Этот путь не меняет checkpoint event-log и не требует ручного SQL; после успешного вызова `governance-manager` в plan появляются safe governance refs. Если risk assessment и gate request уже созданы для target `self_deploy_plan`, `agent-manager` находит active risk assessment через safe `governance-manager` read API по target/project/fingerprint, при пустом результате повторяет lookup по target без project-фильтра и локально требует совпадения target + evidence fingerprint. Затем он читает gate requests по найденному `risk_assessment_id` и локально сверяет target/status перед записью refs. Такой порядок не требует отдельного target-wide gate read для recovery и не принимает risk с другим fingerprint. При ошибке в лог попадает только stage code: `plan_list_failed`, `plan_lookup_failed`, `gate_replay_failed`, `gate_prepare_failed`, `existing_risk_lookup_failed`, `existing_risk_fingerprint_mismatch`, `existing_risk_conflict`, `existing_gate_lookup_failed`, `existing_gate_not_found`, `existing_gate_mismatch`, `existing_gate_conflict`, `gate_response_invalid` или `plan_governance_refs_update_failed`.
- После owner/governance approval build jobs создаются только если `project-catalog.GetSelfDeployBuildPlan` вернул `ready` для affected service keys и ожидаемой `ServicesPolicy` digest/fingerprint/version. Первый вызов может вернуть `build_context_required`: это значит, что image/Dockerfile рецепт уже найден в checked `ServicesPolicy`, но контекст сборки от `runtime-manager` для конкретного repository/source ref/commit ещё не передан. Текущая реализация `agent-manager` фиксирует такой ответ как blocked build dispatch с recipe/fingerprint и не создаёт `JOB_TYPE_BUILD`; подготовка context и повторный read с `materialized_build_contexts` остаются следующим срезом. `build_context_invalid` означает, что переданные context refs/digest или fingerprint не совпали с рецептом. `agent-manager` не парсит `services.yaml`, не подбирает Dockerfile/image refs сам и не передаёт `runtime-manager` значения секретов; любой статус неготовности build plan блокирует `JOB_TYPE_BUILD` безопасной причиной.
