...
| `CreateSelfDeployPlan` / `CreateSelfDeployPlanFromSignal` / `GetSelfDeployPlan` / `ListSelfDeployPlans` | Слой хранения, use-case, gRPC handlers и outbox event готовы; create фиксирует pending self-deploy plan из safe provider/project refs, affected service keys/path categories, `services.yaml` digest, expected runtime job types `build`/`deploy`/`health_check`, governance refs и fingerprint, поддерживает replay/conflict и не создаёт runtime jobs без approval. Signal-вход требует `provider_signal_ref`, возвращает существующий plan при том же fingerprint и конфликтует при изменённом fingerprint; list требует сужающий фильтр | AGO-23 |
...
| `project-catalog` | Готово для AGO-7/AGO-10/AGO-22 | `agent-manager` читает проверенную workspace policy и использует project/repository refs без владения проектной политикой; для self-deploy plan принимает только safe project/repository refs и `services.yaml` ref/digest, а project-owned projection остаётся у `project-catalog`. Deploy-контур включает project-catalog gRPC secret ref только для runtime preparation. |
