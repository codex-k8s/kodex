...
| `CreateSelfDeployPlan` / `CreateSelfDeployPlanFromSignal` / `GetSelfDeployPlan` / `ListSelfDeployPlans` | Слой хранения, use-case, gRPC handlers, outbox event и consumer для self-deploy signal готовы; create фиксирует self-deploy plan из safe provider/project refs, affected service keys/path categories, `services.yaml` digest, expected runtime job types `build`/`deploy`/`health_check`, governance refs и fingerprint, поддерживает replay/conflict и не создаёт runtime jobs без approval. Signal-вход требует `provider_signal_ref`, возвращает существующий plan при том же fingerprint и конфликтует при изменённом fingerprint. Consumer читает `provider.repository.changed`, вызывает `project-catalog.GetSelfDeploySignal` и передаёт в command только `ready` input. После ready-плана `agent-manager` вызывает `governance-manager.PrepareSelfDeployPlanGate`, сохраняет safe gate refs/status/version и не создаёт второй gate request при replay. После `approved` gate `agent-manager` вызывает `project-catalog.GetSelfDeployBuildPlan`, создаёт `JOB_TYPE_BUILD` только по ready build items, сохраняет runtime build job refs/status/fingerprint и блокирует dispatch безопасной диагностикой при non-ready build plan; list требует сужающий фильтр | AGO-23, AGO-24, AGO-25, AGO-26 |
...
| `project-catalog` | Готово для AGO-7/AGO-10/AGO-22/AGO-24/AGO-26 | `agent-manager` читает проверенную workspace policy и использует project/repository refs без владения проектной политикой. Для self-deploy signal wiring он вызывает `GetSelfDeploySignal` по provider signal ref и получает только project-owned safe input: active repository binding, checked `ServicesPolicy`, `services.yaml` ref/digest/version, affected service keys, path categories, expected runtime job types и governance refs или non-ready status. Для approved self-deploy build dispatch он вызывает `GetSelfDeployBuildPlan` и получает ready per-service `BuildExecutionSpec` из checked `ServicesPolicy`; non-ready build plan блокирует runtime dispatch. Deploy-контур включает project-catalog gRPC secret ref для runtime preparation и self-deploy signal/build enrichment. |
