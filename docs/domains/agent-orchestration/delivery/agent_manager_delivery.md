...
| AGO-17 | #968 | Runtime job dispatch заполняет typed `AgentRunExecutionSpec`: `agent-manager` передаёт в `CreateJob` safe refs на run/slot/materialization/workspace/context, digest/fingerprint, runner profile/image, фиксированный runner mode, optional secret refs без значений и reporting targets только после `slot_status=ready` и `workspace_materialization_status=completed`; pending materialization переводит `Run` в waiting, а replay создаёт job после готовности. |
