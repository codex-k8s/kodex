...
| AGO-8 | #809 | Базовая machine acceptance готова: `agent-manager` создаёт pending acceptance result по session/run/stage, записывает `passed`/`failed`/`waiting`/`skipped` через ожидаемую версию, хранит только safe refs/status/bounded details и публикует service-local outbox events без QA runner, Human gate, provider write pipeline и хранения raw payload. |
