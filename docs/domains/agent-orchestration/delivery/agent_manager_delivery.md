...
related_issues: [733, 739, 744, 749, 755, 759, 772, 322, 782, 795, 809, 820, 834, 842, 862, 866, 891, 897, 905, 918, 937, 946, 954, 968, 984, 994, 999, 1011, 1015]
related_prs: []
related_docsets:
...
| `CreateSelfDeployPlan` / `CreateSelfDeployPlanFromSignal` / `GetSelfDeployPlan` / `ListSelfDeployPlans` | Слой хранения, use-case, gRPC handlers, outbox event и consumer для self-deploy signal готовы; create фиксирует pending self-deploy plan из safe provider/project refs, affected service keys/path categories, `services.yaml` digest, expected runtime job types `build`/`deploy`/`health_check`, governance refs и fingerprint, поддерживает replay/conflict и не создаёт runtime jobs без approval. Signal-вход требует `provider_signal_ref`, возвращает существующий plan при том же fingerprint и конфликтует при изменённом fingerprint. Consumer читает `provider.repository.changed`, вызывает `project-catalog.GetSelfDeploySignal` и передаёт в command только `ready` input; list требует сужающий фильтр | AGO-23, AGO-24 |
...
| `risk-and-release-governance` | Для governance-backed gates и policy evidence | `agent-manager` хранит typed governance refs и normalized outcome; governance/risk/release decision body, release evidence и policy engine остаются у `governance-manager`. |
