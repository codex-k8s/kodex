...
| `project-catalog` | Готово для AGO-7/AGO-10/AGO-22/AGO-24 | `agent-manager` читает проверенную workspace policy и использует project/repository refs без владения проектной политикой. Для self-deploy signal wiring он вызывает `GetSelfDeploySignal` по provider signal ref и получает только project-owned safe input: active repository binding, checked `ServicesPolicy`, `services.yaml` ref/digest/version, affected service keys, path categories, expected runtime job types и governance refs или non-ready status. Deploy-контур включает project-catalog gRPC secret ref для runtime preparation и self-deploy signal enrichment. |
