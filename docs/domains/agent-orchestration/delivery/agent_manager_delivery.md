...
related_issues: [733, 739, 744, 749, 755, 759, 772, 322, 782, 795, 809, 820, 834, 842, 862, 866, 891, 897, 905, 918, 937, 946, 954, 968, 984, 994]
related_prs: []
related_docsets:
...
| `ReportAgentRunState` | Слой хранения, use-case и gRPC handler готовы; команда принимает только typed runner states `queued`/`running`/`completed`/`failed`, требует expected version, сверяет run/session/slot/job refs с сохранённым `Run`, поддерживает replay/conflict и сохраняет только bounded summary, diagnostic digest и failure code без raw runner payload | AGO-18 |
