...
related_issues: [733, 739, 744, 749, 755, 759, 772, 322, 782, 795, 809, 820, 834]
related_prs: []
related_docsets:
...
| AGO-9c | не назначено | Интеграция follow-up intent с типизированными provider-командами `provider-hub` для создания или обновления `Issue` без прямого GitHub/GitLab доступа из `agent-manager`. |
...
| `provider-hub` | Перед AGO-9c | AGO-8 хранит acceptance refs/status, AGO-9a фиксирует только follow-up intent, AGO-9b добавляет activity timeline. Для реального создания provider-native `Issue` нужны типизированные provider-операции и безопасный provider write pipeline. |
