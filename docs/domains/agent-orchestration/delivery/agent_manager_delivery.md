...
related_issues: [733, 739, 744, 749, 755, 759, 772, 322, 782, 795, 809, 820, 834, 842, 862, 866, 891, 897, 905, 918, 937, 946]
related_prs: []
related_docsets:
...
| `runtime-manager` | Готово для AGO-7/AGO-10/AGO-14 | `agent-manager` вызывает `PrepareRuntime(agent_run_id, workspace policy, runtime profile, placement constraints)` при включённой runtime preparation и `CreateJob(job_type=JOB_TYPE_AGENT_RUN)` при включённом runtime job dispatch; deploy-контур включает явные env switch и gRPC secret ref, а workspace, job state, Kubernetes и executor остаются у runtime. |
