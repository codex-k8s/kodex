...
related_issues: [733, 739, 744, 749, 755, 759, 772, 322, 782, 795, 809, 820, 834, 842, 862, 866, 891, 897, 905, 918, 937, 946, 954, 968, 984, 994, 999, 1011]
related_prs: []
related_docsets:
...
| `CreateSelfDeployPlan` / `GetSelfDeployPlan` / `ListSelfDeployPlans` | Слой хранения, use-case, gRPC handlers и outbox event готовы; create фиксирует pending self-deploy plan из safe provider/project refs, affected service keys/path categories, `services.yaml` digest, expected runtime job types `build`/`deploy`/`health_check`, governance refs и fingerprint, поддерживает replay/conflict и не создаёт runtime jobs без approval; list требует сужающий фильтр | AGO-22 |
