...
| `DispatchFollowUpIntent` | Слой хранения, use-case, typed provider-hub client и gRPC handler готовы; команда требует expected version, поддерживает idempotency replay/conflict, резервирует dispatch до provider write, вызывает `provider-hub.CreateIssue`, обновляет intent до `created` или `failed` и хранит только safe operation/result refs | AGO-9c |
