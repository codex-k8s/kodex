...
| AGO-17 | #968 | Runtime job dispatch заполняет typed `AgentRunExecutionSpec`: после `PrepareRuntime` `agent-manager` передаёт в `CreateJob` safe refs на run/slot/materialization/workspace/context, digest/fingerprint, runner profile/image, фиксированный runner mode, optional secret refs без значений и reporting targets; неполный spec безопасно отклоняется без создания runtime job. |
