...
| `CreateSelfDeployPlanFromSignal` | gRPC command | `agent.self_deploy.plan` | `provider_signal_ref` + fingerprint | Создаёт тот же pending plan из safe provider/project signal. `provider_signal_ref` обязателен и является signal-level ключом: повтор того же signal с тем же fingerprint возвращает существующий plan даже при новом `command_id`, а другой fingerprint по тому же signal ref получает conflict. |
