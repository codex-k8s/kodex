...
| `runtime-manager` | `PrepareRuntime`, `CreateJob(job_type=JOB_TYPE_AGENT_RUN)` | Состояние runtime остаётся у runtime. `agent-manager` передаёт `WorkspaceSource.kind=guidance_package` для замороженных `guidance_refs` и `WorkspaceSource.kind=generated_context` для `.kodex/context/agent-run.json`; checkout, materialization, job state и будущее исполнение выполняет runtime. |
...
| Интеграция с runtime | Реализованы прямые вызовы `PrepareRuntime` и opt-in `CreateJob(job_type=JOB_TYPE_AGENT_RUN)` для старта `AgentRun`; executor, Kubernetes-размещение и выполнение задания не входят в `agent-manager`. |
