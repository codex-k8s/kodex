...
| `project-catalog` | Чтение workspace policy, release policy, project/repository refs, `GetSelfDeploySignal`, `GetSelfDeployBuildPlan` | Проектная policy остаётся у project. Для self-deploy `agent-manager` использует `GetSelfDeploySignal` для создания plan из provider-owned trigger, а после approved gate использует `GetSelfDeployBuildPlan` для получения safe per-service `BuildExecutionSpec` из checked `ServicesPolicy`. |
