...
| `agent.self_deploy.plan_requested` | Зафиксирован или обновлён safe self-deploy plan после typed input, safe provider/project signal или подготовки governance gate; payload содержит только plan id, project/repository/source refs, affected service keys, path categories, expected runtime job types, governance refs/status и fingerprint без raw webhook/diff/YAML. |
