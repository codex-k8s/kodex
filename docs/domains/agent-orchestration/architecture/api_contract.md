...
| `CreateSelfDeployPlan` | gRPC command | `agent.self_deploy.plan` | `command_id` или `idempotency_key` | Фиксирует pending orchestration plan для self-deploy по typed plan input. Команда сохраняет project/repository refs, provider signal/source/merge commit refs, affected service keys, path categories, `services.yaml` ref/digest, ожидаемые runtime job types `build`/`deploy`/`health_check`, typed governance refs, safe summary и fingerprint. После сохранения ready-плана при включённом `KODEX_AGENT_MANAGER_SELF_DEPLOY_GOVERNANCE_GATE_ENABLED` `agent-manager` вызывает `governance-manager.PrepareSelfDeployPlanGate`, передаёт только safe поля плана, сохраняет `risk_assessment_ref`, `gate_request_ref`/`gate_decision_ref` и статус `pending_approval`/`approved`/`rejected`/`failed`. Если gate уже `approved` и включён `KODEX_AGENT_MANAGER_SELF_DEPLOY_BUILD_DISPATCH_ENABLED`, команда вызывает `project-catalog.GetSelfDeployBuildPlan`: `build_context_required` сохраняется как `runtime_build_status=preparing_context`, остальные non-ready статусы становятся safe `blocked`, а `JOB_TYPE_BUILD` через `runtime-manager.CreateJob` ставится только по ready build items с валидным typed `BuildExecutionSpec`. Raw webhook body, provider response, diff, полный `services.yaml`, prompt/transcript, секреты и токены не принимаются. |
...
| `runtime-manager` | `PrepareRuntime`, `CreateJob(job_type=JOB_TYPE_AGENT_RUN)`, `CreateJob(job_type=JOB_TYPE_BUILD)`, `GetJob` | Состояние runtime остаётся у runtime. `agent-manager` передаёт `WorkspaceSource.kind=guidance_package` для замороженных `guidance_refs` и `WorkspaceSource.kind=generated_context` для `.kodex/context/agent-run.json`; при включённом dispatch собирает typed `AgentRunExecutionSpec` из safe refs/digest/fingerprint и передаёт его в `CreateJob` без raw payload только после `slot_status=ready` и `workspace_materialization_status=completed`. Вложенный `CodexSessionExecutionSpec` строится из safe refs: instruction object ref/digest из `PromptTemplateVersion.TemplateObject`, result schema ref/digest из конфигурации `KODEX_AGENT_MANAGER_CODEX_SESSION_RESULT_SCHEMA_REF` и `KODEX_AGENT_MANAGER_CODEX_SESSION_RESULT_SCHEMA_DIGEST`, session/workspace snapshot ref, hook/callback refs, `KODEX_AGENT_MANAGER_CODEX_SESSION_TIMEOUT`, фиксированный runner profile, output/result refs и secret refs без значений. Если обязательные refs/digest не готовы или не проходят безопасную валидацию, `agent-manager` фиксирует состояние waiting/diagnostic и не вызывает `CreateJob` с неполным input. Для self-deploy build `agent-manager` после approved gate вызывает `CreateJob(JOB_TYPE_BUILD)` только с typed `BuildExecutionSpec`, полученным от `project-catalog.GetSelfDeployBuildPlan(status=ready)` и прошедшим локальную проверку safe refs/digests; при `build_context_unavailable` plan остаётся в `preparing_context`, а `job_input_json` остаётся пустым объектом и deploy jobs не создаются. Checkout, materialization, job state и исполнение выполняет runtime; текущий `agent-runner` исполняет только workspace refs `workspace://.kodex/execution/...` и не строит prompt fallback. Для отчёта runner-а `runtime-manager` добавляет в Kubernetes Job адрес `agent-manager` и ссылку `SecretKeyRef` на gRPC token; `agent-manager` не передаёт значение token в spec и не хранит его в `Run`. Поверхность чтения `GetAgentRunRuntimeStatus` читает актуальное состояние job только через `runtime-manager.GetJob` и возвращает наружу bounded safe поля. |
...
| `project-catalog` | Чтение workspace policy, release policy, project/repository refs, `GetSelfDeploySignal`, `GetSelfDeployBuildPlan` | Проектная policy остаётся у project. Для self-deploy `agent-manager` использует `GetSelfDeploySignal` для создания plan из provider-owned trigger, а после approved gate использует `GetSelfDeployBuildPlan` для получения safe per-service build recipe из checked `ServicesPolicy`. Готовый `BuildExecutionSpec` возвращается только после повторного read с materialized context refs/digests от `runtime-manager`; пока этот runtime context preparation не подключён в `agent-manager`, статус `build_context_required` блокирует dispatch безопасной диагностикой. |
