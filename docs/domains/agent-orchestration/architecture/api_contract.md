...
| `project-catalog` | Чтение workspace policy, release policy, project/repository refs, `GetSelfDeploySignal`, `GetSelfDeployBuildPlan` | Проектная policy остаётся у project. Для self-deploy `agent-manager` использует `GetSelfDeploySignal` для создания plan из provider-owned trigger, а после approved gate вызывает `GetSelfDeployBuildPlan` только после build-plan enrichment с готовыми runtime context refs; `project-catalog` возвращает safe per-service `BuildExecutionSpec` из checked `ServicesPolicy`. |
