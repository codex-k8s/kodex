...
related_issues: [733, 739, 744, 753, 755, 698, 759, 772, 322, 782, 795, 809, 820, 834, 842, 862, 866, 891, 897, 905, 918, 937, 954, 968, 984, 994, 999, 1011, 1015, 1022, 1027]
related_prs: []
approvals:
  required: ["Owner"]
  status: approved
...
| `CreateSelfDeployPlan` | gRPC command | `agent.self_deploy.plan` | `command_id` или `idempotency_key` | Фиксирует pending orchestration plan для self-deploy по typed plan input. Команда сохраняет project/repository/provider refs, provider signal/source/merge commit refs, affected service keys, path categories, `services.yaml` ref/digest, ожидаемые runtime job types `build`/`deploy`/`health_check`, typed governance refs, safe summary и fingerprint. После сохранения ready-плана при включённом `KODEX_AGENT_MANAGER_SELF_DEPLOY_GOVERNANCE_GATE_ENABLED` `agent-manager` вызывает `governance-manager.PrepareSelfDeployPlanGate`, передаёт только safe поля плана, сохраняет `risk_assessment_ref`, `gate_request_ref`/`gate_decision_ref` и статус `pending_approval`/`approved`/`rejected`/`failed`. Если gate уже `approved` и включён `KODEX_AGENT_MANAGER_SELF_DEPLOY_BUILD_DISPATCH_ENABLED`, команда вызывает `project-catalog.GetSelfDeployBuildPlan`: `build_context_required` запускает или переиспользует `runtime-manager.PrepareBuildContext` и сохраняет `runtime_build_status=preparing_context`, остальные non-ready статусы становятся safe `blocked`, а `JOB_TYPE_BUILD` через `runtime-manager.CreateJob` ставится только по ready build items с валидным typed `BuildExecutionSpec`. После successful build `agent-manager` вызывает `project-catalog.GetSelfDeployDeployPlan` и ставит `JOB_TYPE_DEPLOY` только по checked `DeployExecutionSpec`; health-check остаётся отдельным типизированным переходом. Raw webhook body, provider response, diff, полный `services.yaml`, prompt/transcript, секреты и токены не принимаются. |
| `CreateSelfDeployPlanFromSignal` | gRPC command | `agent.self_deploy.plan` | `provider_signal_ref` + fingerprint | Создаёт тот же pending plan из safe project-side signal input. `provider_signal_ref` обязателен и является signal-level ключом: повтор того же signal с тем же fingerprint возвращает существующий plan даже при новом `command_id`, а другой fingerprint по тому же signal ref получает conflict. Встроенный consumer получает `provider.repository.changed` как trigger, вызывает `project-catalog.GetSelfDeploySignal` и передаёт сюда только `ready` input с проверенными `services.yaml` ref/digest и affected service keys; non-ready статусы не создают plan. Повтор после уже подготовленного governance gate переиспользует сохранённые refs и не создаёт второй gate request; повтор после approved gate и созданного build job возвращает сохранённые runtime job refs без второго `CreateJob`. |
| `GetSelfDeployPlan` | gRPC query | `agent.self_deploy.read` | нет | Читает один safe self-deploy plan по id, включая governance refs/status и runtime build refs/status/error summary без runtime logs или build input body. |
| `ListSelfDeployPlans` | gRPC query | `agent.self_deploy.read` | `page_token` | Читает self-deploy plans по scope/project/repository/provider signal/status с bounded pagination; широкий список без ограничителя недопустим. Ответ содержит только safe plan/gate/build summaries и refs. |
...
| `agent.self_deploy.plan` | Создать pending self-deploy plan из typed input или safe provider/project signal, подготовить governance gate и после approved gate поставить build jobs по ready build plan. |
...
| `runtime-manager` | `PrepareRuntime`, `PrepareBuildContext`, `GetBuildContext`, `CreateJob(job_type=JOB_TYPE_AGENT_RUN)`, `CreateJob(job_type=JOB_TYPE_BUILD)`, `CreateJob(job_type=JOB_TYPE_DEPLOY)`, `GetJob` | Состояние runtime остаётся у runtime. `agent-manager` передаёт `WorkspaceSource.kind=guidance_package` для замороженных `guidance_refs` и `WorkspaceSource.kind=generated_context` для `.kodex/context/agent-run.json`; при включённом dispatch собирает typed `AgentRunExecutionSpec` из safe refs/digest/fingerprint и передаёт его в `CreateJob` без raw payload только после `slot_status=ready` и `workspace_materialization_status=completed`. Вложенный `CodexSessionExecutionSpec` строится из safe refs: instruction object ref/digest из `PromptTemplateVersion.TemplateObject`, result schema ref/digest из конфигурации `KODEX_AGENT_MANAGER_CODEX_SESSION_RESULT_SCHEMA_REF` и `KODEX_AGENT_MANAGER_CODEX_SESSION_RESULT_SCHEMA_DIGEST`, session/workspace snapshot ref, hook/callback refs, `KODEX_AGENT_MANAGER_CODEX_SESSION_TIMEOUT`, фиксированный runner profile, output/result refs и secret refs без значений. Instruction и result schema refs считаются готовыми для `CreateJob` только в форме `workspace://.kodex/execution/...`; безопасные `object://` refs означают, что checked execution input ещё не материализован в workspace, поэтому `agent-manager` фиксирует `execution_input_unavailable` и не создаёт runtime job. Если обязательные refs/digest не готовы или не проходят безопасную валидацию, `agent-manager` фиксирует состояние waiting/diagnostic и не вызывает `CreateJob` с неполным input. Для self-deploy `agent-manager` после approved gate сначала вызывает `PrepareBuildContext` с provider/repository/source/commit refs и сохраняет только runtime build context refs/status/digest; готовый context передаётся в `project-catalog.GetSelfDeployBuildPlan`, а `CreateJob(JOB_TYPE_BUILD)` вызывается только с проверенным typed `BuildExecutionSpec`. После successful build `agent-manager` читает `GetSelfDeployDeployPlan` и вызывает `CreateJob(JOB_TYPE_DEPLOY)` только с checked `DeployExecutionSpec`; job state, Kubernetes materialization, Kaniko build, server-side apply и rollout watch остаются у runtime. Для отчёта runner-а `runtime-manager` добавляет в Kubernetes Job адрес `agent-manager` и ссылку `SecretKeyRef` на gRPC token; `agent-manager` не передаёт значение token в spec и не хранит его в `Run`. Поверхность чтения `GetAgentRunRuntimeStatus` читает актуальное состояние job только через `runtime-manager.GetJob` и возвращает наружу bounded safe поля. |
...
| `project-catalog` | Чтение workspace policy, release policy, project/repository refs, `GetSelfDeploySignal`, `GetSelfDeployBuildPlan`, `GetSelfDeployDeployPlan` | Проектная policy остаётся у project. Для self-deploy `agent-manager` использует `GetSelfDeploySignal` для создания plan из provider-owned trigger, после approved gate использует `GetSelfDeployBuildPlan` для получения safe per-service build recipe из checked `ServicesPolicy`, а после successful build использует `GetSelfDeployDeployPlan` для checked deploy specs. Готовый `BuildExecutionSpec` возвращается только после повторного read с materialized context refs/digests от `runtime-manager`; готовый `DeployExecutionSpec` возвращается только по safe build outputs и checked manifest bundle refs/digests. |
| `governance-manager` | `PrepareSelfDeployPlanGate`, событие `governance.gate.resolved`, risk assessment, record review signal, request gate, read gate/release decision | Risk/gate/release decisions остаются у governance. Для self-deploy `agent-manager` после ready-плана вызывает `PrepareSelfDeployPlanGate` с `self_deploy_plan_ref`, project/repository/source refs, commit, `services.yaml` ref/digest, service keys, path categories, expected runtime job types, safe summary и fingerprint; локально сохраняет только `risk_assessment_ref`, `gate_request_ref`, `gate_decision_ref`, status/version и не копирует decision/evidence body. Consumer `governance.gate.resolved` обрабатывает только target `self_deploy_plan` с ref `agent:self-deploy-plan:<uuid>`, сверяет `gate_request_ref`, записывает normalized outcome и запускает build dispatch только для `approve`/`approve_with_conditions`; `reject` и `request_changes`/`revise` завершают plan без build. |
...
| `agent.self_deploy.plan_requested` | Зафиксирован или обновлён safe self-deploy plan после typed input, safe provider/project signal, подготовки governance gate или записи governance decision; payload содержит только plan id, project/repository/source refs, affected service keys, path categories, expected runtime job types, governance refs/status и fingerprint без raw webhook/diff/YAML. |
