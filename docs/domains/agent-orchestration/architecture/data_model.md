...
Для self-deploy `SelfDeployPlan` хранит только safe project/provider refs, affected service keys, `services.yaml` digest/fingerprint/version, expected runtime job type hints и governance refs. После approval `agent-manager` сначала подготавливает runtime-owned build context через `runtime-manager.PrepareBuildContext`; без checked source snapshot ref/digest этот lifecycle остаётся `pending` с `source_snapshot_unavailable`, и `JOB_TYPE_BUILD` не создаётся. После готовности build context отдельный build-plan enrichment передаёт `build_context_ref`/`build_context_digest` в `project-catalog.GetSelfDeployBuildPlan`: `project-catalog` связывает affected service keys с checked `ServicesPolicy` и возвращает `BuildExecutionSpec`-совместимые refs для `runtime-manager`. Если project-side build plan возвращает non-ready статус, включая `build_context_unavailable`, `agent-manager` фиксирует build dispatch как blocked и не создаёт `JOB_TYPE_BUILD`. `agent-manager` не читает raw `services.yaml`, не вычисляет Dockerfile/image refs по путям и не хранит значения секретов.
...
`SelfDeployPlan` — авторитетное состояние `agent-manager` для self-deploy orchestration после safe provider/project signal или typed plan input. Он описывает, какие сервисы и категории путей затронуты после merge/push в `main`, какие runtime job types ожидаются после approval, и какие governance refs нужны для решения владельца. После ready-плана `agent-manager` подготавливает governance gate через `governance-manager.PrepareSelfDeployPlanGate`; затем потребляет `governance.gate.resolved` только для target `self_deploy_plan`, сверяет `gate_request_ref`, сохраняет `gate_decision_ref` и переводит plan дальше. Только `approve` и `approve_with_conditions` разрешают build dispatch; `reject`, `request_changes`/`revise`, `hold`, `rollback` и `escalate` закрывают путь без runtime jobs. После `approved` gate он сначала получает или ждёт готовый runtime build context, затем через build-plan enrichment получает `BuildExecutionSpec`-совместимые refs у `project-catalog.GetSelfDeployBuildPlan` и только после этого создаёт `JOB_TYPE_BUILD` в `runtime-manager`. Проектная, provider-native, runtime и governance истина остаются у сервисов-владельцев.
...
| `runtime_build_status` | enum | нет | `not_requested`, `blocked`, `requested`, `failed`. `requested` означает, что build jobs уже поставлены или переиспользованы; `blocked` фиксирует non-ready build plan или отсутствие approved gate. |
...
`SelfDeployPlan` запрещает хранение raw webhook body, provider response, полного diff, полного `services.yaml`, prompt, transcript, секретов, токенов, kubeconfig, workspace paths и больших логов. Build jobs создаются только после `approved` governance status/decision ref, готового runtime build context и `project-catalog.GetSelfDeployBuildPlan(status=ready)`: `agent-manager` передаёт `runtime-manager.CreateJob(JOB_TYPE_BUILD)` typed `BuildExecutionSpec` из project-owned build plan, а `job_input_json` остаётся пустым объектом. Deploy и health-check jobs не создаются автоматически и остаются отдельными approval-driven срезами.
