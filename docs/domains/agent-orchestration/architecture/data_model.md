...
`SelfDeployPlan` — авторитетное состояние `agent-manager` для self-deploy orchestration после safe provider/project signal или typed plan input. Он описывает, какие сервисы и категории путей затронуты после merge/push в `main`, какие runtime job types ожидаются после approval, и какие governance refs нужны для решения владельца. После ready-плана `agent-manager` подготавливает governance gate через `governance-manager.PrepareSelfDeployPlanGate`, но не создаёт runtime jobs и не переносит в `agent-manager` проектную, provider-native или governance истину.
