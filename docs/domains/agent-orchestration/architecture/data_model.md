...
Для self-deploy `SelfDeployPlan` хранит только safe project/provider refs, affected service keys, `services.yaml` digest/fingerprint/version, expected runtime job type hints и governance refs. После approval `agent-manager` получает build-вход через `project-catalog.GetSelfDeployBuildPlan`: `project-catalog` связывает affected service keys с checked `ServicesPolicy` и возвращает `BuildExecutionSpec`-совместимые refs для `runtime-manager`. Если project-side build plan возвращает `build_context_unavailable`, `agent-manager` фиксирует build dispatch как `preparing_context` и не создаёт `JOB_TYPE_BUILD` до появления runtime-owned build context refs/digest; остальные non-ready статусы становятся safe `blocked` diagnostic. `agent-manager` не читает raw `services.yaml`, не вычисляет Dockerfile/image refs по путям и не хранит значения секретов.
...
`SelfDeployPlan` — авторитетное состояние `agent-manager` для self-deploy orchestration после safe provider/project signal или typed plan input. Он описывает, какие сервисы и категории путей затронуты после merge/push в `main`, какие runtime job types ожидаются после approval, и какие governance refs нужны для решения владельца. После ready-плана `agent-manager` подготавливает governance gate через `governance-manager.PrepareSelfDeployPlanGate`; затем потребляет `governance.gate.resolved` только для target `self_deploy_plan`, сверяет `gate_request_ref`, сохраняет `gate_decision_ref` и переводит plan дальше. Только `approve` и `approve_with_conditions` разрешают build dispatch; `reject`, `request_changes`/`revise`, `hold`, `rollback` и `escalate` закрывают путь без runtime jobs. После `approved` gate он получает build plan через `project-catalog.GetSelfDeployBuildPlan` и создаёт только `JOB_TYPE_BUILD` в `runtime-manager`. Deploy dispatch требует successful build и будущий `project-catalog.GetSelfDeployDeployPlan`, который возвращает checked `DeployExecutionSpec` с manifest bundle refs/digest, rollout targets и expected image refs. Проектная, provider-native, runtime и governance истина остаются у сервисов-владельцев.
...
| `runtime_build_status` | enum | нет | `not_requested`, `preparing_context`, `blocked`, `requested`, `failed`. `preparing_context` означает approved plan, для которого policy уже проверена, но runtime-owned build context refs/digest ещё не готовы; `requested` означает, что build jobs уже поставлены или переиспользованы; `blocked` фиксирует policy conflict, invalid ready build spec, отсутствие approved gate или иной non-ready blocker. |
...
`SelfDeployPlan` запрещает хранение raw webhook body, provider response, полного diff, полного `services.yaml`, prompt, transcript, секретов, токенов, kubeconfig, workspace paths и больших логов. Build jobs создаются только после `approved` governance status/decision ref и `project-catalog.GetSelfDeployBuildPlan(status=ready)`: `agent-manager` передаёт `runtime-manager.CreateJob(JOB_TYPE_BUILD)` typed `BuildExecutionSpec` из project-owned build plan, а `job_input_json` остаётся пустым объектом. Если `project-catalog` возвращает `build_context_unavailable`, план остаётся в `preparing_context` до повторной проверки после подготовки runtime-owned context refs/digest. Deploy jobs не создаются до successful build и checked `GetSelfDeployDeployPlan(status=ready)`; `runtime-manager` принимает только typed `DeployExecutionSpec` и не читает raw YAML, manifests из Git provider, kubeconfig или значения секретов. Health-check jobs остаются отдельным approval-driven срезом.
