...
`guidance_refs` не является manifest cache. В этом поле нельзя хранить `payload_json`, `SKILL.md`, prompt templates, flow files, scripts, assets или package source. Локальные пути workspace также не являются частью модели `AgentRun`: они вычисляются при подготовке runtime policy и фиксируются в `runtime-manager` как `WorkspaceSource.local_path`.
