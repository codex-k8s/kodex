...
Для self-deploy `SelfDeployPlan` хранит только safe project/provider refs, affected service keys, `services.yaml` digest/fingerprint/version, expected runtime job type hints и governance refs. После approval `agent-manager` получает build-вход через `project-catalog.GetSelfDeployBuildPlan`: `project-catalog` связывает affected service keys с checked `ServicesPolicy` и сначала возвращает статический рецепт сборки. Текущая реализация `agent-manager` не готовит materialized build context сама: статус `build_context_required` сохраняется как blocked build dispatch с recipe/fingerprint для следующего среза подготовки context. Только `ready` ответ содержит `BuildExecutionSpec`-совместимые refs для `runtime-manager.CreateJob`. Если project-side build plan возвращает статус неготовности, включая `build_context_required` или `build_context_invalid`, `agent-manager` фиксирует build dispatch как blocked и не создаёт `JOB_TYPE_BUILD`. `agent-manager` не читает raw `services.yaml`, не вычисляет Dockerfile/image refs по путям и не хранит значения секретов.
