...
| `runtime_build_jobs` | jsonb | нет | Ограниченный массив safe refs созданных build jobs: `service_key`, optional `service_ref`, `runtime_job_ref`, optional runtime job status, optional `runtime_job_attempt_ref` для bounded retry terminal job и fingerprint build plan item. `job_input_json`, Kubernetes refs, логи и значения секретов не хранятся. |
...
| `runtime_deploy_jobs` | jsonb | нет | Ограниченный массив safe refs созданных deploy jobs: `service_key`, optional `service_ref`, `runtime_job_ref`, optional runtime job status, optional `runtime_job_attempt_ref` для bounded retry terminal job и fingerprint deploy plan item. Raw manifests, kubeconfig, Kubernetes events, логи и значения секретов не хранятся. |
