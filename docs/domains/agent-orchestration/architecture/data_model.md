...
`SelfDeployPlan` — авторитетное состояние `agent-manager` для self-deploy orchestration после safe provider/project signal или typed plan input. Он описывает, какие сервисы и категории путей затронуты после merge/push в `main`, какие runtime job types ожидаются после approval, и какие governance refs нужны для решения владельца. После ready-плана `agent-manager` подготавливает governance gate через `governance-manager.PrepareSelfDeployPlanGate`; после `approved` gate он получает build plan через `project-catalog.GetSelfDeployBuildPlan` и создаёт только `JOB_TYPE_BUILD` в `runtime-manager`. Проектная, provider-native, runtime и governance истина остаются у сервисов-владельцев.
