...
- optional `CodexSessionExecutionSpec` с safe refs/digest для будущего Codex CLI запуска без raw prompt text;
...
Guidance refs не дублируются отдельным сырым payload в `AgentRunExecutionSpec`: они уже заморожены в `AgentRun.guidance_refs`, переданы в workspace policy как `guidance_package` sources и связаны с исполнением через workspace fingerprint и digest generated context. `job_input_json` не содержит prompt body, transcript, raw tool input/output, provider payload, workspace paths, kubeconfig, значения секретов или большие логи. Проверенный execution input для Codex CLI материализуется отдельно в workspace или объектном хранилище и передаётся runner-у только как `instruction_object_ref`/digest внутри `CodexSessionExecutionSpec`.
