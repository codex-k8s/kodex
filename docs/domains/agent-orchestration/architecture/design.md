...
При включённом `KODEX_AGENT_MANAGER_RUNTIME_JOB_DISPATCH_ENABLED` постановка runtime job требует `KODEX_AGENT_MANAGER_RUNTIME_JOB_RUNNER_IMAGE_REF`. `agent-manager` собирает `AgentRunExecutionSpec` только из безопасных refs: `agent_run_id`, `slot_id`, workspace materialization id/fingerprint, runtime-owned workspace/context refs, `RuntimeContext.workspace_pvc_ref`, digest `.kodex/context/agent-run.json`, `runner_profile_ref`, `runner_image_ref`, фиксированный `runner_mode=codex_agent`, разрешённые secret refs без значений и reporting targets обратно в `agent-manager`. Вложенный `CodexSessionExecutionSpec` формируется на стороне `agent-manager` из уже материализованных безопасных refs/digest: `PromptTemplateVersion.TemplateObject` как checked instruction object, настроенная result schema ref/digest, session snapshot или workspace snapshot ref, hook/callback refs, ограниченный timeout, тот же фиксированный runner profile/mode, output/result refs и allowed secret refs без значений. `CreateJob` вызывается только когда runtime уже подтвердил `slot_status=ready` и `workspace_materialization_status=completed`, а все обязательные refs и digest для `AgentRunExecutionSpec` и `CodexSessionExecutionSpec` доступны и проходят безопасную валидацию; если `PrepareRuntime` вернул `materializing/pending`, `Run` переходит в `waiting` с safe reason `runtime_materialization_pending`, а replay той же команды повторно читает idempotent `PrepareRuntime` и ставит job после готовности. `agent-runner` исполняет только workspace refs `workspace://.kodex/execution/...`, сверяет digest instruction/result schema и не строит fallback prompt из `agent-run.json`, env или provider payload. Если instruction object, result schema, snapshot, context digest, workspace refs или workspace PVC ref ещё не материализованы, dispatch фиксирует retryable diagnostic `execution_input_unavailable` и не создаёт неполный `JOB_TYPE_AGENT_RUN`. Если runtime вернул terminal materialization state `failed`/`cancelled`, failed slot или невалидный execution ref/digest, `agent-manager` фиксирует безопасный `failed` state без создания job.
...
Если plan уже сохранён в `pending_approval`, но ещё не содержит `governance_risk_assessment_ref`, `governance_gate_request_ref` или `governance_gate_decision_ref`, `agent-manager` штатно доводит его через тот же идемпотентный жизненный цикл подготовки gate. При старте сервиса включённый self-deploy signal consumer выполняет ограниченную сверку по настроенному self-project и вызывает `EnsureSelfDeployPlanGovernanceGate` только для таких неполных pending plans. Этот путь не двигает checkpoint event-log, не читает raw provider payload и не требует ручного SQL. Если `governance-manager` уже содержит active risk assessment и requested/awaiting/resolved gate request для target `self_deploy_plan` с тем же fingerprint, `agent-manager` сначала находит risk assessment по target/project/fingerprint, а если project-scoped фильтр не вернул запись, выполняет target-only read и всё равно локально требует совпадения target и evidence fingerprint. Затем он читает gate requests по найденному `risk_assessment_id`, локально сверяет target/status и для resolved gate читает `ListGateDecisions` по `gate_request_id`. Так recovery не создаёт дублей, записывает недостающие refs в plan и для approved decision продолжает build dispatch без ручного replay. Ошибки сверки классифицируются безопасными stage codes вроде `existing_risk_lookup_failed`, `existing_risk_access_denied`, `existing_risk_fingerprint_mismatch`, `existing_gate_request_lookup_failed`, `existing_gate_request_access_denied`, `existing_gate_not_found`, `existing_gate_mismatch`, `existing_gate_decision_lookup_failed`, `existing_gate_decision_access_denied`, `existing_gate_decision_not_found`, `existing_gate_decision_mismatch` или `plan_governance_refs_update_failed`, без raw payload и секретов.

Если gate уже `approved` и включён `KODEX_AGENT_MANAGER_SELF_DEPLOY_BUILD_DISPATCH_ENABLED`, `agent-manager` переводит план в post-approval путь: вызывает `project-catalog.GetSelfDeployBuildPlan` с refs плана, affected service keys и ожидаемым `services_yaml_digest`. Статус `build_context_unavailable` или `build_context_required` означает, что project-side policy уже проверена, но runtime-owned build context ref/digest ещё не готовы; `agent-manager` вызывает или переиспользует `runtime-manager.PrepareBuildContext`, сохраняет `runtime_build_status=preparing_context` и не создаёт build job до готового context. Статус `policy_stale` означает, что сохранённый в plan snapshot `services.yaml` больше не совпадает с checked policy по source ref, digest, fingerprint или version; commit кода из provider signal при code-only изменении может быть новее `ServicesPolicy.SourceCommitSHA` и сам по себе не делает policy устаревшей. Такой plan переводится в terminal `failed` с `runtime_build_status=blocked`, safe code `policy_stale` и operator action `replan_required`, без подстановки latest policy и без runtime jobs. Остальные non-ready статусы фиксируются как безопасный `runtime_build_status=blocked`. Только статус `ready` даёт per-service build items с `BuildExecutionSpec`-совместимыми refs/digests; если ready-ответ не содержит safe build context, Dockerfile, image или fingerprint refs, plan блокируется как `invalid_build_execution_spec` без вызова runtime. Для ready build plan `agent-manager` создаёт `runtime-manager.CreateJob(JOB_TYPE_BUILD)` с typed `BuildExecutionSpec` и пустым `job_input_json`, сохраняет `runtime_job_ref`/status/fingerprint по каждому service key и не читает Kubernetes. Повтор после `preparing_context` снова проверяет `project-catalog` и при готовом context идемпотентно ставит build job; повтор после уже созданных build jobs переиспользует сохранённые refs и не создаёт вторую job. После successful build `agent-manager` вызывает `project-catalog.GetSelfDeployDeployPlan`, передаёт build output вместе с ожидаемым build plan fingerprint и materialized context refs, проверяет checked `DeployExecutionSpec` с manifest bundle refs/digest, rollout targets и expected image refs, затем создаёт `runtime-manager.CreateJob(JOB_TYPE_DEPLOY)` и сохраняет deploy refs/status/fingerprint. Health-check jobs требуют отдельного approval-driven перехода.

Если plan уже `approved`, содержит сохранённый `gate_decision_ref`, но runtime build/deploy остановился на retryable blocker вроде `permission_denied`, `dependency_unavailable`, `conflict`, `config_not_ready` или `materializer_not_ready`, стартовая runtime-сверка `agent-manager` сбрасывает только безопасно повторяемый runtime step и продолжает `PrepareBuildContext`/build или deploy dispatch без нового governance gate и без повторного approval. Для terminal runtime job с сохранённым `runtime_job_ref` retry идёт через отдельный bounded `runtime_job_attempt_ref`, поэтому `CreateJob` не replay-ит старый terminal command id и при повторе recovery не создаёт дубли; terminal результат самого retry attempt остаётся явным blocker. `policy_stale`, fingerprint/digest mismatch, `failed_precondition`, `invalid_argument`, unsafe contract mismatch и stale checked policy не считаются transient и не ретраятся.
