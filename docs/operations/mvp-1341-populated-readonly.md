...
## Длительный сетевой журнал (#1407)

BUI записывает `read-network-checkpoint` после каждого завершённого шага.
Запись содержит монотонный `checkpointSequence`, накопительные счётчики и
закрытые детали failures текущего интервала. Итоговый `read-network` сохраняет
накопительный результат и детали последнего интервала. Для разбора причины
нужно читать оба типа записей: итоговая пустая коллекция деталей не означает,
что прежних ошибок не было. Ошибка сохранения checkpoint закрывает весь тест.

После материализации checkpoint удаляются только terminal requests и связанные
с ними private signal/document записи. Незавершённые запросы остаются под тем
же лимитом 4096; лимит не увеличивается. Диагностика failures ограничена 32
записями на интервал; превышение остаётся FAIL. Закрытая классификация уже
сохранённых отмен не меняется из-за поздней ошибки, переполнения либо callback.
Поздняя причина не превращает сохранённый unexplained failure в PASS. Для
нативных ID остаётся bounded watermark документа; повтор ID отвергается.

Регрессии: `e2e/ui-read-network.test.ts`, `e2e/ui-document-navigation.test.ts`
и `e2e/ui-network-checkpoint.synthetic.spec.ts`. Последний входит в обычный
synthetic профиль Chromium/Firefox/WebKit: более 4096 реальных завершённых
запросов, сохранённая точная отмена и последующий настоящий socket reset.
Добавление этих тестов не является заявлением их фактического PASS; локальное
выполнение фиксируется отдельно на SHA общего пакета перед публикацией.
