...
Документ описывает реализованный owner lifecycle D5 и границы его локальной
проверки. Исторические промежуточные checkpoints ниже не заменяют итоговые
проверки согласованного дерева #1046 и сквозной baseline #1031.
...
В том же unit реализованы runtime catalog/TOML, effective actor∩agent
capabilities, VFS eligibility, Home query/total и RoleImage/IntegrationDefinition
CFG; их самостоятельные матрицы и evidence находятся в профильных документах
`docs/operations`. Итоговый пакет #1076/#1077/#1078 дополняет их Automation
preview, Provider usage и Workflow launch readiness. HTTP/PWA consumer baseline,
live provider acceptance и общий product/security/architecture review #1031
проверяются отдельно. READY доставки не означает SMTP/IMAP/POP provider readiness.
