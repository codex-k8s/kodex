...
## Карточки каталогов #1098 и сообщение expiry #1085

Producer checkpoint `19f297de22506a074aff217504fbdc41ab08ad58` включает
`602e82ee3d7e122515e06a459d2343dc6e98dc41` поверх normalized CP `6e823604a`.
В HTTP эти два clean prerequisite включены последовательно, без изменения
вручную CP source или generated Proto. Source ownership сохраняется за #1046.

| Owner DTO | HTTP/SDK output | Presence и проверка |
| --- | --- | --- |
| Project.last_activity_at14 | lastActivityAt? | отсутствует при nil, не подменяется updatedAt или epoch |
| Project.integration_state15 | integrationState | обязательный NONE/READY/DEGRADED/UNKNOWN, пустое или неизвестное значение отклоняется |
| Agent.current_run_ref24 | currentRunRef? | пустое отсутствует; доступный owner ref передаётся буквально |
| Workflow.card_summary12 | cardSummary | обязательная owner projection, не вычисляется из steps/page |
| WorkflowCardSummary | stageCount, uniqueAgentCount, parallelGroupCount, hasHumanGate, activeRunCount, pendingGateCount, lastActivityAt? | пять nonnegative int32 и bool обязательны, zero/false сохраняются; timestamp optional |

Browser session → existing generated CP client с проверенным transport/authority
→ owner query/command/replay projection → recursive messageMap/ProtoMap →
строгая проверка и общий HTTP single/page mapping → canonical OpenAPI Go/TS →
PWA. Project/global filters и cursor не выдают полномочий и передаются в прежний
owner RPC. HTTP не делает дополнительного fan-out, не пересчитывает counts и
не расширяет eligibility. Project counters сохраняют обязательные нули.

Покрыты четыре Project wrappers List/Get/Create/Update; шестнадцать Agent
wrappers, включая lifecycle/instructions/capability/integration commands,
Upload/Set/RemoveAvatar и ListRuntimeEnvironmentAgents; семь Workflow wrappers
List/Get/Create/UpdateDraft/ValidateDraft/PublishDraft/Archive. Поэтому поля
сохраняются также в командном результате и replay. Owner повторно проверяет
eligibility; HTTP не переисполняет mutation. Для read projection нового event
нет: authoritative path остаётся get/list. ETag/OCC относятся к прежней owner
version, а HTTP ответы имеют Cache-Control no-store.

Повреждённая обязательная проекция даёт 502 до публикации body: negative count,
missing summary, неизвестное integration state, invalid timestamp/ref.
UNKNOWN не подменяется NONE. Необязательная ссылка не нормализуется как enum.
Regression проходит все 27 generated response envelopes с active/empty данными,
project/global/single HTTP paths, literal ref, cursor/ETag и malformed denial.
Counts fixture намеренно не равны длине локальных steps.

`i18n:RUNTIME_LEASE_EXPIRED` зарегистрирован в обоих ru/en HTTP message catalogs;
HTTP и realtime используют общий safe localizer, без raw runtime/provider text.
Canonical OpenAPI генерирует Go output и TS SDK; handwritten PWA не меняется.
Context7 resolve→query_docs подтвердил optional pointer/omitempty oapi-codegen.
Для Go protobuf Context7 не нашёл подходящую библиотеку; использована официальная
документация [protoregistry](https://pkg.go.dev/google.golang.org/protobuf/reflect/protoregistry)
и [protoreflect](https://pkg.go.dev/google.golang.org/protobuf/reflect/protoreflect)
для проверки всех generated wrappers без дублирования списка методов.

Действующий список Run поддерживает targetType/targetRef только вместе с
resumableSessionsOnly. Полного отдельного workflow-run-list endpoint нет;
действие карточки «Запуски» открывает существующий каталог проекта без
несуществующего target filter. Новый фильтр этим исправлением не добавляется.
Exact локальные проверки фиксируются в handoff; live/browser/provider/deploy
и независимое review нового assembled SHA не подменяются mapper fixtures.
