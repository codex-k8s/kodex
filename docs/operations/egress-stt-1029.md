...
| Readiness | Тот же STT workload и listener | `GET /readyz` возвращает revision/digest/profile/workload/operation фактически загруженной policy; STT затем проверяет provider через тот же CONNECT | `204` при валидной policy, настроенном resolver и запущенном listener; внешние DNS/провайдер проверяются в рабочем сценарии, но не меняют готовность Pod |
