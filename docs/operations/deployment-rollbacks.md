...
Для `000024_cluster_admin_freeze.sql` и `000025_cluster_admin_session_key_inventory.sql` физический `down` запрещён: команда должна завершиться ошибкой, сохранить последнюю применённую `goose` version и все объекты заморозки. После такого отказа повторный `up` допустим и не создаёт дублирующие триггеры или записи инвентаря. Откат приложения разрешён только на заранее проверенный exact N-1 SHA с отдельным runtime DML login; migration/schema-owner DSN нельзя передавать старому процессу как fallback. Перед переключением повторяют adversarial проверку runtime-атрибутов, видимости profiles/roles, запрета DDL/TEMP/disable-trigger, отсутствия расширения frozen state и запрета повторного использования frozen `session_key`.
