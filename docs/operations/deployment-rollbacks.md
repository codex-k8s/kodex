...
Для `000023_cluster_admin_freeze.sql` физический `down` запрещён: команда должна завершиться ошибкой, сохранить `goose` version `23` и все объекты заморозки. После такого отказа повторный `up` допустим и не создаёт дублирующие триггеры. Откат приложения разрешён только на заранее проверенный exact N-1 SHA с отдельным runtime DML login; migration/schema-owner DSN нельзя передавать старому процессу как fallback. Перед переключением повторяют adversarial проверку runtime-атрибутов, видимости profiles/roles, запрета DDL/TEMP/disable-trigger и отсутствия расширения frozen state.
