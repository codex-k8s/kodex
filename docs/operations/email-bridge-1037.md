...
## Обновление pgx по #1081

Общий govulncheck на интеграции `afa9de5d` завершился FAIL: EMAIL сохранял
pgx v5.8.0 с достижимым символом SanitizeSQL (GO-2026-5004). Зависимость
обновлена до принятой в остальных модулях v5.10.0; go.sum воспроизведён
через `go mod tidy`. SQL, миграции, режим протокола и runtime guards не менялись.

[Официальный advisory](https://pkg.go.dev/vuln/GO-2026-5004) указывает исправление
начиная с v5.9.2. Достижимость символа scanner не доказывает эксплуатацию:
требуются нестандартный simple protocol, placeholder внутри dollar-quoted
литерала и контролируемый атакующим аргумент. Context7 `/jackc/pgx` проверен
по QueryExecMode и интерполяции simple protocol; обычный extended protocol
не подменяется настройкой этого исправления.

Локально поверх source `f977638e1`: полный EMAIL race/vet/build — PASS;
`make test-email-bridge` — PASS, disposable PostgreSQL up/status/up и полный
protocol/component suite за 26.498 s. Module govulncheck v1.6.0 с
`-mode=source -scan=symbol -show=traces,version ./...` — PASS,
`No vulnerabilities found`. Общий многомодульный baseline повторяет #1031;
live provider и staging в этом исправлении не запускались.
