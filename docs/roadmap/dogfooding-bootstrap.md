...
- `manager` — координация волн, обратных вызовов и ручной приемки, слияние и запуск `improver` после обычного принятого результата; слияние результата самого `improver` завершает исходный цикл без автоматического повторного запуска.
...
- ежедневный `improver` только по новым материальным замечаниям слитых PR и рецензий; сам факт слияния результата `improver` не создаёт рекурсивный запуск;
