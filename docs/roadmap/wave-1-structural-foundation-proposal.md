...
- Делегирование ограничивает UTF-8 bytes/runes всех пользовательских metadata до storage; возврат строит обе точные audit-публикации до durable callback transition. Невалидный и повторный невалидный вызовы дают ноль durable/network effects, а исправленный допустимый вызов проходит.
- `make test-go-postgres` самостоятельно сериализует database-global `CREATE EXTENSION vector` advisory lock, устанавливает extension один раз в `public`, использует `search_path=<isolated_schema>,public` и удаляет только пакетную схему. DSN разрешён только для одноразовой тестовой database; live/external DSN запрещён. Приёмка начинается с чистой database без `vector`, повторяет default target без внешнего `GOFLAGS` и проверяет единственный extension и доступный `public.vector`.
- `000024`, `000025` и `000026` являются forward-only. Каждый `down` fail-closed, а rollback приложения разрешён только на заранее проверенный exact N-1 reader/runtime без удаления scoped fence/central trigger и без отката мер PR #74/#75.
...
Остаточные риски: checks GitHub могут отсутствовать и не считаются успешным CI; partial network после уже зафиксированного callback остаётся идемпотентным частичным исходом без автоматического дубля; test lifecycle требует права `CREATE DATABASE` только для чистой одноразовой regression database. Физический rollback миграций `000024`–`000026` не поддерживается.
