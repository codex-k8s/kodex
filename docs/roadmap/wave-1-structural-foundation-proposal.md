...
- `000025`, `000026`, `000027`, `000028` и `000029` являются forward-only. Каждый `down` fail-closed, а rollback приложения разрешён только на заранее проверенный exact N-1 reader/runtime при отключённом callback route, без удаления scoped fence/outbox/manifest/triggers и без отката мер PR #74/#75.
