...
**Результат:** по решениям 1A, 3A и 6A вводятся разрешающий список Ingress; внутренние маршруты, MCP и метрики доступны только внутри кластера; `action`/`dialog` идут через кластерный Service; непрозрачная одноразовая серверная возможность доступа хранится только в виде хеша и связывается с операцией, ресурсом, каналом, постом, сроком и погашением; субъект удостоверяется отдельно от серверной проверки допуска `allowed|denied|indeterminate`, где неопределённость означает отказ; действует политика `response_url` из раздела 2.3. Новые назначения профилей `cluster-admin` заморожены; только снимок уже настроенных профилей и назначений может пройти явно заданные на сервере право профиля и допуск с аудитом. Полные `Organization`, `Membership`, `IntegrationGrant`, права и квоты не создаются.

**Автоматические проверки:** отрицательная матрица хеша возможности доступа, срока, погашения, операции, ресурса, канала, поста, субъекта, допуска, повторного воспроизведения и SSRF с нулём побочных эффектов; разрешение только уже настроенного `cluster-admin` профиля и назначения, отказ новому или изменённому назначению и аудит обоих исходов; снимки зарегистрированных и опубликованных маршрутов; отрендерованный Ingress; существующие контрактные тесты `slash`/`action`/`dialog`/GitHub.
...
**Автоматические проверки:** все цели из раздела 5.1; явный тест, что обязательный режим PostgreSQL не пропускается молча; обязательная характеристическая проверка текущего исхода `failed` для сценария #51 и проверка отсутствия `skip`/ожидаемого падения в этом сценарии; снимок уже настроенных `cluster-admin` профилей и назначений, отказ новым назначениям и наличие аудита без расширения полномочий; проверка `front matter`, уникальности `id`, относительных ссылок и архитектурных импортов.
...
- владелец дал финальный OK точной версии `0.6.0` и SHA после полного повторного прохода рецензента.
