...
1. `PR-0` закрывает публичную границу Mattermost: узкий Ingress, удостоверение обратных вызовов `action`/`dialog`, проверенный субъект, защита от повторного воспроизведения и SSRF.
...
4. `PR-3` вводит сериализацию сессии, аренду, пульс и ограждение устаревшего исполнителя, единую PostgreSQL-транзакцию завершения после загрузки архива, атомарное намерение обратного вызова, локальный исходящий журнал результата и контрольную точку `S3b bridge-ready`.
5. `PR-4` вводит версионированный конверт, одну квитанцию события поставщика, детерминированное разветвление в `N` команд/ходов и честную доставку Mattermost с семантикой `at-least-once`.
...
Возможность доступа удостоверяет обратный вызов, но не заменяет авторизацию. После неё сервер отдельно проверяет разрешение субъекта на операцию. Полная модель `Organization`, прав и квот из #66 не входит в PR-0, однако точка сопряжения обязана возвращать типизированный результат допуска `allowed|denied|indeterminate` с кодом причины; `indeterminate` трактуется как отказ. Промпт, `user_name`, инструкции, метки Kubernetes и ключ идемпотентности не являются правом.

Допустим эквивалентный механизм доверенного прокси или плагина с подписанным запросом и теми же привязками. Простая проверка IP источника, существования `user_id` или общий статический URL-токен не защищает от подмены субъекта и повторного воспроизведения.
...
| `bot-service` → PostgreSQL | `direct`, текущий | Secret `${MATTERCODEX_POSTGRES_SECRET}`: переменная среды `MATTERCODEX_DATABASE_DSN` ← ключ `mattermost-datasource`; подключение отсутствует; ServiceAccount `matter-codex-bot-service` | те же `automount` по умолчанию и Role среды выполнения, не нужные для PostgreSQL | установщик направляет DSN на `mattermost-postgres.${MATTERCODEX_NAMESPACE}.svc.cluster.local:5432`; фактические назначение и порт остаются в DSN, манифест не ограничивает исходящий трафик; роль PostgreSQL задаётся вне репозитория, и её SQL-права здесь не доказаны | допуск — только наличие и действительность DSN; один `admin.Repository` имеет широкий доступ | DSN не передаётся `agent-runner`/DTO и не журналируется; широкий исходный SQL-доступ не выдаётся за минимально необходимые права |
...
`organization_scope` в волне 1 является стабильной ссылкой на одну установку и проходит через порт авторизации; создание общей модели `Organization`, `Membership`, прав и квот остаётся в #66. Неизвестная область, субъект или результат допуска дают закрытый отказ `rejected`, а не значение по умолчанию.
...
- один обработчик, защищённый ограждением, выполняет доставку `at-least-once` с ограниченной экспоненциальной задержкой и случайным разбросом;
- состояния: `pending|claimed|retry_wait|delivered|ambiguous|dead_letter`;
- успешный `post_id`, число попыток, безопасный код ошибки и идентификатор корреляции сохраняются;
- состояния очереди необработанных событий `dead_letter` и неоднозначной доставки `ambiguous` доступны оператору и процедуре сверки.

| Окно отказа | Доказуемый результат |
| --- | --- |
| До внешнего вызова | намерение остаётся и безопасно повторяется; внешнего эффекта нет |
| После ошибки до состояния повтора | аренда истекает, намерение повторяется |
| После подтверждённого ответа и локального подтверждения | `delivered`, повтор не выполняется |
| После внешнего побочного эффекта, но до локального подтверждения | состояние неоднозначно; повтор может создать внешний дубль |
| Истечение срока, когда поставщик мог принять запрос | `ambiguous`; автоматический повтор допускается только принятой политикой риска |

Сверка использует чтение результата у поставщика только после отдельного тестового доказательства. Без него оператор сверяет целевое обсуждение, метаданные корреляции и идемпотентности и локальный исходящий журнал, затем помечает намерение `delivered` либо разрешает повтор. Ручная проверка специально воспроизводит отказ до и после побочного эффекта и подтверждает отсутствие потери локального намерения; возможный внешний дубль в последнем окне фиксируется как ограничение, а не скрывается.

## 5. Обязательные тестовые шлюзы

### 5.1 Тестовый контур PostgreSQL как результат PR-1

Текущие команды ниже отсутствуют; PR-1 обязан их создать согласно [действующему руководству PostgreSQL](../design-guidelines/go/infrastructure_integration_requirements.md):

- `make test-go` — только герметичные модульные и компонентные тесты, без PostgreSQL, Docker и тестовых DSN;
- `make test-go-postgres` — обязательный режим тестов репозиториев, миграций и конкурентности; использует имена вида `MATTERCODEX_*_TEST_DATABASE_DSN`, не печатает значения и завершается явным исходом «ошибка» или «не запущено», а не `t.Skip`;
...
| Сессия | FIFO, захват двумя исполнителями, потерянный ответ, повторный захват после истечения аренды, устаревшее завершение, перезапуск, CAS остановки и завершения; загрузка и проверка архива, откат транзакции завершения, сверка непривязанной версии и следующий захват с новым снимком; обязательный MatterCodex MCP недоступен до появления Codex-событий → ход сохраняет `queued` или переходит в `retry_wait`, после восстановления зависимости выполняется и не становится необратимо `failed` (#51) |
| Обратный вызов | конкурентный возврат, отказ и перезапуск, детерминированный ключ обработчика, один результирующий ход |
| Доставка | уникальный локальный `OutboxEvent`, повтор с задержкой и очередь `dead_letter`, отказ до или после побочного эффекта, сверка `ambiguous` |
...
| `S1 containment` | PR-0/PR-1/PR-2: публичная граница, тестовый контур, сдерживание очистки | полезная маршрутизация сохранена, опасные обратные вызовы и очистка закрыты по умолчанию |
...
| `S3a durable behavior` | PR-3: аренда, ограждение, CAS, единая транзакция завершения, намерение обратного вызова и переключение доставки на исходящий журнал | транспорт и DTO маршрутизации прежние; меняется только надёжность среды выполнения и доставки |
| `S3b bridge-ready` | финальная контрольная точка PR-3: S3a плюс включённый в унаследованный входной путь адаптер совместимости для чтения и устранения повторов будущих квитанций, команд и результатов S4 | `event_path=legacy`; адаптер только читает чужие записи S4 при откате или повторе и не создаёт их на обычном трафике S3 |
| `S4 inbound durable` | PR-4: конверт, квитанция и переключение команд `1 -> N` | тот же транспорт WebSocket/HTTP; один долговечный владелец записи и обработчик |
...
| `S3a durable behavior` | Предыдущий исполняемый файл с поддержкой расширенной схемы знает переключатели и новые записи и при чужой эпохе не выполняет побочные эффекты обработчика | Поддерживает аренду, CAS, завершение, обратный вызов и исходящий журнал | после атомарного переключения только обработчик среды выполнения и доставки N, защищённый ограждением; владелец записи транспорта прежний | Развернуть N с переключателями `legacy`, убедиться, что все экземпляры их понимают; остановить приём завершений и обратных вызовов, дренировать старые захваты и прямые доставки, выдать новую эпоху, включить `delivery_path=outbox` | Приостановить новые терминальные команды и обратные вызовы, оградить N, дождаться отсутствия захватов; доставить либо пометить `OutboxEvent` как `ambiguous`, вернуть `legacy`, затем N-1. Старые аренды не принимаются, ходы `queued` читаются N-1; локальных дублей и потерь нет |
...
Рекомендуемый контракт — добавить адаптер совместимости в более ранний PR-3. Поддерживаемый `N-1` для S4 имеет точное логическое имя `S3b bridge-ready`: это финальные принятые SHA исходного кода и дайджест образа PR-3, а не любой совместимый с S3 исполняемый файл. До шлюза ручной приемки раздела 10 это рекомендация; если владелец выберет вариант без адаптера, переключение S4 с откатом на PR-3 запрещено и граница сужается согласно варианту B таблицы решений.
...
4. Для исходящего журнала доставки: захваченная аренда обработчика S4 завершается или освобождается; `pending|retry_wait` продолжит точный обработчик доставки S3b; `ambiguous` остаётся только для сверки, `dead_letter` — для оператора. Удалять или объявлять доставленными эти записи при откате запрещено.
...
**Результат:** разрешающий список Ingress; внутренние маршруты, MCP и метрики доступны только внутри кластера; `action`/`dialog` идут через кластерный Service; одноразовая серверная возможность доступа; проверенный субъект; привязки, защита от повторного воспроизведения и срок действия; политика `response_url` из раздела 2.3.

**Автоматические проверки:** отрицательная матрица удостоверения, субъекта, повторного воспроизведения и SSRF с нулём побочных эффектов; снимки зарегистрированных и опубликованных маршрутов; отрендерованный Ingress; существующие контрактные тесты `slash`/`action`/`dialog`/GitHub.

**Ручная проверка:** легитимная цепочка `slash -> action -> dialog` работает; подделанный обратный вызов отклоняется; с внешнего адреса доступны только разрешённые пути, а внутренние маршруты, MCP и метрики недоступны.

**Ограничение:** PR не переносит транспортные пакеты, не меняет доменную модель и остаётся блокирующим условием любого следующего слияния или развертывания волны.

### PR-1. Обязательный тестовый контур и характеристические доказательства

**Результат:** создаются `make test-go`, `make test-go-postgres`, `make test-go-all`; обязательный режим PostgreSQL с `MATTERCODEX_*_TEST_DATABASE_DSN`; миграции с нуля и обновление копии; минимум два соединения; контур внесения отказов в маршрутизацию, сессии, обратные вызовы и доставку; полная матрица синтетических секретов. Для #51 обязательный характеристический сценарий проверяет недоступность MatterCodex MCP до первых Codex-событий: ход сохраняет `queued` или переходит в `retry_wait`, после восстановления зависимости выполняется и не завершается необратимо в `failed`.

**Автоматические проверки:** все цели из раздела 5.1; явный тест, что обязательный режим PostgreSQL не пропускается молча; проверка `front matter`, уникальности `id`, относительных ссылок и архитектурных импортов.

**Ручная проверка:** владелец видит отдельные исходы «герметичные тесты пройдены», «PostgreSQL пройден» либо явное «не запущен/упал»; успешный общий статус невозможен без обязательного PostgreSQL-контура для PR хранения.

**Ограничение:** это новый результат PR, а не описание существующих команд. Прикладные схема и поведение не меняются.

### PR-2. Запрещающее по умолчанию сдерживание очистки сессий

**Результат:** автоматическое разрушительное удаление PVC сессии и Secret токена сессии выключено; хранение работает как инвентаризация и предварительный просмотр без удаления для сессионных данных; повтор после ошибки квоты не вызывает разрушительную очистку и возвращает типизированную ошибку ёмкости. Предикат допустимости принимает факты PostgreSQL и архива, но до доказанного архива S3 любое `unknown` даёт отказ.
...
**Результат:** поля конверта из раздела 4.1; точка сопряжения проверенного субъекта; области установки, рабочей области и сессии без полной #66; одна уникальная квитанция на событие поставщика; неизменяемый снимок целей; `N` команд и ходов по целям; типизированный результат допуска; идемпотентные обработчики; этап S4.
...
**Ограничение:** транспорт WebSocket/HTTP остаётся на месте. Новые `Organization`, `Membership`, права и квоты не создаются.
...
4. **Надёжность #51 — обязательная зависимость до любого физического разделения очереди и `runtime-controller`.** Классы временных ошибок, ограниченные повторы с задержкой, лидер для всех циклов. Характеристический шлюз: если обязательный MatterCodex MCP недоступен до появления Codex-событий, ход сохраняет `queued` или переходит в `retry_wait`; после восстановления зависимости он выполняется и не становится необратимо `failed`. Только после этого шлюза допустимы физическое разделение и `replicas >= 2`.
5. **Хранение: доказанная допустимость по PostgreSQL и S3 и включение очистки.** Контрольная сумма и ссылка архива, отсрочка, блокировка хранения, аренда, аудит, отчёт предварительного просмотра и ручной шлюз включения; зависит от ADR-MC-006 и PR-2.
6. **Инфраструктура #58: воспроизводимые настройки ботов Mattermost.** Отдельный результат SRE без переноса транспорта.
7. **Инструкции #60:** версии исходных данных, контрольная сумма, расхождение и управляемое обновление без молчаливой перезаписи.
8. **Учётная запись поставщика #61:** безопасное обозначение, неизменяемая привязка сессии и `RuntimeRevision` по ADR-MC-004.
9. **Авторизация #66:** отдельные архитектурный, схемный и поведенческие срезы `Organization`, `Membership`, прав и квот после стабилизации конверта и точки сопряжения допуска.

Физическое разделение очереди и `runtime-controller`, включая выделение `runtime-controller` в отдельный процесс, начинается только после задач 1–4. Задача #51 и её характеристический шлюз блокируют это физическое разделение и `replicas >= 2`, но не блокируют документационную работу или общий dogfooding. `interaction-gateway`, `control-plane`, `integration-gateway` и `automation-scheduler` не создаются в первых пяти PR.

## 9. Основные риски и ограничения

| Риск | Шлюз |
| --- | --- |
| Подделанный обратный вызов/SSRF | PR-0 до любого структурного слияния или развертывания |
| Тихо пропущенные тесты PostgreSQL | обязательная цель PR-1; PR хранения без неё не принимается |
| Потеря состояния PVC или сессии | PR-2 запрещает удаление при неопределённости; полное удаление только после доказательства по БД и S3 |
...
- подтвердил, что структурные порты начинаются только после PR-4;
- принял внешний контракт Mattermost `at-least-once` и ограничение окна неоднозначного отказа;
- подтвердил поддерживаемый диапазон `N-1/N` и запрет опасного отката без дренирования и ограждения.

Точная цель финального полного прохода рецензента на новом SHA:

1. повторно проверить все факты по указанным ссылкам на код и SQL;
2. подтвердить, что #51 является обязательной зависимостью до любого физического разделения очереди и `runtime-controller`, условие изменено на задачи 1–4, характеристический шлюз сохраняет `queued|retry_wait` и исключает необратимое `failed`, а блокировка не распространяется на документацию и общий dogfooding;
3. убедиться, что PR-0 закрывает публикацию маршрутов, подмену субъекта, повторное воспроизведение и SSRF с нулём побочных эффектов;
...
6. доказать `N-1/N` для S2/S3a/S3b/S4, совместимый адаптер на фактическом исполняемом файле, единственных владельца записи и обработчика, переключатели, дренирование, откат и повторное воспроизведение;
7. проверить закрытое при неопределённости хранение, обязательный PostgreSQL-контур, синтетические секреты и матрицу полномочий;
8. подтвердить одного владельца каждой таблицы и мутации и отсутствие разделённого владения внешними идентификаторами;
9. убедиться, что PR-0…PR-4, совместимость S3b/S4, транзакция завершения, матрица полномочий и хранение не ослаблены, а все шесть решений владельца остаются открытыми;
10. проверить русскую проектную прозу и убедиться, что точные идентификаторы, пути, команды, API, протоколы, переменные, Secret и канонические состояния сохранены без искажения.
