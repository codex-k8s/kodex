...
**Ручная проверка:** владелец видит отдельные исходы «герметичные тесты пройдены», «PostgreSQL пройден» либо явное «не запущен/упал»; успешный общий статус невозможен без обязательного PostgreSQL-контура для PR хранения. На явном ephemeral endpoint после каждого post-`CREATE` fault число target database возвращается к исходному, proof остаётся consumed, а подменённая replacement database сохраняется. Отдельно воспроизводится недоступность обязательного MatterCodex MCP до первых Codex-событий: после восстановления зависимости проверка PR-1 проходит, только если наблюдает фактический `failed` и отсутствие автоматического продолжения того же хода, а отчёт называет этот исход текущим долгом #51, а не восстановлением или допустимым целевым поведением.
