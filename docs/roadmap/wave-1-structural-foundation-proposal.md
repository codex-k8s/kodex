...
- `000025`, `000026`, `000027`, `000028`, `000029` и `000031`–`000033` являются forward-only. Каждый `down` fail-closed, а rollback приложения разрешён только на заранее проверенный exact N-1 reader/runtime при отключённом callback route, без удаления scoped fence/outbox/manifest/triggers/functions, восстановления удаленных identity placeholders и без отката мер PR #74/#75.
...
Остаточные риски: checks GitHub могут отсутствовать и не считаются успешным CI; retry callback запускается повторным MCP-вызовом, отдельного фонового worker пока нет; callback delivery rows и manifests не имеют временной очистки в MVP, а внешний ключ блокирует удаление delegation; test bootstrap требует право `CREATE DATABASE` только после проверенного one-shot proof. Любой target до exact applied marker и любой существующий target на внешнем/shared endpoint намеренно сохраняются с явной передачей ручной очистки владельцу: catalog identity не доказывает происхождение, wildcard/prefix cleanup запрещён. Физический rollback миграций `000025`–`000029` и `000031`–`000033` не поддерживается.

## Краткий вывод

Первые пять результатов волны должны идти в таком порядке:

1. `PR-0` по обязательным решениям 1A, 3A и 6A закрывает публичную границу Mattermost: узкий Ingress, кластерный обратный вызов `action`/`dialog`, непрозрачная одноразовая серверная возможность доступа, сохранённая как хеш, проверенный субъект, отдельный допуск, защита от повторного воспроизведения и SSRF; одновременно новые назначения `cluster-admin` замораживаются, уже настроенные профили сохраняются только как серверные policy/audit records, а их materialization в shared-node runtime закрыто запрещена до managed gateway.
...
| Полномочия | `bot-service` может управлять Pod/PVC/Secret/Job и `pods/exec`; shared-node `agent-runner` получает только ServiceAccount для чтения, прямые учётные данные OpenAI/GitHub и токены сессии/MCP. Назначение `cluster-admin` в этом runtime закрыто отклоняется до Kubernetes side effects. |
...
| Профиль с `kubernetes_access=cluster-admin`, включая начальную настройку `mattercodex-admin` | `direct`, отключён в shared-node runtime | те же введённые переменные среды и проецируемые файлы, но Kubernetes credential не выдаётся; legacy `${MATTERCODEX_AGENT_RUNNER_CLUSTER_ADMIN_SERVICE_ACCOUNT}` используется installer только как точное имя для отзыва | ServiceAccount и ClusterRoleBinding для agent pod отсутствуют; adapter возвращает fail-closed ошибку до Job, Pod, PVC и session Secret | прямой Kubernetes API path не материализуется | новые назначения заморожены; точный серверный снимок сохраняет policy/audit, но не превращается в credential; [`system_roles.go`](../../services/external/bot-service/internal/domain/service/system_roles.go) создаёт `mattercodex-admin`, но промпт и само значение перечисления не являются правом | безопасное ограничение 3A: административное исполнение возможно только после отдельного принятого managed gateway; legacy binding/ServiceAccount удаляются при upgrade, а inventory блокирует rollout при legacy pod |
...
| `S1 containment` | PR-0/PR-1/PR-2: 1A — кластерный обратный вызов и одноразовая возможность доступа; 3A — заморозка новых `cluster-admin` назначений, отзыв legacy agent credential и запрет materialization в shared-node runtime; 6A — проверенный субъект и точка сопряжения допуска без схемы #66; тестовый контур; 2A — разрушительное удаление и quota cleanup выключены | полезная маршрутизация и audit policy сохранены; опасные обратные вызовы, прямой административный credential в недоверенном pod, новые широкие назначения, неопределённый допуск и очистка закрыты по умолчанию |
