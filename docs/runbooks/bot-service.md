...
- `000034` также является forward-only: откат приложения выполняется только при `MATTERCODEX_ARTIFACTS_ENABLED=false`; artifact metadata, bindings, deliveries, retention hold и S3 objects не удаляются и не изменяются вручную.
