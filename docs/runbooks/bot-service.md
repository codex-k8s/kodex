...
`make test-go-postgres` и PostgreSQL-часть `make test-go-all` всегда запускаются через `cmd/postgres-test-target`. Предпочтительный вход — локальный `MATTERCODEX_BOT_SERVICE_TEST_BOOTSTRAP_DSN`: wrapper создаёт случайную database из `template0`, записывает высокоэнтропийный ограниченный по времени per-run marker в комментарий database, передаёт дочернему процессу `MATTERCODEX_BOT_SERVICE_TEST_DATABASE_DSN` и `MATTERCODEX_BOT_SERVICE_TEST_DATABASE_MARKER`, а после команды повторно проверяет identity и удаляет только эту database. Уже созданный target допустим лишь при одновременно заданных DSN и точном marker.

Admission сначала без подключения разбирает URL и keyword DSN одинаковым `pgx`, требует имя database, детерминированно связанное с marker, и запрещает `mattermost`, `postgres`, `template*`, настроенную production identity, альтернативный fallback endpoint и отсутствующий, несовпадающий или просроченный marker. По умолчанию разрешены только loopback и Unix socket. CI service endpoint включается отдельно точным списком `host:port` в `MATTERCODEX_POSTGRES_TEST_EPHEMERAL_ENDPOINTS`; bootstrap database такого endpoint дополнительно обязана иметь точный `MATTERCODEX_POSTGRES_TEST_EPHEMERAL_ENDPOINT_MARKER`. Непосредственно перед каждым helper-owned `CREATE EXTENSION`, `CREATE SCHEMA`, `CREATE DATABASE`, `DROP SCHEMA` и `DROP DATABASE` тот же connection выполняет только read-only сверку `current_database()`, фактического endpoint и комментария marker. Ошибка закрыто прекращает DDL. DSN, credentials, endpoint marker и per-run marker не выводятся.
...
- После применения `000024`, `000025`, `000026` и `000027` exact N-1 runtime login нормально читает profiles/roles и выполняет обычный repository bootstrap/DML. Самодекларируемый GUC больше не участвует в допуске. Runtime login не может создавать `pg_temp`, DDL, удалять callback outbox, менять immutable delivery plan или отключать триггеры; функции имеют фиксированный доверенный `search_path` с `pg_temp` последним, а триггеры закрыто блокируют прямое расширение frozen state, повторное использование или перепривязку frozen `session_key`, обход scoped delivery fence и обратный переход из `delivered`. Владелец схемы, superuser и `BYPASSRLS` явно находятся вне защищаемой runtime-стороны и не используются приложением.
...
- `000024`, `000025`, `000026` и `000027` являются forward-only: каждый `goose down` закрыто возвращает ошибку, версия остаётся последней применённой, а frozen state, inventory, scoped fence, callback outbox и triggers физически сохраняются; повторный `up` после неуспешного `down` является успешным no-op.
- Для callback `ReturnToRequester` точный prompt и обе точные audit-публикации с UTF-8 byte chunks строятся и проверяются до enqueue. Подготовка turn/process/run, запись `CallbackRunID` и все строки неизменяемого delivery plan миграции `000027` выполняются одной guarded transaction. Уникальность связывает delegation, callback run, destination и publication; каждая строка хранит точные channel/root/message/props, SHA-256 payload и детерминированную внешнюю identity. Ошибка после durable transition возвращается вызывающему, а не отбрасывается. Повтор с существующим `CallbackRunID` не создаёт новый callback и работает только с незавершёнными строками.
...
- `pending_post_id` не считается долговечной гарантией Mattermost. Адаптер до отправки и после неоднозначной ошибки ищет детерминированную callback identity в точном треде, затем полностью сверяет channel/root/message и весь набор props. Точное совпадение подтверждает прежнюю доставку; чужая payload, любая лишняя prop или несколько совпадений закрыто возвращают ошибку. Если сеть была успешна, а DB mark завершился ошибкой, worker по возможности освобождает строку в `pending/confirmation_ambiguous`; при общей недоступности БД она остаётся `in_flight` до истечения lease. Следующий повтор выполняет сверку и не создаёт второй post. Успех возвращается только когда все обязательные строки имеют `delivered`.
...
- Откат приложения выполняется без `goose down` только на заранее проверенный exact N-1 SHA/дайджест с reader/runtime, совместимыми со схемой `000024`–`000027`. Физически удалять fence/outbox/triggers или откатывать меры PR #74/#75 запрещено. Exact N-1 не обрабатывает новый outbox, поэтому на время отката callback route выключается; незавершённые строки сохраняются для исправления вперёд.
