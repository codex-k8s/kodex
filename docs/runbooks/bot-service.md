...
- `000025`, `000026`, `000027`, `000028`, `000029` и `000030` являются forward-only: каждый `goose down` закрыто возвращает ошибку, версия остаётся последней применённой, а frozen state, inventory, scoped fence, callback outbox, manifest, integration approvals, recording receipts и triggers физически сохраняются; повторный `up` после неуспешного `down` является успешным no-op.
...
- Откат приложения выполняется без `goose down` только на заранее проверенный exact N-1 SHA/дайджест с reader/runtime, совместимыми со схемой `000025`–`000030`. Физически удалять fence/outbox/manifest/integration receipts/triggers или откатывать меры PR #74/#75 запрещено. Exact N-1 не обрабатывает новый outbox и integration execution, поэтому на время отката callback route и integration MCP/action/worker выключаются; незавершённые строки, manifests, fence, revocations, approvals и receipts сохраняются для исправления вперёд.
