...
- `000024`, `000025` и `000026` являются forward-only: `goose down` закрыто возвращает ошибку, версия остаётся последней применённой, объекты не помечаются как предыдущая версия; повторный `up` после неуспешного `down` является успешным no-op.
- Для callback `ReturnToRequester` подготовка нового turn, process/run и запись callback выполняются атомарно одним guarded repository и завершаются до Mattermost-публикаций. Перед любым DB lock проверяются byte/chunk/concurrency bounds. Каждый фактический source/requester audit post выполняется внутри отдельного exact source+child final guard с каноническим порядком `session → role → chat → participant → dependency → delivery fence`; после захвата точных fence выполняется повторная проверка binding/revocation непосредственно перед network boundary. Миграция `000026` добавляет по одной fence-строке на `session_key`; центральный trigger для явных revoke и всех dependency-trigger вставок берёт `FOR UPDATE` только затронутых fence. Поэтому committed revoke до fence даёт ноль публикаций, revoke после fence сериализуется только с точным source/child, а несвязанный revoke продолжает работу. Глобальная блокировка таблицы revocation во время сети отсутствует.
...
- Откат приложения выполняется возвратом к предыдущему образу без `goose down`: additive fence-таблица и trigger остаются активными и совместимы с прежним reader. После применения `000026` удалять fence или trigger запрещено; при дефекте публикации отключают callback route либо возвращают предыдущий образ, сохраняют строки revocation и исправляют только вперёд.
