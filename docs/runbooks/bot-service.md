...
1. До запуска зафиксировать только количество `mc_test_*` database без вывода имён, DSN, marker или proof.
2. Последовательно внести отказ после `CREATE`, до `COMMENT`, после применённого `COMMENT`, при построении DSN и до/во время финальной admission; после каждого исхода количество database должно вернуться к исходному, а повтор того же proof должен быть отклонён.
3. Повторить transient reconnect и неоднозначный ответ `DROP`; итогом должно быть подтверждённое отсутствие exact target.
4. После создания заменить target database объектом с тем же именем, владельцем и комментарием, но другим OID. Bootstrap обязан завершиться безопасной ошибкой, replacement обязан сохраниться; wildcard/prefix cleanup запрещён.
...
- Откат приложения выполняется без `goose down` только на заранее проверенный exact N-1 SHA/дайджест с reader/runtime, совместимыми со схемой `000024`–`000028`. Физически удалять fence/outbox/manifest/triggers или откатывать меры PR #74/#75 запрещено. Exact N-1 не обрабатывает новый outbox, поэтому на время отката callback route выключается; незавершённые строки, manifests, fence и revocations сохраняются для исправления вперёд.
