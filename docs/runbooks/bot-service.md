...
- Поля `user_name`, `prompt`, `labels`, `state` и `submission` не предоставляют права. Новое назначение `cluster-admin` запрещено. Миграция `000024` фиксирует точное состояние существующих профилей и ролей, влияющее на полномочия: состояние включения; bot identity; развёрнутые состояния учётных записей и credentials OpenAI/GitHub; SHA-256 содержимого, UID и resourceVersion Secret без raw values; привязки репозиториев, переменных и сессий; Kubernetes и `sandbox`/`config`; а также фактически потребляемый instruction-контекст profile template, project, chat и role (`name`, `description`, `advanced_settings`, `prompt_template`, `work_policy`, `settings`, `system_purpose`, root issue и тип). Изменение снимка и повторное включение после понижения прав, выключения, блокировки поставщиком или удаления закрыто отклоняются, а отзыв сохраняется монотонно. Перед каждым административным adapter callback final guard повторно сверяет БД и текущее безопасное описание Secret; изменение значения при прежнем ref/key и пересоздание Secret отклоняются до Job/Secret/account-status/publish side effects. Для frozen session token один проверенный `Get` одновременно возвращает bytes и точную integrity tuple; runner создаёт отдельный collision-safe immutable Secret с защитным finalizer, а Pod ссылается только на эту версию. На каждой границе повторно проверяются исходная tuple, immutable data, session label и finalizer. Retention снимает finalizer отдельным metadata-only patch с UID/resourceVersion fence и удаляет Secret с тем же UID и post-mutation resourceVersion, не передавая token в Pod spec, patch или журнал. Raw values не сохраняются в БД, аудит, prompt или журнал.
- После применения `000024`, `000025` и `000026` exact N-1 runtime login нормально читает profiles/roles и выполняет обычный repository bootstrap/DML. Самодекларируемый GUC больше не участвует в допуске. Runtime login не может создавать `pg_temp`, DDL или отключать триггеры; функции имеют фиксированный доверенный `search_path` с `pg_temp` последним, а триггеры закрыто блокируют прямое расширение frozen state, повторное использование или перепривязку frozen `session_key` и обход scoped delivery fence. Владелец схемы, superuser и `BYPASSRLS` явно находятся вне защищаемой runtime-стороны и не используются приложением.
- Перед созданием уникального индекса `000025` берёт блокировку таблицы от конкурентных записей и выполняет обезличенную предварительную проверку. При унаследованных дубликатах миграция возвращает только код `MCV25_DUPLICATE_SESSION_KEY_GROUPS` и число групп, полностью откатывается и оставляет версию `goose` `24`. Для исправления остановите записи и работайте локально через временную таблицу соответствий с внутренним суррогатным идентификатором строки: определите число групп и число лишних строк агрегатами, выберите каноническую строку внутри каждой группы через `row_number()`, перенесите внешние ссылки по внутренним идентификаторам и удалите лишние строки. Не выбирайте и не перенаправляйте в терминал, журнал или тикет значения `session_key`, `role_id` и иные идентификаторы. После исправления повторите только агрегатную предварительную проверку и `goose up`.
...
- `000024`, `000025` и `000026` являются forward-only: каждый `goose down` закрыто возвращает ошибку, версия остаётся последней применённой, а frozen state, inventory, scoped fence и центральный trigger физически сохраняются; повторный `up` после неуспешного `down` является успешным no-op.
- Для callback `ReturnToRequester` точный prompt и обе точные audit-публикации с UTF-8 byte chunks строятся и проверяются до enqueue, изменения turn/process/run и записи `CallbackRunID`; внутри guarded transaction metadata повторно сверяется fail-closed. `StartAgentThread` до чтения session/token/storage ограничивает bytes/runes у title, message, target chat/agent и work item key. Невалидный повтор не создаёт состояние идемпотентности, поэтому исправленный допустимый вызов не подавляется. Подготовка нового turn, process/run и запись callback выполняются атомарно одним guarded repository. Каждый фактический source/requester audit post выполняется внутри отдельного exact source+child final guard с каноническим порядком `session → role → chat → participant → dependency → delivery fence`; после захвата точных fence выполняется повторная проверка binding/revocation непосредственно перед network boundary. Миграция `000026` добавляет по одной fence-строке на `session_key`; центральный trigger для явных revoke и всех dependency-trigger вставок берёт `FOR UPDATE` только затронутых fence. Поэтому committed revoke до fence даёт ноль публикаций, revoke после fence сериализуется только с точным source/child, а несвязанный revoke продолжает работу. Глобальная блокировка таблицы revocation во время сети отсутствует.
- Разбиение и окончательная проверка chunks выполняются до fence, поэтому publisher внутри блокировки не обращается обратно в БД. Обе публикации разделяют один короткий server-owned deadline. Все основные, административные и token-specific `Client4` используют один HTTP client с общим timeout и ограничениями dial, TLS handshake, response headers, idle connections и числа соединений. Сетевой сбой сохраняет уже зафиксированный callback и частичный результат, а идемпотентный повтор не публикует дубликаты. Другие транзакционные adapter callbacks всё ещё могут удерживать DB locks во время сети, а commit allowed-audit после callback сохраняет риск неоднозначного исхода; они должны оставаться идемпотентными, поскольку общий протокол outbox/lease в этот PR не входит.
- Откат приложения выполняется без `goose down` только на заранее проверенный exact N-1 SHA/дайджест с reader/runtime, совместимыми со схемой `000024`–`000026`: additive fence-таблица и центральный trigger остаются активными. После применения `000026` удалять fence/trigger или откатывать меры PR #74/#75 запрещено; при дефекте публикации отключают callback route либо возвращают доказанный exact N-1 образ, сохраняют строки revocation и исправляют только вперёд.
