...
## Ручная приёмка integration approval recording-среза

Сценарий выполняется только на одноразовой PostgreSQL 15/16 из `make test-go-postgres`; промышленный DSN, connection credential, Kubernetes и внешняя система не используются.

1. Подготовьте тестовые `Connection` и exact `IntegrationGrant` для одной активной AgentSession через тестовую оснастку. `credential_ref` должен оставаться пустым.
2. Убедитесь, что `tools/list` разрешённой сессии содержит только `deployment.restart_workload`, а чужая сессия capability не видит.
3. Вызовите инструмент с новым idempotency key. Ожидается `pending`, карточка exact root/direct human initiator и ноль строк в `matter_codex_integration_test_executions`.
4. Отклоните запрос. Число recording receipts остаётся нулём.
5. Создайте второй запрос и согласуйте его кнопкой карточки. После worker ожидается ровно одна recording receipt; никакого Kubernetes mutation или внешнего действия не происходит.
6. Повторите исходный MCP-вызов с тем же idempotency key и неизменным binding. Ожидается сохранённый `succeeded` с тем же execution result; повторный callback, параллельный worker и восстановление после истёкшей lease не создают вторую receipt.
7. Для отдельного запроса согласуйте карточку и отзовите grant до записи receipt. Ожидается `cancelled` с `execution.authorization_changed` и ноль новых строк executor. Отзыв после уже зафиксированной receipt не отменяет факт: recovery финализирует ту же receipt без второй записи.
8. Проверьте только безопасные агрегаты и публичные идентификаторы. Не выводите DSN, bearer, Secret, credential value или полные строки аудита.

Автоматический новый ход Codex, `resume` и перевод AgentSession в `waiting_approval` в этот сценарий не входят и не считаются успешным результатом.
...
- После применения `000025`–`000030` и `000032` exact N-1 runtime login нормально читает profiles/roles и выполняет обычный repository bootstrap/DML при отключённых callback route и integration MCP/action/worker. Самодекларируемый GUC больше не участвует в допуске. Runtime login не может создавать `pg_temp`, DDL, удалять callback outbox/manifest или integration receipts, менять immutable delivery plan/tool binding или отключать триггеры; функции имеют фиксированный доверенный `search_path` с `pg_temp` последним, а триггеры закрыто блокируют прямое расширение frozen state, повторное использование или перепривязку frozen `session_key`, обход scoped delivery fence, неполный callback manifest, обратный переход из `delivered` и немонотонный переход integration state. Владелец схемы, superuser и `BYPASSRLS` явно находятся вне защищаемой runtime-стороны и не используются приложением.
...
- Integration approval использует ту же атомарную callback-границу: consume одноразовой capability и exact root/direct human decision выполняются одной PostgreSQL-транзакцией через узкий transactional integration adapter. Решение не запускает worker синхронно. Fresh authorization до первой recording receipt закрыто отменяет invocation при revoke; уже сохранённая immutable receipt при recovery только читается и финализируется с тем же fence/hash.
- `000025`, `000026`, `000027`, `000028`, `000029`, `000030` и `000032` являются forward-only: каждый `goose down` закрыто возвращает ошибку, версия остаётся последней применённой, а frozen state, inventory, scoped fence, callback outbox, manifest, integration approvals, recording receipts и triggers физически сохраняются; повторный `up` после неуспешного `down` является успешным no-op. Версия `000031` зарезервирована независимой волной runtime revisions и не входит в этот результат.
...
- Откат приложения выполняется без `goose down` только на заранее проверенный exact N-1 SHA/дайджест с reader/runtime, совместимыми со схемой `000025`–`000030` и `000032`. Физически удалять fence/outbox/manifest/integration receipts/triggers или откатывать меры PR #74/#75 запрещено. Exact N-1 не обрабатывает новый outbox и integration execution, поэтому на время отката callback route и integration MCP/action/worker выключаются; незавершённые строки, manifests, fence, revocations, approvals и receipts сохраняются для исправления вперёд.
