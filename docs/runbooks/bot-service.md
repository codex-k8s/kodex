...
- Поля `user_name`, `prompt`, `labels`, `state` и `submission` не предоставляют права. Новое назначение `cluster-admin` запрещено. Миграция `000023` фиксирует точное состояние существующих профилей и ролей, влияющее на полномочия: состояние включения, bot identity, имена и состояния учётных записей OpenAI/GitHub вместе со ссылками на Secret учётных данных, привязки репозиториев проекта и чата, Kubernetes и `sandbox`/`config`, переменные среды выполнения, точные `project_id`, `chat_id`, `mattermost_channel_id`, существовавшие `session_key` и привязку Kubernetes/Secret сессии. Изменение снимка и повторное включение после понижения прав, выключения, блокировки поставщиком или удаления закрыто отклоняются, а отзыв сохраняется монотонно. Создание новой привязки, смена канала или новая сессия отклоняются до Mattermost, Kubernetes и прикладных изменений БД. Транзакционный guard удерживает блокировки точной роли, участника, чата, зависимостей учётных записей, репозиториев и среды выполнения, frozen binding и при необходимости сессии на время каждого административного callback в bootstrap и `RequestAgent`; общее согласование административную роль не обрабатывает. Административный bootstrap только проверяет уже замороженное состояние репозитория и чата и не восстанавливает его записью. `CreateChat` принимает решение и изменяет чат, участников и привязки репозиториев в одной транзакции с тем же порядком блокировок.
- После применения `000023` исполняемый файл N-1 без маркера writer `v23` не видит таблицы профилей и ролей через принудительную RLS и не может выполнить старый безусловный upsert. Триггеры дополнительно не дают старому `CreateChat` восстановить отозванного участника или изменить точную привязку. Это DB-level ограждение сохраняет заморозку при запуске exact N-1, но не обещает доступность старого процесса: его bootstrap закрыто завершится ошибкой до административных внешних эффектов. Перед откатом всё равно обязательны точный SHA, изолированная PostgreSQL-проверка и подтверждение отсутствия расширения frozen state.
