...
4. Воспроизвести исчерпание квоты на создание session PVC и убедиться, что вызывающий код различает `AgentSessionCapacityError`, создание не повторяется внутри адаптера, а сохранённые PVC/Secret не получают `delete` или `patch`.
