...
До любых действий apply-режима installer отзывает legacy ClusterRoleBinding и ServiceAccount, затем читает inventory всех pod в namespace. Pod со старым `PriorityClass`, старым request/limit либо любым legacy `cluster-admin` ServiceAccount закрыто останавливает upgrade до применения новой quota или rollout. Для восстановления остановите активные ходы, удалите только точный legacy session pod либо точный управляющий utility Job и повторите installer. Не удаляйте и не пересоздавайте PVC, session token Secret или архив. На следующем запуске session runner сам пересоздаёт только несовместимый pod по UID/resourceVersion preconditions; несовместимый utility Job не переиспользуется автоматически.
