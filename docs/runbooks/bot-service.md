...
- `MATTERCODEX_AGENT_RUNNER_CLUSTER_ADMIN_SERVICE_ACCOUNT` - optional, только точное legacy-имя ServiceAccount для отзыва при upgrade; bot-service не передаёт его agent workload и не создаёт заново;
...
Проверка перечисляет каждый непустой YAML document как `Kind/name`, исключает пустые документы и сравнивает число объектов с результатами `kubectl create --dry-run=client --validate=false`. Для полного профиля с управляемым registry, Kaniko и runtime limits ожидаются 8 файлов и 18 объектов; legacy `cluster-admin` ServiceAccount и ClusterRoleBinding отсутствуют. Число разделителей `---` доказательством не является. Команда не обращается к Kubernetes API и ничего не применяет.
...
make test-kubernetes-admission
```

Первые команды проверяют render и числовой aggregate invariant без обращения к Kubernetes API. `make test-kubernetes-admission` отдельно скачивает закреплённые и проверенные по SHA-256 `kube-apiserver v1.36.1` и `etcd v3.6.6`, запускает их только на loopback во временном каталоге и применяет отрендеренные `PriorityClass`, scoped `ResourceQuota` и RBAC в одноразовый API. Тест наблюдает `status.used`, допускает pod до полного бюджета, на конкурентной границе проверяет ровно один `Forbidden/exceeded quota` и запрещает недоверенному ServiceAccount создавать Pod/Job либо изменять quota/priority. Завершение процесса удаляет только временное тестовое хранилище; production kubeconfig и cluster API не используются.

До любых изменений Kubernetes в apply-режиме installer читает общий список `Pod`, `ReplicaSet`, `Deployment` и `StatefulSet` в runtime namespace. Разрешаются только pod Mattermost, bot-service, PostgreSQL, OAuth2 proxy и управляемого registry с точной живой цепочкой UID-владения к явно разрешённому контроллеру и с исполняемым шаблоном, совпадающим с шаблоном владельца. Label и ServiceAccount сами по себе не являются доказательством доверия. Любой standalone pod, pod неизвестного контроллера и ранее созданный agent или utility pod закрыто останавливает upgrade до отзыва legacy ClusterRoleBinding/ServiceAccount, применения новой quota и rollout.

Для восстановления остановите активные ходы и удалите только точный session pod либо точный управляющий utility Job, указанный диагностикой, затем повторите installer. Доверенные рабочие нагрузки платформы не удаляются. Не удаляйте и не пересоздавайте PVC, session token Secret или архив. После успешной инвентаризации installer отзывает legacy credential, а на следующем запуске session runner создаёт pod уже под действующей quota; несовместимый utility Job не переиспользуется автоматически.

Все BinarySI Quantity shell и Go приводят к KiB до любых Kubernetes mutations. Максимум — `9007199254740991` KiB; большее значение, потеря точности, переполнение и насыщение закрыто отклоняются на env, render и runtime границах.
...
- Межкомнатное делегирование уже зафиксированной роли с `cluster-admin` по-прежнему проходит server-side role/chat/participant/dependency/Secret проверки и сохраняет аудит, но shared-node Kubernetes adapter закрыто отклоняет материализацию до Job, Pod, PVC или session Secret. Штатного `cluster-admin` credential в недоверенном agent pod нет; для административного исполнения нужен отдельный управляемый шлюз вне этого runtime.
...
- `000025`, `000026`, `000027`, `000028`, `000029` и `000031`–`000033` являются forward-only: каждый `goose down` закрыто возвращает ошибку, версия остаётся последней применённой, а frozen state, inventory, scoped fence, callback outbox, manifest, triggers, bootstrap function и очистка identity placeholders физически сохраняются; повторный `up` после неуспешного `down` является успешным no-op.
...
- Получение delivery claim использует `FOR UPDATE SKIP LOCKED`, ограниченную lease и последовательность chunks внутри destination. Проверка манифеста и чтение плана делят один server-owned предел plan preflight до получения ownership. Каждая следующая попытка использует один общий, а не повторно начинающийся, preflight budget для claim и точной source+child final guard со всеми lock waits. Поэтому claim/reclaim lease формально равна `preflight budget + CallbackPublishDeadline + 2s safety margin`: она покрывает полный оставшийся server-owned путь от начала claim до внешней границы и подтверждения результата. Канонический порядок остаётся `session → role → chat → participant → dependency → delivery fence`, после чего guarded repository непосредственно перед transport атомарным CAS подтверждает только `in_flight` строку с точным ещё действующим `lease_owner`; SQL сверяет срок по серверным часам PostgreSQL, а сервис проверяет остаток на полный `CallbackPublishDeadline`. Просроченный или уже reclaim-нутый owner закрыто завершает свой вызов до транспорта и не переходит к другой delivery. После успешного CAS отдельный `CallbackPublishDeadline` ограничивает только внешний reconcile/POST. Зафиксированный revoke/remap оставляет строку `blocked` с `last_error_code=final_binding_denied`, а потеря lease — незавершённую строку; эти состояния не считаются успешными. Глобальная блокировка и отдельное соединение в обход guarded repository запрещены.
...
- Runtime namespace получает общую namespace-level ResourceQuota/LimitRange и отдельную scoped `ResourceQuota` по `PriorityClass` agent workloads. CPU/memory requests сохраняют scheduler accounting, а aggregate `requests.memory` и `limits.memory` недоверенных session/utility pod ограничены одним бюджетом и проверяются Kubernetes admission. CPU limits не задаются; `/dev/shm` имеет отдельный ceiling. Оператор продолжает контролировать node pressure системных workloads и число работающих pod.
