...
- Для callback `ReturnToRequester` подготовка нового turn, process/run и запись callback выполняются атомарно одним guarded repository; Mattermost-публикации выполняются после commit и нового точного final guard без удержания сетевого вызова под блокировкой. Другие транзакционные adapter callbacks всё ещё могут удерживать DB locks во время сети, а commit allowed-audit после callback сохраняет риск неоднозначного исхода; они должны оставаться идемпотентными, поскольку общий протокол outbox/lease в этот PR не входит.
