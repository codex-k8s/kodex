...
- После применения `000024` и `000025` exact N-1 runtime login нормально читает profiles/roles и выполняет обычный repository bootstrap/DML. Самодекларируемый GUC больше не участвует в допуске. Runtime login не может создавать `pg_temp`, DDL или отключать триггеры; функции имеют фиксированный доверенный `search_path` с `pg_temp` последним, а триггеры закрыто блокируют прямое расширение frozen state и повторное использование или перепривязку frozen `session_key`. Владелец схемы, superuser и `BYPASSRLS` явно находятся вне защищаемой runtime-стороны и не используются приложением.
...
- `000024` и `000025` являются forward-only: `goose down` закрыто возвращает ошибку, версия остаётся последней применённой, объекты не помечаются как предыдущая версия; повторный `up` после неуспешного `down` является успешным no-op.
- Для callback `ReturnToRequester` подготовка нового turn, process/run и запись callback выполняются атомарно одним guarded repository и завершаются до Mattermost-публикаций. Каждый фактический source/requester audit post выполняется внутри отдельного combined exact source+child final guard: сессии, роли, чаты и участники блокируются в детерминированном порядке, новые revocation сериализуются до возврата сетевого вызова, а вложенные проверки используют тот же transaction/savepoint без нового соединения. Разбиение текста на chunks выполняется до guard, поэтому publisher внутри блокировки не обращается обратно в БД; сетевой сбой сохраняет уже зафиксированный callback и частичный результат, а идемпотентный повтор не публикует дубликаты. Другие транзакционные adapter callbacks всё ещё могут удерживать DB locks во время сети, а commit allowed-audit после callback сохраняет риск неоднозначного исхода; они должны оставаться идемпотентными, поскольку общий протокол outbox/lease в этот PR не входит.
