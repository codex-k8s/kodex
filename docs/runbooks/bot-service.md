...
Entrypoint, `bootstrap.sh`, `env.sh`, render helper и agent-runner build wrapper сверяются с blob одного закреплённого exact `HEAD`, интерпретируемого с отключёнными Git replacement objects в очищенном окружении. Исполняемые helper-файлы открываются один раз, их topology и content commitment проверяются через удержанные descriptors, а `source` и локальный запуск выполняются через те же `/proc/<pid>/fd/<n>`. Remote archive также получает build wrapper из проверенного descriptor, а не через повторное разрешение pathname. Статический guard дополнительно закрепляет SHA-256 всего раннего closure. Явный запуск через `bash script.sh` не является поддерживаемым защищённым entrypoint.
