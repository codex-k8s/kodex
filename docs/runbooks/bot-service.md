...
- `000024` является forward-only: `goose down` закрыто возвращает ошибку, версия остаётся `24`, объекты не помечаются как `v23`; повторный `up` после неуспешного `down` является успешным no-op.
