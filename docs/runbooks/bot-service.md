...
- Поля `user_name`, `prompt`, `labels`, `state` и `submission` не предоставляют права. Новое назначение `cluster-admin` запрещено. Миграция `000023` фиксирует точное состояние существующих профилей и ролей, влияющее на полномочия: состояние включения; bot identity; развёрнутые состояния учётных записей и credentials OpenAI/GitHub; SHA-256 содержимого, UID и resourceVersion Secret без raw values; привязки репозиториев, переменных и сессий; Kubernetes и `sandbox`/`config`; а также фактически потребляемый instruction-контекст profile template, project, chat и role (`name`, `description`, `advanced_settings`, `prompt_template`, `work_policy`, `settings`, root issue и тип). Изменение снимка и повторное включение после понижения прав, выключения, блокировки поставщиком или удаления закрыто отклоняются, а отзыв сохраняется монотонно. Перед каждым административным adapter callback final guard повторно сверяет БД и текущее безопасное описание Secret; изменение значения при прежнем ref/key и пересоздание Secret отклоняются до Job/Secret/account-status/publish side effects. Raw values не сохраняются в БД, аудит, prompt или журнал.
- После применения `000023` exact N-1 runtime login нормально читает profiles/roles и выполняет обычный repository bootstrap/DML. Самодекларируемый GUC больше не участвует в допуске. Runtime login не может создавать `pg_temp`, DDL или отключать триггеры; функции имеют фиксированный доверенный `search_path` с `pg_temp` последним, а триггеры закрыто блокируют прямое расширение frozen state. Владелец схемы, superuser и `BYPASSRLS` явно находятся вне защищаемой runtime-стороны и не используются приложением.
...
- `000023` является forward-only: `goose down` закрыто возвращает ошибку, версия остаётся `23`, объекты не помечаются как `v22`; повторный `up` после неуспешного `down` является успешным no-op.
