...
- Получение delivery claim использует `FOR UPDATE SKIP LOCKED`, ограниченную lease и последовательность chunks внутри destination. Claim, чтение плана и точная source+child final guard получают отдельный server-owned предел preflight; отсутствие срока у входящего запроса не оставляет callback slot или соединение ждать блокировку неограниченно. Канонический порядок остаётся `session → role → chat → participant → dependency → delivery fence`, после чего guarded repository атомарным CAS продлевает только ещё действующую lease того же owner на весь ограниченный reconcile/POST. Просроченный или уже reclaim-нутый owner закрыто останавливается до транспорта. После успешного CAS каждая фактическая попытка получает новый отдельный `CallbackPublishDeadline`, который начинается после final preflight и ограничивает только этот внешний вызов. Зафиксированный revoke/remap оставляет строку `blocked` с `last_error_code=final_binding_denied`, а потеря lease — незавершённую строку; эти состояния не считаются успешными. Глобальная блокировка и отдельное соединение в обход guarded repository запрещены.
