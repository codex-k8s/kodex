...
- принимать разрешённые вложения Mattermost, сохранять неизменяемые metadata/object, материализовывать проверенный manifest/inbox для точного session turn и публиковать выбранный результат обратно в исходный thread через `publish_artifact`.
...
1. Сначала примените миграцию `000030`, подготовьте отдельный S3-compatible bucket и Kubernetes Secret, затем включите `MATTERCODEX_ARTIFACTS_ENABLED`. Значения учётных данных не выводите в render, журналы или отчёт.
...
- Миграция `000030_artifacts_vertical.sql` forward-only: immutable triggers запрещают изменение metadata, terminal state и перепривязку scope; object key включает project/session/artifact/version, а S3 `PutObject` использует checksum и `If-None-Match: *`.
...
- `000030` также является forward-only: откат приложения выполняется только при `MATTERCODEX_ARTIFACTS_ENABLED=false`; artifact metadata, bindings, deliveries, retention hold и S3 objects не удаляются и не изменяются вручную.
...
- Откат приложения выполняется без `goose down` только на заранее проверенный exact N-1 SHA/дайджест с reader/runtime, совместимыми со схемой `000025`–`000030`. Физически удалять fence/outbox/manifest/artifact metadata/triggers или откатывать меры PR #74/#75 запрещено. Exact N-1 не обрабатывает новый outbox и artifact delivery, поэтому на время отката callback route и `MATTERCODEX_ARTIFACTS_ENABLED` выключаются; незавершённые строки, manifests, artifact metadata, fence и revocations сохраняются для исправления вперёд.
