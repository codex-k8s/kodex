...
- Получение delivery claim использует `FOR UPDATE SKIP LOCKED`, ограниченную lease и последовательность chunks внутри destination. Получение claim и предварительные проверки PostgreSQL в точной source+child final guard выполняются на контексте входящего запроса и не расходуют срок внешней публикации. После захвата final guard и повторной проверки привязки и отзыва непосредственно перед сетевой границей каждая фактическая попытка получает новый отдельный server-owned контекст публикации. Канонический порядок защитной блокировки остаётся `session → role → chat → participant → dependency → delivery fence`; зафиксированный revoke/remap оставляет строку `blocked` с `last_error_code=final_binding_denied`, и это состояние не считается успешным. Глобальная блокировка и отдельное соединение в обход guarded repository запрещены.
