...
- Транзакционный final guard всё ещё удерживает DB locks во время сетевого callback, а commit allowed-audit после callback оставляет риск неоднозначного исхода при ошибке commit. Adapter-операции должны оставаться идемпотентными; отдельный протокол outbox/lease в этот PR не входит, поэтому этот долг не считается устранённым.
