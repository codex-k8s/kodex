...
## TL;DR
- Проблема: текущий onboarding умеет проверить репозиторий (`preflight`), но не умеет довести пустой GitHub-репозиторий до состояния, в котором его уже можно вести через `kodex`.
...
- В итоге новый проект нельзя быстро завести в платформу “с нуля” без ручной подготовки репозитория вне `kodex`.
...
- Handover package:
  - этот epic;
  - `docs/deprecated/pre-refactor/delivery/epics/s3/epic-s3-day14-repository-onboarding-preflight.md`;
  - `docs/deprecated/pre-refactor/delivery/epics/s3/epic-s3-day12-docset-import-and-safe-sync.md`;
  - `docs/deprecated/pre-refactor/delivery/epics/s3/epic-s3-day9-declarative-full-env-deploy-and-runtime-parity.md`;
