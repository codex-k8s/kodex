...
- Ключевая ценность: единый source of truth для full-env runtime, dogfooding `kodex`, prompt context и первичного развертывания платформы.
- Фактический результат: webhook-driven `kodex` разворачивает окружения по typed execution-plan из `services.yaml`, c repo-sync/pvc snapshot контуром и idempotent reconcile в runtime deploy.

## Priority
- `P0`.

## Контекст
- В `codexctl` orchestration строился вокруг `services.yaml` и GitHub workflows; в `kodex` source-of-truth остаётся `services.yaml`, но запуск теперь webhook-driven через `control-plane`.
- Для MVP требуется одинаковая логика:
  - для любых внешних проектов (любой стек);
  - для dogfooding проекта `kodex` (саморазвёртывание в ai-slot namespace).
- Нужен одноразовый bootstrap binary для первичной инициализации чистого Ubuntu 24.04 сервера: подготовка Kubernetes/зависимостей/секретов/репозитория, деплой `kodex`, передача управления `control-plane`.
...
- `R4`: для проекта `kodex` окружение `production` в `services.yaml` задаётся шаблоном `{{ .Project }}-production`; для `project=kodex` это даёт namespace `kodex-prod`.
...
- Dogfooding без конфликтов:
  - `kodex` в ai-slot разворачивает изолированную копию себя в отдельном namespace;
  - для `kodex` `production` задаётся шаблоном `{{ .Project }}-production`.
- Bootstrap binary (одноразовый):
  ...
  - деплой `kodex` и handoff в webhook-driven `control-plane`.
...
- Story-6: Bootstrap binary для первичной установки Ubuntu 24.04 + Kubernetes + deploy `kodex` + handoff.
- Story-7: Dogfooding safeguards:
  - namespace isolation для ai-slot;
  - шаблон для `kodex production`: `{{ .Project }}-production`;
...
- Story-8: Full E2E на новом чистом VPS:
  ...
  - сценарий: установка зависимостей на Ubuntu 24.04, поднятие Kubernetes, деплой `kodex`, проверка webhook-driven lifecycle;
...
- Story-1 (`done`):
  - typed контракт `services.yaml` зафиксирован в `libs/go/servicescfg` (`apiVersion=kodex.works/v1alpha1`, `kind=ServiceStack`);
...
- Story-7 (`done`):
  - правило `kodex production => {{ .Project }}-production` валидируется в loader;
  - namespace-level изоляция runtime и anti-conflict guardrails включены в текущий full-env путь.
- Story-8 (`moved`):
  - full e2e на чистом VPS вынесен в финальный закрывающий эпик `docs/deprecated/pre-refactor/delivery/epics/s3/epic-s3-day20-e2e-regression-and-mvp-closeout.md`;
...
- `done`: full-env сценарий для `kodex` подтверждён на production; typed runtime deploy работает через persisted reconcile loop.
...
- `done`: правило `production={{ .Project }}-production` для `kodex` соблюдается и валидируется.
...
## Критерии приемки
- Для минимум двух проектов (`project-example` и `kodex`) full-env поднимается из `services.yaml` через typed execution-plan.
...
- Для `kodex` подтверждено правило шаблона: `production` задаётся как `{{ .Project }}-production` и для `project=kodex` резолвится в `kodex-prod`.
...
- Решение: approved
- Комментарий: включает выбранные решения `1-1`, `2-1`, `3-1`, обязательный Story-8 e2e и namespace правило для `kodex production`.
