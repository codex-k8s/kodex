...
- Локальный контур использует k3s, SeaweedFS S3 и два авторизованных аккаунта
  OpenAI Codex.
- Kubernetes API публикуется внутри кластера через стабильный
  host-only адрес, не зависящий от DHCP и VPN; пользовательские локальные URL
  остаются на `127.0.0.1.nip.io`.
