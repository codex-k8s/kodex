...
Внешние репозитории документации подключены как необязательные submodule в `docs/sources/**`, а исходники платформенных пакетов — в `packages/**`.
Обычная работа с `kodex` не требует их инициализации.
Правила работы описаны в `docs/sources/AGENTS.md` и `packages/AGENTS.md`.
