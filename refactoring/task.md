...
Ключевой принцип:
- guidance package живёт во внешнем репозитории;
...
| Пакетная платформа | built-in packages, пользовательские sources, локальная установка plugins и guidance packages, verification flag | marketplace, verified package versions, publisher profiles, paid/free packages, revenue share |
| Контур серверов и кластеров | один production cluster с явным слоем реестра и политикой размещения | multi-cluster, multi-server, per-org/per-project cluster placement, multi-zone |
| Billing | записи затрат, распределение использования, billing summaries | invoices, payment providers, package marketplace billing, automated charging |
| Release policy | release policy, release branch rules, release line, задел для canary и staged rollout | full release programs, tenant-aware release policies, complex trains |
...
1. пакетная платформа = общий слой для plugins и guidance packages;
...
7. guidance packages должны жить во внешних репозиториях и попадать в рабочий контур локально;
