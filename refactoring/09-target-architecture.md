...
5. настраиваемые внешние каналы взаимодействия и уведомлений, включая текст, голос и временные медиа-вложения;
6. встроенные и пользовательские роли с шаблонами промптов, хранящимися в репозитории платформы;
7. package-платформу для plugins и guidance packages;
8. многоорганизационную access-модель, fleet management и billing;
9. постепенное вытеснение старого `control-plane` без сохранения его как "новой стабильной формы".
...
- первичную настройку и preflight-проверки репозиториев;
- branch rules;
- release policies;
- package bindings;
...
- mentions, relationships и метаинформацию провайдера;
...
Отвечает за:
- package catalog;
- plugins и guidance packages как package entries;
- package source repositories и версии;
- verification status;
- package install/import state;
- secret schemas и package capabilities;
- package pricing и marketplace metadata.

Этот сервис не подменяет `provider-hub`: операции чтения и изменения внешних репозиториев для packages идут через provider-контур.

### `agent-manager`
Отвечает за:
- разбор пользовательского намерения;
- выбор flow;
- каталог встроенных и пользовательских ролей;
- выбор `work`/`revise` шаблонов по профилю и правилам;
- запуск role-агентов;
- запуск параллельных ролей проверки и контроля после появления нужных артефактов;
...
При этом канонический текст шаблонов промптов остаётся версионируемым содержимым репозитория платформы, а не внутренней БД owner-сервиса.
...
- placement policy;
- привязку организаций, проектов, репозиториев и runtime-контуров к server/cluster scopes;
- задел на multi-cluster и delegated clusters.
...
- исполнение на уже выбранном fleet scope.

### `billing-hub`
Отвечает за:
- cost records;
- allocation затрат по организациям, проектам и другим scopes;
- invoice basis;
- usage внешних провайдеров и package-экономики;
- payment-provider seams.

### `interaction-hub`
Отвечает за:
- диалоговые ветки пользователя с agent-manager;
- сообщения и краткие статус-отчёты, приходящие в пользовательские каналы;
- ссылки на временные медиа-вложения и их retention-контур;
- approvals;
- уведомления;
- delivery attempts;
- callback handling;
...
- шов для speech-to-text, text-to-speech и объектного хранилища временных медиафайлов.
...
5. Если требуется package catalog, import guidance package или plugin install, `platform-mcp-server` маршрутизирует вызов в `package-hub`.
6. Если требуется slot/runtime, `platform-mcp-server` маршрутизирует вызов в `fleet-manager` и `runtime-manager`.
7. Если нужно уведомление, Human gate или внешний канал, используется `interaction-hub`.
8. Оператор видит агрегированное состояние через `operations-hub`.

### 6.3. Build/deploy/cleanup
1. `fleet-manager` определяет допустимый infrastructure scope.
2. `runtime-manager` создаёт и ведёт platform jobs.
3. `worker` или специализированные исполнители выполняют нужные действия.
4. Статус и краткий хвост лога возвращаются в контур-владелец `runtime-manager`.
5. `interaction-hub` и `operations-hub` распространяют это в уведомления и UI-проекции.
...
- auth/admin/org/group -> `access-manager`
- project/repo config, branch rules, release policy -> `project-catalog`
- provider semantics/webhooks/rate limits -> `provider-hub`
- package catalog and package install/import policy -> `package-hub`
- run/session/acceptance/flow/automation -> `agent-manager`
- fleet inventory and placement -> `fleet-manager`
- slots/build/deploy/cleanup -> `runtime-manager`
- billing and cost accounting -> `billing-hub`
