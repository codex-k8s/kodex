...
- Wave 5.1 добавляет к onboarding-модели не только репозиторный старт, но и старт через organization, guidance package, plugin install и fleet scope, чтобы дальнейшая реализация не осталась однопроектной и однокластерной.
...
К базовым трём дорожкам wave 5.1 добавляет обязательные дополнительные admin-маршруты:
1. первичное создание или подключение организации;
2. назначение пользователей и групп;
3. привязка проекта к допустимому server/cluster scope;
4. импорт базового guidance package;
...
Это не отменяет базовую метрику `time to first successful run`, но фиксирует, что в enterprise-модели первый успешный run может зависеть не только от репозитория, но и от корректного package/access/fleet bootstrap.
