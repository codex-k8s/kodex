...
## TL;DR
- Wave 5.1 добавляет в канонику платформы шесть обязательных доменных направлений до старта runtime/deploy волны: пакетную платформу, многоорганизационную модель доступа, контур серверов и кластеров, billing, release policy и automation rules.
- Новая модель проектируется сразу на enterprise-уровень, но для каждого направления фиксируется отдельный MVP floor, чтобы волны реализации не расползлись.
...
- С этого момента wave 6 и последующие волны реализации обязаны учитывать tenancy, каталоги пакетов, billing, release trains и автоматизацию по триггерам как обязательные точки расширения, а не как расширения когда-нибудь потом.
...
Wave 5.1 фиксирует общий пакетный слой с разными runtime-путями для разных видов пакетов.

## 4. MVP floor против enterprise target

| Направление | MVP floor | Enterprise target |
|---|---|---|
| Пакетная платформа | встроенные пакеты, пользовательские источники пакетов, локальная установка плагина и guidance package без открытого marketplace billing | публичный marketplace, paid/free packages, publisher profiles, verified versions, revenue share |
...
| Контур серверов и кластеров | один сервер и один cluster в production, но с явным слоем реестра и политикой размещения | несколько серверов, несколько кластеров, multi-zone, dedicated cluster per org/project |
...
| Release policy | release branch rules, release line, политики canary и staged rollout на уровне модели | полные release programs, multi-env trains, tenant-aware rollout policies |
| Automation rules | cron и минимальный набор event triggers, включая создание задач по alert | гибкие event buses, сложные trigger pipelines, cross-org automation templates |
...
- doc-governance для внешних репозиториев руководящей документации;
...
- владение данными для org/group/fleet/billing/automation objects;
- provider-модель для репозиториев-источников пакетов, платёжных провайдеров и branch semantics.
...
7. guidance packages как отдельный слой, управляемый репозиториями.
