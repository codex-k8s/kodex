...
- Wave 5.1 расширяет карту консоли на каталоги пакетов, организации и группы, контур серверов и кластеров, billing, release policy и automation rules; детальная визуализация этих административных разделов может идти следующим UX-срезом, но сами разделы уже считаются обязательными.
...
| Каталоги и расширения | какие плагины, пакеты руководящей документации, роли и шаблоны доступны и разрешены | запись каталога пакетов, plugin, guidance package, профиль роли, шаблон промпта |
...
| Биллинг и учёт | сколько стоит работа платформы и что выставляется клиентам | billing account, cost record, invoice basis |
...
## 8. Обязательные административные разделы wave 5.1
Wave 5.1 добавляет обязательные экранные направления, даже если их детальный пакет макетов будет вынесен в следующий UX-срез:
- каталог пакетов для plugins и guidance packages;
- поверхности установки, импорта и верификации пакетов;
- organizations, groups и inheritance-права;
- реестр серверов и кластеров и политика размещения;
- billing summaries, allocation и invoice basis;
- release policy, релизные линии и automation rules.

Эти разделы не считаются необязательными корпоративными дополнениями. Они входят в каноническую карту консоли уже до начала волн реализации.
