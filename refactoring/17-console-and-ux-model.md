...
### 4.2. Кнопка `Создать`
Вместо большого постоянного composer в центре экрана консоль использует одну явную точку старта: кнопку `Создать`.

Меню `Создать` должно уметь:
...
### 4.3. Голос как глобальное действие
Голосовой запуск — не отдельный экран и не дублирующая кнопка в шапке.

Канонический паттерн:
- одна заметная плавающая кнопка микрофона;
- доступна в ключевых рабочих экранах;
- открывает тот же диалоговый контур agent-manager, что и текстовый чат;
- не перекрывает полезный контент.
...
2. блок быстрого старта через `Создать`;
3. виджеты центра внимания;
4. список последних активных задач, `PR/MR`, `run` и `job`;
...
## 10. Визуальные варианты командного центра

### Вариант A
Приоритет — центральный чат, а блокировки и решения вынесены в правую колонку.

![Вариант A командного центра](images/wave5-command-center-variant-a.svg)

### Вариант B
Приоритет — верхняя лента решений и активной работы, чат уходит в левую основную колонку.

![Вариант B командного центра](images/wave5-command-center-variant-b.svg)

## 11. Что wave 5 intentionally не фиксирует
