...
- Wave 5.1 расширяет карту консоли на package catalogs, organizations и groups, fleet, billing, release policy и automation rules; детальная визуализация этих admin-секций может идти следующим UX-срезом, но сами surfaces уже считаются обязательными.

Полный пакет утверждённых и review-ready макетов wave 5 находится в [refactoring/images/wave5/index.md](images/wave5/index.md). Для экранов с вкладками рядом с основным состоянием лежат отдельные PNG по каждой вкладке.
...
| Исполнения и среда | какие agent `run`, platform `job`, slot и cluster operations выполняются или упали | `run`, `job`, slot, cluster scope, хвост лога |
| Каталоги и расширения | какие plugins, guidance packages, роли и шаблоны доступны и разрешены | package entry, plugin, guidance package, role profile, prompt template |
| Организации и доступы | кто к чему допущен и как устроены users, groups и org scopes | user, group, organization, membership, access policy |
| Инфраструктура | где живёт платформа и куда могут уходить slots и workloads | server, cluster, placement policy, runtime scope |
| Биллинг и учёт | сколько стоит работа платформы и что выставляется клиентам | billing account, cost record, invoice basis |
| Настройка и automation | как заданы flow, release policy, schedule rules, triggers и каналы | flow, release line, automation rule, external account, channel |
...
- настроечные экраны используют вкладки для inventory, policy, limits, usage и history внутри одной сущности.
...
## 8. Обязательные admin-surfaces wave 5.1
Wave 5.1 добавляет обязательные экранные направления, даже если их detailed mockup package будет вынесен в следующий UX-срез:
- package catalog для plugins и guidance packages;
- install/import и verification surfaces для packages;
- organizations, groups и inheritance-права;
- server/cluster inventory и placement policy;
- billing summaries, allocation и invoice basis;
- release policy, release lines и automation rules.

Эти разделы не считаются "optional enterprise extras". Они входят в каноническую карту консоли уже до начала implementation waves.
