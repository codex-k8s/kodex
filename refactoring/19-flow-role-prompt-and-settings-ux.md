...
- Роли и шаблоны промптов разделены: профиль роли и политика исполнения живут в платформе, а канонические файлы flow и шаблонов — в репозиториях конфигурации.
...
- UI может запускать редактирование шаблонов, flow и ролевых политик, но публикует изменения через обычный `PR/MR` в целевой репозиторий конфигурации, а не через магическое сохранение в БД.
...
Для самого flow UI должен показывать источник конфигурации:
- репозиторий;
- путь к файлу;
- активную версию;
- область применения: платформа, организация, команда или проект;
- кто может менять этот источник.
...
Для каждой роли UI должен показывать не только файл шаблона, но и репозиторий конфигурации, из которого он взят. Это важно для разделения встроенных, организационных, командных и проектных ролей.
...
Текст шаблона промпта хранится в репозитории конфигурации.
...
- история изменений, ревью и слияние идут через обычный `PR/MR`;
- разные организации и команды могут использовать разные репозитории конфигурации без смешивания всех flow и ролей в одном месте.

### 4.2. Что должен уметь UI
UI должен:
- показать, из какого репозитория конфигурации взят шаблон;
...
- создать ветку и `PR/MR` для изменения файла;
- связать `PR/MR` с затронутыми ролями и flow.
...
5. платформа создаёт ветку и `PR/MR` в целевом репозитории конфигурации;
6. после слияния новая версия становится доступной для запуска.
...
Если изменение затрагивает репозиторий конфигурации, проектный репозиторий или репозиторий проектной документации, UI не должен сохранять его скрыто.

Канонический паттерн:
- изменить локальную форму;
- показать diff и затронутые сущности;
- создать `PR/MR`;
- после слияния обновить активную конфигурацию.
...
- публикацию изменений через `PR/MR` в целевой репозиторий конфигурации.
...
Но обязательная каноника уже закреплена: последовательный редактор flow, каталог ролей с поддержкой встроенных и пользовательских ролей, тексты шаблонов в репозиториях конфигурации и публикация изменений только через `PR/MR`.
