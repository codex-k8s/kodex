...
- policy layer должен объяснять, как GitHub-аккаунты привязаны к ролям и проектам;
...
- `Аккаунты` — inventory аккаунтов, их scopes, статусы и auth state;
- `Права` — policy layer: какие роли и проекты могут использовать какие provider-аккаунты и для каких операций;
...
Если активна вкладка `Права`, экран не должен одновременно показывать inventory-секции вкладки `Аккаунты`.
...
Для утверждённой модели этого экрана каноническим считается режим, где при выборе категории `GitHub` активная вкладка `Права` показывает именно policy layer, а inventory аккаунтов остаётся во вкладке `Аккаунты`.
