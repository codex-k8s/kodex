...
- Старые документы и delivery-пакеты из `docs/**` не используются как база новой реализации; они либо заменяются переходными документами, либо переводятся в `docs/deprecated/pre-refactor/**`.
- Новая реализация волн `7+` пишется только с нуля в новых сервисах и новых каталогах целевой архитектуры; устаревший код остаётся только справочным материалом до удаления.
