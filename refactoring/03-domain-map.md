...
- пакет публичного сайта и пользовательской документации платформы: API, разработка пакетов, добавление пакетов в магазин и каталог;
...
- магазин пакетов не является жёстко встроенной частью ядра: авторский магазин поставляется из приватного репозитория как пакет, а каждая установка платформы сама выбирает источники доступных пакетов. Пакетная платформа в этом контуре является клиентом магазина и владельцем локального состояния установки.
