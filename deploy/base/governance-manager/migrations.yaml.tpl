...
kind: Job
metadata:
  name: governance-manager-migrations
  namespace: {{ envOr "KODEX_PRODUCTION_NAMESPACE" "" }}
  labels:
    app.kubernetes.io/name: governance-manager-migrations
  ...
  template:
    ...
    spec:
      ...
      securityContext:
        runAsNonRoot: true
      initContainers:
        - name: wait-governance-manager-db
      ...
      containers:
        - name: migrations
          image: {{ envOr "KODEX_GOVERNANCE_MANAGER_MIGRATIONS_IMAGE" "" }}
          imagePullPolicy: IfNotPresent
          securityContext:
            runAsNonRoot: true
            runAsUser: 10001
          ...
          command: ["/bin/sh", "-ec"]
          args:
            - exec /usr/local/bin/goose -dir /migrations postgres "$KODEX_GOVERNANCE_MANAGER_DATABASE_DSN" up
          env:
            - name: KODEX_GOVERNANCE_MANAGER_DATABASE_DSN
              valueFrom:
                secretKeyRef:
