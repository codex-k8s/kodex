...
kind: Job
metadata:
  name: agent-manager-migrations
  namespace: {{ envOr "KODEX_PRODUCTION_NAMESPACE" "" }}
  labels:
    app.kubernetes.io/name: agent-manager-migrations
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: migrations
          image: {{ envOr "KODEX_AGENT_MANAGER_MIGRATIONS_IMAGE" "" }}
          imagePullPolicy: IfNotPresent
          securityContext:
            runAsNonRoot: true
            runAsUser: 10001
          ...
          command: ["/bin/sh", "-ec"]
          args:
            - exec /usr/local/bin/goose -dir /migrations postgres "$KODEX_AGENT_MANAGER_DATABASE_DSN" up
          env:
            - name: KODEX_AGENT_MANAGER_DATABASE_DSN
              valueFrom:
                secretKeyRef:
