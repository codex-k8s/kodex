...
metadata:
  ...
  labels:
    app.kubernetes.io/name: runtime-manager-migrations
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: migrations
          image: {{ envOr "KODEX_RUNTIME_MANAGER_MIGRATIONS_IMAGE" "" }}
          imagePullPolicy: IfNotPresent
          securityContext:
            runAsNonRoot: true
            runAsUser: 10001
          ...
          command: ["/bin/sh", "-ec"]
          args:
            - exec /usr/local/bin/goose -dir /migrations postgres "$KODEX_RUNTIME_MANAGER_DATABASE_DSN" up
          env:
            - name: KODEX_RUNTIME_MANAGER_DATABASE_DSN
              valueFrom:
                secretKeyRef:
